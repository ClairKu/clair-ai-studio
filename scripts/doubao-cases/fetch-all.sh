#!/bin/zsh
# 豆包个例分析台取数：候选用户 + 全部明细，一条命令跑完（redash 数据源 41 / dw-tidb，需 VPN + REDASH_API_KEY）。
# 用法：CASE_CUT='YYYY-MM-DD HH:MM:SS' CASE_AD='YYYY-MM-DD' CASE_WORK=<目录> scripts/doubao-cases/fetch-all.sh
#   CASE_CUT 必须与主看板 latest.json 的 data_cutoff 一致（validate-alignment.py 会校验）；CASE_AD 为资产快照日。
# 与千问个例台同一套字段（见 assemble-users.py 头部），只有两处不同：
#   1. 绑定基表 = 豆包 OAuth 授权（client_id='doubao'，当前仍有未撤销授权），首次绑定 = MIN(granted_at)
#   2. m_tokens = 豆包 OAuth 会话令牌（api_oauth_refresh_token）；豆包侧走 MCP，提问文本协议层不可得，
#      d_asks 只剩且慢 App 内小顾 3.0 的会话（agent_dj_sessions 非千问部分）
set -eu
source ~/.zshrc 2>/dev/null
CUT="${CASE_CUT:?}"; AD="${CASE_AD:?}"; W="${CASE_WORK:?}"; mkdir -p "$W"
S="$(cd "$(dirname "$0")" && pwd)"
F() { python3 "$S/fetch.py" "$@"; }
START='2026-08-28'                      # 豆包上架首日 = 主看板窗口起点
...
B="(SELECT CAST(user_id AS SIGNED) AS pmid, MIN(granted_at) AS fb FROM ying99_oap.api_user_consent_record
    WHERE client_id='doubao' GROUP BY user_id
    HAVING SUM(revoked_at IS NULL)>0 AND MIN(granted_at)>='$START 00:00:00' AND MIN(granted_at)<'$CUT') b
...
echo "== 2/3 明细（交易 / 风测 / App 小顾会话 / 资产序列 / 设备 / ROOT 流水）"
...
F "$W/d_asks.json" "SELECT s.qmuser_user_id uid, s.session_id, m.dj_gmt_create ts, m.user_input text FROM ying99_xiaogu3_qa.agent_dj_sessions s JOIN ying99_xiaogu3_qa.agent_dj_messages m ON m.session_id=s.session_id AND m.is_deleted=0 LEFT JOIN ying99_qieman.qwen_a2a_session_map q ON CAST(q.session_id AS CHAR)=CAST(s.session_id AS CHAR) AND q.is_deleted=0 WHERE s.qmuser_user_id IN ($IDS) AND s.is_deleted=0 AND q.session_id IS NULL AND m.role='USER' AND m.user_input IS NOT NULL AND m.user_input<>'' AND m.dj_gmt_create<'$CUT' ORDER BY uid, ts" 300
...
F "$W/m_tokens.json" "SELECT user_id, created_at, expires_at, revoked_at FROM ying99_oap.api_oauth_refresh_token WHERE client_id='doubao' AND user_id IN ($IDQ) AND created_at<'$CUT' ORDER BY user_id, created_at" 300
...
F "$W/m_sensors.json" "SELECT broker_user_id, event, event_category, page_name, title, element_content, screen_name, lib, event_time FROM qm_meta.ai_insight_sensors_event_detail WHERE event_date>='$SENS_FROM' AND broker_user_id IN ($IDQ) AND event_time<'$CUT' ORDER BY event_time LIMIT 30000" 600
echo "取数完成 → $W"
