...
SCOPES = [
  {"id": "new_inv", "label": "新投", "flag": "zero_at_bind",
   "def": "绑定豆包时没有投资资产，绑定后开始投入资金，新客和老客都包括在内。"},
  {"id": "first_inv", "label": "首投", "flag": "first_invest_after",
   "def": "绑定豆包前从未投资，绑定后完成了人生第一笔投资，新客和老客都包括在内。"},
  {"id": "new_first_inv", "label": "新户首投", "flag": "new_first_invest",
   "def": "在绑定豆包时新注册且慢，之后完成了人生第一笔投资。"},
  ...
  {"id": "existing_first_inv", "label": "老户首投", "flag": "existing_first_invest",
   "def": "已有且慢账号但从未投资，绑定豆包后完成了人生第一笔投资。"},
]
    ...
    if decision_at:
        ...
        for row in asks:
            ...
            if when and when > decision_at:
                post.append((when, "App 小顾", row.get("text") or ""))
...
def top_stats(u):
    ...
    return (f'<div class="case-facts" aria-label="个例关键结果">'
            ...
            f'<div class="fact"><span>首笔决策时长</span><b class="num">{dur(u["fb"], first_in)}</b><small>豆包绑定 → 首笔入金</small></div>'
...
def case_insight(u):
    ...
    focus_by_style = {
        "规划推荐型": "先用豆包明确资金金额、投资期限、收益目标与提醒需求",
...
<thead><tr><th>用户</th><th>客群</th><th>画像</th><th>绑定</th><th>绑定后首笔投资</th><th>间隔</th><th title="充值盈米宝或银行卡直接投资">入金</th><th>当前资产</th><th>风测</th><th title="豆包会话令牌次数 / 且慢 App 小顾提问条数">豆包会话 / 提问</th><th>下单终端</th></tr></thead>
...
def scope_section(sc):
    ...
    # 决策时长：绑定豆包 → 第一笔入金；无入金记录的用户不进入时长计算。
    import statistics
  ...
  <div class="cell decision" title="绑定豆包到第一笔入金，按 {len(durs)} 位有入金记录的用户计算"><b>{avg_txt}</b><span>平均决策时长</span><em>中位数 {med_txt} · 样本 {len(durs)} 人</em></div>
</div>'''
...
<title>豆包用户转化分析｜截至 {CUT[:10]}</title>
<style>
{CSS}
...
<div class="page-wrap">
  ...
  <footer>豆包 X 且慢AI小顾 · 绑定用户个例分析台 · 生成于 {META["generated"]} · <a href="../doubao-user-acquisition-dashboard/">返回用户数据看板</a></footer>
</div>
...
function renderAsks(){{
  ...
  document.getElementById('asks-title').textContent=`用户 ${{u.letter}} 的会话与提问`;
  ...
  if(!rows.length){{ const li=document.createElement('li'), t=document.createElement('span'), d=document.createElement('span');
    t.className='qt'; t.textContent='—'; d.className='qd'; d.textContent=asksChannel ? `该用户没有${{asksChannel}}渠道的记录。` : '该用户在豆包、且慢、微信三端都没有会话或提问记录。';
