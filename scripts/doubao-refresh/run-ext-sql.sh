...
# 用法：QW_CUT='YYYY-MM-DD HH:MM:SS' QW_AD='YYYY-MM-DD' QW_WORK=<目录> scripts/doubao-refresh/run-ext-sql.sh
# 与 qianwen-refresh/run-ext-sql.sh 同构，仅绑定基表换成豆包 OAuth 授权（client_id='doubao'，当前仍有未撤销授权）。
...
INFLOW_PER_USER="(SELECT t.user_id, SUM($INFLOW_CASE) AS inflow FROM qm_meta.trade_detail t
   JOIN (SELECT CAST(user_id AS SIGNED) AS pmid, MIN(granted_at) AS fb FROM ying99_oap.api_user_consent_record
         WHERE client_id='doubao' GROUP BY user_id HAVING SUM(revoked_at IS NULL)>0) bb ON bb.pmid=t.user_id
...
LEFT JOIN (SELECT t.user_id, SUM($INFLOW_CASE) inflow,
  ...
  FROM qm_meta.trade_detail t JOIN (SELECT CAST(user_id AS SIGNED) AS pmid, MIN(granted_at) AS fb FROM ying99_oap.api_user_consent_record WHERE client_id='doubao' GROUP BY user_id HAVING SUM(revoked_at IS NULL)>0) bb ON bb.pmid=t.user_id
