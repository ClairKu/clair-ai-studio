#!/usr/bin/env python3
"""把豆包看板取数各段的表格输出整理成 assemble.py 需要的 q1..q5.json（与 qianwen-refresh/build-q.py 同构，窗口起点 2026-08-28）。
...
# 豆包新户几乎没有资金账户，SUM(...) 在空集上返回 None：一律按 0 处理。
_NULLS = ("None", "NULL", "")
I = lambda v: 0 if v in _NULLS else int(float(v))
F = lambda v: 0.0 if v in _NULLS else float(v)
W = Path(sys.argv[1])
...
assert days and days[0]["date"] >= "2026-08-28", "逐日出现窗口外日期（检查 run-sql.sh 的绑定基表）"
cur, full = date(2026, 8, 28), []
...
# 人数为 0 的客群（如尚无首投用户）在 SQL 里不会出现行：补一行全 0，面板照常显示「暂无」。
_SEG_ALL = ["all", "new_inv", "first_inv", "reinvested", "new", "new_first_inv", "existing", "existing_reactivated", "existing_first_inv"]
_zero_row = {k: "0" for k in next(iter(sg.values())) if k != "seg"}
for _k in _SEG_ALL:
    sg.setdefault(_k, {"seg": _k, **_zero_row})
