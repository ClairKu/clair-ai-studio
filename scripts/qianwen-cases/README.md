...
- `narratives.json` 为每位用户维护四个作者研判字段：`insight`（个案定义）、`conversion_path_label`（转化路径类型）、`conversion_path`（四段式跨端路径）与 `behavior_insight`（行为特性和主要影响因素）。这些内容必须逐例结合提问、App 行为和交易时点判断，不能套固定信息摘要模板。
- 个案卡头部直接展示「路径 · 类型」；洞察引言不再出现「一句话」标签。
- 「且慢行为」默认只展示转化路径、行为判断和下单关键时点；高频逐日浏览/点击日志折叠到「查看按日浏览与操作明细」。时序链只说明行为先后和伴随关系，不把单一提问或页面归因为下单原因。
