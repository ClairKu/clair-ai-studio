...
const reportRoot = join(root, "public", "reports", "doubao-user-acquisition-dashboard");
...
const preview = readFileSync(join(root, "public", "previews", "doubao-user-acquisition-dashboard.svg"), "utf8");
const workbench = readFileSync(join(root, "src", "app.js"), "utf8");
const fail = (message) => { throw new Error(`豆包用户数据看板校验失败：${message}`); };
// 豆包 2026-08-28 上架当日即有用户绑定，没有单独灰度期：上架时刻与统计窗口起点一致。
const launchAt = "2026-08-28T00:00:00+08:00";
const windowStartAt = "2026-08-28T00:00:00+08:00";
const schemaVersion = "doubao-user-acquisition-v1";
...
function assertBusinessItem(item, path, population, minimumPublicCell) {
  ...
  // 笔数指标没有 amount_wan；豆包新户暂无入金，人数为 0 时只校验笔数（见下）。
  if (item.accounts === 0 && !isTransactionCount && item.amount_wan !== 0) fail(`${path} 无人涉及却有金额`);
  if (Object.hasOwn(item, "event_count")) {
    if (!isCount(item.event_count)) fail(`${path}.event_count 必须为非负整数`);
...
for (const removed of ["所选用户数据明细", 'id="audience-table"', 'id="audience-footnote"', "千问", "qianwen", "QIANWEN", "首发来", "上线前灰度", "since-launch"]) {
...
const reportEntryStart = workbench.indexOf('id: "doubao-user-acquisition-dashboard"');
...
if (process.argv.includes("--write-fallback")) {
  writeFileSync(fallbackPath, `window.DOUBAO_ACQUISITION_DATA = ${JSON.stringify(data, null, 2)};\n`);
}

console.log(`豆包用户数据看板通过：${windowStartAt.slice(0, 10)} 起 ${data.metrics.bound_accounts} 个绑定用户，${data.daily.length} 天趋势，截止 ${data.meta.data_cutoff}。`);
