...
- 桌面导航压缩至 54px、移动端 50px；当前章节自动平滑移动到导航中间，前后章节保持可见。
- 全页面统一宋体，章节标题降至 30—44px，标题与副标合并为紧凑标题带，把空间留给核心内容。
- 所有章节复用同一高度、内边距、标题区与内容起点；架构模块改为横向紧凑编排，减少无效内部滚动。
- 移除左下角常驻操作提示，保留键盘能力与无障碍标签。
