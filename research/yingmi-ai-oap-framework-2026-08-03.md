...
report_cutoff: 2026-09-01 19:38（当日部分数据）；年度目标与用户增长实时聚合；千问传播材料截至 8 月 13 日；其余历史证据保留各自截点
...
- 累计调用 10,058,883 / 13,000,000 = 77.4%（年末正式 OKR；距目标 2,941,117 次）· 截至 2026-09-01 19:38（当日部分数据） · OAP 生产库实时聚合
- 累计申请 12,097 / 10,000 = 121%（超目标 2,097）· 截至 2026-09-01 19:38（当日部分数据） · 申请记录累计，排除内部测试 apiKey
- 近 30 日活跃 2,899 / 2,000 = 145%（超目标 899）· 截至 2026-09-01 19:38（当日部分数据） · 滚动近 30 天有调用记录的去重 apiKey
- 历史累计调用用户：本次公开实时快照未提供该字段，不沿用 8 月 16 日旧值（missing）
...
累计调用 10.06M（OKR 77.4%）、累计申请 12,097（121%）、近 30 日活跃 2,899（145%）；申请与活跃已超年度目标，平台下一阶段要把规模转成有效使用、服务结果与商化闭环。
