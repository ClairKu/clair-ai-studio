...
- 累计调用 9,458,599 / 13,000,000 = 72.8%（年末正式 OKR；距目标 3,541,401 次）· 截至 2026-08-16 23:59:59，完整自然日 · OAP 生产数仓只读聚合
- 批准申请用户 9,855 / 10,000 = 98.6%（距目标 145 人）· 截至 2026-08-16 23:59:59，完整自然日 · OAP 生产数仓只读聚合
- 近 30 日活跃用户 2,266 / 2,000 = 113.3%（超目标 266 人）· 最近 30 个完整自然日 · OAP 生产数仓只读聚合
- 历史调用用户 5,816；占批准申请用户 9,855 的 59.0% · 截至 2026-08-16 23:59:59 · OAP 生产数仓只读聚合
...
累计调用 9.46M（OKR 72.8%）、批准申请用户 9,855（98.6%）、近 30 日活跃用户 2,266（113.3%，已超年度目标）；平台跨过能力验证期，下半场主线是补足调用规模、放大新入口并验证商化。关键举措遵循「盈米 AI 开放平台底座 → 四个持续 → 两类接入模式 → 更多群体」放大势能；业务调用 TOP20 进一步表明，真实需求从资讯与基金发现起步，再进入诊断、配置与策略服务，行业机会在于把头部入口编排成完整投顾任务链。
