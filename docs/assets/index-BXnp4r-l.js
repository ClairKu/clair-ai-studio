(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=[`.pdf`,`.html`,`.htm`,`.png`,`.jpg`,`.jpeg`,`.webp`,`.doc`,`.docx`,`.xls`,`.xlsx`,`.ppt`,`.pptx`,`.md`,`.markdown`].join(`,`),t=[{kind:`pdf`,label:`PDF`,extensions:[`pdf`],mime:[`application/pdf`],preview:`pdf`},{kind:`html`,label:`HTML`,extensions:[`html`,`htm`],mime:[`text/html`],preview:`html`},{kind:`image`,label:`PNG`,extensions:[`png`],mime:[`image/png`],preview:`image`},{kind:`image`,label:`IMAGE`,extensions:[`jpg`,`jpeg`,`webp`],mime:[`image/jpeg`,`image/webp`],preview:`image`},{kind:`word`,label:`WORD`,extensions:[`doc`,`docx`],mime:[`application/msword`,`application/vnd.openxmlformats-officedocument.wordprocessingml.document`],preview:`download`},{kind:`excel`,label:`EXCEL`,extensions:[`xls`,`xlsx`],mime:[`application/vnd.ms-excel`,`application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`],preview:`download`},{kind:`ppt`,label:`PPT`,extensions:[`ppt`,`pptx`],mime:[`application/vnd.ms-powerpoint`,`application/vnd.openxmlformats-officedocument.presentationml.presentation`],preview:`download`},{kind:`markdown`,label:`MD`,extensions:[`md`,`markdown`],mime:[`text/markdown`],preview:`text`}];function n(e=``){return String(e).split(`.`).pop()?.toLowerCase()||``}function r(e={}){let r=n(e.name),i=String(e.type||``).toLowerCase(),a=t.find(e=>e.extensions.includes(r)||e.mime.includes(i));return a?{...a,extension:r,supported:!0}:{kind:`file`,label:r?r.toUpperCase().slice(0,8):`FILE`,extension:r,preview:`download`,supported:!1}}function i(e={}){return r(e).supported}var a=[{id:`save`,name:`Save`,hint:`Recognize and add to the library`},{id:`decision`,name:`Decide`,hint:`Copy a decision brief`},{id:`review`,name:`Review`,hint:`Copy a review brief`}],o={save:`
    ...
    </svg>`},s=[{id:`requirement`,name:`需求评审`},{id:`solution`,name:`方案评审`},{id:`decision`,name:`决策推演`},{id:`agreement`,name:`协议审查`},{id:`career`,name:`履历评估`}],c=l();function l(){return{material:``,files:[]}}function u(){return crypto.randomUUID?.()||`${Date.now()}-${Math.random()}`}function d(e){let t=e.toLowerCase(),n=[[`agreement`,[`协议`,`合同`,`条款`,`保密`,`签署`,`数据处理`]],[`career`,[`简历`,`履历`,`候选人`,`晋升`,`岗位`,`面试`]],[`decision`,[`决策`,`选型`,`取舍`,`是否推进`,`选择`]],[`requirement`,[`需求`,`prd`,`用户故事`,`验收`,`原型`]],[`solution`,[`方案`,`流程`,`架构`,`设计`,`上线`]]].find(([,e])=>e.some(e=>t.includes(e)))?.[0]||`solution`;return s.find(e=>e.id===n)||s[1]}function f(e,t,n){let r=(e.files||[]).map(e=>`- ${e.name}${e.sizeLabel?`（${e.sizeLabel}）`:``}`).join(`
`);return[`Task: ${n===`decision`?`Decision`:`Review`}`,`Matched skill: ${t.name}`,``,`Material:`,e.material||`(No pasted text)`,r?`\nAttachments:\n${r}`:``].filter(Boolean).join(`
`)}function p(e){return e<1024?`${e} B`:e<1024*1024?`${Math.ceil(e/1024)} KB`:`${(e/1024/1024).toFixed(1)} MB`}async function m(e){let t=[...e];return Promise.all(t.map(async e=>{let t=r(e),n=e.type.startsWith(`text/`)||/\.(md|txt|csv|json|html|xml)$/i.test(e.name),i=/\.html?$/i.test(e.name),a=``,o=``;if(n&&e.size<=1024*1024)try{let t=await e.text();a=t.slice(0,12e3),i&&(o=t)}catch{a=``,o=``}return{id:u(),name:e.name,type:e.type||`application/octet-stream`,size:e.size,sizeLabel:p(e.size),kind:t.kind,format:t.label,extension:t.extension,previewMode:t.preview,excerpt:a,content:o,blob:e}}))}async function h(e){let t=[...e],n=t.filter(i),r=Math.max(0,20-c.files.length),a=n.slice(0,r),o=await m(a);return c.files.push(...o),{added:o.length,unsupported:t.length-n.length,overflow:Math.max(0,n.length-a.length)}}function ee(e){let t=[`已加入 ${e.added} 个文件`];return e.unsupported&&t.push(`${e.unsupported} 个格式不支持`),e.overflow&&t.push(`${e.overflow} 个超过 20 个上限`),t.join(` · `)}function te(e){return c.files.length?`<div class="attachment-list">${c.files.map(t=>`
    <span class="attachment-chip file-kind-${e(t.kind||`file`)}" title="${e(t.format||`FILE`)} · ${e(t.name)}">
      ...
      <button type="button" aria-label="移除 ${e(t.name)}"
        data-task-action="remove-file" data-file-id="${t.id}">${o.close}</button>
    </span>`).join(``)}</div>`:``}function ne(e){return a.map(t=>`
    <button class="intake-action intake-icon-action" type="submit"
      ...
      ${o[t.id]}
    </button>`).join(``)}function re(t){return`
    <section class="inline-task-launcher prompt-launcher simple-intake" aria-label="新增内容">
      <form class="prompt-composer compact-intake-composer ${c.material.trim()||c.files.length?`has-intake-content`:``}" id="task-composer">
        <div class="compact-intake-row">
          ...
          <textarea id="task-goal" rows="1" aria-label="Set an idea in motion"
            placeholder="Set an idea in motion">${t(c.material)}</textarea>
          <div class="intake-actions compact-task-actions" aria-label="Actions">
            <label class="intake-action intake-icon-action compact-upload-button"
              ...
              <input id="task-files" type="file" accept="${e}" multiple />
              ${o.upload}
            </label>
            ${ne(t)}
          </div>
    ...
    </section>`}function ie({render:e,showToast:t,saveToLibrary:n}){document.querySelectorAll(`[data-task-action]`).forEach(t=>{t.addEventListener(`click`,async t=>{t.currentTarget.dataset.taskAction===`remove-file`&&(ae(),c.files=c.files.filter(e=>e.id!==t.currentTarget.dataset.fileId),e())})});let r=document.getElementById(`task-composer`);r?.addEventListener(`submit`,async i=>{if(i.preventDefault(),ae(),!c.material.trim()&&!c.files.length){t(`先粘贴内容，或加入一份材料`),document.getElementById(`task-goal`)?.focus();return}let a=i.submitter?.dataset.submitAction||`save`,o=i.submitter,u={material:c.material.trim(),files:c.files};if(a===`save`){let i=r.querySelector(`#intake-save-status`),a=[...r.querySelectorAll(`button, textarea, input`)],s=e=>{a.forEach(e=>{e.disabled=!0}),r.setAttribute(`aria-busy`,`true`),r.classList.add(`is-saving`),i.hidden=!1,i.querySelector(`strong`).textContent=e,o.setAttribute(`aria-label`,`保存中`),o.innerHTML=`<span class="mini-spinner"></span>`};s(`正在检查成果库与页面访问状态…`);try{let r=await n(u,s);if(r.rejected){e(),t(r.reason);return}if(r.duplicate){e(),t(`成果库已有“${r.title}” · 位于“${r.groupName}”，未重复保存`);return}c=l(),e(),t(`已保存到“${r.groupName}” · ${r.workTypeName} · 标签：${r.tags.join(` / `)||`待补标签`}`)}catch{a.forEach(e=>{e.disabled=!1}),e(),t(`保存失败，请稍后重试`)}return}let p=d([u.material,...u.files.map(e=>`${e.name}\n${e.excerpt}`)].join(`
`)),m=a===`decision`?s.find(e=>e.id===`decision`):p.id===`decision`?s.find(e=>e.id===`solution`):p;try{await navigator.clipboard.writeText(f(u,m,a)),t(`${a===`decision`?`Decision`:`Review`} brief copied`)}catch{t(`Copy failed — select the material and try again`);return}c=l(),e()}),document.getElementById(`task-files`)?.addEventListener(`change`,async n=>{ae();let r=await h(n.target.files);e(),t(ee(r))});let i=document.querySelector(`.prompt-composer`);i?.addEventListener(`dragover`,e=>{e.preventDefault(),i.classList.add(`drag-over`)}),i?.addEventListener(`dragleave`,()=>i.classList.remove(`drag-over`)),i?.addEventListener(`drop`,async n=>{n.preventDefault(),n.stopPropagation(),i.classList.remove(`drag-over`),ae();let r=n.dataTransfer.files,a=await h(r);e(),t(ee(a))});let a=document.getElementById(`task-goal`);requestAnimationFrame(()=>oe(a)),a?.addEventListener(`input`,()=>{c.material=a.value,i?.classList.toggle(`has-intake-content`,!!(a.value.trim()||c.files.length)),oe(a)}),a?.addEventListener(`paste`,async n=>{let r=[...n.clipboardData?.items||[]].filter(e=>e.kind===`file`).map(e=>e.getAsFile()).filter(Boolean);if(!r.length)return;n.preventDefault();let i=n.clipboardData.getData(`text/plain`),o=a.selectionStart??a.value.length,s=a.selectionEnd??o;c.material=`${a.value.slice(0,o)}${i}${a.value.slice(s)}`;let l=await h(r);e(),t(`已从剪贴板${ee(l)}`)}),le({render:e,showToast:t})}function ae(){let e=document.getElementById(`task-goal`);e&&(c.material=e.value)}function oe(e){if(!e)return;e.style.height=`auto`;let t=Math.min(Math.max(e.scrollHeight,40),180);e.style.height=`${t}px`,e.style.overflowY=e.scrollHeight>180?`auto`:`hidden`}function se(){document.querySelector(`.prompt-composer`)&&requestAnimationFrame(()=>{document.getElementById(`task-goal`)?.focus({preventScroll:!0})})}function ce(e){return!!e?.closest?.(`input, textarea, select, [contenteditable='true']`)}function le({render:e,showToast:t}){document.onpaste=async n=>{if(ce(n.target)||!document.querySelector(`.prompt-composer`))return;let r=[...n.clipboardData?.items||[]].filter(e=>e.kind===`file`).map(e=>e.getAsFile()).filter(Boolean),i=n.clipboardData?.getData(`text/plain`)||``;if(!r.length&&!i.trim())return;n.preventDefault(),c.material=[c.material.trim(),i.trim()].filter(Boolean).join(`

`);let a=r.length?await h(r):null;e(),requestAnimationFrame(se),t(a?`已从剪贴板${ee(a)}`:`已把粘贴内容放入输入框`)},document.ondragover=e=>{[...e.dataTransfer?.types||[]].includes(`Files`)&&e.preventDefault()},document.ondrop=async n=>{if(n.target?.closest?.(`.prompt-composer`))return;let r=n.dataTransfer?.files||[];if(!r.length)return;n.preventDefault();let i=await h(r);e(),requestAnimationFrame(se),t(ee(i))}}var g=`clair-report-editor-v1`,ue=`https://api.github.com`,de=`clair-report-editor-draft-v1:`,_={reportId:``,reportTitle:``,reportUrl:``,status:`idle`,error:``,html:``,editorDocument:``,dirty:!1,hasDraft:!1,draftHtml:``,draftAt:``,target:null,token:``,settingsOpen:!1,publishConfirmOpen:!1,pendingSave:!1,saving:!1,lastCommit:``,isLocal:!1,saveLocal:null,protection:null,loadPromise:null,render:null,showToast:null,currentPage:0,pageCount:1},fe=new Map,pe=!1;function me(e){return[...new Set(e.filter(Boolean))]}function he(e=_.target){return e?{...e.path&&e.sha?{[e.path]:e.sha}:{},...Object.fromEntries((e.mirrors||[]).map(e=>[e.path,e.sha])),...e.baseFiles||{}}:{}}function ge(e){return`${de}${e}`}function _e(e){try{let t=sessionStorage.getItem(ge(e));if(!t)return null;let n=JSON.parse(t);return!n?.html||typeof n.html!=`string`?null:n}catch{return null}}function ve(e=_.reportId){try{sessionStorage.removeItem(ge(e))}catch{}}function ye(){return _.dirty&&_.hasDraft?{tone:`changed`,label:_.isLocal?`有新修订 · 上次暂存待保存`:`有新修订 · 上次暂存待推送`}:_.dirty?{tone:`changed`,label:`已修订 · 未暂存`}:_.hasDraft?{tone:`staged`,label:_.isLocal?`已暂存 · 待保存成果库`:`已暂存 · 待推送生产`}:_.lastCommit?{tone:`published`,label:_.isLocal?`成果库 HTML 已更新`:`生产档案已更新`}:{tone:`clean`,label:`未修改`}}function v(){let e=ye(),t=document.querySelector(`.editor-revision-status`);t&&(t.className=`editor-revision-status is-${e.tone}`,t.textContent=e.label);let n=document.querySelector(`[data-editor-action="stash"]`);if(n){n.disabled=_.status!==`ready`||_.saving||!_.dirty;let e=!_.dirty&&_.hasDraft?`已暂存`:`暂存修改`;n.setAttribute(`aria-label`,e),n.title=e}let r=document.querySelector(`[data-editor-action="publish"]`);if(r){r.disabled=_.status!==`ready`||_.saving||!_.dirty&&!_.hasDraft;let e=_.saving?_.isLocal?`正在保存到成果库`:`正在推送生产`:_.isLocal?`保存到成果库`:`推送生产`;r.setAttribute(`aria-label`,e),r.title=e,r.classList.toggle(`is-saving`,_.saving)}let i=document.querySelector(`[data-editor-action="preview"]`);i&&(i.disabled=_.status!==`ready`||_.saving||!_.hasDraft);let a=document.querySelector(`[data-editor-page-counter]`),o=document.querySelector(`[data-editor-page-controls]`);a&&(a.textContent=`${_.currentPage+1} / ${Math.max(1,_.pageCount)}`),o&&(o.hidden=_.pageCount<=1);let s=document.querySelector(`[data-editor-action="prev-page"]`),c=document.querySelector(`[data-editor-action="next-page"]`);s&&(s.disabled=_.currentPage<=0),c&&(c.disabled=_.currentPage>=_.pageCount-1)}function be(e=``){return String(e).replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`).replaceAll(`'`,`&#39;`)}function xe(e){let t=atob(String(e||``).replace(/\s/g,``)),n=Uint8Array.from(t,e=>e.charCodeAt(0));return new TextDecoder().decode(n)}function Se(e){let t=new TextEncoder().encode(e),n=``,r=32768;for(let e=0;e<t.length;e+=r)n+=String.fromCharCode(...t.subarray(e,e+r));return btoa(n)}async function Ce(e){return{html:e,protection:null}}async function we(e){return e}function Te(e){try{let t=new URL(e);if(t.hostname.toLowerCase()!==`clairku.github.io`)return null;let n=t.pathname.split(`/`).filter(Boolean).map(decodeURIComponent),r=n.shift()||`ClairKu.github.io`,i=n.join(`/`);(!i||t.pathname.endsWith(`/`))&&(i=`${i?`${i}/`:``}index.html`);let a=me([`docs/${i}`,i,`public/${i}`]);return{owner:`ClairKu`,repository:r,branch:`main`,path:a[0],candidates:a,source:`auto`}}catch{return null}}async function Ee(e,{token:t=``,method:n=`GET`,body:r}={}){let i={Accept:`application/vnd.github+json`,"X-GitHub-Api-Version":`2022-11-28`};t&&(i.Authorization=`Bearer ${t}`),r!==void 0&&(i[`Content-Type`]=`application/json`);let a=await fetch(`${ue}${e}`,{method:n,headers:i,body:r===void 0?void 0:JSON.stringify(r)});if(!a.ok){let e=``;try{e=(await a.json())?.message||``}catch{e=await a.text()}let t=Error(e||`GitHub API ${a.status}`);throw t.status=a.status,t}return a.status===204?null:a.json()}async function De(e){e.branch=(await Ee(`/repos/${encodeURIComponent(e.owner)}/${encodeURIComponent(e.repository)}`)).default_branch||e.branch||`main`;let t=me(e.candidates?.length?e.candidates:[e.path]),n=null,r=null,i=[];for(let a of t)try{let n=a.split(`/`).map(encodeURIComponent).join(`/`),o=await Ee(`/repos/${encodeURIComponent(e.owner)}/${encodeURIComponent(e.repository)}/contents/${n}?ref=${encodeURIComponent(e.branch)}`),s=``;if(o.encoding===`base64`&&o.content)s=xe(o.content);else if(o.download_url){let e=await fetch(o.download_url,{cache:`no-store`});if(!e.ok)throw Error(`无法读取 GitHub 原始文件`);s=await e.text()}if(!s)throw Error(`GitHub 文件内容为空`);r?s===r.html&&i.push({path:a,sha:o.sha}):r={html:s,target:{...e,path:a,sha:o.sha,candidates:t}}}catch(e){if(n=e,e.status&&![403,404].includes(e.status))break}if(r)return r.target.mirrors=i,r;throw n||Error(`没有找到对应的 GitHub HTML 文件`)}function Oe(e){e.querySelectorAll(`script`).forEach(e=>{e.dataset.clairOriginalType=e.getAttribute(`type`)??`__empty__`,e.setAttribute(`type`,`application/x-clair-disabled`)}),e.querySelectorAll(`*`).forEach(e=>{[...e.attributes].forEach(t=>{/^on/i.test(t.name)&&(e.setAttribute(`data-clair-event-${t.name.toLowerCase()}`,t.value),e.removeAttribute(t.name))});let t=e.getAttribute(`href`);t&&/^\s*javascript:/i.test(t)&&(e.dataset.clairJavascriptHref=t,e.removeAttribute(`href`))})}function ke(){return`
(() => {
...
`}function Ae(e,t){let n=new DOMParser().parseFromString(e,`text/html`);n.querySelectorAll(`meta[http-equiv="Content-Security-Policy" i]`).forEach(e=>{e.dataset.clairEditorHttpEquiv=e.getAttribute(`http-equiv`)||`Content-Security-Policy`,e.setAttribute(`http-equiv`,`x-clair-csp-disabled`)}),Oe(n);let r=n.createElement(`base`);r.href=t,r.dataset.clairEditorBase=`true`,n.head.prepend(r);let i=n.createElement(`style`);i.id=`clair-editor-style`,i.textContent=`
  ...
  `,n.head.append(i);let a=n.createElement(`script`);return a.id=`clair-editor-bridge`,a.textContent=ke(),n.body.append(a),`<!DOCTYPE html>\n${n.documentElement.outerHTML}`}function je(e){if(e.url)return``;if(e.savedHtml)return e.savedHtml;let t=(e.savedFiles||[]).find(e=>/\.html?$/i.test(e.name||``));return t?.content||t?.excerpt?t.content||t.excerpt:/<!doctype\s+html|<html[\s>]/i.test(e.savedContent||``)?e.savedContent.trim():``}async function Me(e){try{let t=je(e),n=t?null:Te(e.url),r=null;if(t)r={html:t,target:null};else if(n)try{r=await De(n)}catch{}if(!r&&e.url){let t=await fetch(e.url,{cache:`no-store`});if(!t.ok)throw Error(`报告读取失败（HTTP ${t.status}）`);r={html:await t.text(),target:n}}let i=await Ce(r.html);_.protection=i.protection,_.target=r.target||n;let a=i.html,o=_e(e.id);if(o?.html)try{let e=await Ce(o.html);a=e.html,_.hasDraft=!0,_.draftHtml=e.html,_.draftAt=o.savedAt||``,o.baseFiles&&_.target&&(_.target.baseFiles=o.baseFiles)}catch{ve(e.id)}_.html=a,_.editorDocument=Ae(a,e.url||window.location.href),_.status=`ready`,_.error=``}catch(e){_.status=`error`,_.error=e?.message||`无法读取这份 HTML`}finally{_.loadPromise=null,_.render?.()}}function Ne(){let e=_.render,t=_.showToast;Object.assign(_,{reportId:``,reportTitle:``,reportUrl:``,status:`idle`,error:``,html:``,editorDocument:``,dirty:!1,hasDraft:!1,draftHtml:``,draftAt:``,target:null,settingsOpen:!1,publishConfirmOpen:!1,pendingSave:!1,saving:!1,lastCommit:``,isLocal:!1,saveLocal:null,protection:null,loadPromise:null,currentPage:0,pageCount:1,render:e,showToast:t})}function Pe(){return document.querySelector(`.report-editor-frame`)}function Fe(e,t=null){Pe()?.contentWindow?.postMessage({channel:g,type:`command`,command:e,value:t},`*`)}function y(e,t={}){Pe()?.contentWindow?.postMessage({channel:g,type:e,...t},`*`)}function Ie(e=``){return String(e).replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`)}function Le(e){let t=Pe();if(!t?.contentWindow)return;let n=Math.max(0,Math.min(_.pageCount-1,Number(e)||0));_.currentPage=n,t.contentWindow.postMessage({channel:g,type:`set-page`,page:n},`*`),v()}function Re(){let e=Pe();if(!e?.contentWindow)return Promise.reject(Error(`编辑画布尚未就绪`));let t=crypto.randomUUID?.()||`${Date.now()}-${Math.random()}`;return new Promise((n,r)=>{let i=window.setTimeout(()=>{fe.delete(t),r(Error(`读取编辑内容超时`))},1e4);fe.set(t,{resolve:e=>{clearTimeout(i),n(e)}}),e.contentWindow.postMessage({channel:g,type:`serialize`,requestId:t},`*`)})}function ze(e){return`${String(e||`report`).replace(/[\\/:*?"<>|]+/g,`-`).replace(/\s+/g,`-`).replace(/-+/g,`-`).replace(/^-|-$/g,``).slice(0,80)||`report`}.html`}function Be(e,t){let n=new Blob([e],{type:`text/html;charset=utf-8`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=ze(t),document.body.append(i),i.click(),i.remove(),window.setTimeout(()=>URL.revokeObjectURL(r),1e3)}async function Ve(e){await navigator.clipboard.writeText(e)}function He(e,t){let n=new DOMParser().parseFromString(e,`text/html`);n.querySelector(`base[data-clair-preview-base]`)?.remove();let r=n.createElement(`base`);return r.href=t,r.dataset.clairPreviewBase=`true`,n.head.prepend(r),`<!DOCTYPE html>\n${n.documentElement.outerHTML}`}function Ue(e){if(!_.hasDraft||!_.draftHtml)throw Error(`请先暂存当前修订，再另开预览`);let t=new Blob([He(_.draftHtml,e.url||window.location.href)],{type:`text/html;charset=utf-8`}),n=URL.createObjectURL(t),r=window.open(n,`_blank`);if(!r)throw URL.revokeObjectURL(n),Error(`浏览器拦截了新窗口，请允许弹窗后重试`);r.opener=null,window.setTimeout(()=>URL.revokeObjectURL(n),6e4)}async function We(e,{silent:t=!1}={}){let n=await Re(),r=await we(n),i=new Date().toISOString();try{sessionStorage.setItem(ge(e.id),JSON.stringify({reportId:e.id,reportUrl:e.url,savedAt:i,baseFiles:he(),html:r}))}catch{throw Error(`浏览器暂存空间不足，请先下载 HTML 备份`)}return _.html=n,_.draftHtml=n,_.draftAt=i,_.hasDraft=!0,_.dirty=!1,_.lastCommit=``,v(),t||_.showToast?.(_.isLocal?`已暂存在当前浏览器会话，尚未写回成果库`:`已暂存在当前浏览器会话，尚未更新 GitHub`),n}async function Ge(e){if(!(_.saving||!_.saveLocal)){_.saving=!0,v();try{let t=_.dirty?await We(e,{silent:!0}):_.draftHtml||await Re();await _.saveLocal(t),_.html=t,_.dirty=!1,_.hasDraft=!1,_.draftHtml=``,_.draftAt=``,_.lastCommit=`local`,ve(e.id),_.showToast?.(`已更新成果库中的 HTML`)}catch(e){_.showToast?.(e?.message||`保存失败，请下载 HTML 备份`)}finally{_.saving=!1,v()}}}async function Ke(e){let t=_.target;if(!t?.owner||!t.repository||!t.path||!t.branch)throw Error(`请先填写 GitHub 仓库、分支和 HTML 路径`);if(!_.token)throw Error(`请先提供 GitHub Fine-grained Token`);let n=await we(e),r=(t.mirrors||[]).map(e=>e.path),i=me([...r.filter(e=>e.startsWith(`public/`)),...r.filter(e=>!e.startsWith(`public/`)&&e!==t.path),t.path]),a=``,o=[];for(let e of i)try{let r=e.split(`/`).map(encodeURIComponent).join(`/`),i=`/repos/${encodeURIComponent(t.owner)}/${encodeURIComponent(t.repository)}/contents/${r}`,s=await Ee(`${i}?ref=${encodeURIComponent(t.branch)}`,{token:_.token}),c=he(t)[e];if(c&&s.sha!==c)throw Error(`生产文件 ${e} 已在本次编辑后更新，请重新打开报告合并修改`);let l=await Ee(i,{token:_.token,method:`PUT`,body:{message:`Update ${_.reportTitle} from Clair's Studio`,content:Se(n),sha:s.sha,branch:t.branch}});a=l?.commit?.sha||a,t.baseFiles={...he(t),[e]:l?.content?.sha||s.sha},o.push(e)}catch(t){throw o.length?Error(`已更新 ${o.join(`、`)}，但 ${e} 同步失败：${t.message}`):t}return{commit:a,files:o.length}}async function qe(e){if(!_.saving){_.saving=!0,v();try{let t=_.dirty?await We(e,{silent:!0}):_.draftHtml||await Re(),n=await Ke(t);_.html=t,_.dirty=!1,_.hasDraft=!1,_.draftHtml=``,_.draftAt=``,_.lastCommit=n.commit,ve(e.id),_.showToast?.(n.files>1?`已同步 ${n.files} 个 GitHub 文件，Pages 正在更新`:`已提交 GitHub，Pages 正在更新`)}catch(e){_.showToast?.(e?.message||`保存失败，请下载 HTML 备份`)}finally{_.saving=!1,v()}}}function Je(e){let t=_.target||{owner:`ClairKu`,repository:``,branch:`main`,path:``};return`
    ...
    </div>`}function Ye(e){let t=_.target?`${_.target.owner}/${_.target.repository} · ${_.target.path}`:`尚未识别 GitHub 文件路径`;return`
    ...
    </div>`}function Xe({pendingSave:e=!1}={}){_.settingsOpen=!0,_.pendingSave=e;let t=document.querySelector(`.editor-settings-backdrop`);if(!t)return;t.hidden=!1;let n=t.querySelector(`#editor-settings-form`),r=_.target||{};if(n){n.elements.owner.value=r.owner||`ClairKu`,n.elements.repository.value=r.repository||``,n.elements.branch.value=r.branch||`main`,n.elements.path.value=r.path||``;let t=n.querySelector(`button[type="submit"]`);t&&(t.textContent=e?`Connect & save`:`Save settings`)}}function Ze(){_.settingsOpen=!1,_.pendingSave=!1;let e=document.querySelector(`.editor-settings-backdrop`);e&&(e.hidden=!0)}function Qe(){_.publishConfirmOpen=!0;let e=document.querySelector(`.editor-publish-backdrop`);e&&(e.hidden=!1)}function $e(){_.publishConfirmOpen=!1;let e=document.querySelector(`.editor-publish-backdrop`);e&&(e.hidden=!0)}function et(e=``){return!!(_.reportId&&(!e||_.reportId===e))}function tt(e,{render:t,showToast:n,saveLocal:r=null}){Ne(),Object.assign(_,{reportId:e.id,reportTitle:e.title,reportUrl:e.url,status:`loading`,render:t,showToast:n,isLocal:!!(je(e)&&r),saveLocal:r,currentPage:0,pageCount:1}),t(),_.loadPromise=Me(e)}function nt(e,t){let n=_.isLocal?`本地成果 · 保存在当前浏览器`:_.target?`${_.target.owner}/${_.target.repository} · ${_.target.path}${_.target.mirrors?.length?` · 同步 ${_.target.mirrors.length+1} 处`:``}`:`尚未识别 GitHub 源文件`,r=ye(),i=_.status===`ready`?`
    ...
    </svg>`};function hn(e){return mn[e]||``}function gn(e){try{return new URL(e).hostname.replace(/^www\./,``)}catch{return e}}function _n(e){try{return[`http:`,`https:`].includes(new URL(e).protocol)}catch{return!1}}function G(e=``){return[...new Set(String(e).split(/[、,，;；\n]+/).map(e=>e.trim()).filter(Boolean).map(e=>e.slice(0,20)))].slice(0,8)}function vn(){let e=new Set(vt);return T.reports.forEach(t=>{(t.tags||[]).forEach(t=>e.add(t))}),[...e].filter(e=>![`HTML`,`手动保存`,`生产`].includes(e))}function K(e){document.querySelector(`.toast`)?.remove();let t=document.createElement(`div`);t.className=`toast`,t.setAttribute(`role`,`status`),t.textContent=e,document.body.append(t),clearTimeout(Et),Et=window.setTimeout(()=>t.remove(),2600)}function yn(e=`auto`){q(),L=requestAnimationFrame(()=>{L=0,window.scrollTo({top:0,left:0,behavior:e})})}function q(){I&&cancelAnimationFrame(I),L&&cancelAnimationFrame(L),R&&cancelAnimationFrame(R),I=0,L=0,R=0}function bn(){let e=document.querySelector(`.topbar`)?.getBoundingClientRect().bottom||0,t=document.querySelector(`.topic-nav`),n=t?getComputedStyle(t):null,r=t?.getBoundingClientRect();if(!window.matchMedia(`(max-width: 840px)`).matches){let t=n?.position===`sticky`&&Number.parseFloat(n.top)||0;return Math.max(e+22,t)}let i=n?.position===`sticky`&&r?.bottom>0?r.bottom:0;return Math.max(e,i)+10}function xn(e,t=`smooth`){if(!e)return;q();let n=Math.max(0,document.documentElement.scrollHeight-window.innerHeight),r=Math.max(0,Math.min(n,window.scrollY+e.getBoundingClientRect().top-bn())),i=window.scrollY,a=r-i;if(Math.abs(a)<2)return;if(t!==`smooth`||matchMedia(`(prefers-reduced-motion: reduce)`).matches){window.scrollTo({top:r,left:0,behavior:`auto`});return}let o=Math.min(360,Math.max(180,Math.abs(a)*.22)),s=performance.now(),c=t=>{if(!e.isConnected){I=0;return}let n=Math.min(1,(t-s)/o),r=1-(1-n)**3,l=Math.max(0,Math.min(Math.max(0,document.documentElement.scrollHeight-window.innerHeight),window.scrollY+e.getBoundingClientRect().top-bn()));a=l-i,window.scrollTo(0,i+a*r),n<1?I=requestAnimationFrame(c):(I=0,window.scrollTo(0,l))};I=requestAnimationFrame(c)}function J(e,t){return document.querySelector(`.group-column[data-bucket-kind="${CSS.escape(e)}"][data-bucket-id="${CSS.escape(t)}"]`)}function Y(e){return document.querySelector(`.board .report-card[data-report-id="${CSS.escape(e)}"]`)}function Sn(e){if(!e)return null;let t=e.closest?.(`.report-card[data-report-id]`);if(t){let e=t.closest(`.group-column[data-bucket-kind][data-bucket-id]`);return{type:`report`,id:t.dataset.reportId,bucketKind:e?.dataset.bucketKind||``,bucketId:e?.dataset.bucketId||``}}let n=e.closest?.(`.group-column[data-bucket-kind][data-bucket-id]`);if(n)return{type:`bucket`,kind:n.dataset.bucketKind,id:n.dataset.bucketId};let r=e.closest?.(`.results-toolbar, .archive-search, .prompt-composer, .groups-section, .library-layout`);return r?{type:`selector`,selector:r.classList.contains(`results-toolbar`)?`.results-toolbar`:r.classList.contains(`archive-search`)?`.archive-search`:r.classList.contains(`prompt-composer`)?`.prompt-composer`:r.classList.contains(`groups-section`)?`.groups-section`:`.library-layout`}:null}function Cn(e){return e?e.type===`report`?(e.bucketKind&&e.bucketId?J(e.bucketKind,e.bucketId):null)?.querySelector(`.report-card[data-report-id="${CSS.escape(e.id)}"]`)||Y(e.id):e.type===`bucket`?J(e.kind,e.id):e.type===`selector`?document.querySelector(e.selector):null:null}function wn(){let e=bn(),t=t=>[...document.querySelectorAll(t)].filter(t=>{let n=t.getBoundingClientRect();return n.bottom>e&&n.top<window.innerHeight}).sort((t,n)=>Math.abs(t.getBoundingClientRect().top-e)-Math.abs(n.getBoundingClientRect().top-e))[0];return t(`.board .report-card[data-report-id]`)||t(`.group-column[data-bucket-id]`)||t(`.results-toolbar, .archive-search, .prompt-composer`)||document.querySelector(`.results-toolbar, .archive-search, .groups-section, .library-layout`)}function X(e=null){q();let t=e||wn();return{scrollY:window.scrollY,identity:Sn(t),viewportTop:t?.getBoundingClientRect().top??null}}function Tn(e){if(!e)return;q();let t=()=>{let t=Cn(e.identity),n=Math.max(0,document.documentElement.scrollHeight-window.innerHeight),r=t&&Number.isFinite(e.viewportTop)?window.scrollY+t.getBoundingClientRect().top-e.viewportTop:e.scrollY;window.scrollTo({top:Math.max(0,Math.min(n,r)),left:0,behavior:`auto`})};t(),R=requestAnimationFrame(()=>{R=requestAnimationFrame(()=>{R=0,t()})})}function Z(e){$(),Tn(e)}function En(e,t=null){let n=t||Y(e),r=n?.nextElementSibling?.matches?.(`.report-card[data-report-id]`)?n.nextElementSibling:n?.previousElementSibling?.matches?.(`.report-card[data-report-id]`)?n.previousElementSibling:null;if(!r){let e=n?.closest(`.group-column[data-bucket-id]`),t=e?.nextElementSibling?.matches?.(`.group-column[data-bucket-id]`)?e.nextElementSibling:e?.previousElementSibling?.matches?.(`.group-column[data-bucket-id]`)?e.previousElementSibling:null;r=t?.querySelector(`.report-card[data-report-id]`)||t||n}return X(r)}function Dn(e,t){let n=J(e,t);return X(n?.nextElementSibling?.matches?.(`.group-column[data-bucket-id]`)?n.nextElementSibling:n?.previousElementSibling?.matches?.(`.group-column[data-bucket-id]`)?n.previousElementSibling:n)}function On(e,t=`smooth`){q(),L=requestAnimationFrame(()=>{L=requestAnimationFrame(()=>{L=0,xn(e(),t)})})}function Q(e=null){Z(X(typeof e==`function`?e():e))}[`wheel`,`touchstart`,`pointerdown`].forEach(e=>{window.addEventListener(e,q,{passive:!0})});var kn=null,An=new Set;function jn(){return kn||(kn=new Promise((e,t)=>{if(!window.indexedDB){t(Error(`IndexedDB unavailable`));return}let n=indexedDB.open(gt,1);n.onupgradeneeded=()=>{let e=n.result;e.objectStoreNames.contains(x)||e.createObjectStore(x,{keyPath:`id`}).createIndex(`reportId`,`reportId`,{unique:!1})},n.onsuccess=()=>e(n.result),n.onerror=()=>t(n.error||Error(`File database failed`))}),kn)}async function Mn(e,t=[]){let n=t.map(t=>{let{blob:n,...r}=t;return{...r,storageId:n instanceof Blob?`${e}:${t.id}`:t.storageId||``,blob:n}}),r=n.filter(e=>e.blob instanceof Blob&&e.storageId);if(r.length){let t=await jn();await new Promise((n,i)=>{let a=t.transaction(x,`readwrite`),o=a.objectStore(x);r.forEach(t=>{o.put({id:t.storageId,reportId:e,name:t.name,type:t.type,size:t.size,blob:t.blob,updatedAt:new Date().toISOString()})}),a.oncomplete=n,a.onerror=()=>i(a.error||Error(`File save failed`)),a.onabort=()=>i(a.error||Error(`File save aborted`))})}return n.map(({blob:e,...t})=>t)}async function Nn(e){if(e?.storageId)try{let t=await jn(),n=await new Promise((n,r)=>{let i=t.transaction(x,`readonly`).objectStore(x).get(e.storageId);i.onsuccess=()=>n(i.result),i.onerror=()=>r(i.error||Error(`File read failed`))});if(n?.blob instanceof Blob)return n.blob}catch{return null}let t=e?.content||e?.excerpt;return t?new Blob([t],{type:e.type||`text/plain;charset=utf-8`}):null}async function Pn(e){try{let t=await jn();await new Promise((n,r)=>{let i=t.transaction(x,`readwrite`),a=i.objectStore(x),o=a.index(`reportId`).openKeyCursor(IDBKeyRange.only(e));o.onsuccess=()=>{let e=o.result;e&&(a.delete(e.primaryKey),e.continue())},o.onerror=()=>r(o.error||Error(`File cleanup failed`)),i.oncomplete=n,i.onerror=()=>r(i.error||Error(`File cleanup failed`))})}catch{}}function Fn(){An.forEach(e=>URL.revokeObjectURL(e)),An.clear()}function In(e){let t=URL.createObjectURL(e);return An.add(t),t}async function Ln(){let e=[...document.querySelectorAll(`[data-saved-file-preview]`)];await Promise.all(e.map(async e=>{let t=T.reports.find(t=>t.id===e.dataset.reportId)?.savedFiles?.find(t=>t.id===e.dataset.fileId);if(!t)return;let n=await Nn(t);if(!n||!e.isConnected)return;let r=In(n),i=e.dataset.previewMode;if(i===`image`){let n=document.createElement(`img`);n.src=r,n.alt=t.name||`图片预览`,n.draggable=!1,e.replaceChildren(n)}else if(i===`pdf`||i===`html`){let n=document.createElement(`iframe`);n.src=i===`pdf`?`${r}#toolbar=0&navpanes=0&view=FitH`:r,n.title=`${t.name||(i===`pdf`?`PDF`:`HTML`)}预览`,n.tabIndex=-1,i===`html`&&n.setAttribute(`sandbox`,`allow-forms allow-modals allow-popups allow-scripts`),e.replaceChildren(n)}e.classList.add(`is-ready`)}))}async function Rn(e){let t=await Nn(e);if(!t)return!1;let n=In(t),r=document.createElement(`a`);return r.href=n,r.download=e.name||`download`,document.body.append(r),r.click(),r.remove(),window.setTimeout(()=>{URL.revokeObjectURL(n),An.delete(n)},1e3),!0}async function zn(e){let t=window.open(``,`_blank`),n=await Nn(e);if(!n)return t?.close(),!1;let r=In(n);return t?t.location.href=r:window.open(r,`_blank`,`noopener,noreferrer`),window.setTimeout(()=>{URL.revokeObjectURL(r),An.delete(r)},6e4),!0}function Bn(e){return e.savedHtml||Jt(e.savedContent,e.savedFiles)}function Vn(e){return`${String(e.title||`report`).replace(/[\\/:*?"<>|]+/g,`-`).replace(/\s+/g,` `).trim().slice(0,80)||`report`}.html`}function Hn(e){let t=Bn(e);return t?URL.createObjectURL(new Blob([t],{type:`text/html;charset=utf-8`})):``}function Un(e){let t=Hn(e);if(!t)return!1;let n=document.createElement(`a`);return n.href=t,n.download=Vn(e),document.body.append(n),n.click(),n.remove(),window.setTimeout(()=>URL.revokeObjectURL(t),1e3),!0}function Wn(e){let t=e.url||Hn(e);return t?(window.open(t,`_blank`,`noopener,noreferrer`),e.url||window.setTimeout(()=>URL.revokeObjectURL(t),6e4),!0):!1}function Gn(e,t,n=!1){let i=r(t),a=t.format||i.label,o=t.storageId&&[`image`,`pdf`,`html`].includes(i.preview)?`data-saved-file-preview data-report-id="${W(e.id)}" data-file-id="${W(t.id)}" data-preview-mode="${i.preview}"`:``,s=i.preview===`text`&&(t.excerpt||t.content)?`<pre>${W((t.excerpt||t.content).slice(0,n?280:8e3))}</pre>`:``;return`
    <div class="saved-file-visual file-kind-${i.kind} ${n?`compact`:``}" ${o}>
      <span class="saved-file-format">${W(a)}</span>
      ${s||`<div class="saved-file-fallback">
        <strong>${W(a)}</strong>
        <small>${W(t.name||`未命名文件`)}</small>
      </div>`}
    </div>`}function Kn(e){let t=e.savedFiles||[];return t.length?`<section class="saved-file-list rich-file-list" aria-label="已保存档案">
    <strong>已保存档案 · ${t.length}</strong>
    ${t.map(t=>{let n=r(t),i=t.format||n.label,a=[`image`,`pdf`,`html`,`text`].includes(n.preview)&&!!(t.storageId||t.content||t.excerpt),o=[`image`,`pdf`,`text`].includes(n.preview)&&!!(t.storageId||t.content||t.excerpt);return`<article class="saved-file-card file-kind-${n.kind}">
        ${Gn(e,t)}
        <div class="saved-file-details">
          <span class="saved-file-format">${W(i)}</span>
          <div><b>${W(t.name)}</b><small>${W(t.sizeLabel||``)}</small></div>
          <p>${a?`文件已保存在当前浏览器，可直接查看并下载。`:`该格式由系统完整保存，请下载后使用对应应用打开。`}</p>
          <div class="saved-file-actions">
            ${o?`<button type="button" data-action="preview-saved-file" data-id="${W(e.id)}" data-file-id="${W(t.id)}">预览</button>`:``}
            <button type="button" data-action="download-saved-file" data-id="${W(e.id)}" data-file-id="${W(t.id)}">下载原文件</button>
          </div>
        </div>
      </article>`}).join(``)}
  </section>`:``}function qn(e,t=!1,n={}){let r=!e.url&&(!!e.savedContent||!!(e.savedFiles||[]).length),i=[`org`,`account`].includes(e.access),a=e.access===`org`?`需组织登录`:e.access===`account`?`需账号登录`:`生产可访问`,o=Bn(e),s=!!e.pinned,c=T.groups.find(t=>t.id===e.groupId)?.name||`未归类`,l=new Set([`HTML`,`手动保存`,`生产`]),u=[...new Set([c,on(e.workType),...e.tags||[]])].filter(e=>!l.has(e)),d=!i&&w.reports.some(t=>t.id===e.id),f=e.preview||`${e.id}.png`,p=(e.savedFiles||[])[0],m=o&&e.isHtml?`<iframe class="local-html-preview-frame" title="${W(e.title)}视觉预览"
        srcdoc="${W(o)}" sandbox="allow-scripts" loading="lazy"
        tabindex="-1" aria-hidden="true"></iframe>`:d?`<img src="./previews/${W(f)}" alt="" loading="lazy" decoding="async" draggable="false" />`:r&&p?Gn(e,p,!0):`
    ...
    <article class="report-card ${i?`restricted-card`:``} ${t?`archived-card`:``} ${s?`is-featured`:``} ${M===e.id?`is-move-selected`:``}"
      data-report-id="${W(e.id)}" ${t||k===`time`?``:`data-report-draggable="true"`}>
      <button class="card-main" type="button" data-action="open" data-id="${W(e.id)}" aria-label="打开${W(e.title)}">
        ...
        <span class="report-copy">
          ...
          <span class="report-tags">${u.map((e,t)=>`<span class="${t<2?`report-context-tag`:``}">${W(e)}</span>`).join(``)}</span>
      ...
      <div class="card-actions">
        ${t?`
            ...
            <button type="button" class="studio-icon-button card-icon-action" data-action="edit" data-id="${W(e.id)}" title="编辑成果" aria-label="编辑成果">
              ${C.edit}
            </button>
    ...
    </article>`}function Jn(){if(!F)return``;if(F.type===`group`){let e=F.mode===`edit`?T.groups.find(e=>e.id===F.groupId):null;return`
      ...
      </div>`}let e=F.mode===`edit`?T.reports.find(e=>e.id===F.reportId):null,t=e?.groupId||F.groupId||T.groups[0]?.id||``,n=G((e?.tags||[]).join(`、`)).filter(e=>![`HTML`,`手动保存`,`生产`].includes(e));return`
    <div class="dialog-backdrop">
      <form class="dialog" id="report-form">
        ...
        <label>网站地址
          <div class="url-input-row">
            <input name="url" type="url" value="${W(e?.url||``)}" placeholder="https://..." ${!e||e.url?`required`:``} autofocus />
        ...
        <label>放入分组
          <select name="groupId">
            ${T.groups.map(e=>`<option value="${W(e.id)}" ${e.id===t?`selected`:``}>${W(e.name)}</option>`).join(``)}
          </select>
        </label>
        <label>工作类型
          <select name="workType">
            ${_t.map(t=>`<option value="${W(t.id)}" ${t.id===(e?.workType||`product-planning`)?`selected`:``}>${W(t.name)}</option>`).join(``)}
          </select>
        </label>
        <fieldset class="report-tag-field">
          ...
          <div class="report-tag-picker" aria-label="选择关键标签">
            ${vn().map(e=>`<button type="button" class="${n.includes(e)?`selected`:``}"
    ...
    </div>`}function Yn(e){if(et(e.id))return nt(e,W);let t=!e.url&&(!!e.savedContent||!!(e.savedFiles||[]).length),n=[`org`,`account`].includes(e.access),r=e.loginProvider||Yt(e.url)?.provider||(e.access===`org`?`组织帐号`:`站点帐号`),i=e.savedHtml||Jt(e.savedContent,e.savedFiles),a=i?`edit-local-document`:e.url?n?`edit`:`edit-document`:``,o=i?`
...
`,`<br />`)}</div>`:``}
      ...
      <div class="login-handoff-wrap">
        <section class="login-handoff-card" aria-labelledby="login-handoff-title">
          ...
          <p class="login-handoff-domain">${W(gn(e.url))}</p>
        </section>
      </div>`:`
      <div class="reader-frame-wrap">
    ...
    <main class="reader-shell compact-reader-shell">
      <header class="reader-header compact-reader-header">
        <button class="reader-icon-button back-button" type="button" data-action="back"
          aria-label="返回成果库" title="返回成果库">${hn(`back`)}</button>
        <div class="reader-title">
          <strong>${W(e.title)}</strong>
          <span>${t?`本地保存`:W(gn(e.url))}</span>
        </div>
    ...
    </header>`}function Zn(){let e=T.reports.filter(e=>e.archived).filter(e=>ct(e,E,{group:T.groups.find(t=>t.id===e.groupId),workTypeName:on(e.workType)})).sort((e,t)=>new Date(t.archivedAt||0)-new Date(e.archivedAt||0)),t=T.reports.filter(e=>e.archived).length;return`
    <main class="app-shell archive-shell">
      ...
      <section class="workspace archive-workspace">
        ...
        <label class="search archive-search">
          ...
          <input id="search-input" value="${W(E)}"
            placeholder="搜索归档标题、来源或网址" aria-label="搜索归档" />
          ${E?`<button type="button" class="studio-icon-button search-clear-button" data-action="clear-search" title="清除搜索" aria-label="清除搜索">${C.close}</button>`:``}
        </label>
        ${e.length?`
          <section class="archive-results">
            ...
            <div class="archive-grid">${e.map(e=>qn(e,!0)).join(``)}</div>
          </section>`:`
      ...
      ${Jn()}
    </main>`}function Qn(e,t){if(![`topic`,`type`,`tag`].includes(e.kind))return``;let n=t.filter(t=>t.kind===e.kind),r=n.findIndex(t=>t.id===e.id);if(r<0)return``;let i=r===0,a=r===n.length-1,o=(t,n,r,i)=>`
    <button type="button" class="studio-icon-button group-order-button" data-action="move-group"
      ...
      title="${n}" aria-label="${n}" ${i?`disabled`:``}>${r}</button>`;return[o(`top`,`置顶`,C.top,i),o(`up`,`上移`,C.up,i),o(`down`,`下移`,C.down,a),o(`bottom`,`置底`,C.bottom,a)].join(``)}function $n(){if(O)return Zn();let e=b(E),t=T.reports.filter(e=>!e.archived),n=e?dn(t,e):t,r=nn(t.filter(e=>e.pinned),`featured`,`featured`,(e,t)=>U(t)-U(e)),i={id:`featured`,name:`精选成果`,kind:`featured`,accent:`violet`,reports:r},a=T.reports.filter(e=>e.archived).length,o=t.filter(e=>e.access===`production`).length,s=t.filter(e=>e.access!==`production`).length,c=cn(t,``),l=k===`topic`&&r.length?[i,...c]:c,u=k===`time`&&c[0]?.reports||[],d=e?[]:(k===`topic`&&r.length?[i,...c]:c).filter(e=>e.reports.length||M||k===`topic`),f=k===`type`?`工作类型`:k===`tag`?`关键标签`:k===`time`?`新增时间`:`工作主题`;return`
    <main class="app-shell">
        ...
        <div class="results-toolbar unified-results-toolbar">
          ...
          <div class="results-toolbar-side">
            <label class="search results-search">
              <span aria-hidden="true">⌕</span>
              <input id="search-input" type="search" value="${W(E)}"
        ...
        <section class="groups-section">
          ${M?`
            <div class="move-mode-banner" role="status">
              <div><strong>正在整理报告</strong><span>选择目标${un()}的“移到这里”，或直接拖动卡片。</span></div>
              <button type="button" data-action="cancel-move">Cancel</button>
            </div>`:``}
          ${e||d.length?`
            <div class="library-layout">
              <nav class="topic-nav" aria-label="报告${f}">
              ...
              ${e?`
                <section class="search-results-panel" aria-live="polite">
                  <header class="search-results-header">
                    <div><span>SEARCH RESULTS</span><h2>“${W(E.trim())}”</h2></div>
                    <strong>${n.length} 份匹配</strong>
                  </header>
                  ${n.length?`<div class="group-cards search-results-cards">${n.map(t=>qn(t,!1,{searchExcerpt:fn(t,e)})).join(``)}</div>`:`<div class="no-results search-no-results">
                        <strong>没有找到“${W(E.trim())}”</strong>
                  ...
                  <header class="group-header">
                    <div class="group-heading-area">
                      ...
                      ${e.kind===`topic`?`<div class="group-primary-actions" aria-label="分组快捷操作">
                        <button type="button" class="studio-icon-button" data-action="add-to-group" data-id="${W(e.id)}" title="新增成果" aria-label="新增成果">${C.plus}</button>
                        <button type="button" class="studio-icon-button" data-action="rename-group" data-id="${W(e.id)}" title="编辑分组" aria-label="编辑分组">${C.edit}</button>
                      </div>`:``}
                    </div>
                    <div class="group-menu" aria-label="分组排序操作">
                      ...
                      ${Qn(e,c)}
                      ${e.kind===`topic`?`<button type="button" class="studio-icon-button group-delete-button" data-action="delete-group" data-id="${W(e.id)}" title="删除分组" aria-label="删除分组">${C.minus}</button>`:``}
                    </div>
                  </header>
                  <div class="group-cards">
                    ${e.reports.length?e.reports.map(e=>qn(e)).join(``):e.kind===`topic`?`<button class="empty-topic-drop" type="button" data-action="add-to-group" data-id="${W(e.id)}">
      ...
      ${Jn()}
    </main>`}function $(){let e=document.getElementById(`app`),t=D&&T.reports.find(e=>e.id===D);Fn(),e.innerHTML=t?Yn(t):$n(),nr(),ie({render:()=>Q(()=>document.querySelector(`.prompt-composer`)),showToast:K,saveToLibrary:Qt}),Ln()}async function er(e){let t=e.elements.url,n=e.elements.title,r=e.querySelector(`[data-action="detect-title"]`),i=e.querySelector(`.field-hint`),a=t.value.trim();if(!_n(a))return i.textContent=`请输入完整的 http 或 https 网址`,``;r.disabled=!0,r.innerHTML=`<span class="mini-spinner"></span>`,i.textContent=`正在读取网页标题…`;try{let{title:e}=await Xt(a);if(!e)throw Error(`read failed`);return n.value=e,i.textContent=`已识别网页标题`,n.value}catch{let e=gn(a);return n.value||=e,i.textContent=`网页暂时无法读取，已用域名作为标题，你可以手动修改`,n.value}finally{r.disabled=!1,r.textContent=`Detect title`}}function tr(){let e=document.querySelector(`.board`);if(!e)return;let t=null,n=()=>{document.querySelectorAll(`.report-card, .group-column, .topic-nav a`).forEach(e=>{e.classList.remove(`is-nav-drop-target`)})},r=e=>e?[e.bucketKind,e.bucketId,e.targetReportId,Number(e.placeAfter),Number(e.nav)].join(`|`):``,i=()=>{let e=t.sourceCard.getBoundingClientRect(),n=t.sourceCard.cloneNode(!0);return n.removeAttribute(`id`),n.className=`report-card report-drag-preview`,n.style.width=`${e.width}px`,n.style.height=`${e.height}px`,n.querySelectorAll(`button, [role='button'], iframe`).forEach(e=>{e.removeAttribute(`data-action`),e.setAttribute(`tabindex`,`-1`)}),document.body.append(n),t.previewWidth=e.width,t.previewHeight=e.height,t.previewOffsetX=Math.max(18,Math.min(e.width-18,t.startX-e.left)),t.previewOffsetY=Math.max(18,Math.min(e.height-18,t.startY-e.top)),n},a=()=>{if(!t?.preview)return;let e=Math.max(8,window.innerWidth-t.previewWidth-8),n=Math.max(8,window.innerHeight-t.previewHeight-8),r=Math.max(8,Math.min(e,t.x-t.previewOffsetX)),i=Math.max(8,Math.min(n,t.y-t.previewOffsetY));t.preview.style.transform=`translate3d(${r}px, ${i}px, 0)`},o=()=>{!t?.active||t.updateFrame||(t.updateFrame=requestAnimationFrame(()=>{t?.active&&(t.updateFrame=0,a(),u())}))},s=(e,n=null,r=!1)=>{if(!(!e||!t?.placeholder)){if(!n||n.parentElement!==e){e.append(t.placeholder);return}e.insertBefore(t.placeholder,r?n.nextSibling:n)}},c=()=>{let e=document.elementFromPoint(t.x,t.y);if(e?.closest(`.report-card-placeholder`))return t.target;let n=e?.closest(`.topic-nav a[data-nav-bucket-id]`);if(n)return{bucketKind:n.dataset.navBucketKind,bucketId:n.dataset.navBucketId,targetReportId:``,placeAfter:!1,nav:!0,element:n};let r=e?.closest(`.report-card:not(.report-card-placeholder):not(.report-drag-preview)`),i=e?.closest(`.group-column`);if(r&&r!==t.sourceCard){let e=r.closest(`.group-column`);if(!e||e.dataset.bucketKind===`time`)return null;let n=r.getBoundingClientRect(),i=t.y>n.bottom-n.height*.22||t.y>=n.top+n.height*.22&&t.y<=n.bottom-n.height*.22&&t.x>n.left+n.width/2;return{bucketKind:e.dataset.bucketKind||k,bucketId:e.dataset.bucketId||``,targetReportId:r.dataset.reportId||``,placeAfter:i,nav:!1,element:r,container:e.querySelector(`.group-cards`)}}return i&&i.dataset.bucketKind!==`time`?{bucketKind:i.dataset.bucketKind||k,bucketId:i.dataset.bucketId||``,targetReportId:``,placeAfter:!1,nav:!1,element:i,container:i.querySelector(`.group-cards`)}:null},l=e=>{if(!(!t||r(e)===r(t.target))&&(n(),t.target=e,N=e,e)){if(e.nav){e.element.classList.add(`is-nav-drop-target`);return}if(e.targetReportId){s(e.container,e.element,e.placeAfter);return}s(e.container)}},u=()=>{t?.active&&l(c())},d=()=>{let e=Math.min(window.innerHeight*.34,bn()+72),n=window.innerHeight-72;if(t.y<e){let n=Math.min(1,(e-t.y)/84);return-Math.max(1,Math.round(12*n*n))}if(t.y>n){let e=Math.min(1,(t.y-n)/84);return Math.max(1,Math.round(12*e*e))}return 0},f=()=>{if(!t?.active)return;let e=d();if(e){let t=window.scrollY;window.scrollBy(0,e),window.scrollY!==t&&u()}t.autoScrollFrame=requestAnimationFrame(f)},p=()=>{!t||t.active||(clearTimeout(t.holdTimer),t.sourceCard.setPointerCapture?.(t.pointerId),t.active=!0,j=t.reportId,t.preview=i(),t.placeholder=document.createElement(`div`),t.placeholder.className=`report-card report-card-placeholder`,t.placeholder.innerHTML=`<span>放在这里</span>`,t.placeholder.style.minHeight=`${t.sourceCard.getBoundingClientRect().height}px`,t.sourceCard.before(t.placeholder),t.sourceCard.classList.add(`is-dragging`),document.body.classList.add(`report-drag-session`),a(),u(),t.autoScrollFrame=requestAnimationFrame(f))},m=()=>{t&&(clearTimeout(t.holdTimer),t.autoScrollFrame&&cancelAnimationFrame(t.autoScrollFrame),t.updateFrame&&cancelAnimationFrame(t.updateFrame),t.preview?.remove(),t.placeholder?.remove(),t.sourceCard.classList.remove(`is-dragging`),document.body.classList.remove(`report-drag-session`),n(),j=``,N=null)},h=()=>{if(!t)return;let e=t,n=e.active?e.target:null,r=e.reportId;e.active&&(Dt=r,Ot=Date.now()+500),m(),t=null,!(!n?.bucketId||n.bucketKind===`time`)&&ln(r,n.bucketKind,n.bucketId,n.targetReportId||``,!!n.placeAfter)&&(Q(()=>Y(r)),n.nav&&On(()=>J(n.bucketKind,n.bucketId)),K(n.bucketKind===`featured`?`已加入精选成果`:n.bucketKind===`tag`?`已添加目标标签`:n.bucketKind===`type`?`工作类型已更新`:n.targetReportId?`报告顺序已更新`:`已移入新主题`))};e.addEventListener(`pointerdown`,e=>{if(e.button!==0||e.target.closest(`.card-actions`))return;let n=e.target.closest(`.report-card[data-report-draggable="true"]`);n?.closest(`.group-column`)&&(e.pointerType===`mouse`&&e.preventDefault(),t={pointerId:e.pointerId,reportId:n.dataset.reportId,sourceCard:n,startX:e.clientX,startY:e.clientY,x:e.clientX,y:e.clientY,active:!1,target:null,preview:null,placeholder:null,autoScrollFrame:0,updateFrame:0,holdTimer:0},t.holdTimer=window.setTimeout(()=>p(),240))}),e.addEventListener(`pointermove`,e=>{!t||e.pointerId!==t.pointerId||(t.x=e.clientX,t.y=e.clientY,!t.active&&Math.hypot(t.x-t.startX,t.y-t.startY)>=8&&p(),t.active&&(e.preventDefault(),o()))}),e.addEventListener(`pointerup`,e=>{!t||e.pointerId!==t.pointerId||h()}),e.addEventListener(`pointercancel`,()=>{m(),t=null})}function nr(){let e=document.getElementById(`search-input`),t=!1,n=e=>{let t=e?.value||``;if(t===E)return;let n=e.selectionStart,r=e.selectionEnd;E=t,Q(()=>document.querySelector(`.results-toolbar, .archive-search`));let i=document.getElementById(`search-input`);i?.focus({preventScroll:!0}),i?.setSelectionRange(n,r)};e?.addEventListener(`compositionstart`,()=>{t=!0}),e?.addEventListener(`compositionend`,e=>{t=!1,n(e.currentTarget)}),e?.addEventListener(`input`,e=>{e.isComposing||t||n(e.currentTarget)}),e?.addEventListener(`search`,e=>n(e.currentTarget)),e?.addEventListener(`keydown`,e=>{e.key!==`Escape`||!E||(e.preventDefault(),E=``,Q(()=>document.querySelector(`.results-toolbar, .archive-search`)),document.getElementById(`search-input`)?.focus({preventScroll:!0}))}),document.querySelectorAll(`[data-action]`).forEach(e=>{e.addEventListener(`click`,async e=>{let t=e.currentTarget.dataset.action,n=e.currentTarget.dataset.id,r=e.currentTarget.closest(`.report-card`);if(t===`scroll-top`)yn(`smooth`);else if(t===`open`){if(n===Dt&&Date.now()<Ot)return;kt=X(r||Y(n)),D=n,$(),yn()}else if(t===`preview-saved-file`||t===`download-saved-file`){let r=T.reports.find(e=>e.id===n)?.savedFiles?.find(t=>t.id===e.currentTarget.dataset.fileId);if(!r)return;(t===`preview-saved-file`?await zn(r):await Rn(r))||K(`原文件未找到，请重新上传后保存`)}else if(t===`edit-document`){let e=T.reports.find(e=>e.id===n);if(!e||e.access!==`production`)return;tt(e,{render:$,showToast:K})}else if(t===`edit-local-document`){let e=T.reports.find(e=>e.id===n);if(!e||!Bn(e))return;tt(e,{render:$,showToast:K,saveLocal:async t=>{let n=e.savedHtml;e.savedHtml=t,e.isHtml=!0,e.tags=St(e,e.workType),H(e);try{V()}catch{throw e.savedHtml=n,Error(`修改后的 HTML 超过当前浏览器可保存容量，请先下载备份`)}}})}else if(t===`download-report`){let e=T.reports.find(e=>e.id===n);if(!e)return;Bn(e)?Un(e)&&K(`HTML 已下载`):await it(e,K)}else if(t===`share-report`||t===`copy-production-url`){let e=T.reports.find(e=>e.id===n);e?.url&&await at(e,e=>{K(e===`报告链接已复制`?`生产 URL 已复制`:e)})}else if(t===`open-browser`){let e=T.reports.find(e=>e.id===n);if(!e)return;Wn(e)||K(`浏览器未能打开该报告`)}else if(t===`back`)D=``,F=null,Z(kt||{scrollY:0}),kt=null;else if(t===`clear-search`)E=``,Q(()=>document.querySelector(`.results-toolbar, .archive-search`)),document.getElementById(`search-input`)?.focus({preventScroll:!0});else if(t===`set-view`){if(![`topic`,`type`,`tag`,`time`].includes(n))return;k=n,M=``,localStorage.setItem(ft,k),Q()}else if(t===`toggle-time-sort`)A=A===`created`?`modified`:`created`,localStorage.setItem(pt,A),Q();else if(t===`cancel-move`)M=``,Q();else if(t===`move-here`){let t=e.currentTarget.dataset.bucketKind||k;if(M&&ln(M,t,n)){let e=M;M=``,Q(()=>Y(e)),K(t===`tag`?`已添加目标标签`:`报告已移入目标${un()}`)}}else if(t===`show-archive`)O=!0,E=``,D=``,$(),yn();else if(t===`show-catalog`)O=!1,E=``,D=``,$(),yn();else if(t===`add-report`)z=X(document.querySelector(`.results-toolbar`)),F={type:`report`,mode:`create`,groupId:T.groups[0]?.id},Z(z);else if(t===`add-to-group`)z=X(J(`topic`,n)),F={type:`report`,mode:`create`,groupId:n},Z(z);else if(t===`edit`)z=X(r||Y(n)),F={type:`report`,mode:`edit`,reportId:n},Z(z);else if(t===`toggle-pin`){let e=T.reports.find(e=>e.id===n);if(!e)return;let t=e.pinned&&r?.closest(`[data-bucket-kind="featured"]`)?En(n,r):X(r||Y(n));e.pinned=!e.pinned,H(e),V(),Z(t),K(e.pinned?`已加入精选成果`:`已移出精选成果`)}else if(t===`close-modal`)F=null,Z(z||X()),z=null;else if(t===`detect-title`)await er(e.currentTarget.closest(`form`));else if(t===`archive`){let e=T.reports.find(e=>e.id===n);if(!e)return;let t=En(n,r);e.archived=!0,e.archivedAt=new Date().toISOString(),V(),Z(t),K(`已归档，可随时恢复`)}else if(t===`restore`){let e=T.reports.find(e=>e.id===n);if(!e)return;let t=En(n);e.archived=!1,e.archivedAt=``,V(),Z(t),K(`报告已恢复到原主题`)}else if(t===`delete`){let e=T.reports.find(e=>e.id===n);if(e?.archived&&confirm(`二次确认：永久删除“${e.title}”？\n\n删除后无法从归档区恢复。`)){let e=En(n);T.reports=T.reports.filter(e=>e.id!==n),D===n&&(D=``),V(),await Pn(n),Z(e),K(`报告已永久删除`)}}else if(t===`add-group`)z=X(document.querySelector(`.results-toolbar`)),F={type:`group`,mode:`create`},Z(z);else if(t===`rename-group`)T.groups.find(e=>e.id===n)&&(z=X(J(`topic`,n)),F={type:`group`,mode:`edit`,groupId:n},Z(z));else if(t===`move-group`){let t=e.currentTarget.dataset.bucketKind,r=e.currentTarget.dataset.direction,i=X(J(t,n));en(n,r,t)&&(Z(i),requestAnimationFrame(()=>{document.querySelector(`[data-action="move-group"][data-id="${CSS.escape(n)}"][data-direction="${CSS.escape(r)}"]`)?.focus({preventScroll:!0})}),K(`分组顺序已更新`))}else if(t===`delete-group`){let e=T.groups.find(e=>e.id===n),t=T.groups.find(e=>e.id!==n);if(e&&!t)K(`请先新增另一个分组，再删除当前分组`);else if(e&&confirm(`删除“${e.name}”？其中的报告会移到“${t.name}”。`)){let e=Dn(`topic`,n);T.reports.forEach(e=>{e.groupId===n&&(e.groupId=t.id)}),T.groups=T.groups.filter(e=>e.id!==n),V(),Z(e),K(`分组已删除，报告已移到“${t.name}”`)}}})}),document.querySelector(`.topbar`)?.addEventListener(`click`,e=>{e.target.closest(`button, a`)||yn(`smooth`)}),document.querySelectorAll(`.topic-nav a[data-nav-bucket-id]`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault();let n=e.dataset.navBucketKind,r=e.dataset.navBucketId;if(E){E=``,Q(),On(()=>J(n,r));return}xn(J(n,r))})}),document.querySelectorAll(`.topic-nav a[data-nav-report-id]`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault(),xn(Y(e.dataset.navReportId))})}),tr(),document.querySelectorAll(`.legacy-report-drag-handle`).forEach(e=>{let t=null,n=!1,r=null,i=0,a=0,o=()=>{i&&=(cancelAnimationFrame(i),0)},s=()=>{if(!j)return o();let e=Math.min(110,window.innerHeight*.18),t=a<e?a-e:a>window.innerHeight-e?a-(window.innerHeight-e):0;if(!t)return o();let n=Math.sign(t)*Math.min(24,5+Math.abs(t)*.2);window.scrollBy(0,n),i=requestAnimationFrame(s)},c=t=>{let n=e.closest(`.report-card`);if(n){if(!r){let e=n.getBoundingClientRect();r=n.cloneNode(!0),r.className=`report-card report-drag-preview`,r.style.width=`${e.width}px`,r.style.height=`${e.height}px`,r.querySelectorAll(`button, [role='button']`).forEach(e=>{e.removeAttribute(`data-action`),e.setAttribute(`tabindex`,`-1`)}),document.body.append(r)}r.style.transform=`translate3d(${t.clientX+16}px, ${t.clientY+16}px, 0)`}},l=()=>{N=null,P?.remove(),P=null,document.querySelectorAll(`.report-card, .group-column, .topic-nav a`).forEach(e=>e.classList.remove(`is-card-drop-target`,`is-card-drop-before`,`is-card-drop-after`,`is-drop-ready`,`is-nav-drop-target`))},u=(t,n,r)=>{if(t){if(!P){P=document.createElement(`div`),P.className=`report-card report-card-placeholder`,P.innerHTML=`<span>放在这里</span>`;let t=e.closest(`.report-card`)?.getBoundingClientRect().height;t&&(P.style.minHeight=`${t}px`)}if(!n||n.parentElement!==t){t.append(P);return}t.insertBefore(P,r?n.nextSibling:n)}},d=t=>{let n=document.elementFromPoint(t.clientX,t.clientY),r=n?.closest(`.topic-nav a[data-nav-bucket-id]`);if(l(),r){r.classList.add(`is-nav-drop-target`),N={bucketKind:r.dataset.navBucketKind,bucketId:r.dataset.navBucketId,targetReportId:``,placeAfter:!1,nav:!0};return}let i=e.closest(`.report-card`),a=n?.closest(`.report-card:not(.report-card-placeholder)`),o=n?.closest(`.group-column`);if(a&&a!==i){let e=a.closest(`.group-column`),n=a.getBoundingClientRect(),r=t.clientY>n.top+n.height/2;a.classList.add(`is-card-drop-target`,r?`is-card-drop-after`:`is-card-drop-before`),u(e?.querySelector(`.group-cards`),a,r),N={bucketKind:e?.dataset.bucketKind||k,bucketId:e?.dataset.bucketId||``,targetReportId:a.dataset.reportId||``,placeAfter:r,nav:!1};return}o&&o.dataset.bucketKind!==`time`&&(o.classList.add(`is-drop-ready`),u(o.querySelector(`.group-cards`),null,!1),N={bucketKind:o.dataset.bucketKind||k,bucketId:o.dataset.bucketId||``,targetReportId:``,placeAfter:!1,nav:!1})},f=()=>{j=``,t=null,n=!1,o(),r?.remove(),r=null,l(),e.closest(`.report-card`)?.classList.remove(`is-dragging`)};e.addEventListener(`pointerdown`,r=>{r.preventDefault(),j=e.dataset.reportDragId,t={x:r.clientX,y:r.clientY},n=!1,e.setPointerCapture?.(r.pointerId),e.closest(`.report-card`)?.classList.add(`is-dragging`)}),e.addEventListener(`pointermove`,e=>{j&&(t&&Math.hypot(e.clientX-t.x,e.clientY-t.y)<7||(n=!0,a=e.clientY,c(e),d(e),i||=requestAnimationFrame(s)))}),e.addEventListener(`pointerup`,e=>{if(!j)return;let t=j;if(!n){M=t,f(),Q(()=>Y(t)),K(`请选择目标${un()}`);return}let r=N,i=r?.targetReportId||``,a=r?.bucketId||``,o=r?.bucketKind||k,s=a&&o!==`time`?ln(t,o,a,i,!!r?.placeAfter):!1;f(),s&&(Q(()=>Y(t)),requestAnimationFrame(()=>{let e=`.group-column[data-bucket-kind="${CSS.escape(o)}"][data-bucket-id="${CSS.escape(a)}"]`,n=document.querySelector(`${e} .report-card[data-report-id="${CSS.escape(t)}"]`)||document.querySelector(`.search-results-cards .report-card[data-report-id="${CSS.escape(t)}"]`);xn(n),n?.classList.add(`is-drop-landed`),window.setTimeout(()=>n?.classList.remove(`is-drop-landed`),900)}),K(o===`featured`?`已加入精选成果`:o===`tag`?`已添加目标标签`:o===`type`?`工作类型已更新`:i?`报告顺序已更新`:`已移入新主题`))}),e.addEventListener(`pointercancel`,f)});let r=document.getElementById(`group-form`);r?.addEventListener(`submit`,e=>{e.preventDefault();let t=new FormData(r).get(`name`)?.trim(),n=new FormData(r).get(`description`)?.trim();if(!t)return;if(F.mode===`edit`){let e=T.groups.find(e=>e.id===F.groupId);if(!e)return;e.name=t.slice(0,60),e.description=n?.slice(0,80)||`自定义工作主题`}else T.groups.push({id:pn(`group`),name:t.slice(0,60),description:n?.slice(0,80)||`自定义工作主题`,accent:[`blue`,`violet`,`amber`,`green`][T.groups.length%4],position:T.groups.length}),k=`topic`,localStorage.setItem(ft,k);V();let i=F.mode===`edit`?`工作主题已更新`:`工作主题已创建，可直接拖入报告`;F=null,Z(z||X()),z=null,K(i)});let i=document.getElementById(`report-form`),a=i?.elements.tags,o=e=>{let t=G(a?.value).includes(e.dataset.reportTag);e.classList.toggle(`selected`,t),e.setAttribute(`aria-pressed`,String(t))},s=e=>{e.addEventListener(`click`,()=>{let t=G(a.value),n=e.dataset.reportTag;a.value=t.includes(n)?t.filter(e=>e!==n).join(`、`):[...t,n].slice(0,8).join(`、`),o(e)})};i?.querySelectorAll(`[data-report-tag]`).forEach(s);let c=i?.querySelector(`[data-add-report-tag]`),l=i?.querySelector(`.new-report-tag-row`),u=i?.querySelector(`[data-new-report-tag]`),d=()=>{l.hidden=!1,u.focus()},f=()=>{let[e]=G(u.value);if(!e)return;let t=G(a.value);if(!t.includes(e)&&t.length>=8){K(`最多选择 8 个标签`);return}a.value=[...new Set([...t,e])].slice(0,8).join(`、`);let n=[...i.querySelectorAll(`[data-report-tag]`)].find(t=>t.dataset.reportTag===e);n||(n=document.createElement(`button`),n.type=`button`,n.dataset.reportTag=e,n.textContent=e,c.before(n),s(n)),o(n),u.value=``,l.hidden=!0,c.focus()};c?.addEventListener(`click`,d),i?.querySelector(`[data-confirm-report-tag]`)?.addEventListener(`click`,f),u?.addEventListener(`keydown`,e=>{e.key===`Enter`?(e.preventDefault(),f()):e.key===`Escape`&&(e.preventDefault(),u.value=``,l.hidden=!0,c.focus())}),i?.addEventListener(`submit`,async e=>{e.preventDefault();let t=i.elements.url.value.trim(),n=F.mode===`edit`?F.reportId:``,r=n?T.reports.find(e=>e.id===n):null,a=!!(r&&!r.url&&!t);if(!_n(t)&&!a)return;let o=i.querySelector(`button[type="submit"]`),s=i.querySelector(`.field-hint`);if(o.disabled=!0,o.innerHTML=`<span class="mini-spinner"></span>`,a){let e=i.elements.title.value.trim()||r.title,t=i.elements.groupId.value,n=i.elements.workType.value,a=G(i.elements.tags.value).filter(e=>![`HTML`,`手动保存`,`生产`].includes(e));Object.assign(r,{title:e,groupId:t,workType:n,tags:a}),H(r),V(),F=null,Z(z||X()),z=null,K(`报告已保存`);return}let c=Gt({material:t,files:[],url:t,excludeId:n});if(c){o.disabled=!1,o.textContent=`Save`,s.textContent=`成果库已有“${c.title}”，未重复保存`,K(`成果库已有“${c.title}”，未重复保存`);return}let l=await Zt({material:t,files:[],url:t},e=>{s.textContent=e});if(!l.allowed){o.disabled=!1,o.textContent=`Save`,s.textContent=l.reason,K(l.reason);return}let u=i.elements.title.value.trim()||l.metadata.title,d=i.elements.groupId.value,f=i.elements.workType.value,p=G(i.elements.tags.value),m={title:u||gn(t),url:t,groupId:d,workType:f,source:`手动添加`,access:l.access,detectedDescription:l.metadata.description,manualSaved:!0,isProduction:l.access===`production`,isPersonal:Kt(t),isHtml:l.isHtml,loginProvider:l.loginProvider},h=F.mode===`edit`?p:[...new Set([...St(m,f),...p])].slice(0,8);if(F.mode===`edit`){let e=T.reports.find(e=>e.id===F.reportId);Object.assign(e,m,{tags:h}),H(e)}else{let e=new Date().toISOString(),t={id:pn(`report`),groupId:d,...m,pinned:!1,position:T.reports.filter(e=>e.groupId===d).length,createdAt:e,modifiedAt:e,archived:!1,archivedAt:``,tags:h};T.reports.push(t)}V(),F=null,Z(z||X()),z=null,K(`报告已保存`)});let p=D&&T.reports.find(e=>e.id===D);p&&rt(p)}function rr(){Pt(),$()}rr(document.getElementById(`app`));