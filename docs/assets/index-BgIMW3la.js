const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./lib-20jBwWzq.js","./jszip.min-BzLaYm7C.js","./pptx-preview.es-DzmrXZc6.js"])))=>i.map(i=>d[i]);
...
`)}function At(e=``){let t=document.createElement(`template`);return t.innerHTML=String(e),t.content.querySelectorAll(`script, style, link, meta, base, iframe, frame, object, embed, form, input, button, textarea, select`).forEach(e=>e.remove()),t.content.querySelectorAll(`*`).forEach(e=>{[...e.attributes].forEach(t=>{let n=t.name.toLowerCase(),r=t.value.trim().toLowerCase();(n.startsWith(`on`)||n===`srcdoc`||(n===`href`||n===`src`||n===`xlink:href`)&&/^(javascript|vbscript):/.test(r))&&e.removeAttribute(t.name)})}),t.innerHTML}function jt(e,t,n=`document`){return`<!doctype html>
  ...
  </div>`}function er(e){let t=z.size?new Set(z):new Set(H.map(e=>e.id)),n=new Map(H.map(({id:e})=>[e,[]]));return e.forEach(e=>{let r=H.find(({id:n})=>t.has(n)&&e.dimensions.includes(n));r&&n.get(r.id).push(e)}),`<div class="search-priority-groups" aria-label="按最高匹配优先级排列的搜索结果">
    ${H.map((e,t)=>({...e,priority:String(t+1).padStart(2,`0`),hits:n.get(e.id).sort(Qn)})).filter(({id:e})=>t.has(e)).filter(e=>e.hits.length).map(e=>`<section class="search-priority-group" data-search-dimension="${e.id}">
      ...
      <div class="group-cards search-results-cards">
        ${e.hits.map(({report:e})=>Wr(e)).join(``)}
      </div>
    </section>`).join(``)}
  </div>`}function tr(e){return`${e}-${crypto.randomUUID?.()||`${Date.now()}-${Math.random()}`}`}function G(e=``){return String(e).replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`).replaceAll(`'`,`&#39;`)}var nr={back:`
    <svg viewBox="0 0 24 24" aria-hidden="true">
    ...
    </svg>`};function rr(e){return nr[e]||``}function ir(e){try{return new URL(e).hostname.replace(/^www\./,``)}catch{return e}}function ar(e){try{return[`http:`,`https:`].includes(new URL(e).protocol)}catch{return!1}}function K(e=``){return[...new Set(String(e).split(/[、,，;；\n]+/).map(e=>e.trim()).filter(Boolean).map(e=>e.slice(0,20)))].slice(0,8)}function or(){let e=new Set($t);return T.reports.forEach(t=>{(t.tags||[]).forEach(t=>e.add(t))}),[...e].filter(e=>![`HTML`,`手动保存`,`生产`].includes(e))}function q(e,{duration:t=2600,actionLabel:n=``,onAction:r=null}={}){document.querySelector(`.toast`)?.remove();let i=document.createElement(`div`);i.className=`toast`,i.setAttribute(`role`,`status`);let a=document.createElement(`span`);if(a.textContent=e,i.append(a),n&&typeof r==`function`){let e=document.createElement(`button`);e.type=`button`,e.className=`toast-action`,e.textContent=n,e.addEventListener(`click`,()=>{clearTimeout(cn),i.remove(),r()}),i.append(e)}document.body.append(i),clearTimeout(cn),cn=window.setTimeout(()=>i.remove(),t)}function sr(e,t){q(e,{duration:8e3,actionLabel:`撤销`,onAction:()=>{t(),q(`已撤销刚才的操作`)}})}function cr(e){let t=e?.closest?.(`[data-action]`);return t?{action:t.dataset.action||``,id:t.dataset.id||``,bucketKind:t.dataset.bucketKind||``,direction:t.dataset.direction||``}:null}function lr(e){if(!e?.action)return null;let t=[`[data-action="${CSS.escape(e.action)}"]`,e.id?`[data-id="${CSS.escape(e.id)}"]`:``,e.bucketKind?`[data-bucket-kind="${CSS.escape(e.bucketKind)}"]`:``,e.direction?`[data-direction="${CSS.escape(e.direction)}"]`:``].join(``);return document.querySelector(t)}function ur(e,t,n){hn=t||Z(),ln=cr(n||document.activeElement),un=window.scrollY,I=e,Q(hn)}function J({fallbackSelector:e=`.results-toolbar, .archive-search, .reader-header`}={}){if(!I)return;let t=hn||{scrollY:un},n=ln;I=null,Q(t),hn=null,ln=null,requestAnimationFrame(()=>{let t=document.querySelector(e);(lr(n)||t?.querySelector(`button, input, [tabindex]:not([tabindex='-1'])`)||t)?.focus?.({preventScroll:!0})})}function dr(){let e=document.querySelector(`.app-shell > .dialog-backdrop`);if(e?document.body.style.setProperty(`--studio-modal-scroll-top`,`${-un}px`):document.body.style.removeProperty(`--studio-modal-scroll-top`),document.body.classList.toggle(`studio-modal-open`,!!e),!e)return;[...e.parentElement.children].forEach(t=>{t!==e&&(t.inert=!0,t.setAttribute(`aria-hidden`,`true`))});let t=e.querySelector(`[role="dialog"]`);if(!t)return;let n=()=>[...t.querySelectorAll([`a[href]`,`button:not([disabled])`,`input:not([disabled]):not([type='hidden'])`,`select:not([disabled])`,`textarea:not([disabled])`,`[tabindex]:not([tabindex='-1'])`].join(`,`))].filter(e=>!e.hidden&&e.getClientRects().length);e.addEventListener(`keydown`,e=>{if(e.key===`Escape`){e.preventDefault(),J();return}if(e.key!==`Tab`)return;let r=n();if(!r.length){e.preventDefault(),t.focus({preventScroll:!0});return}let i=r[0],a=r.at(-1);e.shiftKey&&document.activeElement===i?(e.preventDefault(),a.focus({preventScroll:!0})):!e.shiftKey&&document.activeElement===a&&(e.preventDefault(),i.focus({preventScroll:!0}))}),e.addEventListener(`click`,t=>{t.target===e&&J()}),requestAnimationFrame(()=>{(t.querySelector(`[autofocus]`)||n()[0]||t)?.focus?.({preventScroll:!0})})}function fr(e=`auto`){pr(),R=requestAnimationFrame(()=>{R=0,window.scrollTo({top:0,left:0,behavior:e})})}function pr(){L&&cancelAnimationFrame(L),R&&cancelAnimationFrame(R),dn&&cancelAnimationFrame(dn),L=0,R=0,dn=0}function mr(){let e=document.querySelector(`.topbar`)?.getBoundingClientRect().bottom||0,t=document.querySelector(`.topic-nav`),n=t?getComputedStyle(t):null,r=t?.getBoundingClientRect();if(!window.matchMedia(`(max-width: 840px)`).matches){let t=n?.position===`sticky`&&Number.parseFloat(n.top)||0;return Math.max(e+22,t)}let i=n?.position===`sticky`&&r?.bottom>0?r.bottom:0;return Math.max(e,i)+10}function hr(e,t=`smooth`){if(!e)return;pr();let n=Math.max(0,document.documentElement.scrollHeight-window.innerHeight),r=Math.max(0,Math.min(n,window.scrollY+e.getBoundingClientRect().top-mr())),i=window.scrollY,a=r-i;if(Math.abs(a)<2)return;if(t!==`smooth`||matchMedia(`(prefers-reduced-motion: reduce)`).matches){window.scrollTo({top:r,left:0,behavior:`auto`});return}let o=Math.min(360,Math.max(180,Math.abs(a)*.22)),s=performance.now(),c=t=>{if(!e.isConnected){L=0;return}let n=Math.min(1,(t-s)/o),r=1-(1-n)**3,l=Math.max(0,Math.min(Math.max(0,document.documentElement.scrollHeight-window.innerHeight),window.scrollY+e.getBoundingClientRect().top-mr()));a=l-i,window.scrollTo(0,i+a*r),n<1?L=requestAnimationFrame(c):(L=0,window.scrollTo(0,l))};L=requestAnimationFrame(c)}function Y(e,t){return document.querySelector(`.group-column[data-bucket-kind="${CSS.escape(e)}"][data-bucket-id="${CSS.escape(t)}"]`)}function X(e){return document.querySelector(`.board .report-card[data-report-id="${CSS.escape(e)}"]`)}function gr(e){if(!e)return null;let t=e.closest?.(`.report-card[data-report-id]`);if(t){let e=t.closest(`.group-column[data-bucket-kind][data-bucket-id]`);return{type:`report`,id:t.dataset.reportId,bucketKind:e?.dataset.bucketKind||``,bucketId:e?.dataset.bucketId||``}}let n=e.closest?.(`.group-column[data-bucket-kind][data-bucket-id]`);if(n)return{type:`bucket`,kind:n.dataset.bucketKind,id:n.dataset.bucketId};let r=e.closest?.(`.results-toolbar, .archive-search, .prompt-composer, .groups-section, .library-layout`);return r?{type:`selector`,selector:r.classList.contains(`results-toolbar`)?`.results-toolbar`:r.classList.contains(`archive-search`)?`.archive-search`:r.classList.contains(`prompt-composer`)?`.prompt-composer`:r.classList.contains(`groups-section`)?`.groups-section`:`.library-layout`}:null}function _r(e){return e?e.type===`report`?(e.bucketKind&&e.bucketId?Y(e.bucketKind,e.bucketId):null)?.querySelector(`.report-card[data-report-id="${CSS.escape(e.id)}"]`)||X(e.id):e.type===`bucket`?Y(e.kind,e.id):e.type===`selector`?document.querySelector(e.selector):null:null}function vr(){let e=mr(),t=t=>[...document.querySelectorAll(t)].filter(t=>{let n=t.getBoundingClientRect();return n.bottom>e&&n.top<window.innerHeight}).sort((t,n)=>Math.abs(t.getBoundingClientRect().top-e)-Math.abs(n.getBoundingClientRect().top-e))[0];return t(`.board .report-card[data-report-id]`)||t(`.group-column[data-bucket-id]`)||t(`.results-toolbar, .archive-search, .prompt-composer`)||document.querySelector(`.results-toolbar, .archive-search, .groups-section, .library-layout`)}function Z(e=null){pr();let t=e||vr();return{scrollY:window.scrollY,identity:gr(t),viewportTop:t?.getBoundingClientRect().top??null}}function yr(e){if(!e)return;pr();let t=()=>{let t=_r(e.identity),n=Math.max(0,document.documentElement.scrollHeight-window.innerHeight),r=t&&Number.isFinite(e.viewportTop)?window.scrollY+t.getBoundingClientRect().top-e.viewportTop:e.scrollY;window.scrollTo({top:Math.max(0,Math.min(n,r)),left:0,behavior:`auto`})};t(),dn=requestAnimationFrame(()=>{dn=requestAnimationFrame(()=>{dn=0,t()})})}function Q(e){Zr(),yr(e)}function br(e,t=null){let n=t||X(e),r=n?.nextElementSibling?.matches?.(`.report-card[data-report-id]`)?n.nextElementSibling:n?.previousElementSibling?.matches?.(`.report-card[data-report-id]`)?n.previousElementSibling:null;if(!r){let e=n?.closest(`.group-column[data-bucket-id]`),t=e?.nextElementSibling?.matches?.(`.group-column[data-bucket-id]`)?e.nextElementSibling:e?.previousElementSibling?.matches?.(`.group-column[data-bucket-id]`)?e.previousElementSibling:null;r=t?.querySelector(`.report-card[data-report-id]`)||t||n}return Z(r)}function xr(e,t){let n=Y(e,t);return Z(n?.nextElementSibling?.matches?.(`.group-column[data-bucket-id]`)?n.nextElementSibling:n?.previousElementSibling?.matches?.(`.group-column[data-bucket-id]`)?n.previousElementSibling:n)}function Sr(e,t=`smooth`){pr(),R=requestAnimationFrame(()=>{R=requestAnimationFrame(()=>{R=0,hr(e(),t)})})}function $(e=null){Q(Z(typeof e==`function`?e():e))}function Cr(){if(O||k||I)return!1;let e=document.querySelector(`.results-toolbar-side`),t=e?.querySelector(`.results-search`),n=document.querySelector(`.groups-section`);if(!e||!t||!n)return!1;let r=document.createElement(`template`);r.innerHTML=Xr();let i=r.content.querySelector(`.results-toolbar-side`),a=i?.querySelector(`.results-search`),o=r.content.querySelector(`.groups-section`);if(!i||!a||!o)return!1;t.querySelector(`.search-clear-button`)?.remove();let s=a.querySelector(`.search-clear-button`);return s&&t.append(s),[...e.children].filter(e=>e!==t).forEach(e=>e.remove()),[...i.children].filter(e=>!e.classList.contains(`results-search`)).forEach(t=>e.append(t)),e.className=i.className,n.replaceWith(o),ei(),Nr(),!0}function wr(){let e=window.scrollY;if(!Cr()){Q({scrollY:e});return}window.scrollTo({top:e,left:0,behavior:`auto`})}[`wheel`,`touchstart`,`pointerdown`].forEach(e=>{window.addEventListener(e,pr,{passive:!0})});var Tr=null,Er=new Set;function Dr(){return Tr||(Tr=new Promise((e,t)=>{if(!window.indexedDB){t(Error(`IndexedDB unavailable`));return}let n=indexedDB.open(Xt,1);n.onupgradeneeded=()=>{let e=n.result;e.objectStoreNames.contains(S)||e.createObjectStore(S,{keyPath:`id`}).createIndex(`reportId`,`reportId`,{unique:!1})},n.onsuccess=()=>e(n.result),n.onerror=()=>t(n.error||Error(`File database failed`))}),Tr)}async function Or(e,t=[]){let n=t.map(t=>{let{blob:n,...r}=t;return{...r,storageId:n instanceof Blob?`${e}:${t.id}`:t.storageId||``,blob:n}}),r=n.filter(e=>e.blob instanceof Blob&&e.storageId);if(r.length){let t=await Dr();await new Promise((n,i)=>{let a=t.transaction(S,`readwrite`),o=a.objectStore(S);r.forEach(t=>{o.put({id:t.storageId,reportId:e,name:t.name,type:t.type,size:t.size,blob:t.blob,updatedAt:new Date().toISOString()})}),a.oncomplete=n,a.onerror=()=>i(a.error||Error(`File save failed`)),a.onabort=()=>i(a.error||Error(`File save aborted`))})}return n.map(({blob:e,...t})=>t)}async function kr(e){if(e?.storageId)try{let t=await Dr(),n=await new Promise((n,r)=>{let i=t.transaction(S,`readonly`).objectStore(S).get(e.storageId);i.onsuccess=()=>n(i.result),i.onerror=()=>r(i.error||Error(`File read failed`))});if(n?.blob instanceof Blob)return n.blob}catch{return null}let t=e?.content||e?.excerpt;return t?new Blob([t],{type:e.type||`text/plain;charset=utf-8`}):null}async function Ar(e){try{let t=await Dr();await new Promise((n,r)=>{let i=t.transaction(S,`readwrite`),a=i.objectStore(S),o=a.index(`reportId`).openKeyCursor(IDBKeyRange.only(e));o.onsuccess=()=>{let e=o.result;e&&(a.delete(e.primaryKey),e.continue())},o.onerror=()=>r(o.error||Error(`File cleanup failed`)),i.oncomplete=n,i.onerror=()=>r(i.error||Error(`File cleanup failed`))})}catch{}}function jr(){Er.forEach(e=>URL.revokeObjectURL(e)),Er.clear()}function Mr(e){let t=URL.createObjectURL(e);return Er.add(t),t}async function Nr(){let e=[...document.querySelectorAll(`[data-saved-file-preview]`)];await Promise.all(e.map(async e=>{let t=T.reports.find(t=>t.id===e.dataset.reportId)?.savedFiles?.find(t=>t.id===e.dataset.fileId);if(!t)return;e.classList.add(`is-loading`),e.setAttribute(`aria-busy`,`true`);let n=await kr(t);if(!n||!e.isConnected){e.classList.remove(`is-loading`),e.setAttribute(`aria-busy`,`false`);return}let r=e.dataset.previewMode;try{if(r===`image`){let r=document.createElement(`img`);r.src=Mr(n),r.alt=t.name||`图片预览`,r.draggable=!1,e.replaceChildren(r)}else if(r===`html`){let r=document.createElement(`iframe`);r.src=Mr(n),r.title=`${t.name||`HTML`}内容`,r.setAttribute(`sandbox`,`allow-forms allow-modals allow-popups allow-scripts`),e.replaceChildren(r)}else await Lt(e,n,t,r);e.classList.add(`is-ready`)}catch(t){let n=document.createElement(`div`);n.className=`saved-file-render-error`;let r=document.createElement(`strong`);r.textContent=`暂时无法直接显示这个文件`;let i=document.createElement(`p`);i.textContent=t?.message||`请下载原文件后使用对应应用打开`,n.append(r,i),e.replaceChildren(n),e.classList.add(`has-error`)}finally{e.classList.remove(`is-loading`),e.setAttribute(`aria-busy`,`false`)}}))}async function Pr(e){let t=await kr(e);if(!t)return!1;let n=Mr(t),r=document.createElement(`a`);return r.href=n,r.download=e.name||`download`,document.body.append(r),r.click(),r.remove(),window.setTimeout(()=>{URL.revokeObjectURL(n),Er.delete(n)},1e3),!0}async function Fr(e){let t=window.open(``,`_blank`),n=await kr(e);if(!n)return t?.close(),!1;let r=Mr(n);return t?t.location.href=r:window.open(r,`_blank`,`noopener,noreferrer`),window.setTimeout(()=>{URL.revokeObjectURL(r),Er.delete(r)},6e4),!0}function Ir(e){return e.savedHtml||Pn(e.savedContent,e.savedFiles)}function Lr(e){return`${String(e.title||`report`).replace(/[\\/:*?"<>|]+/g,`-`).replace(/\s+/g,` `).trim().slice(0,80)||`report`}.html`}function Rr(e){let t=Ir(e);return t?URL.createObjectURL(new Blob([t],{type:`text/html;charset=utf-8`})):``}function zr(e){let t=Rr(e);if(!t)return!1;let n=document.createElement(`a`);return n.href=t,n.download=Lr(e),document.body.append(n),n.click(),n.remove(),window.setTimeout(()=>URL.revokeObjectURL(t),1e3),!0}function Br(e){let t=e.url||Rr(e);return t?(window.open(t,`_blank`,`noopener,noreferrer`),e.url||window.setTimeout(()=>URL.revokeObjectURL(t),6e4),!0):!1}function Vr(e,t,n=!1){let r=g(t),i=t.format||r.label,a=!!(t.storageId&&[`image`,`pdf`,`html`].includes(r.preview)),o=n&&r.preview===`pdf`?`pdf-thumb`:r.preview,s=a?`data-saved-file-preview data-report-id="${G(e.id)}" data-file-id="${G(t.id)}" data-preview-mode="${o}"`:``,c=r.preview===`text`&&(t.excerpt||t.content)?`<pre>${G((t.excerpt||t.content).slice(0,n?280:8e3))}</pre>`:``;return`
    ...
    </div>`}function Hr(e,t,n,r){let i=g(t),a=t.format||i.label,o=t.storageId||t.content||t.excerpt?`data-saved-file-preview data-report-id="${G(e.id)}" data-file-id="${G(t.id)}" data-preview-mode="${G(i.preview)}"`:``;return`<article class="saved-file-embed file-kind-${i.kind}">
  ...
  </article>`}function Ur(e){let t=e.savedFiles||[];return t.length?`<section class="saved-file-list embedded-file-list" aria-label="档案正文">
    <div class="embedded-file-list-heading"><strong>档案正文</strong><span>${t.length} 个文件 · 已直接展开</span></div>
    ${t.map((n,r)=>Hr(e,n,r,t.length)).join(``)}
  </section>`:``}function Wr(e,t=!1){let n=!e.url&&(!!e.savedContent||!!(e.savedFiles||[]).length),r=[`org`,`account`].includes(e.access),i=e.access===`org`?`需组织登录`:e.access===`account`?`需账号登录`:`生产可访问`,a=Ir(e),o=!!e.pinned,s=T.groups.find(t=>t.id===e.groupId)?.name||`未归类`,c=new Set([`HTML`,`手动保存`,`生产`]),l=[...new Set([s,Gn(e.workType),...e.tags||[]])].filter(e=>!c.has(e)),u=!r&&en.reports.some(t=>t.id===e.id),d=e.preview||`${e.id}.png`,f=(e.savedFiles||[])[0],p=a&&e.isHtml?`<iframe class="local-html-preview-frame" title="${G(e.title)}视觉预览"
        srcdoc="${G(a)}" sandbox="allow-scripts" loading="lazy"
        tabindex="-1" aria-hidden="true"></iframe>`:u?`<img src="./previews/${G(d)}" alt="" loading="lazy" decoding="async" draggable="false" />`:n&&f?Vr(e,f,!0):`
    ...
    </article>`}function Gr(){if(!I)return``;if(I.type===`delete-report`){let e=T.reports.find(e=>e.id===I.reportId);return e?`
      ...
      <form class="dialog" id="report-form" role="dialog" aria-modal="true"
        ...
        <label>工作类型
          <select name="workType">
            ${Qt.map(t=>`<option value="${G(t.id)}" ${t.id===(e?.workType||`product-planning`)?`selected`:``}>${G(t.name)}</option>`).join(``)}
          </select>
        </label>
    ...
    </div>`}function Kr(e){if(_t(e.id))return yt(e,G);let t=!e.url&&(!!e.savedContent||!!(e.savedFiles||[]).length),n=[`org`,`account`].includes(e.access),r=e.loginProvider||Fn(e.url)?.provider||(e.access===`org`?`组织帐号`:`站点帐号`),i=e.savedHtml||Pn(e.savedContent,e.savedFiles),a=i?`edit-local-document`:e.url?n?`edit`:`edit-document`:``,o=i?`
...
`,`<br />`)}</div>`:``}
      ...
      <div class="login-handoff-wrap">
        <section class="login-handoff-card" aria-labelledby="login-handoff-title">
          ...
          <p class="login-handoff-domain">${G(ir(e.url))}</p>
        </section>
      </div>`:`
      <div class="reader-frame-wrap">
    ...
    <main class="reader-shell compact-reader-shell">
      <header class="reader-header compact-reader-header">
        <button class="reader-icon-button back-button" type="button" data-action="back"
          aria-label="返回成果库" title="返回成果库">${rr(`back`)}</button>
        <div class="reader-title">
          <strong>${G(e.title)}</strong>
          <span>${t?`本地保存`:G(ir(e.url))}</span>
        </div>
    ...
    </header>`}function Jr(){let e=T.reports.filter(e=>e.archived).filter(e=>Wt(e,D,{group:T.groups.find(t=>t.id===e.groupId),workTypeName:Gn(e.workType)})).sort((e,t)=>new Date(t.archivedAt||0)-new Date(e.archivedAt||0)),t=T.reports.filter(e=>e.archived).length;return`
    <main class="app-shell archive-shell">
      ...
      <section class="workspace archive-workspace">
        ...
        ${e.length?`
          <section class="archive-results">
            ...
            <div class="archive-grid">${e.map(e=>Wr(e,!0)).join(``)}</div>
          </section>`:`
      ...
      ${Gr()}
    </main>`}function Yr(e,t){if(![`topic`,`type`,`tag`].includes(e.kind))return``;let n=t.filter(t=>t.kind===e.kind),r=n.findIndex(t=>t.id===e.id);if(r<0)return``;let i=r===0,a=r===n.length-1,o=(t,n,r,i)=>`
    <button type="button" class="studio-icon-button group-order-button" data-action="move-group"
      ...
      title="${n}" aria-label="${n}" ${i?`disabled`:``}>${r}</button>`;return[o(`top`,`置顶`,w.top,i),o(`up`,`上移`,w.up,i),o(`down`,`下移`,w.down,a),o(`bottom`,`置底`,w.bottom,a)].join(``)}function Xr(){if(k)return Jr();let e=x(D),t=T.reports.filter(e=>!e.archived),n=e?Zn(t,e):[],r=z.size?n.filter(e=>e.dimensions.some(e=>z.has(e))):n,i=Hn(t.filter(e=>e.pinned),`featured`,`featured`,(e,t)=>qn(t)-qn(e)),a={id:`featured`,name:`精选成果`,kind:`featured`,accent:`violet`,reports:i},o=T.reports.filter(e=>e.archived).length,s=t.filter(e=>e.access===`production`).length,c=t.filter(e=>e.access!==`production`).length,l=Jn(t,``),u=A===`topic`&&i.length?[a,...l]:l,d=A===`time`&&l[0]?.reports||[],f=e?[]:(A===`topic`&&i.length?[a,...l]:l).filter(e=>e.reports.length||N||A===`topic`),p=A===`type`?`工作类型`:A===`tag`?`关键标签`:A===`time`?`新增时间`:`工作主题`;return`
      ...
      <section class="workspace">
        ...
        <div class="results-toolbar unified-results-toolbar">
          ...
          <div class="results-toolbar-side ${e?`has-search-results`:``}">
            ...
            ${e?$n(n,r.length):`<div class="studio-summary compact-summary" aria-label="成果统计">
        ...
        <section class="groups-section">
          ${N?`
            <div class="move-mode-banner" role="status">
              <div><strong>正在整理报告</strong><span>选择目标${Xn()}的“移到这里”，或直接拖动卡片。</span></div>
            ...
            <div class="library-layout">
              ...
              ${e?`
                <section class="search-results-panel">
                  ...
                  ${r.length?er(r):n.length?`<div class="no-results search-no-results">
                ...
                <section class="group-column topic-section bucket-${G(e.kind)} accent-${G(e.accent||`blue`)}"
                  ...
                  <div class="group-cards">
                    ${e.reports.length?e.reports.map(e=>Wr(e)).join(``):e.kind===`topic`?`<button class="empty-topic-drop" type="button" data-action="add-to-group" data-id="${G(e.id)}">
