(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))a(r);new MutationObserver(r=>{for(const n of r)if(n.type==="childList")for(const c of n.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&a(c)}).observe(document,{childList:!0,subtree:!0});function i(r){const n={};return r.integrity&&(n.integrity=r.integrity),r.referrerPolicy&&(n.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?n.credentials="include":r.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function a(r){if(r.ep)return;r.ep=!0;const n=i(r);fetch(r.href,n)}})();const xt="clair-ai-studio-tasks-v1",Wt=[{id:"save",name:"保存",hint:"自动识别并进入成果库"},{id:"decision",name:"决策",hint:"发起决策推演"},{id:"review",name:"评审",hint:"自动匹配合适的评审 Skill"}],z=[{id:"requirement",name:"需求评审"},{id:"solution",name:"方案评审"},{id:"decision",name:"决策推演"},{id:"agreement",name:"协议审查"},{id:"career",name:"履历评估"}];let $=st();function st(){return{material:"",files:[]}}function Lt(){var t;return((t=crypto.randomUUID)==null?void 0:t.call(crypto))||`${Date.now()}-${Math.random()}`}function Yt(t){var r;const e=t.toLowerCase(),a=((r=[["agreement",["协议","合同","条款","保密","签署","数据处理"]],["career",["简历","履历","候选人","晋升","岗位","面试"]],["decision",["决策","选型","取舍","是否推进","选择"]],["requirement",["需求","prd","用户故事","验收","原型"]],["solution",["方案","流程","架构","设计","上线"]]].find(([,n])=>n.some(c=>e.includes(c))))==null?void 0:r[0])||"solution";return z.find(n=>n.id===a)||z[1]}function Jt(t){return t<1024?`${t} B`:t<1024*1024?`${Math.ceil(t/1024)} KB`:`${(t/1024/1024).toFixed(1)} MB`}async function M(t){const e=[...t].slice(0,20);return Promise.all(e.map(async i=>{const a=i.type.startsWith("text/")||/\.(md|txt|csv|json|html|xml)$/i.test(i.name);let r="";if(a&&i.size<=1024*1024)try{r=(await i.text()).slice(0,12e3)}catch{r=""}return{id:Lt(),name:i.name,type:i.type||"文件",size:i.size,sizeLabel:Jt(i.size),excerpt:r}}))}function Vt(t){return $.files.length?`<div class="attachment-list">${$.files.map(e=>`
    ...
    </span>`).join("")}</div>`:""}function Xt(t){return Wt.map(e=>`
    <button class="intake-action ${e.id==="save"?"primary-intake-action":""}"
      type="submit" data-submit-action="${e.id}" title="${t(e.hint)}">
      <strong>${t(e.name)}</strong>
    </button>`).join("")}function Qt(t){return`
    <section class="inline-task-launcher prompt-launcher simple-intake" aria-label="新增内容">
      <form class="prompt-composer compact-intake-composer" id="task-composer">
        <div class="compact-intake-row">
          ...
          <textarea id="task-goal" rows="1" aria-label="输入或粘贴内容">${t($.material)}</textarea>
    ...
    </section>`}function te({render:t,showToast:e,saveToLibrary:i}){document.querySelectorAll("[data-task-action]").forEach(s=>{s.addEventListener("click",l=>{l.currentTarget.dataset.taskAction==="remove-file"&&(j(),$.files=$.files.filter(p=>p.id!==l.currentTarget.dataset.fileId),t())})});const a=document.getElementById("task-composer");a==null||a.addEventListener("submit",async s=>{var g,w;if(s.preventDefault(),j(),!$.material.trim()&&!$.files.length){e("先粘贴内容，或加入一份材料"),(g=document.getElementById("task-goal"))==null||g.focus();return}const l=((w=s.submitter)==null?void 0:w.dataset.submitAction)||"save",d=s.submitter,p={material:$.material.trim(),files:$.files};if(l==="save"){const b=a.querySelector("#intake-save-status"),D=[...a.querySelectorAll("button, textarea, input")],wt=P=>{D.forEach(Kt=>{Kt.disabled=!0}),a.setAttribute("aria-busy","true"),a.classList.add("is-saving"),b.hidden=!1,b.querySelector("strong").textContent=P,d.innerHTML='<span class="mini-spinner"></span><strong>保存中</strong>'};wt(/^https?:\/\/\S+$/i.test(p.material)?"正在读取页面标题与摘要…":"正在识别标题、分组、类型与标签…");try{const P=await i(p,wt);$=st(),t(),e(`已保存到“${P.groupName}” · ${P.workTypeName} · 标签：${P.tags.join(" / ")||"待补标签"}`)}catch{D.forEach(P=>{P.disabled=!1}),t(),e("保存失败，请稍后重试")}return}d.disabled=!0;const u=Yt([p.material,...p.files.map(b=>`${b.name}
${b.excerpt}`)].join(`
`)),m=l==="decision"?z.find(b=>b.id==="decision"):u.id==="decision"?z.find(b=>b.id==="solution"):u,y=new Date().toISOString(),A=ee();A.push({id:Lt(),title:ae(p),mode:l,skillId:m.id,skillName:m.name,material:p.material,files:p.files,status:"queued",createdAt:y,updatedAt:y}),localStorage.setItem(xt,JSON.stringify(A)),$=st(),t(),e(`${l==="decision"?"已发起决策":"已发起评审"} · ${m.name}`)});const r=document.getElementById("task-files");r==null||r.addEventListener("change",async s=>{j(),$.files.push(...await M(s.target.files)),t(),e(`已加入 ${s.target.files.length} 个文件`)});const n=document.querySelector(".prompt-composer");n==null||n.addEventListener("dragover",s=>{s.preventDefault(),n.classList.add("drag-over")}),n==null||n.addEventListener("dragleave",()=>n.classList.remove("drag-over")),n==null||n.addEventListener("drop",async s=>{s.preventDefault(),s.stopPropagation(),n.classList.remove("drag-over"),j();const l=s.dataTransfer.files;$.files.push(...await M(l)),t(),e(`已加入 ${l.length} 个文件`)});const c=document.getElementById("task-goal");requestAnimationFrame(()=>kt(c)),c==null||c.addEventListener("input",()=>{$.material=c.value,kt(c)}),c==null||c.addEventListener("paste",async s=>{var m;const l=[...((m=s.clipboardData)==null?void 0:m.items)||[]].filter(y=>y.kind==="file").map(y=>y.getAsFile()).filter(Boolean);if(!l.length)return;s.preventDefault();const d=s.clipboardData.getData("text/plain"),p=c.selectionStart??c.value.length,u=c.selectionEnd??p;$.material=`${c.value.slice(0,p)}${d}${c.value.slice(u)}`,$.files.push(...await M(l)),t(),e(`已从剪贴板加入 ${l.length} 个材料`)}),re({render:t,showToast:e})}function ee(){try{const t=JSON.parse(localStorage.getItem(xt));return Array.isArray(t)?t:[]}catch{return[]}}function ae(t){var i;return(t.material.split(/\n/).map(a=>a.trim()).find(Boolean)||((i=t.files[0])==null?void 0:i.name)||"未命名任务").replace(/[。；;！!？?]+$/,"").slice(0,64)}function j(){const t=document.getElementById("task-goal");t&&($.material=t.value)}function kt(t){if(!t)return;t.style.height="auto";const e=Math.min(Math.max(t.scrollHeight,40),180);t.style.height=`${e}px`,t.style.overflowY=t.scrollHeight>180?"auto":"hidden"}function $t(){const t=document.querySelector(".prompt-composer");t==null||t.scrollIntoView({behavior:"smooth",block:"center"}),requestAnimationFrame(()=>{var e;return(e=document.getElementById("task-goal"))==null?void 0:e.focus()})}function ie(t){var e;return!!((e=t==null?void 0:t.closest)!=null&&e.call(t,"input, textarea, select, [contenteditable='true']"))}function re({render:t,showToast:e}){document.onpaste=async i=>{var c,s;if(ie(i.target)||!document.querySelector(".prompt-composer"))return;const r=[...((c=i.clipboardData)==null?void 0:c.items)||[]].filter(l=>l.kind==="file").map(l=>l.getAsFile()).filter(Boolean),n=((s=i.clipboardData)==null?void 0:s.getData("text/plain"))||"";!r.length&&!n.trim()||(i.preventDefault(),$.material=[$.material.trim(),n.trim()].filter(Boolean).join(`

`),r.length&&$.files.push(...await M(r)),t(),requestAnimationFrame($t),e(r.length?`已从剪贴板加入 ${r.length} 个材料`:"已把粘贴内容放入输入框"))},document.ondragover=i=>{var a;[...((a=i.dataTransfer)==null?void 0:a.types)||[]].includes("Files")&&i.preventDefault()},document.ondrop=async i=>{var r,n,c;if((n=(r=i.target)==null?void 0:r.closest)!=null&&n.call(r,".prompt-composer"))return;const a=((c=i.dataTransfer)==null?void 0:c.files)||[];a.length&&(i.preventDefault(),$.files.push(...await M(a)),t(),requestAnimationFrame($t),e(`已拖入 ${a.length} 个文件`))}}const V="clair-report-editor-v1",ne="https://api.github.com",Ct="2026",oe="clair-report-editor-draft-v1:",o={reportId:"",reportTitle:"",reportUrl:"",status:"idle",error:"",html:"",editorDocument:"",dirty:!1,hasDraft:!1,draftHtml:"",draftAt:"",target:null,token:"",settingsOpen:!1,publishConfirmOpen:!1,pendingSave:!1,saving:!1,lastCommit:"",protection:null,loadPromise:null,render:null,showToast:null},K=new Map;let At=!1;function pt(t){return[...new Set(t.filter(Boolean))]}function ct(t=o.target){return t?{...t.path&&t.sha?{[t.path]:t.sha}:{},...Object.fromEntries((t.mirrors||[]).map(e=>[e.path,e.sha])),...t.baseFiles||{}}:{}}function mt(t){return`${oe}${t}`}function se(t){try{const e=sessionStorage.getItem(mt(t));if(!e)return null;const i=JSON.parse(e);return!(i!=null&&i.html)||typeof i.html!="string"?null:i}catch{return null}}function Dt(t=o.reportId){try{sessionStorage.removeItem(mt(t))}catch{}}function Pt(){return o.dirty&&o.hasDraft?{tone:"changed",label:"有新修订 · 上次暂存待推送"}:o.dirty?{tone:"changed",label:"已修订 · 未暂存"}:o.hasDraft?{tone:"staged",label:"已暂存 · 待推送生产"}:o.lastCommit?{tone:"published",label:"生产档案已更新"}:{tone:"clean",label:"未修改"}}function W(){const t=Pt(),e=document.querySelector(".editor-revision-status");e&&(e.className=`editor-revision-status is-${t.tone}`,e.textContent=t.label);const i=document.querySelector('[data-editor-action="stash"]');i&&(i.disabled=o.status!=="ready"||o.saving||!o.dirty,i.textContent=!o.dirty&&o.hasDraft?"已暂存":"暂存");const a=document.querySelector('[data-editor-action="publish"]');a&&(a.disabled=o.status!=="ready"||o.saving||!o.dirty&&!o.hasDraft,a.textContent=o.saving?"推送中…":"推送生产");const r=document.querySelector('[data-editor-action="preview"]');r&&(r.disabled=o.status!=="ready"||o.saving||!o.hasDraft)}function ce(t=""){return String(t).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function le(t){const e=atob(String(t||"").replace(/\s/g,"")),i=Uint8Array.from(e,a=>a.charCodeAt(0));return new TextDecoder().decode(i)}function de(t){const e=new TextEncoder().encode(t);let i="";const a=32768;for(let r=0;r<e.length;r+=a)i+=String.fromCharCode(...e.subarray(r,r+a));return btoa(i)}function at(t){let e="";for(let a=0;a<t.length;a+=32768)e+=String.fromCharCode(...t.subarray(a,a+32768));return btoa(e)}function it(t){return Uint8Array.from(atob(t),e=>e.charCodeAt(0))}async function Ot(t,e){const i=await crypto.subtle.importKey("raw",new TextEncoder().encode(t),"PBKDF2",!1,["deriveKey"]);return crypto.subtle.deriveKey({name:"PBKDF2",salt:e,iterations:21e4,hash:"SHA-256"},i,{name:"AES-GCM",length:256},!1,["encrypt","decrypt"])}async function It(t){const e=t.match(/const\s+payload\s*=\s*(\{"salt":"[^"]+","iv":"[^"]+","data":"[^"]+"\})\s*;/);if(!e)return{html:t,protection:null};try{const i=JSON.parse(e[1]),a=it(i.salt),r=it(i.iv),n=await Ot(Ct,a),c=await crypto.subtle.decrypt({name:"AES-GCM",iv:r},n,it(i.data)),s=new TextDecoder().decode(c);if(!/<html[\s>]/i.test(s))throw new Error("解密结果不是 HTML");return{html:s,protection:{type:"aes-gcm-wrapper",wrapperHtml:t,payloadSource:e[1]}}}catch{throw new Error("检测到加密报告，但无法用工作台口令解锁")}}async function gt(t){var c;if(((c=o.protection)==null?void 0:c.type)!=="aes-gcm-wrapper")return t;const e=crypto.getRandomValues(new Uint8Array(16)),i=crypto.getRandomValues(new Uint8Array(12)),a=await Ot(Ct,e),r=await crypto.subtle.encrypt({name:"AES-GCM",iv:i},a,new TextEncoder().encode(t)),n=JSON.stringify({salt:at(e),iv:at(i),data:at(new Uint8Array(r))});return o.protection.wrapperHtml.replace(o.protection.payloadSource,n)}function ue(t){try{const e=new URL(t);if(e.hostname.toLowerCase()!=="clairku.github.io")return null;const i=e.pathname.split("/").filter(Boolean).map(decodeURIComponent),a=i.shift()||"ClairKu.github.io";let r=i.join("/");(!r||e.pathname.endsWith("/"))&&(r=`${r?`${r}/`:""}index.html`);const n=pt([`docs/${r}`,r,`public/${r}`]);return{owner:"ClairKu",repository:a,branch:"main",path:n[0],candidates:n,source:"auto"}}catch{return null}}async function Y(t,{token:e="",method:i="GET",body:a}={}){var c;const r={Accept:"application/vnd.github+json","X-GitHub-Api-Version":"2022-11-28"};e&&(r.Authorization=`Bearer ${e}`),a!==void 0&&(r["Content-Type"]="application/json");const n=await fetch(`${ne}${t}`,{method:i,headers:r,body:a===void 0?void 0:JSON.stringify(a)});if(!n.ok){let s="";try{s=((c=await n.json())==null?void 0:c.message)||""}catch{s=await n.text()}const l=new Error(s||`GitHub API ${n.status}`);throw l.status=n.status,l}return n.status===204?null:n.json()}async function pe(t){var c;const e=await Y(`/repos/${encodeURIComponent(t.owner)}/${encodeURIComponent(t.repository)}`);t.branch=e.default_branch||t.branch||"main";const i=pt((c=t.candidates)!=null&&c.length?t.candidates:[t.path]);let a=null,r=null;const n=[];for(const s of i)try{const l=s.split("/").map(encodeURIComponent).join("/"),d=`/repos/${encodeURIComponent(t.owner)}/${encodeURIComponent(t.repository)}/contents/${l}?ref=${encodeURIComponent(t.branch)}`,p=await Y(d);let u="";if(p.encoding==="base64"&&p.content)u=le(p.content);else if(p.download_url){const m=await fetch(p.download_url,{cache:"no-store"});if(!m.ok)throw new Error("无法读取 GitHub 原始文件");u=await m.text()}if(!u)throw new Error("GitHub 文件内容为空");r?u===r.html&&n.push({path:s,sha:p.sha}):r={html:u,target:{...t,path:s,sha:p.sha,candidates:i}}}catch(l){if(a=l,l.status&&![403,404].includes(l.status))break}if(r)return r.target.mirrors=n,r;throw a||new Error("没有找到对应的 GitHub HTML 文件")}function me(t){t.querySelectorAll("script").forEach(e=>{e.dataset.clairOriginalType=e.getAttribute("type")??"__empty__",e.setAttribute("type","application/x-clair-disabled")}),t.querySelectorAll("*").forEach(e=>{[...e.attributes].forEach(a=>{/^on/i.test(a.name)&&(e.setAttribute(`data-clair-event-${a.name.toLowerCase()}`,a.value),e.removeAttribute(a.name))});const i=e.getAttribute("href");i&&/^\s*javascript:/i.test(i)&&(e.dataset.clairJavascriptHref=i,e.removeAttribute("href"))})}function ge(){return`
(() => {
...
`}function fe(t,e){const a=new DOMParser().parseFromString(t,"text/html");a.querySelectorAll('meta[http-equiv="Content-Security-Policy" i]').forEach(s=>{s.dataset.clairEditorHttpEquiv=s.getAttribute("http-equiv")||"Content-Security-Policy",s.setAttribute("http-equiv","x-clair-csp-disabled")}),me(a);const r=a.createElement("base");r.href=e,r.dataset.clairEditorBase="true",a.head.prepend(r);const n=a.createElement("style");n.id="clair-editor-style",n.textContent=`
  ...
  `,a.head.append(n);const c=a.createElement("script");return c.id="clair-editor-bridge",c.textContent=ge(),a.body.append(c),`<!DOCTYPE html>
${a.documentElement.outerHTML}`}async function Rt(t){var e;try{const i=ue(t.url);let a=null;if(i)try{a=await pe(i)}catch{}if(!a){const s=await fetch(t.url,{cache:"no-store"});if(!s.ok)throw new Error(`报告读取失败（HTTP ${s.status}）`);a={html:await s.text(),target:i}}const r=await It(a.html);o.protection=r.protection,o.target=a.target||i;let n=r.html;const c=se(t.id);if(c!=null&&c.html)try{const s=await It(c.html);n=s.html,o.hasDraft=!0,o.draftHtml=s.html,o.draftAt=c.savedAt||"",c.baseFiles&&o.target&&(o.target.baseFiles=c.baseFiles)}catch{Dt(t.id)}o.html=n,o.editorDocument=fe(n,t.url),o.status="ready",o.error=""}catch(i){o.status="error",o.error=(i==null?void 0:i.message)||"无法读取这份 HTML"}finally{o.loadPromise=null,(e=o.render)==null||e.call(o)}}function Mt(){const t=o.render,e=o.showToast;Object.assign(o,{reportId:"",reportTitle:"",reportUrl:"",status:"idle",error:"",html:"",editorDocument:"",dirty:!1,hasDraft:!1,draftHtml:"",draftAt:"",target:null,settingsOpen:!1,publishConfirmOpen:!1,pendingSave:!1,saving:!1,lastCommit:"",protection:null,loadPromise:null,render:t,showToast:e})}function ft(){return document.querySelector(".report-editor-frame")}function rt(t,e=null){var a;const i=ft();(a=i==null?void 0:i.contentWindow)==null||a.postMessage({channel:V,type:"command",command:t,value:e},"*")}function ht(){var i;const t=ft();if(!(t!=null&&t.contentWindow))return Promise.reject(new Error("编辑画布尚未就绪"));const e=((i=crypto.randomUUID)==null?void 0:i.call(crypto))||`${Date.now()}-${Math.random()}`;return new Promise((a,r)=>{const n=window.setTimeout(()=>{K.delete(e),r(new Error("读取编辑内容超时"))},1e4);K.set(e,{resolve:c=>{clearTimeout(n),a(c)}}),t.contentWindow.postMessage({channel:V,type:"serialize",requestId:e},"*")})}function he(t){return`${String(t||"report").replace(/[\\/:*?"<>|]+/g,"-").replace(/\s+/g,"-").replace(/-+/g,"-").replace(/^-|-$/g,"").slice(0,80)||"report"}.html`}function Bt(t,e){const i=new Blob([t],{type:"text/html;charset=utf-8"}),a=URL.createObjectURL(i),r=document.createElement("a");r.href=a,r.download=he(e),document.body.append(r),r.click(),r.remove(),window.setTimeout(()=>URL.revokeObjectURL(a),1e3)}async function Ut(t){await navigator.clipboard.writeText(t)}function be(t,e){var r;const i=new DOMParser().parseFromString(t,"text/html");(r=i.querySelector("base[data-clair-preview-base]"))==null||r.remove();const a=i.createElement("base");return a.href=e,a.dataset.clairPreviewBase="true",i.head.prepend(a),`<!DOCTYPE html>
${i.documentElement.outerHTML}`}function ve(t){if(!o.hasDraft||!o.draftHtml)throw new Error("请先暂存当前修订，再另开预览");const e=new Blob([be(o.draftHtml,t.url)],{type:"text/html;charset=utf-8"}),i=URL.createObjectURL(e),a=window.open(i,"_blank");if(!a)throw URL.revokeObjectURL(i),new Error("浏览器拦截了新窗口，请允许弹窗后重试");a.opener=null,window.setTimeout(()=>URL.revokeObjectURL(i),6e4)}async function lt(t,{silent:e=!1}={}){var n;const i=await ht(),a=await gt(i),r=new Date().toISOString();try{sessionStorage.setItem(mt(t.id),JSON.stringify({reportId:t.id,reportUrl:t.url,savedAt:r,baseFiles:ct(),html:a}))}catch{throw new Error("浏览器暂存空间不足，请先下载 HTML 备份")}return o.html=i,o.draftHtml=i,o.draftAt=r,o.hasDraft=!0,o.dirty=!1,o.lastCommit="",W(),e||(n=o.showToast)==null||n.call(o,"已暂存在当前浏览器会话，尚未更新 GitHub"),i}async function ye(t){var s,l;const e=o.target;if(!(e!=null&&e.owner)||!e.repository||!e.path||!e.branch)throw new Error("请先填写 GitHub 仓库、分支和 HTML 路径");if(!o.token)throw new Error("请先提供 GitHub Fine-grained Token");const i=await gt(t),a=(e.mirrors||[]).map(d=>d.path),r=pt([...a.filter(d=>d.startsWith("public/")),...a.filter(d=>!d.startsWith("public/")&&d!==e.path),e.path]);let n="";const c=[];for(const d of r)try{const p=d.split("/").map(encodeURIComponent).join("/"),u=`/repos/${encodeURIComponent(e.owner)}/${encodeURIComponent(e.repository)}/contents/${p}`,m=await Y(`${u}?ref=${encodeURIComponent(e.branch)}`,{token:o.token}),y=ct(e)[d];if(y&&m.sha!==y)throw new Error(`生产文件 ${d} 已在本次编辑后更新，请重新打开报告合并修改`);const A=await Y(u,{token:o.token,method:"PUT",body:{message:`Update ${o.reportTitle} from Clair's Studio`,content:de(i),sha:m.sha,branch:e.branch}});n=((s=A==null?void 0:A.commit)==null?void 0:s.sha)||n,e.baseFiles={...ct(e),[d]:((l=A==null?void 0:A.content)==null?void 0:l.sha)||m.sha},c.push(d)}catch(p){throw c.length?new Error(`已更新 ${c.join("、")}，但 ${d} 同步失败：${p.message}`):p}return{commit:n,files:c.length}}async function St(t){var e,i;if(!o.saving){o.saving=!0,W();try{const a=o.dirty?await lt(t,{silent:!0}):o.draftHtml||await ht(),r=await ye(a);o.html=a,o.dirty=!1,o.hasDraft=!1,o.draftHtml="",o.draftAt="",o.lastCommit=r.commit,Dt(t.id),(e=o.showToast)==null||e.call(o,r.files>1?`已同步 ${r.files} 个 GitHub 文件，Pages 正在更新`:"已提交 GitHub，Pages 正在更新")}catch(a){(i=o.showToast)==null||i.call(o,(a==null?void 0:a.message)||"保存失败，请下载 HTML 备份")}finally{o.saving=!1,W()}}}function we(t){const e=o.target||{owner:"ClairKu",repository:"",branch:"main",path:""};return`
    ...
    </div>`}function ke(t){const e=o.target?`${o.target.owner}/${o.target.repository} · ${o.target.path}`:"尚未识别 GitHub 文件路径";return`
    <div class="dialog-backdrop editor-publish-backdrop" ${o.publishConfirmOpen?"":"hidden"}>
    ...
    </div>`}function qt({pendingSave:t=!1}={}){o.settingsOpen=!0,o.pendingSave=t;const e=document.querySelector(".editor-settings-backdrop");if(!e)return;e.hidden=!1;const i=e.querySelector("#editor-settings-form"),a=o.target||{};if(i){i.elements.owner.value=a.owner||"ClairKu",i.elements.repository.value=a.repository||"",i.elements.branch.value=a.branch||"main",i.elements.path.value=a.path||"";const r=i.querySelector('button[type="submit"]');r&&(r.textContent=t?"连接并保存":"保存设置")}}function F(){o.settingsOpen=!1,o.pendingSave=!1;const t=document.querySelector(".editor-settings-backdrop");t&&(t.hidden=!0)}function $e(){o.publishConfirmOpen=!0;const t=document.querySelector(".editor-publish-backdrop");t&&(t.hidden=!1)}function Z(){o.publishConfirmOpen=!1;const t=document.querySelector(".editor-publish-backdrop");t&&(t.hidden=!0)}function Ht(t=""){return!!(o.reportId&&(!t||o.reportId===t))}function Ae(t,{render:e,showToast:i}){Mt(),Object.assign(o,{reportId:t.id,reportTitle:t.title,reportUrl:t.url,status:"loading",render:e,showToast:i}),e(),o.loadPromise=Rt(t)}function Ie(t,e){var c;const i=o.target?`${o.target.owner}/${o.target.repository} · ${o.target.path}${(c=o.target.mirrors)!=null&&c.length?` · 同步 ${o.target.mirrors.length+1} 处`:""}`:"尚未识别 GitHub 源文件",a=Pt(),r=o.status==="ready"?`
      <div class="editor-toolbar" role="toolbar" aria-label="文本排版工具">
        ...
        <button type="button" data-editor-command="redo" title="重做">↷</button>
          ...
          sandbox="allow-scripts allow-modals" srcdoc="${ce(o.editorDocument)}"></iframe></div>`;return`
    ...
    <article class="report-card ${a?"restricted-card":""} ${e?"archived-card":""} ${C===t.id?"is-move-selected":""}" data-report-id="${f(t.id)}">
    ...
    </article>`}function yt(){var i;if(!k)return"";if(k.type==="tags"){const a=h.reports.find(r=>r.id===k.reportId);return a?`
      ...
      </div>`:""}if(k.type==="group"){const a=k.mode==="edit"?h.groups.find(r=>r.id===k.groupId):null;return`
      <div class="dialog-backdrop">
      ...
      </div>`}const t=k.mode==="edit"?h.reports.find(a=>a.id===k.reportId):null,e=(t==null?void 0:t.groupId)||k.groupId||((i=h.groups[0])==null?void 0:i.id)||"";return`
    <div class="dialog-backdrop">
      <form class="dialog" id="report-form">
        ...
        <label>工作类型
          <select name="workType">
            ${X.map(a=>`<option value="${f(a.id)}" ${a.id===((t==null?void 0:t.workType)||"product-planning")?"selected":""}>${f(a.name)}</option>`).join("")}
          </select>
        </label>
        <label>关键标签
    ...
    </main>`}function Me(t){if(Ht(t.id))return Ie(t,f);const e=!!t.savedContent&&!t.url,i=t.access!=="production",a=t.access==="org"?"组织账号":"站点账号",r=e?`
...
`,"<br />")}</div>`:""}
    ...
    </header>`}function Be(){const t=h.reports.filter(i=>i.archived).filter(i=>{if(!q.trim())return!0;const a=q.trim().toLowerCase();return`${i.title} ${i.url} ${i.source||""}`.toLowerCase().includes(a)}).sort((i,a)=>new Date(a.archivedAt||0)-new Date(i.archivedAt||0)),e=h.reports.filter(i=>i.archived).length;return`
    <main class="app-shell archive-shell">
      ...
      <section class="workspace archive-workspace">
        ...
        ${t.length?`
          <section class="archive-results">
            ...
            <div class="archive-grid">${t.map(i=>_t(i,!0)).join("")}</div>
          </section>`:`
      ...
      ${yt()}
    </main>`}function Ue(){if(U)return Be();const t=q.trim().toLowerCase(),e=t.split(/\s+/).filter(Boolean),i=h.reports.filter(d=>!d.archived),a=e.length?i.filter(d=>{const p=`${d.title} ${d.source||""} ${d.access||""} ${Gt(d.workType)} ${(d.tags||[]).join(" ")}`.toLowerCase();return e.every(u=>p.includes(u))}):i,r=h.reports.filter(d=>d.archived).length,n=i.filter(d=>d.access==="production").length,c=i.filter(d=>d.access!=="production").length,s=Oe(a,t).filter(d=>d.reports.length||C),l=I==="type"?"工作类型":I==="tag"?"关键标签":"工作主题";return`
    <main class="app-shell">
          ...
          ${s.length?`
            <div class="library-layout">
              <nav class="topic-nav" aria-label="报告${l}">
                ${s.map((d,p)=>`<a href="#bucket-${p}"><span class="nav-index">${String(p+1).padStart(2,"0")}</span>${f(d.name)}<span>${d.reports.length}</span></a>`).join("")}
              ...
              ${s.map((d,p)=>`
                <section id="bucket-${p}" class="group-column topic-section bucket-${f(d.kind)} accent-${f(d.accent||"blue")}"
                  data-bucket-kind="${f(d.kind)}"
                  data-bucket-id="${f(d.id)}"
                  ${d.kind==="topic"?`data-group-id="${f(d.id)}"`:""}>
                  <header class="group-header">
                    ${d.kind==="topic"?`<span class="group-drag-handle" role="button" tabindex="0" data-group-drag-id="${f(d.id)}"
                          aria-label="拖动“${f(d.name)}”调整主题顺序" title="拖动调整主题顺序；也可用左右方向键">
                          <span aria-hidden="true">⠿</span>
                    ...
                    <div class="group-heading-copy">
                      <div><h2>${f(d.name)}</h2></div>
                      <span class="count">${d.reports.length} 份</span>
                    </div>
                    <div class="group-menu">
                      ${C?`<button class="move-here-button" type="button" data-action="move-here" data-id="${f(d.id)}" data-bucket-kind="${f(d.kind)}">移到这里</button>`:""}
                      ${d.kind==="topic"?`<button type="button" data-action="add-to-group" data-id="${f(d.id)}">添加报告</button>
                           <button type="button" data-action="rename-group" data-id="${f(d.id)}">编辑主题</button>
                           ${d.id!=="inbox"?`<button type="button" data-action="delete-group" data-id="${f(d.id)}">删除</button>`:""}`:""}
                    </div>
                  </header>
                  <div class="group-cards">
                    ${d.reports.length?d.reports.map(u=>_t(u)).join(""):d.kind==="topic"?`<button class="empty-topic-drop" type="button" data-action="add-to-group" data-id="${f(d.id)}">
      ...
      ${yt()}
    </main>`}function v(){const t=document.getElementById("app");if(sessionStorage.getItem(vt)!=="ok"){t.innerHTML=Re(),He();return}const e=L&&h.reports.find(i=>i.id===L);t.innerHTML=e?Me(e):Ue(),Ne(),te({render:v,showToast:S,saveToLibrary:De})}function He(){const t=document.getElementById("login-form");t==null||t.addEventListener("submit",e=>{if(e.preventDefault(),new FormData(t).get("password")!=="2026"){const a=t.querySelector(".form-error");a.hidden=!1,a.textContent="口令不正确，请再试一次";return}sessionStorage.setItem(vt,"ok"),v()})}async function Tt(t){const e=t.elements.url,i=t.elements.title,a=t.querySelector('[data-action="detect-title"]'),r=t.querySelector(".field-hint"),n=e.value.trim();if(!et(n))return r.textContent="请输入完整的 http 或 https 网址","";a.disabled=!0,a.innerHTML='<span class="mini-spinner"></span>',r.textContent="正在读取网页标题…";try{const{title:c}=await Zt(n);if(!c)throw new Error("read failed");return i.value=c,r.textContent="已识别网页标题",i.value}catch{const c=N(n);return i.value||(i.value=c),r.textContent="网页暂时无法读取，已用域名作为标题，你可以手动修改",i.value}finally{a.disabled=!1,a.textContent="识别标题"}}function Ne(){var r;(r=document.getElementById("search-input"))==null||r.addEventListener("input",n=>{q=n.target.value,v();const c=document.getElementById("search-input");c==null||c.focus(),c==null||c.setSelectionRange(q.length,q.length)}),document.querySelectorAll("[data-action]").forEach(n=>{n.addEventListener("click",async c=>{var d,p;const s=c.currentTarget.dataset.action,l=c.currentTarget.dataset.id;if(s==="open")L=l,v();else if(s==="edit-document"){const u=h.reports.find(m=>m.id===l);if(!u||u.access!=="production")return;Ae(u,{render:v,showToast:S})}else if(s==="download-report"){const u=h.reports.find(m=>m.id===l);u&&await Nt(u,S)}else if(s==="share-report"){const u=h.reports.find(m=>m.id===l);u&&await qe(u,S)}else if(s==="back")L="",k=null,v();else if(s==="lock")sessionStorage.removeItem(vt),v();else if(s==="clear-search")q="",v();else if(s==="set-view"){if(!["topic","type","tag"].includes(l))return;I=l,C="",localStorage.setItem(J,I),v()}else if(s==="cancel-move")C="",v();else if(s==="move-here"){const u=c.currentTarget.dataset.bucketKind||I;C&&G(C,u,l)&&(C="",v(),S(u==="tag"?"已添加目标标签":`报告已移入目标${H()}`))}else if(s==="show-archive")U=!0,q="",L="",v();else if(s==="show-catalog")U=!1,q="",L="",v();else if(s==="add-report")k={type:"report",mode:"create",groupId:((d=h.groups[1])==null?void 0:d.id)||((p=h.groups[0])==null?void 0:p.id)},v();else if(s==="add-to-group")k={type:"report",mode:"create",groupId:l},v();else if(s==="edit")k={type:"report",mode:"edit",reportId:l},v();else if(s==="edit-tags")k={type:"tags",reportId:l},v();else if(s==="close-modal")k=null,v();else if(s==="detect-title")await Tt(c.currentTarget.closest("form"));else if(s==="archive"){const u=h.reports.find(m=>m.id===l);if(!u)return;u.archived=!0,u.archivedAt=new Date().toISOString(),T(),v(),S("已归档，可随时恢复")}else if(s==="restore"){const u=h.reports.find(m=>m.id===l);if(!u)return;u.archived=!1,u.archivedAt="",T(),v(),S("报告已恢复到原主题")}else if(s==="delete"){const u=h.reports.find(m=>m.id===l);u!=null&&u.archived&&confirm(`二次确认：永久删除“${u.title}”？

删除后无法从归档区恢复。`)&&(h.reports=h.reports.filter(m=>m.id!==l),L===l&&(L=""),T(),v(),S("报告已永久删除"))}else if(s==="add-group")k={type:"group",mode:"create"},v();else if(s==="rename-group")h.groups.find(m=>m.id===l)&&(k={type:"group",mode:"edit",groupId:l},v());else if(s==="delete-group"){const u=h.groups.find(m=>m.id===l);u&&confirm(`删除“${u.name}”？其中的报告会移到“待整理”。`)&&(h.reports.forEach(m=>{m.groupId===l&&(m.groupId="inbox")}),h.groups=h.groups.filter(m=>m.id!==l),T(),v(),S("分组已删除，报告已移到待整理"))}})}),document.querySelectorAll(".report-drag-handle").forEach(n=>{let c=null,s=!1;const l=()=>{var d;x="",c=null,s=!1,(d=n.closest(".report-card"))==null||d.classList.remove("is-dragging"),document.querySelectorAll(".report-card, .group-column").forEach(p=>{p.classList.remove("is-card-drop-target","is-drop-ready")})};n.addEventListener("pointerdown",d=>{var p,u;d.preventDefault(),x=n.dataset.reportDragId,E="",c={x:d.clientX,y:d.clientY},s=!1,(p=n.setPointerCapture)==null||p.call(n,d.pointerId),(u=n.closest(".report-card"))==null||u.classList.add("is-dragging")}),n.addEventListener("pointermove",d=>{if(!x||c&&Math.hypot(d.clientX-c.x,d.clientY-c.y)<7)return;s=!0;const p=document.elementFromPoint(d.clientX,d.clientY),u=p==null?void 0:p.closest(".report-card"),m=p==null?void 0:p.closest(".group-column");document.querySelectorAll(".report-card").forEach(y=>{y.classList.toggle("is-card-drop-target",!!(u&&u!==n.closest(".report-card")&&y===u))}),document.querySelectorAll(".group-column").forEach(y=>{y.classList.toggle("is-drop-ready",!!(m&&y===m))})}),n.addEventListener("pointerup",d=>{if(!x)return;const p=x;if(!s){C=p,l(),v(),S(`请选择目标${H()}`);return}const u=document.elementFromPoint(d.clientX,d.clientY),m=u==null?void 0:u.closest(".report-card"),y=u==null?void 0:u.closest(".group-column"),A=(m==null?void 0:m.dataset.reportId)||"",g=(y==null?void 0:y.dataset.bucketId)||"",w=(y==null?void 0:y.dataset.bucketKind)||I,b=A&&A!==p?G(p,w,g,A):g?G(p,w,g):!1;l(),b&&(v(),S(w==="tag"?"已添加目标标签":w==="type"?"工作类型已更新":A?"报告顺序已更新":"已移入新主题"))}),n.addEventListener("pointercancel",l)}),document.querySelectorAll(".group-drag-handle").forEach(n=>{const c=()=>{var s;E="",(s=n.closest(".group-column"))==null||s.classList.remove("is-group-dragging"),document.querySelectorAll(".group-column").forEach(l=>{l.classList.remove("is-group-drop-target","is-drop-ready")})};n.addEventListener("pointerdown",s=>{var l,d;s.preventDefault(),E=n.dataset.groupDragId,x="",(l=n.setPointerCapture)==null||l.call(n,s.pointerId),(d=n.closest(".group-column"))==null||d.classList.add("is-group-dragging")}),n.addEventListener("pointermove",s=>{E&&document.querySelectorAll(".group-column").forEach(l=>{var d;l.classList.toggle("is-group-drop-target",l===((d=document.elementFromPoint(s.clientX,s.clientY))==null?void 0:d.closest(".group-column")))})}),n.addEventListener("pointerup",s=>{var p;if(!E)return;const l=E,d=(p=document.elementFromPoint(s.clientX,s.clientY))==null?void 0:p.closest(".group-column");if(d&&nt(l,d.dataset.groupId)){E="",v(),S("分组顺序已更新");return}c()}),n.addEventListener("pointercancel",c),n.addEventListener("keydown",s=>{var u;if(!["ArrowLeft","ArrowRight"].includes(s.key))return;s.preventDefault();const l=h.groups.findIndex(m=>m.id===n.dataset.groupDragId),d=s.key==="ArrowLeft"?l-1:l+1,p=h.groups[d];!p||!nt(n.dataset.groupDragId,p.id)||(v(),S("分组顺序已更新"),(u=document.querySelector(`[data-group-drag-id="${CSS.escape(n.dataset.groupDragId)}"]`))==null||u.focus())})}),document.querySelectorAll(".group-column").forEach(n=>{n.addEventListener("dragover",c=>{c.preventDefault(),n.classList.add(E?"is-group-drop-target":"is-drop-ready")}),n.addEventListener("dragleave",()=>{n.classList.remove("is-drop-ready","is-group-drop-target")}),n.addEventListener("drop",c=>{if(c.preventDefault(),E){if(n.dataset.bucketKind==="topic"&&nt(E,n.dataset.groupId)){E="",v(),S("分组顺序已更新");return}E="",n.classList.remove("is-group-drop-target");return}const s=h.reports.find(d=>d.id===x),l=n.dataset.bucketKind||I;s&&G(x,l,n.dataset.bucketId)&&(x="",v(),S(l==="tag"?"已添加目标标签":l==="type"?"工作类型已更新":"已移入新主题")),x=""})}),document.querySelectorAll("[data-tag-suggestion]").forEach(n=>{n.addEventListener("click",()=>{const c=document.querySelector('#tag-form input[name="tags"]');if(!c)return;const s=ot(c.value),l=n.dataset.tagSuggestion;c.value=s.includes(l)?s.filter(d=>d!==l).join("、"):[...s,l].slice(0,8).join("、"),n.classList.toggle("selected",!s.includes(l)),c.focus()})});const t=document.getElementById("tag-form");t==null||t.addEventListener("submit",n=>{n.preventDefault();const c=h.reports.find(s=>s.id===k.reportId);c&&(c.tags=ot(new FormData(t).get("tags")),T(),k=null,v(),S("标签已更新"))});const e=document.getElementById("group-form");e==null||e.addEventListener("submit",n=>{var d,p;n.preventDefault();const c=(d=new FormData(e).get("name"))==null?void 0:d.trim(),s=(p=new FormData(e).get("description"))==null?void 0:p.trim();if(!c)return;if(k.mode==="edit"){const u=h.groups.find(m=>m.id===k.groupId);if(!u)return;u.name=c.slice(0,60),u.description=(s==null?void 0:s.slice(0,80))||"自定义工作主题"}else h.groups.push({id:ut("group"),name:c.slice(0,60),description:(s==null?void 0:s.slice(0,80))||"自定义工作主题",accent:["blue","violet","amber","green"][h.groups.length%4],position:h.groups.length});T();const l=k.mode==="edit"?"工作主题已更新":"工作主题已创建，可直接拖入报告";k=null,v(),S(l)});const i=document.getElementById("report-form");i==null||i.addEventListener("submit",async n=>{n.preventDefault();const c=i.elements.url.value.trim();if(!et(c))return;const s=i.querySelector('button[type="submit"]');s.disabled=!0,s.innerHTML='<span class="mini-spinner"></span>';let l=i.elements.title.value.trim();l||(l=await Tt(i));const d=i.elements.groupId.value,p=i.elements.workType.value,u=ot(i.elements.tags.value);if(k.mode==="edit"){const m=h.reports.find(y=>y.id===k.reportId);Object.assign(m,{title:l,url:c,groupId:d,workType:p,tags:u})}else{const m={id:ut("report"),groupId:d,title:l||N(c),url:c,pinned:!1,position:h.reports.filter(y=>y.groupId===d).length,createdAt:new Date().toISOString(),source:"手动添加",access:"production",archived:!1,archivedAt:"",workType:p,tags:u};m.tags.length||(m.tags=tt(m,m.workType)),h.reports.push(m)}T(),k=null,v(),S("报告已保存")});const a=L&&h.reports.find(n=>n.id===L);a&&Se(a)}function je(){v()}je(document.getElementById("app"));
