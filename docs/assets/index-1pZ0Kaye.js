...
`}function Ee(e,t){let n=new DOMParser().parseFromString(e,`text/html`);n.querySelectorAll(`meta[http-equiv="Content-Security-Policy" i]`).forEach(e=>{e.dataset.clairEditorHttpEquiv=e.getAttribute(`http-equiv`)||`Content-Security-Policy`,e.setAttribute(`http-equiv`,`x-clair-csp-disabled`)}),we(n);let r=n.createElement(`base`);r.href=t,r.dataset.clairEditorBase=`true`,n.head.prepend(r);let i=n.createElement(`style`);i.id=`clair-editor-style`,i.textContent=`
  ...
  `,n.head.append(i);let a=n.createElement(`script`);return a.id=`clair-editor-bridge`,a.textContent=Te(),n.body.append(a),`<!DOCTYPE html>\n${n.documentElement.outerHTML}`}function De(e){if(e.url)return``;if(e.savedHtml)return e.savedHtml;let t=(e.savedFiles||[]).find(e=>/\.html?$/i.test(e.name||``));return t?.content||t?.excerpt?t.content||t.excerpt:/<!doctype\s+html|<html[\s>]/i.test(e.savedContent||``)?e.savedContent.trim():``}async function Oe(e){try{let t=De(e),n=t?null:Se(e.url),r=null;if(t)r={html:t,target:null};else if(n)try{r=await Ce(n)}catch{}if(!r&&e.url){let t=await fetch(e.url,{cache:`no-store`});if(!t.ok)throw Error(`报告读取失败（HTTP ${t.status}）`);r={html:await t.text(),target:n}}let i=await be(r.html);_.protection=i.protection,_.target=r.target||n;let a=i.html,o=de(e.id);if(o?.html)try{let e=await be(o.html);a=e.html,_.hasDraft=!0,_.draftHtml=e.html,_.draftAt=o.savedAt||``,o.baseFiles&&_.target&&(_.target.baseFiles=o.baseFiles)}catch{fe(e.id)}_.html=a,_.editorDocument=Ee(a,e.url||window.location.href),_.status=`ready`,_.error=``}catch(e){_.status=`error`,_.error=e?.message||`无法读取这份 HTML`}finally{_.loadPromise=null,_.render?.()}}function ke(){let e=_.render,t=_.showToast;Object.assign(_,{reportId:``,reportTitle:``,reportUrl:``,status:`idle`,error:``,html:``,editorDocument:``,dirty:!1,hasDraft:!1,draftHtml:``,draftAt:``,target:null,settingsOpen:!1,publishConfirmOpen:!1,pendingSave:!1,saving:!1,lastCommit:``,isLocal:!1,saveLocal:null,protection:null,loadPromise:null,currentPage:0,pageCount:1,render:e,showToast:t})}function Ae(){return document.querySelector(`.report-editor-frame`)}function je(e,t=null){Ae()?.contentWindow?.postMessage({channel:g,type:`command`,command:e,value:t},`*`)}function Me(e){let t=Ae();if(!t?.contentWindow)return;let n=Math.max(0,Math.min(_.pageCount-1,Number(e)||0));_.currentPage=n,t.contentWindow.postMessage({channel:g,type:`set-page`,page:n},`*`),v()}function Ne(){let e=Ae();if(!e?.contentWindow)return Promise.reject(Error(`编辑画布尚未就绪`));let t=crypto.randomUUID?.()||`${Date.now()}-${Math.random()}`;return new Promise((n,r)=>{let i=window.setTimeout(()=>{oe.delete(t),r(Error(`读取编辑内容超时`))},1e4);oe.set(t,{resolve:e=>{clearTimeout(i),n(e)}}),e.contentWindow.postMessage({channel:g,type:`serialize`,requestId:t},`*`)})}function Pe(e){return`${String(e||`report`).replace(/[\\/:*?"<>|]+/g,`-`).replace(/\s+/g,`-`).replace(/-+/g,`-`).replace(/^-|-$/g,``).slice(0,80)||`report`}.html`}function Fe(e,t){let n=new Blob([e],{type:`text/html;charset=utf-8`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=Pe(t),document.body.append(i),i.click(),i.remove(),window.setTimeout(()=>URL.revokeObjectURL(r),1e3)}async function Ie(e){await navigator.clipboard.writeText(e)}function Le(e,t){let n=new DOMParser().parseFromString(e,`text/html`);n.querySelector(`base[data-clair-preview-base]`)?.remove();let r=n.createElement(`base`);return r.href=t,r.dataset.clairPreviewBase=`true`,n.head.prepend(r),`<!DOCTYPE html>\n${n.documentElement.outerHTML}`}function Re(e){if(!_.hasDraft||!_.draftHtml)throw Error(`请先暂存当前修订，再另开预览`);let t=new Blob([Le(_.draftHtml,e.url||window.location.href)],{type:`text/html;charset=utf-8`}),n=URL.createObjectURL(t),r=window.open(n,`_blank`);if(!r)throw URL.revokeObjectURL(n),Error(`浏览器拦截了新窗口，请允许弹窗后重试`);r.opener=null,window.setTimeout(()=>URL.revokeObjectURL(n),6e4)}async function ze(e,{silent:t=!1}={}){let n=await Ne(),r=await xe(n),i=new Date().toISOString();try{sessionStorage.setItem(ue(e.id),JSON.stringify({reportId:e.id,reportUrl:e.url,savedAt:i,baseFiles:le(),html:r}))}catch{throw Error(`浏览器暂存空间不足，请先下载 HTML 备份`)}return _.html=n,_.draftHtml=n,_.draftAt=i,_.hasDraft=!0,_.dirty=!1,_.lastCommit=``,v(),t||_.showToast?.(_.isLocal?`已暂存在当前浏览器会话，尚未写回成果库`:`已暂存在当前浏览器会话，尚未更新 GitHub`),n}async function Be(e){if(!(_.saving||!_.saveLocal)){_.saving=!0,v();try{let t=_.dirty?await ze(e,{silent:!0}):_.draftHtml||await Ne();await _.saveLocal(t),_.html=t,_.dirty=!1,_.hasDraft=!1,_.draftHtml=``,_.draftAt=``,_.lastCommit=`local`,fe(e.id),_.showToast?.(`已更新成果库中的 HTML`)}catch(e){_.showToast?.(e?.message||`保存失败，请下载 HTML 备份`)}finally{_.saving=!1,v()}}}async function Ve(e){let t=_.target;if(!t?.owner||!t.repository||!t.path||!t.branch)throw Error(`请先填写 GitHub 仓库、分支和 HTML 路径`);if(!_.token)throw Error(`请先提供 GitHub Fine-grained Token`);let n=await xe(e),r=(t.mirrors||[]).map(e=>e.path),i=ce([...r.filter(e=>e.startsWith(`public/`)),...r.filter(e=>!e.startsWith(`public/`)&&e!==t.path),t.path]),a=``,o=[];for(let e of i)try{let r=e.split(`/`).map(encodeURIComponent).join(`/`),i=`/repos/${encodeURIComponent(t.owner)}/${encodeURIComponent(t.repository)}/contents/${r}`,s=await y(`${i}?ref=${encodeURIComponent(t.branch)}`,{token:_.token}),c=le(t)[e];if(c&&s.sha!==c)throw Error(`生产文件 ${e} 已在本次编辑后更新，请重新打开报告合并修改`);let l=await y(i,{token:_.token,method:`PUT`,body:{message:`Update ${_.reportTitle} from Clair's Studio`,content:ge(n),sha:s.sha,branch:t.branch}});a=l?.commit?.sha||a,t.baseFiles={...le(t),[e]:l?.content?.sha||s.sha},o.push(e)}catch(t){throw o.length?Error(`已更新 ${o.join(`、`)}，但 ${e} 同步失败：${t.message}`):t}return{commit:a,files:o.length}}async function He(e){if(!_.saving){_.saving=!0,v();try{let t=_.dirty?await ze(e,{silent:!0}):_.draftHtml||await Ne(),n=await Ve(t);_.html=t,_.dirty=!1,_.hasDraft=!1,_.draftHtml=``,_.draftAt=``,_.lastCommit=n.commit,fe(e.id),_.showToast?.(n.files>1?`已同步 ${n.files} 个 GitHub 文件，Pages 正在更新`:`已提交 GitHub，Pages 正在更新`)}catch(e){_.showToast?.(e?.message||`保存失败，请下载 HTML 备份`)}finally{_.saving=!1,v()}}}function Ue(e){let t=_.target||{owner:`ClairKu`,repository:``,branch:`main`,path:``};return`
    ...
    </div>`}function We(e){let t=_.target?`${_.target.owner}/${_.target.repository} · ${_.target.path}`:`尚未识别 GitHub 文件路径`;return`
    ...
    </svg>`};function J(e){return nn[e]||``}function Y(e){try{return new URL(e).hostname.replace(/^www\./,``)}catch{return e}}function rn(e){try{return[`http:`,`https:`].includes(new URL(e).protocol)}catch{return!1}}function an(e=``){return[...new Set(String(e).split(/[、,，;；\n]+/).map(e=>e.trim()).filter(Boolean).map(e=>e.slice(0,20)))].slice(0,8)}function X(e){document.querySelector(`.toast`)?.remove();let t=document.createElement(`div`);t.className=`toast`,t.setAttribute(`role`,`status`),t.textContent=e,document.body.append(t),clearTimeout(gt),gt=window.setTimeout(()=>t.remove(),2600)}function Z(e=`auto`){requestAnimationFrame(()=>{window.scrollTo({top:0,left:0,behavior:e})})}function Q(e){return e.savedHtml||Pt(e.savedContent,e.savedFiles)}function on(e){return`${String(e.title||`report`).replace(/[\\/:*?"<>|]+/g,`-`).replace(/\s+/g,` `).trim().slice(0,80)||`report`}.html`}function sn(e){let t=Q(e);return t?URL.createObjectURL(new Blob([t],{type:`text/html;charset=utf-8`})):``}function cn(e){let t=sn(e);if(!t)return!1;let n=document.createElement(`a`);return n.href=t,n.download=on(e),document.body.append(n),n.click(),n.remove(),window.setTimeout(()=>URL.revokeObjectURL(t),1e3),!0}function ln(e){let t=e.url||sn(e);return t?(window.open(t,`_blank`,`noopener,noreferrer`),e.url||window.setTimeout(()=>URL.revokeObjectURL(t),6e4),!0):!1}function un(e,t=!1,n={}){let r=!e.url&&(!!e.savedContent||!!(e.savedFiles||[]).length),i=[`org`,`account`].includes(e.access),a=e.access===`org`?`需组织登录`:e.access===`account`?`需账号登录`:`生产可访问`,o=Q(e),s=!!e.pinned,c=N===`time`?Xt(P===`modified`&&e.modifiedAt||e.createdAt,P):e.source||`手动添加`,l=!i&&T.reports.some(t=>t.id===e.id),u=e.preview||`${e.id}.png`,d=o&&e.isHtml?`<iframe class="local-html-preview-frame" title="${q(e.title)}视觉预览"
        srcdoc="${q(o)}" sandbox="allow-scripts" loading="lazy"
        tabindex="-1" aria-hidden="true"></iframe>`:l?`<img src="./previews/${q(u)}" alt="" loading="lazy" decoding="async" />`:`
      <div class="preview-placeholder ${i?`preview-restricted`:``}">
        <span>${i?`ACCESS`:q(e.title.slice(0,2))}</span>
        <strong>${i?a:r?`本地内容`:`预览待补充`}</strong>
      </div>`;return`
    <article class="report-card ${i?`restricted-card`:``} ${t?`archived-card`:``} ${s?`is-featured`:``} ${L===e.id?`is-move-selected`:``}" data-report-id="${q(e.id)}">
      <button class="card-main" type="button" data-action="open" data-id="${q(e.id)}" aria-label="打开${q(e.title)}">
        <span class="report-preview">
          ${d}
        </span>
        <span class="report-copy">
          <span class="report-source">${q(c)}</span>
          <strong>${q(e.title)}</strong>
          ${s?`<span class="featured-mark">精选</span>`:``}
          ${(e.tags||[]).length?`<span class="report-tags">${e.tags.slice(0,3).map(e=>`<span>${q(e)}</span>`).join(``)}</span>`:``}
          ${n.searchExcerpt?`<span class="report-search-excerpt">${q(n.searchExcerpt)}</span>`:``}
          ${i?`<span class="report-access-note">${q(a)}</span>`:``}
        </span>
      </button>
      ${t||N===`time`?``:`
        <span class="report-drag-handle" role="button" tabindex="0" data-report-drag-id="${q(e.id)}"
          aria-label="拖动《${q(e.title)}》到其他${K()}" title="拖动到其他${K()}">
          <span aria-hidden="true">⠿</span>
        </span>`}
      ${t?``:`
        <button type="button" class="feature-action" data-action="toggle-pin" data-id="${q(e.id)}"
          title="${s?`取消精选`:`设为精选`}" aria-label="${s?`取消精选`:`设为精选`}">${s?`★`:`☆`}</button>`}
      <div class="card-actions">
        ${t?`
            <button type="button" data-action="restore" data-id="${q(e.id)}">Restore</button>
            <button type="button" data-action="delete" data-id="${q(e.id)}">Delete permanently</button>`:`
            <button type="button" class="tag-edit-action" data-action="edit-tags" data-id="${q(e.id)}" title="编辑标签" aria-label="编辑标签">#</button>
            ${e.url?`<button type="button" data-action="edit" data-id="${q(e.id)}">Edit</button>`:``}
            <button type="button" data-action="archive" data-id="${q(e.id)}">Archive</button>`}
      </div>
    </article>`}function dn(){if(!B)return``;if(B.type===`tags`){let e=O.reports.find(e=>e.id===B.reportId);return e?`
      <div class="dialog-backdrop">
        <form class="dialog compact-dialog tag-dialog" id="tag-form">
          ...
          <p class="tag-dialog-title">${q(e.title)}</p>
          <label>标签
            <input name="tags" value="${q((e.tags||[]).join(`、`))}" placeholder="例如：本体、飞书、调研" autofocus />
          </label>
          <div class="tag-suggestions">
            ${w.map(t=>`<button type="button" class="${(e.tags||[]).includes(t)?`selected`:``}" data-tag-suggestion="${q(t)}">${q(t)}</button>`).join(``)}
          </div>
          <div class="dialog-actions">
      ...
      </div>`:``}if(B.type===`group`){let e=B.mode===`edit`?O.groups.find(e=>e.id===B.groupId):null;return`
      <div class="dialog-backdrop">
        <form class="dialog compact-dialog" id="group-form">
          ...
          <label>主题 / 分组名称
            <input name="name" value="${q(e?.name||``)}" placeholder="例如：AI 产品、投研报告" maxlength="60" required autofocus />
          </label>
          <label>主题说明
            <input name="description" value="${q(e?.description||``)}" placeholder="这个主题主要收纳什么" maxlength="80" />
          </label>
          <div class="dialog-actions">
            <button type="button" class="quiet-button" data-action="close-modal">Cancel</button>
      ...
      </div>`}let e=B.mode===`edit`?O.reports.find(e=>e.id===B.reportId):null,t=e?.groupId||B.groupId||O.groups[0]?.id||``;return`
    <div class="dialog-backdrop">
      <form class="dialog" id="report-form">
        ...
        <label>网站地址
          <div class="url-input-row">
            <input name="url" type="url" value="${q(e?.url||``)}" placeholder="https://..." required autofocus />
        ...
        <label>报告标题
          <input name="title" value="${q(e?.title||``)}" placeholder="保存时自动识别，也可手动输入" maxlength="180" />
        </label>
        <label>放入分组
          <select name="groupId">
            ${O.groups.map(e=>`<option value="${q(e.id)}" ${e.id===t?`selected`:``}>${q(e.name)}</option>`).join(``)}
          </select>
        </label>
        <label>工作类型
          <select name="workType">
            ${C.map(t=>`<option value="${q(t.id)}" ${t.id===(e?.workType||`product-planning`)?`selected`:``}>${q(t.name)}</option>`).join(``)}
          </select>
        </label>
        <label>关键标签
          <input name="tags" value="${q((e?.tags||[]).join(`、`))}" placeholder="本体、飞书、调研" />
        </label>
        <div class="dialog-actions">
          <button type="button" class="quiet-button" data-action="close-modal">Cancel</button>
    ...
    <main class="gate-shell">
    ...
    </main>`}function pn(e){if(Ye(e.id))return Ze(e,q);let t=!e.url&&(!!e.savedContent||!!(e.savedFiles||[]).length),n=[`org`,`account`].includes(e.access),r=e.loginProvider||Ft(e.url)?.provider||(e.access===`org`?`组织帐号`:`站点帐号`),i=e.savedHtml||Pt(e.savedContent,e.savedFiles),a=i?`edit-local-document`:e.url?n?`edit`:`edit-document`:``,o=i?`
      <div class="reader-frame-wrap">
        <iframe class="reader-frame" title="${q(e.title)}"
          srcdoc="${q(i)}"
      ...
      <div class="saved-material-wrap">
        <article class="saved-material-card">
          ...
          <h1>${q(e.title)}</h1>
          ${e.savedContent?`<div class="saved-material-content">${q(e.savedContent).replaceAll(`
`,`<br />`)}</div>`:``}
          ${(e.savedFiles||[]).length?`<section class="saved-file-list">
                <strong>附件记录</strong>
                ${e.savedFiles.map(e=>`<span><b>${q(e.name)}</b><small>${q(e.sizeLabel||``)}</small></span>`).join(``)}
              </section>`:``}
      ...
      <div class="login-handoff-wrap">
        <section class="login-handoff-card" aria-labelledby="login-handoff-title">
          ...
          <div class="login-handoff-actions">
            <a class="primary-button" href="${q(e.url)}" target="_blank" rel="noreferrer">打开${q(r)}登录页 ↗</a>
            <button class="quiet-button" type="button" data-action="back">Back</button>
          </div>
          <p class="login-handoff-domain">${q(Y(e.url))}</p>
        </section>
      </div>`:`
      <div class="reader-frame-wrap">
        <iframe class="reader-frame" title="${q(e.title)}" src="${q(e.url)}"
    ...
    </header>`}function hn(){let e=O.reports.filter(e=>e.archived).filter(e=>rt(e,k,{group:O.groups.find(t=>t.id===e.groupId),workTypeName:Kt(e.workType)})).sort((e,t)=>new Date(t.archivedAt||0)-new Date(e.archivedAt||0)),t=O.reports.filter(e=>e.archived).length;return`
    <main class="app-shell archive-shell">
      ...
      <section class="workspace archive-workspace">
        ...
        <label class="search archive-search">
          <span aria-hidden="true">⌕</span>
          <input id="search-input" value="${q(k)}"
            placeholder="搜索归档标题、来源或网址" aria-label="搜索归档" />
          ${k?`<button type="button" data-action="clear-search">Clear</button>`:``}
        </label>
        ${e.length?`
          <section class="archive-results">
            ...
            <div class="archive-grid">${e.map(e=>un(e,!0)).join(``)}</div>
          </section>`:`
      ...
      ${dn()}
    </main>`}function gn(){if(j)return hn();let e=b(k),t=O.reports.filter(e=>!e.archived),n=e?$t(t,e):t,r=Ut(t.filter(e=>e.pinned),`featured`,`featured`,(e,t)=>G(t)-G(e)),i={id:`featured`,name:`精选成果`,kind:`featured`,accent:`violet`,reports:r},a=O.reports.filter(e=>e.archived).length,o=t.filter(e=>e.access===`production`).length,s=t.filter(e=>e.access!==`production`).length,c=Zt(t,``),l=N===`topic`&&r.length?[i,...c]:c,u=e?[]:(M&&N===`topic`?[i]:c).filter(e=>e.reports.length||L||N===`topic`),d=N===`type`?`工作类型`:N===`tag`?`关键标签`:N===`time`?`新增时间`:`工作主题`;return`
    <main class="app-shell">
        ...
        <section class="groups-section">
          ${L?`
            <div class="move-mode-banner" role="status">
              <div><strong>正在整理报告</strong><span>选择目标${K()}的“移到这里”，或直接拖动卡片。</span></div>
          ...
          ${e||u.length?`
            <div class="library-layout">
              <nav class="topic-nav" aria-label="报告${d}">
                <div class="library-nav-controls">
                  <div class="library-view-switcher" role="tablist" aria-label="成果分类方式">
                    <button type="button" role="tab" aria-selected="${N===`topic`}" class="${N===`topic`?`active`:``}" data-action="set-view" data-id="topic">Topic</button>
                    <button type="button" role="tab" aria-selected="${N===`type`}" class="${N===`type`?`active`:``}" data-action="set-view" data-id="type">Type</button>
                    <button type="button" role="tab" aria-selected="${N===`tag`}" class="${N===`tag`?`active`:``}" data-action="set-view" data-id="tag">Tag</button>
                    <button type="button" role="tab" aria-selected="${N===`time`}" class="${N===`time`?`active`:``}" data-action="set-view" data-id="time">Time</button>
                  </div>
                  <button class="add-topic-icon" type="button" data-action="add-group"
                    aria-label="Add topic" title="Add topic">＋</button>
                </div>
                ${N===`time`?`
                  <label class="library-time-sort">
                    <span>TIME ORDER</span>
                    <select data-action="set-time-sort" aria-label="时间排序">
                      <option value="created"${P===`created`?` selected`:``}>创建时间倒序</option>
                      <option value="modified"${P===`modified`?` selected`:``}>修改时间倒序</option>
                    </select>
                  </label>`:``}
                ${l.map((e,t)=>`
                  <a href="#bucket-${t}" data-nav-bucket-kind="${q(e.kind)}"
                    data-nav-bucket-id="${q(e.id)}" ${e.kind===`featured`?`data-action="show-featured"`:``}
                    class="${M&&e.kind===`featured`?`is-current`:``}">
                    ${q(e.name)}<span>${e.reports.length}</span>
                  </a>`).join(``)}
              ...
              <div class="board catalog-view-${N}">
              ${e?`
                <section class="search-results-panel" aria-live="polite">
                  <header class="search-results-header">
                    <div><span>SEARCH RESULTS</span><h2>“${q(k.trim())}”</h2></div>
                    <strong>${n.length} 份匹配</strong>
                  </header>
                  ${n.length?`<div class="group-cards search-results-cards">${n.map(t=>un(t,!1,{searchExcerpt:en(t,e)})).join(``)}</div>`:`<div class="no-results search-no-results">
                        <strong>没有找到“${q(k.trim())}”</strong>
                        <span>可搜索标题、标签、成果正文、来源、任务类型或主题</span>
                        <button type="button" data-action="clear-search">Clear search</button>
                      </div>`}
                </section>`:u.map((e,t)=>`
                <section id="bucket-${t}" class="group-column topic-section bucket-${q(e.kind)} accent-${q(e.accent||`blue`)}"
                  data-bucket-kind="${q(e.kind)}"
                  data-bucket-id="${q(e.id)}"
                  data-group-id="${q(e.id)}">
                  <header class="group-header">
                    <div class="group-heading-copy ${[`time`,`featured`].includes(e.kind)?``:`group-drag-handle`}" ${[`time`,`featured`].includes(e.kind)?``:`role="button" tabindex="0"`}
                      data-group-drag-id="${q(e.id)}"
                      data-group-drag-kind="${q(e.kind)}"
                      aria-label="Drag ${q(e.name)} to reorder"
                      title="Drag to reorder · use left or right arrow keys">
                      <div><h2>${q(e.name)}</h2></div>
                      <span class="count">${e.reports.length} 份</span>
                    </div>
                    <div class="group-menu">
                      ${L&&e.kind!==`time`?`<button class="move-here-button" type="button" data-action="move-here" data-id="${q(e.id)}" data-bucket-kind="${q(e.kind)}">Move here</button>`:``}
                      ${e.kind===`topic`?`<button type="button" data-action="add-to-group" data-id="${q(e.id)}">Add report</button>
                           <button type="button" data-action="rename-group" data-id="${q(e.id)}">Rename</button>
                           ${e.id===`inbox`?``:`<button type="button" data-action="delete-group" data-id="${q(e.id)}">Delete</button>`}`:``}
                    </div>
                  </header>
                  <div class="group-cards">
                    ${e.reports.length?e.reports.map(e=>un(e)).join(``):e.kind===`topic`?`<button class="empty-topic-drop" type="button" data-action="add-to-group" data-id="${q(e.id)}">
      ...
      ${dn()}
    </main>`}function $(){let e=document.getElementById(`app`);if(sessionStorage.getItem(st)!==`ok`){e.innerHTML=fn(),_n();return}let t=A&&O.reports.find(e=>e.id===A);e.innerHTML=t?pn(t):gn(),yn(),p({render:$,showToast:X,saveToLibrary:Rt})}function _n(){let e=document.getElementById(`login-form`);e?.addEventListener(`submit`,t=>{if(t.preventDefault(),new FormData(e).get(`password`)!==`2026`){let t=e.querySelector(`.form-error`);t.hidden=!1,t.textContent=`口令不正确，请再试一次`;return}sessionStorage.setItem(st,`ok`),$()})}async function vn(e){let t=e.elements.url,n=e.elements.title,r=e.querySelector(`[data-action="detect-title"]`),i=e.querySelector(`.field-hint`),a=t.value.trim();if(!rn(a))return i.textContent=`请输入完整的 http 或 https 网址`,``;r.disabled=!0,r.innerHTML=`<span class="mini-spinner"></span>`,i.textContent=`正在读取网页标题…`;try{let{title:e}=await It(a);if(!e)throw Error(`read failed`);return n.value=e,i.textContent=`已识别网页标题`,n.value}catch{let e=Y(a);return n.value||=e,i.textContent=`网页暂时无法读取，已用域名作为标题，你可以手动修改`,n.value}finally{r.disabled=!1,r.textContent=`Detect title`}}function yn(){let e=document.getElementById(`search-input`);e?.addEventListener(`input`,e=>{if(e.isComposing)return;k=e.target.value,k&&(M=!1);let t=e.target.selectionStart,n=e.target.selectionEnd;$();let r=document.getElementById(`search-input`);r?.focus(),r?.setSelectionRange(t,n)}),e?.addEventListener(`keydown`,e=>{e.key!==`Escape`||!k||(e.preventDefault(),k=``,$(),document.getElementById(`search-input`)?.focus())}),document.querySelector(`[data-action="set-time-sort"]`)?.addEventListener(`change`,e=>{[`created`,`modified`].includes(e.target.value)&&(P=e.target.value,localStorage.setItem(ct,P),$())}),document.querySelectorAll(`[data-action]`).forEach(e=>{e.addEventListener(`click`,async e=>{let t=e.currentTarget.dataset.action,n=e.currentTarget.dataset.id;if(t===`scroll-top`)Z(`smooth`);else if(t===`open`)A=n,$(),Z();else if(t===`edit-document`){let e=O.reports.find(e=>e.id===n);if(!e||e.access!==`production`)return;Xe(e,{render:$,showToast:X})}else if(t===`edit-local-document`){let e=O.reports.find(e=>e.id===n);if(!e||!Q(e))return;Xe(e,{render:$,showToast:X,saveLocal:async t=>{let n=e.savedHtml;e.savedHtml=t,e.isHtml=!0,e.tags=D(e,e.workType),W(e);try{U()}catch{throw e.savedHtml=n,Error(`修改后的 HTML 超过当前浏览器可保存容量，请先下载备份`)}}})}else if(t===`download-report`){let e=O.reports.find(e=>e.id===n);if(!e)return;Q(e)?cn(e)&&X(`HTML 已下载`):await $e(e,X)}else if(t===`share-report`||t===`copy-production-url`){let e=O.reports.find(e=>e.id===n);e?.url&&await et(e,e=>{X(e===`报告链接已复制`?`生产 URL 已复制`:e)})}else if(t===`open-browser`){let e=O.reports.find(e=>e.id===n);if(!e)return;ln(e)||X(`浏览器未能打开该报告`)}else if(t===`back`)A=``,B=null,$(),Z();else if(t===`lock`)sessionStorage.removeItem(st),$();else if(t===`clear-search`)k=``,M=!1,$(),document.getElementById(`search-input`)?.focus();else if(t===`show-featured`)e.preventDefault(),M=!0,k=``,$(),document.querySelector(`.group-column[data-bucket-kind="featured"]`)?.scrollIntoView({behavior:`smooth`,block:`start`});else if(t===`set-view`){if(![`topic`,`type`,`tag`,`time`].includes(n))return;N=n,M=!1,L=``,localStorage.setItem(x,N),$(),Z()}else if(t===`cancel-move`)L=``,$();else if(t===`move-here`){let t=e.currentTarget.dataset.bucketKind||N;L&&Qt(L,t,n)&&(L=``,$(),X(t===`tag`?`已添加目标标签`:`报告已移入目标${K()}`))}else if(t===`show-archive`)j=!0,k=``,A=``,$(),Z();else if(t===`show-catalog`)j=!1,k=``,A=``,$(),Z();else if(t===`add-report`)B={type:`report`,mode:`create`,groupId:O.groups[1]?.id||O.groups[0]?.id},$();else if(t===`add-to-group`)B={type:`report`,mode:`create`,groupId:n},$();else if(t===`edit`)B={type:`report`,mode:`edit`,reportId:n},$();else if(t===`edit-tags`)B={type:`tags`,reportId:n},$();else if(t===`toggle-pin`){let e=O.reports.find(e=>e.id===n);if(!e)return;e.pinned=!e.pinned,W(e),U(),$(),X(e.pinned?`已加入精选成果`:`已移出精选成果`)}else if(t===`close-modal`)B=null,$();else if(t===`detect-title`)await vn(e.currentTarget.closest(`form`));else if(t===`archive`){let e=O.reports.find(e=>e.id===n);if(!e)return;e.archived=!0,e.archivedAt=new Date().toISOString(),U(),$(),X(`已归档，可随时恢复`)}else if(t===`restore`){let e=O.reports.find(e=>e.id===n);if(!e)return;e.archived=!1,e.archivedAt=``,U(),$(),X(`报告已恢复到原主题`)}else if(t===`delete`){let e=O.reports.find(e=>e.id===n);e?.archived&&confirm(`二次确认：永久删除“${e.title}”？\n\n删除后无法从归档区恢复。`)&&(O.reports=O.reports.filter(e=>e.id!==n),A===n&&(A=``),U(),$(),X(`报告已永久删除`))}else if(t===`add-group`)B={type:`group`,mode:`create`},$();else if(t===`rename-group`)O.groups.find(e=>e.id===n)&&(B={type:`group`,mode:`edit`,groupId:n},$());else if(t===`delete-group`){let e=O.groups.find(e=>e.id===n);e&&confirm(`删除“${e.name}”？其中的报告会移到“待整理”。`)&&(O.reports.forEach(e=>{e.groupId===n&&(e.groupId=`inbox`)}),O.groups=O.groups.filter(e=>e.id!==n),U(),$(),X(`分组已删除，报告已移到待整理`))}})}),document.querySelector(`.topbar`)?.addEventListener(`click`,e=>{e.target.closest(`button, a`)||Z(`smooth`)}),document.querySelectorAll(`.topic-nav a[data-nav-bucket-id]:not([data-action="show-featured"])`).forEach(e=>{e.addEventListener(`click`,t=>{if(!M&&!k)return;t.preventDefault();let n=e.dataset.navBucketKind,r=e.dataset.navBucketId;M=!1,k=``,$(),requestAnimationFrame(()=>{document.querySelector(`.group-column[data-bucket-kind="${CSS.escape(n)}"][data-bucket-id="${CSS.escape(r)}"]`)?.scrollIntoView({behavior:`smooth`,block:`start`})})})}),document.querySelectorAll(`.report-drag-handle`).forEach(e=>{let t=null,n=!1,r=null,i=0,a=0,o=()=>{i&&=(cancelAnimationFrame(i),0)},s=()=>{if(!F)return o();let e=Math.min(110,window.innerHeight*.18),t=a<e?a-e:a>window.innerHeight-e?a-(window.innerHeight-e):0;if(!t)return o();let n=Math.sign(t)*Math.min(24,5+Math.abs(t)*.2);window.scrollBy(0,n),i=requestAnimationFrame(s)},c=t=>{let n=e.closest(`.report-card`);if(n){if(!r){let e=n.getBoundingClientRect();r=n.cloneNode(!0),r.className=`report-card report-drag-preview`,r.style.width=`${e.width}px`,r.style.height=`${e.height}px`,r.querySelectorAll(`button, [role='button']`).forEach(e=>{e.removeAttribute(`data-action`),e.setAttribute(`tabindex`,`-1`)}),document.body.append(r)}r.style.transform=`translate3d(${t.clientX+16}px, ${t.clientY+16}px, 0)`}},l=()=>{R=null,z?.remove(),z=null,document.querySelectorAll(`.report-card, .group-column, .topic-nav a`).forEach(e=>e.classList.remove(`is-card-drop-target`,`is-card-drop-before`,`is-card-drop-after`,`is-drop-ready`,`is-nav-drop-target`))},u=(t,n,r)=>{if(t){if(!z){z=document.createElement(`div`),z.className=`report-card report-card-placeholder`,z.innerHTML=`<span>放在这里</span>`;let t=e.closest(`.report-card`)?.getBoundingClientRect().height;t&&(z.style.minHeight=`${t}px`)}if(!n||n.parentElement!==t){t.append(z);return}t.insertBefore(z,r?n.nextSibling:n)}},d=t=>{let n=document.elementFromPoint(t.clientX,t.clientY),r=n?.closest(`.topic-nav a[data-nav-bucket-id]`);if(l(),r){r.classList.add(`is-nav-drop-target`),R={bucketKind:r.dataset.navBucketKind,bucketId:r.dataset.navBucketId,targetReportId:``,placeAfter:!1,nav:!0};return}let i=e.closest(`.report-card`),a=n?.closest(`.report-card:not(.report-card-placeholder)`),o=n?.closest(`.group-column`);if(a&&a!==i){let e=a.closest(`.group-column`),n=a.getBoundingClientRect(),r=t.clientY>n.top+n.height/2;a.classList.add(`is-card-drop-target`,r?`is-card-drop-after`:`is-card-drop-before`),u(e?.querySelector(`.group-cards`),a,r),R={bucketKind:e?.dataset.bucketKind||N,bucketId:e?.dataset.bucketId||``,targetReportId:a.dataset.reportId||``,placeAfter:r,nav:!1};return}o&&o.dataset.bucketKind!==`time`&&(o.classList.add(`is-drop-ready`),u(o.querySelector(`.group-cards`),null,!1),R={bucketKind:o.dataset.bucketKind||N,bucketId:o.dataset.bucketId||``,targetReportId:``,placeAfter:!1,nav:!1})},f=()=>{F=``,t=null,n=!1,o(),r?.remove(),r=null,l(),e.closest(`.report-card`)?.classList.remove(`is-dragging`)};e.addEventListener(`pointerdown`,r=>{r.preventDefault(),F=e.dataset.reportDragId,I=``,t={x:r.clientX,y:r.clientY},n=!1,e.setPointerCapture?.(r.pointerId),e.closest(`.report-card`)?.classList.add(`is-dragging`)}),e.addEventListener(`pointermove`,e=>{F&&(t&&Math.hypot(e.clientX-t.x,e.clientY-t.y)<7||(n=!0,a=e.clientY,c(e),d(e),i||=requestAnimationFrame(s)))}),e.addEventListener(`pointerup`,e=>{if(!F)return;let t=F;if(!n){L=t,f(),$(),X(`请选择目标${K()}`);return}let r=R,i=r?.targetReportId||``,a=r?.bucketId||``,o=r?.bucketKind||N,s=a&&o!==`time`?Qt(t,o,a,i,!!r?.placeAfter):!1;f(),s&&($(),requestAnimationFrame(()=>{let e=`.group-column[data-bucket-kind="${CSS.escape(o)}"][data-bucket-id="${CSS.escape(a)}"]`,n=document.querySelector(`${e} .report-card[data-report-id="${CSS.escape(t)}"]`)||document.querySelector(`.search-results-cards .report-card[data-report-id="${CSS.escape(t)}"]`);n?.scrollIntoView({behavior:`smooth`,block:`center`,inline:`nearest`}),n?.classList.add(`is-drop-landed`),window.setTimeout(()=>n?.classList.remove(`is-drop-landed`),900)}),X(o===`featured`?`已加入精选成果`:o===`tag`?`已添加目标标签`:o===`type`?`工作类型已更新`:i?`报告顺序已更新`:`已移入新主题`))}),e.addEventListener(`pointercancel`,f)}),document.querySelectorAll(`.group-drag-handle`).forEach(e=>{let t=()=>{I=``,e.closest(`.group-column`)?.classList.remove(`is-group-dragging`),document.querySelectorAll(`.group-column`).forEach(e=>{e.classList.remove(`is-group-drop-target`,`is-drop-ready`)})};e.addEventListener(`pointerdown`,t=>{t.preventDefault(),I=e.dataset.groupDragId,F=``,e.setPointerCapture?.(t.pointerId),e.closest(`.group-column`)?.classList.add(`is-group-dragging`)}),e.addEventListener(`pointermove`,e=>{I&&document.querySelectorAll(`.group-column`).forEach(t=>{t.classList.toggle(`is-group-drop-target`,t===document.elementFromPoint(e.clientX,e.clientY)?.closest(`.group-column`))})}),e.addEventListener(`pointerup`,e=>{if(!I)return;let n=I,r=document.elementFromPoint(e.clientX,e.clientY)?.closest(`.group-column`);if(r&&Vt(n,r.dataset.bucketId,r.dataset.bucketKind)){I=``,$(),X(`分组顺序已更新`);return}t()}),e.addEventListener(`pointercancel`,t),e.addEventListener(`keydown`,t=>{if(![`ArrowLeft`,`ArrowRight`].includes(t.key))return;t.preventDefault();let n=[...document.querySelectorAll(`.group-column`)],r=n.findIndex(t=>t.dataset.bucketId===e.dataset.groupDragId),i=n[t.key===`ArrowLeft`?r-1:r+1];!i||!Vt(e.dataset.groupDragId,i.dataset.bucketId,e.dataset.groupDragKind)||($(),X(`分组顺序已更新`),document.querySelector(`[data-group-drag-id="${CSS.escape(e.dataset.groupDragId)}"]`)?.focus())})}),document.querySelectorAll(`.group-column`).forEach(e=>{e.addEventListener(`dragover`,t=>{t.preventDefault(),e.classList.add(I?`is-group-drop-target`:`is-drop-ready`)}),e.addEventListener(`dragleave`,()=>{e.classList.remove(`is-drop-ready`,`is-group-drop-target`)}),e.addEventListener(`drop`,t=>{if(t.preventDefault(),I){if(Vt(I,e.dataset.bucketId,e.dataset.bucketKind)){I=``,$(),X(`分组顺序已更新`);return}I=``,e.classList.remove(`is-group-drop-target`);return}let n=O.reports.find(e=>e.id===F),r=e.dataset.bucketKind||N;n&&Qt(F,r,e.dataset.bucketId)&&(F=``,$(),X(r===`tag`?`已添加目标标签`:r===`type`?`工作类型已更新`:`已移入新主题`)),F=``})}),document.querySelectorAll(`[data-tag-suggestion]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=document.querySelector(`#tag-form input[name="tags"]`);if(!t)return;let n=an(t.value),r=e.dataset.tagSuggestion;t.value=n.includes(r)?n.filter(e=>e!==r).join(`、`):[...n,r].slice(0,8).join(`、`),e.classList.toggle(`selected`,!n.includes(r)),t.focus()})});let t=document.getElementById(`tag-form`);t?.addEventListener(`submit`,e=>{e.preventDefault();let n=O.reports.find(e=>e.id===B.reportId);n&&(n.tags=an(new FormData(t).get(`tags`)),W(n),U(),B=null,$(),X(`标签已更新`))});let n=document.getElementById(`group-form`);n?.addEventListener(`submit`,e=>{e.preventDefault();let t=new FormData(n).get(`name`)?.trim(),r=new FormData(n).get(`description`)?.trim();if(!t)return;if(B.mode===`edit`){let e=O.groups.find(e=>e.id===B.groupId);if(!e)return;e.name=t.slice(0,60),e.description=r?.slice(0,80)||`自定义工作主题`}else O.groups.push({id:tn(`group`),name:t.slice(0,60),description:r?.slice(0,80)||`自定义工作主题`,accent:[`blue`,`violet`,`amber`,`green`][O.groups.length%4],position:O.groups.length}),N=`topic`,localStorage.setItem(x,N);U();let i=B.mode===`edit`?`工作主题已更新`:`工作主题已创建，可直接拖入报告`;B=null,$(),X(i)});let r=document.getElementById(`report-form`);r?.addEventListener(`submit`,async e=>{e.preventDefault();let t=r.elements.url.value.trim();if(!rn(t))return;let n=r.querySelector(`button[type="submit"]`),i=r.querySelector(`.field-hint`);n.disabled=!0,n.innerHTML=`<span class="mini-spinner"></span>`;let a=jt({material:t,files:[],url:t,excludeId:B.mode===`edit`?B.reportId:``});if(a){n.disabled=!1,n.textContent=`Save`,i.textContent=`成果库已有“${a.title}”，未重复保存`,X(`成果库已有“${a.title}”，未重复保存`);return}let o=await Lt({material:t,files:[],url:t},e=>{i.textContent=e});if(!o.allowed){n.disabled=!1,n.textContent=`Save`,i.textContent=o.reason,X(o.reason);return}let s=r.elements.title.value.trim()||o.metadata.title,c=r.elements.groupId.value,l=r.elements.workType.value,u=an(r.elements.tags.value),d={title:s||Y(t),url:t,groupId:c,workType:l,source:`手动添加`,access:o.access,detectedDescription:o.metadata.description,manualSaved:!0,isProduction:o.access===`production`,isPersonal:Mt(t),isHtml:o.isHtml,loginProvider:o.loginProvider},f=[...new Set([...D(d,l),...u])].slice(0,8);if(B.mode===`edit`){let e=O.reports.find(e=>e.id===B.reportId);Object.assign(e,d,{tags:f}),W(e)}else{let e=new Date().toISOString(),t={id:tn(`report`),groupId:c,...d,pinned:!1,position:O.reports.filter(e=>e.groupId===c).length,createdAt:e,modifiedAt:e,archived:!1,archivedAt:``,tags:f};O.reports.push(t)}U(),B=null,$(),X(`报告已保存`)});let i=A&&O.reports.find(e=>e.id===A);i&&Qe(i)}function bn(){xt(),$()}bn(document.getElementById(`app`));