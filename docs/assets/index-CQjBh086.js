...
`}function Ee(e,t){let n=new DOMParser().parseFromString(e,`text/html`);n.querySelectorAll(`meta[http-equiv="Content-Security-Policy" i]`).forEach(e=>{e.dataset.clairEditorHttpEquiv=e.getAttribute(`http-equiv`)||`Content-Security-Policy`,e.setAttribute(`http-equiv`,`x-clair-csp-disabled`)}),we(n);let r=n.createElement(`base`);r.href=t,r.dataset.clairEditorBase=`true`,n.head.prepend(r);let i=n.createElement(`style`);i.id=`clair-editor-style`,i.textContent=`
  ...
  `,n.head.append(i);let a=n.createElement(`script`);return a.id=`clair-editor-bridge`,a.textContent=Te(),n.body.append(a),`<!DOCTYPE html>\n${n.documentElement.outerHTML}`}function De(e){if(e.url)return``;if(e.savedHtml)return e.savedHtml;let t=(e.savedFiles||[]).find(e=>/\.html?$/i.test(e.name||``));return t?.content||t?.excerpt?t.content||t.excerpt:/<!doctype\s+html|<html[\s>]/i.test(e.savedContent||``)?e.savedContent.trim():``}async function Oe(e){try{let t=De(e),n=t?null:Se(e.url),r=null;if(t)r={html:t,target:null};else if(n)try{r=await Ce(n)}catch{}if(!r&&e.url){let t=await fetch(e.url,{cache:`no-store`});if(!t.ok)throw Error(`报告读取失败（HTTP ${t.status}）`);r={html:await t.text(),target:n}}let i=await be(r.html);_.protection=i.protection,_.target=r.target||n;let a=i.html,o=de(e.id);if(o?.html)try{let e=await be(o.html);a=e.html,_.hasDraft=!0,_.draftHtml=e.html,_.draftAt=o.savedAt||``,o.baseFiles&&_.target&&(_.target.baseFiles=o.baseFiles)}catch{fe(e.id)}_.html=a,_.editorDocument=Ee(a,e.url||window.location.href),_.status=`ready`,_.error=``}catch(e){_.status=`error`,_.error=e?.message||`无法读取这份 HTML`}finally{_.loadPromise=null,_.render?.()}}function ke(){let e=_.render,t=_.showToast;Object.assign(_,{reportId:``,reportTitle:``,reportUrl:``,status:`idle`,error:``,html:``,editorDocument:``,dirty:!1,hasDraft:!1,draftHtml:``,draftAt:``,target:null,settingsOpen:!1,publishConfirmOpen:!1,pendingSave:!1,saving:!1,lastCommit:``,isLocal:!1,saveLocal:null,protection:null,loadPromise:null,currentPage:0,pageCount:1,render:e,showToast:t})}function b(){return document.querySelector(`.report-editor-frame`)}function Ae(e,t=null){b()?.contentWindow?.postMessage({channel:g,type:`command`,command:e,value:t},`*`)}function je(e){let t=b();if(!t?.contentWindow)return;let n=Math.max(0,Math.min(_.pageCount-1,Number(e)||0));_.currentPage=n,t.contentWindow.postMessage({channel:g,type:`set-page`,page:n},`*`),v()}function Me(){let e=b();if(!e?.contentWindow)return Promise.reject(Error(`编辑画布尚未就绪`));let t=crypto.randomUUID?.()||`${Date.now()}-${Math.random()}`;return new Promise((n,r)=>{let i=window.setTimeout(()=>{oe.delete(t),r(Error(`读取编辑内容超时`))},1e4);oe.set(t,{resolve:e=>{clearTimeout(i),n(e)}}),e.contentWindow.postMessage({channel:g,type:`serialize`,requestId:t},`*`)})}function Ne(e){return`${String(e||`report`).replace(/[\\/:*?"<>|]+/g,`-`).replace(/\s+/g,`-`).replace(/-+/g,`-`).replace(/^-|-$/g,``).slice(0,80)||`report`}.html`}function Pe(e,t){let n=new Blob([e],{type:`text/html;charset=utf-8`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=Ne(t),document.body.append(i),i.click(),i.remove(),window.setTimeout(()=>URL.revokeObjectURL(r),1e3)}async function Fe(e){await navigator.clipboard.writeText(e)}function Ie(e,t){let n=new DOMParser().parseFromString(e,`text/html`);n.querySelector(`base[data-clair-preview-base]`)?.remove();let r=n.createElement(`base`);return r.href=t,r.dataset.clairPreviewBase=`true`,n.head.prepend(r),`<!DOCTYPE html>\n${n.documentElement.outerHTML}`}function Le(e){if(!_.hasDraft||!_.draftHtml)throw Error(`请先暂存当前修订，再另开预览`);let t=new Blob([Ie(_.draftHtml,e.url||window.location.href)],{type:`text/html;charset=utf-8`}),n=URL.createObjectURL(t),r=window.open(n,`_blank`);if(!r)throw URL.revokeObjectURL(n),Error(`浏览器拦截了新窗口，请允许弹窗后重试`);r.opener=null,window.setTimeout(()=>URL.revokeObjectURL(n),6e4)}async function Re(e,{silent:t=!1}={}){let n=await Me(),r=await xe(n),i=new Date().toISOString();try{sessionStorage.setItem(ue(e.id),JSON.stringify({reportId:e.id,reportUrl:e.url,savedAt:i,baseFiles:le(),html:r}))}catch{throw Error(`浏览器暂存空间不足，请先下载 HTML 备份`)}return _.html=n,_.draftHtml=n,_.draftAt=i,_.hasDraft=!0,_.dirty=!1,_.lastCommit=``,v(),t||_.showToast?.(_.isLocal?`已暂存在当前浏览器会话，尚未写回成果库`:`已暂存在当前浏览器会话，尚未更新 GitHub`),n}async function ze(e){if(!(_.saving||!_.saveLocal)){_.saving=!0,v();try{let t=_.dirty?await Re(e,{silent:!0}):_.draftHtml||await Me();await _.saveLocal(t),_.html=t,_.dirty=!1,_.hasDraft=!1,_.draftHtml=``,_.draftAt=``,_.lastCommit=`local`,fe(e.id),_.showToast?.(`已更新成果库中的 HTML`)}catch(e){_.showToast?.(e?.message||`保存失败，请下载 HTML 备份`)}finally{_.saving=!1,v()}}}async function Be(e){let t=_.target;if(!t?.owner||!t.repository||!t.path||!t.branch)throw Error(`请先填写 GitHub 仓库、分支和 HTML 路径`);if(!_.token)throw Error(`请先提供 GitHub Fine-grained Token`);let n=await xe(e),r=(t.mirrors||[]).map(e=>e.path),i=ce([...r.filter(e=>e.startsWith(`public/`)),...r.filter(e=>!e.startsWith(`public/`)&&e!==t.path),t.path]),a=``,o=[];for(let e of i)try{let r=e.split(`/`).map(encodeURIComponent).join(`/`),i=`/repos/${encodeURIComponent(t.owner)}/${encodeURIComponent(t.repository)}/contents/${r}`,s=await y(`${i}?ref=${encodeURIComponent(t.branch)}`,{token:_.token}),c=le(t)[e];if(c&&s.sha!==c)throw Error(`生产文件 ${e} 已在本次编辑后更新，请重新打开报告合并修改`);let l=await y(i,{token:_.token,method:`PUT`,body:{message:`Update ${_.reportTitle} from Clair's Studio`,content:ge(n),sha:s.sha,branch:t.branch}});a=l?.commit?.sha||a,t.baseFiles={...le(t),[e]:l?.content?.sha||s.sha},o.push(e)}catch(t){throw o.length?Error(`已更新 ${o.join(`、`)}，但 ${e} 同步失败：${t.message}`):t}return{commit:a,files:o.length}}async function Ve(e){if(!_.saving){_.saving=!0,v();try{let t=_.dirty?await Re(e,{silent:!0}):_.draftHtml||await Me(),n=await Be(t);_.html=t,_.dirty=!1,_.hasDraft=!1,_.draftHtml=``,_.draftAt=``,_.lastCommit=n.commit,fe(e.id),_.showToast?.(n.files>1?`已同步 ${n.files} 个 GitHub 文件，Pages 正在更新`:`已提交 GitHub，Pages 正在更新`)}catch(e){_.showToast?.(e?.message||`保存失败，请下载 HTML 备份`)}finally{_.saving=!1,v()}}}function He(e){let t=_.target||{owner:`ClairKu`,repository:``,branch:`main`,path:``};return`
    ...
    <article class="report-card ${i?`restricted-card`:``} ${t?`archived-card`:``} ${s?`is-featured`:``} ${I===e.id?`is-move-selected`:``}"
      data-report-id="${K(e.id)}" ${t||M===`time`?``:`data-report-draggable="true"`}>
      ...
      <div class="card-actions">
        ${t?`
            ...
            ${e.url?`<button type="button" class="card-icon-action" data-action="edit" data-id="${K(e.id)}" title="编辑成果" aria-label="编辑成果">
              <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 20h4l10.5-10.5a2.8 2.8 0 0 0-4-4L4 16v4Z"></path><path d="m13 7 4 4"></path></svg>
            </button>`:``}
            <button type="button" class="card-icon-action" data-action="archive" data-id="${K(e.id)}" title="归档成果" aria-label="归档成果">
              <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16v13H4z"></path><path d="M3 4h18v3H3zM9 11h6"></path></svg>
            </button>`}
      </div>
    </article>`}function bn(){if(!z)return``;if(z.type===`group`){let e=z.mode===`edit`?D.groups.find(e=>e.id===z.groupId):null;return`
      ...
      </div>`}let e=z.mode===`edit`?D.reports.find(e=>e.id===z.reportId):null,t=e?.groupId||z.groupId||D.groups[0]?.id||``,n=Y((e?.tags||[]).join(`、`));return`
    <div class="dialog-backdrop">
      <form class="dialog" id="report-form">
        ...
        <label>放入分组
          <select name="groupId">
            ${D.groups.map(e=>`<option value="${K(e.id)}" ${e.id===t?`selected`:``}>${K(e.name)}</option>`).join(``)}
          </select>
        </label>
        <label>工作类型
    ...
    </main>`}function Sn(e){if(Je(e.id))return Xe(e,K);let t=!e.url&&(!!e.savedContent||!!(e.savedFiles||[]).length),n=[`org`,`account`].includes(e.access),r=e.loginProvider||Rt(e.url)?.provider||(e.access===`org`?`组织帐号`:`站点帐号`),i=e.savedHtml||Lt(e.savedContent,e.savedFiles),a=i?`edit-local-document`:e.url?n?`edit`:`edit-document`:``,o=i?`
...
`,`<br />`)}</div>`:``}
    ...
    <header class="topbar">
      ...
      ${A?`<div class="top-actions"><button class="quiet-button" type="button" data-action="show-catalog">← Library</button></div>`:``}
    </header>`}function wn(){let e=D.reports.filter(e=>e.archived).filter(e=>nt(e,O,{group:D.groups.find(t=>t.id===e.groupId),workTypeName:Yt(e.workType)})).sort((e,t)=>new Date(t.archivedAt||0)-new Date(e.archivedAt||0)),t=D.reports.filter(e=>e.archived).length;return`
    <main class="app-shell archive-shell">
      ...
      <section class="workspace archive-workspace">
        ...
        <label class="search archive-search">
          ...
          <input id="search-input" value="${K(O)}"
            placeholder="搜索归档标题、来源或网址" aria-label="搜索归档" />
          ${O?`<button type="button" data-action="clear-search">Clear</button>`:``}
        </label>
        ${e.length?`
      ...
      ${bn()}
    </main>`}function Tn(){if(A)return wn();let e=x(O),t=D.reports.filter(e=>!e.archived),n=e?tn(t,e):t,r=Kt(t.filter(e=>e.pinned),`featured`,`featured`,(e,t)=>G(t)-G(e)),i={id:`featured`,name:`精选成果`,kind:`featured`,accent:`violet`,reports:r},a=D.reports.filter(e=>e.archived).length,o=t.filter(e=>e.access===`production`).length,s=t.filter(e=>e.access!==`production`).length,c=Qt(t,``),l=M===`topic`&&r.length?[i,...c]:c,u=M===`time`&&c[0]?.reports||[],d=e?[]:(j&&M===`topic`?[i]:c).filter(e=>e.reports.length||I||M===`topic`),p=M===`type`?`工作类型`:M===`tag`?`关键标签`:M===`time`?`新增时间`:`工作主题`;return`
    <main class="app-shell">
              ...
              <nav class="topic-nav" aria-label="报告${p}">
                ...
                ${M===`time`?`
                  ...
                  <a href="#" data-nav-bucket-kind="${K(e.kind)}"
                    ...
                    class="${j&&e.kind===`featured`?`is-current`:``}">
              ...
              ${e?`
                <section class="search-results-panel" aria-live="polite">
                  <header class="search-results-header">
                    <div><span>SEARCH RESULTS</span><h2>“${K(O.trim())}”</h2></div>
                ...
                <section class="group-column topic-section bucket-${K(e.kind)} accent-${K(e.accent||`blue`)}"
                  ...
                  <header class="group-header">
                    ...
                    <div class="group-menu">
                      ${I&&e.kind!==`time`?`<button class="move-here-button" type="button" data-action="move-here" data-id="${K(e.id)}" data-bucket-kind="${K(e.kind)}">Move here</button>`:``}
                      ${e.kind===`topic`?`<button type="button" data-action="add-to-group" data-id="${K(e.id)}">Add report</button>
                           ...
                           <button type="button" data-action="delete-group" data-id="${K(e.id)}">Delete</button>`:``}
                    </div>
                  </header>
      ...
      ${bn()}
    </main>`}function $(){let e=document.getElementById(`app`);if(sessionStorage.getItem(ot)!==`ok`){e.innerHTML=xn(),En();return}let t=k&&D.reports.find(e=>e.id===k);e.innerHTML=t?Sn(t):Tn(),kn(),p({render:$,showToast:X,saveToLibrary:Vt})}function En(){let e=document.getElementById(`login-form`);e?.addEventListener(`submit`,t=>{if(t.preventDefault(),new FormData(e).get(`password`)!==`2026`){let t=e.querySelector(`.form-error`);t.hidden=!1,t.textContent=`口令不正确，请再试一次`;return}sessionStorage.setItem(ot,`ok`),$()})}async function Dn(e){let t=e.elements.url,n=e.elements.title,r=e.querySelector(`[data-action="detect-title"]`),i=e.querySelector(`.field-hint`),a=t.value.trim();if(!on(a))return i.textContent=`请输入完整的 http 或 https 网址`,``;r.disabled=!0,r.innerHTML=`<span class="mini-spinner"></span>`,i.textContent=`正在读取网页标题…`;try{let{title:e}=await zt(a);if(!e)throw Error(`read failed`);return n.value=e,i.textContent=`已识别网页标题`,n.value}catch{let e=J(a);return n.value||=e,i.textContent=`网页暂时无法读取，已用域名作为标题，你可以手动修改`,n.value}finally{r.disabled=!1,r.textContent=`Detect title`}}function On(){let e=document.querySelector(`.board`);if(!e)return;let t=null,n=()=>{document.querySelectorAll(`.report-card, .group-column, .topic-nav a`).forEach(e=>{e.classList.remove(`is-card-drop-target`,`is-card-drop-before`,`is-card-drop-after`,`is-drop-ready`,`is-nav-drop-target`)})},r=e=>e?[e.bucketKind,e.bucketId,e.targetReportId,Number(e.placeAfter),Number(e.nav)].join(`|`):``,i=()=>{let e=t.sourceCard.getBoundingClientRect(),n=t.sourceCard.cloneNode(!0);return n.removeAttribute(`id`),n.className=`report-card report-drag-preview`,n.style.width=`${e.width}px`,n.style.height=`${e.height}px`,n.querySelectorAll(`button, [role='button'], iframe`).forEach(e=>{e.removeAttribute(`data-action`),e.setAttribute(`tabindex`,`-1`)}),document.body.append(n),n},a=()=>{t?.preview&&(t.preview.style.transform=`translate3d(${t.x+14}px, ${t.y+14}px, 0)`)},o=(e,n=null,r=!1)=>{if(!(!e||!t?.placeholder)){if(!n||n.parentElement!==e){e.append(t.placeholder);return}e.insertBefore(t.placeholder,r?n.nextSibling:n)}},s=()=>{let e=document.elementFromPoint(t.x,t.y);if(e?.closest(`.report-card-placeholder`))return t.target;let n=e?.closest(`.topic-nav a[data-nav-bucket-id]`);if(n)return{bucketKind:n.dataset.navBucketKind,bucketId:n.dataset.navBucketId,targetReportId:``,placeAfter:!1,nav:!0,element:n};let r=e?.closest(`.report-card:not(.report-card-placeholder):not(.report-drag-preview)`),i=e?.closest(`.group-column`);if(r&&r!==t.sourceCard){let e=r.closest(`.group-column`);if(!e||e.dataset.bucketKind===`time`)return null;let n=r.getBoundingClientRect(),i=t.y>n.bottom-n.height*.22||t.y>=n.top+n.height*.22&&t.y<=n.bottom-n.height*.22&&t.x>n.left+n.width/2;return{bucketKind:e.dataset.bucketKind||M,bucketId:e.dataset.bucketId||``,targetReportId:r.dataset.reportId||``,placeAfter:i,nav:!1,element:r,container:e.querySelector(`.group-cards`)}}return i&&i.dataset.bucketKind!==`time`?{bucketKind:i.dataset.bucketKind||M,bucketId:i.dataset.bucketId||``,targetReportId:``,placeAfter:!1,nav:!1,element:i,container:i.querySelector(`.group-cards`)}:null},c=e=>{if(!(!t||r(e)===r(t.target))&&(n(),t.target=e,L=e,e)){if(e.nav){e.element.classList.add(`is-nav-drop-target`);return}if(e.targetReportId){e.element.classList.add(`is-card-drop-target`,e.placeAfter?`is-card-drop-after`:`is-card-drop-before`),o(e.container,e.element,e.placeAfter);return}e.element.classList.add(`is-drop-ready`),o(e.container)}},l=()=>{t?.active&&c(s())},u=()=>{let e=Math.min(window.innerHeight*.34,ln()+72),n=window.innerHeight-72;if(t.y<e){let n=Math.min(1,(e-t.y)/84);return-Math.max(1,Math.round(12*n*n))}if(t.y>n){let e=Math.min(1,(t.y-n)/84);return Math.max(1,Math.round(12*e*e))}return 0},d=()=>{if(!t?.active)return;let e=u();if(e){let t=window.scrollY;window.scrollBy(0,e),window.scrollY!==t&&l()}t.autoScrollFrame=requestAnimationFrame(d)},f=()=>{!t||t.active||(clearTimeout(t.holdTimer),t.sourceCard.setPointerCapture?.(t.pointerId),t.active=!0,P=t.reportId,F=``,t.preview=i(),t.placeholder=document.createElement(`div`),t.placeholder.className=`report-card report-card-placeholder`,t.placeholder.innerHTML=`<span>放在这里</span>`,t.placeholder.style.minHeight=`${t.sourceCard.getBoundingClientRect().height}px`,t.sourceCard.before(t.placeholder),t.sourceCard.classList.add(`is-dragging`),document.body.classList.add(`report-drag-session`),a(),l(),t.autoScrollFrame=requestAnimationFrame(d))},p=()=>{t&&(clearTimeout(t.holdTimer),t.autoScrollFrame&&cancelAnimationFrame(t.autoScrollFrame),t.preview?.remove(),t.placeholder?.remove(),t.sourceCard.classList.remove(`is-dragging`),document.body.classList.remove(`report-drag-session`),n(),P=``,L=null)},m=()=>{if(!t)return;let e=t,n=e.active?e.target:null,r=e.reportId;e.active&&(vt=r,yt=Date.now()+500),p(),t=null,!(!n?.bucketId||n.bucketKind===`time`)&&$t(r,n.bucketKind,n.bucketId,n.targetReportId||``,!!n.placeAfter)&&(n.nav&&(j=n.bucketKind===`featured`),Q(),n.nav?pn(()=>dn(n.bucketKind,n.bucketId)):requestAnimationFrame(()=>requestAnimationFrame(()=>{let e=fn(r);e?.classList.add(`is-drop-landed`),window.setTimeout(()=>e?.classList.remove(`is-drop-landed`),700)})),X(n.bucketKind===`featured`?`已加入精选成果`:n.bucketKind===`tag`?`已添加目标标签`:n.bucketKind===`type`?`工作类型已更新`:n.targetReportId?`报告顺序已更新`:`已移入新主题`))};e.addEventListener(`pointerdown`,e=>{if(e.button!==0||e.target.closest(`.card-actions`))return;let n=e.target.closest(`.report-card[data-report-draggable="true"]`);n?.closest(`.group-column`)&&(t={pointerId:e.pointerId,reportId:n.dataset.reportId,sourceCard:n,startX:e.clientX,startY:e.clientY,x:e.clientX,y:e.clientY,active:!1,target:null,preview:null,placeholder:null,autoScrollFrame:0,holdTimer:0},t.holdTimer=window.setTimeout(()=>f(),240))}),e.addEventListener(`pointermove`,e=>{!t||e.pointerId!==t.pointerId||(t.x=e.clientX,t.y=e.clientY,!t.active&&Math.hypot(t.x-t.startX,t.y-t.startY)>=8&&f(),t.active&&(e.preventDefault(),a(),l()))}),e.addEventListener(`pointerup`,e=>{!t||e.pointerId!==t.pointerId||m()}),e.addEventListener(`pointercancel`,()=>{p(),t=null})}function kn(){let e=document.getElementById(`search-input`);e?.addEventListener(`input`,e=>{if(e.isComposing)return;O=e.target.value,O&&(j=!1);let t=e.target.selectionStart,n=e.target.selectionEnd;$();let r=document.getElementById(`search-input`);r?.focus(),r?.setSelectionRange(t,n)}),e?.addEventListener(`keydown`,e=>{e.key!==`Escape`||!O||(e.preventDefault(),O=``,$(),document.getElementById(`search-input`)?.focus())}),document.querySelectorAll(`[data-action]`).forEach(e=>{e.addEventListener(`click`,async e=>{let t=e.currentTarget.dataset.action,n=e.currentTarget.dataset.id;if(t===`scroll-top`)Z(`smooth`);else if(t===`open`){if(n===vt&&Date.now()<yt)return;k=n,$(),Z()}else if(t===`edit-document`){let e=D.reports.find(e=>e.id===n);if(!e||e.access!==`production`)return;Ye(e,{render:$,showToast:X})}else if(t===`edit-local-document`){let e=D.reports.find(e=>e.id===n);if(!e||!mn(e))return;Ye(e,{render:$,showToast:X,saveLocal:async t=>{let n=e.savedHtml;e.savedHtml=t,e.isHtml=!0,e.tags=E(e,e.workType),W(e);try{U()}catch{throw e.savedHtml=n,Error(`修改后的 HTML 超过当前浏览器可保存容量，请先下载备份`)}}})}else if(t===`download-report`){let e=D.reports.find(e=>e.id===n);if(!e)return;mn(e)?_n(e)&&X(`HTML 已下载`):await Qe(e,X)}else if(t===`share-report`||t===`copy-production-url`){let e=D.reports.find(e=>e.id===n);e?.url&&await $e(e,e=>{X(e===`报告链接已复制`?`生产 URL 已复制`:e)})}else if(t===`open-browser`){let e=D.reports.find(e=>e.id===n);if(!e)return;vn(e)||X(`浏览器未能打开该报告`)}else if(t===`back`)k=``,z=null,$(),Z();else if(t===`lock`)sessionStorage.removeItem(ot),$();else if(t===`clear-search`)O=``,j=!1,$(),document.getElementById(`search-input`)?.focus();else if(t===`set-view`){if(![`topic`,`type`,`tag`,`time`].includes(n))return;M=n,j=!1,I=``,localStorage.setItem(S,M),Q()}else if(t===`toggle-time-sort`)N=N===`created`?`modified`:`created`,localStorage.setItem(st,N),Q();else if(t===`cancel-move`)I=``,$();else if(t===`move-here`){let t=e.currentTarget.dataset.bucketKind||M;I&&$t(I,t,n)&&(I=``,$(),X(t===`tag`?`已添加目标标签`:`报告已移入目标${en()}`))}else if(t===`show-archive`)A=!0,O=``,k=``,$(),Z();else if(t===`show-catalog`)A=!1,O=``,k=``,$(),Z();else if(t===`add-report`)z={type:`report`,mode:`create`,groupId:D.groups[0]?.id},$();else if(t===`add-to-group`)z={type:`report`,mode:`create`,groupId:n},$();else if(t===`edit`)z={type:`report`,mode:`edit`,reportId:n},$();else if(t===`toggle-pin`){let e=D.reports.find(e=>e.id===n);if(!e)return;e.pinned=!e.pinned,W(e),U(),Q(),X(e.pinned?`已加入精选成果`:`已移出精选成果`)}else if(t===`close-modal`)z=null,$();else if(t===`detect-title`)await Dn(e.currentTarget.closest(`form`));else if(t===`archive`){let e=D.reports.find(e=>e.id===n);if(!e)return;e.archived=!0,e.archivedAt=new Date().toISOString(),U(),$(),X(`已归档，可随时恢复`)}else if(t===`restore`){let e=D.reports.find(e=>e.id===n);if(!e)return;e.archived=!1,e.archivedAt=``,U(),$(),X(`报告已恢复到原主题`)}else if(t===`delete`){let e=D.reports.find(e=>e.id===n);e?.archived&&confirm(`二次确认：永久删除“${e.title}”？\n\n删除后无法从归档区恢复。`)&&(D.reports=D.reports.filter(e=>e.id!==n),k===n&&(k=``),U(),$(),X(`报告已永久删除`))}else if(t===`add-group`)z={type:`group`,mode:`create`},$();else if(t===`rename-group`)D.groups.find(e=>e.id===n)&&(z={type:`group`,mode:`edit`,groupId:n},$());else if(t===`delete-group`){let e=D.groups.find(e=>e.id===n),t=D.groups.find(e=>e.id!==n);e&&!t?X(`请先新增另一个分组，再删除当前分组`):e&&confirm(`删除“${e.name}”？其中的报告会移到“${t.name}”。`)&&(D.reports.forEach(e=>{e.groupId===n&&(e.groupId=t.id)}),D.groups=D.groups.filter(e=>e.id!==n),U(),$(),X(`分组已删除，报告已移到“${t.name}”`))}})}),document.querySelector(`.topbar`)?.addEventListener(`click`,e=>{e.target.closest(`button, a`)||Z(`smooth`)}),document.querySelectorAll(`.topic-nav a[data-nav-bucket-id]`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault();let n=e.dataset.navBucketKind,r=e.dataset.navBucketId,i=e.dataset.navFeatured===`true`;if(O||j!==i){j=i,O=``,Q(),pn(()=>dn(n,r));return}un(dn(n,r))})}),document.querySelectorAll(`.topic-nav a[data-nav-report-id]`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault(),un(fn(e.dataset.navReportId))})}),On(),document.querySelectorAll(`.legacy-report-drag-handle`).forEach(e=>{let t=null,n=!1,r=null,i=0,a=0,o=()=>{i&&=(cancelAnimationFrame(i),0)},s=()=>{if(!P)return o();let e=Math.min(110,window.innerHeight*.18),t=a<e?a-e:a>window.innerHeight-e?a-(window.innerHeight-e):0;if(!t)return o();let n=Math.sign(t)*Math.min(24,5+Math.abs(t)*.2);window.scrollBy(0,n),i=requestAnimationFrame(s)},c=t=>{let n=e.closest(`.report-card`);if(n){if(!r){let e=n.getBoundingClientRect();r=n.cloneNode(!0),r.className=`report-card report-drag-preview`,r.style.width=`${e.width}px`,r.style.height=`${e.height}px`,r.querySelectorAll(`button, [role='button']`).forEach(e=>{e.removeAttribute(`data-action`),e.setAttribute(`tabindex`,`-1`)}),document.body.append(r)}r.style.transform=`translate3d(${t.clientX+16}px, ${t.clientY+16}px, 0)`}},l=()=>{L=null,R?.remove(),R=null,document.querySelectorAll(`.report-card, .group-column, .topic-nav a`).forEach(e=>e.classList.remove(`is-card-drop-target`,`is-card-drop-before`,`is-card-drop-after`,`is-drop-ready`,`is-nav-drop-target`))},u=(t,n,r)=>{if(t){if(!R){R=document.createElement(`div`),R.className=`report-card report-card-placeholder`,R.innerHTML=`<span>放在这里</span>`;let t=e.closest(`.report-card`)?.getBoundingClientRect().height;t&&(R.style.minHeight=`${t}px`)}if(!n||n.parentElement!==t){t.append(R);return}t.insertBefore(R,r?n.nextSibling:n)}},d=t=>{let n=document.elementFromPoint(t.clientX,t.clientY),r=n?.closest(`.topic-nav a[data-nav-bucket-id]`);if(l(),r){r.classList.add(`is-nav-drop-target`),L={bucketKind:r.dataset.navBucketKind,bucketId:r.dataset.navBucketId,targetReportId:``,placeAfter:!1,nav:!0};return}let i=e.closest(`.report-card`),a=n?.closest(`.report-card:not(.report-card-placeholder)`),o=n?.closest(`.group-column`);if(a&&a!==i){let e=a.closest(`.group-column`),n=a.getBoundingClientRect(),r=t.clientY>n.top+n.height/2;a.classList.add(`is-card-drop-target`,r?`is-card-drop-after`:`is-card-drop-before`),u(e?.querySelector(`.group-cards`),a,r),L={bucketKind:e?.dataset.bucketKind||M,bucketId:e?.dataset.bucketId||``,targetReportId:a.dataset.reportId||``,placeAfter:r,nav:!1};return}o&&o.dataset.bucketKind!==`time`&&(o.classList.add(`is-drop-ready`),u(o.querySelector(`.group-cards`),null,!1),L={bucketKind:o.dataset.bucketKind||M,bucketId:o.dataset.bucketId||``,targetReportId:``,placeAfter:!1,nav:!1})},f=()=>{P=``,t=null,n=!1,o(),r?.remove(),r=null,l(),e.closest(`.report-card`)?.classList.remove(`is-dragging`)};e.addEventListener(`pointerdown`,r=>{r.preventDefault(),P=e.dataset.reportDragId,F=``,t={x:r.clientX,y:r.clientY},n=!1,e.setPointerCapture?.(r.pointerId),e.closest(`.report-card`)?.classList.add(`is-dragging`)}),e.addEventListener(`pointermove`,e=>{P&&(t&&Math.hypot(e.clientX-t.x,e.clientY-t.y)<7||(n=!0,a=e.clientY,c(e),d(e),i||=requestAnimationFrame(s)))}),e.addEventListener(`pointerup`,e=>{if(!P)return;let t=P;if(!n){I=t,f(),$(),X(`请选择目标${en()}`);return}let r=L,i=r?.targetReportId||``,a=r?.bucketId||``,o=r?.bucketKind||M,s=a&&o!==`time`?$t(t,o,a,i,!!r?.placeAfter):!1;f(),s&&($(),requestAnimationFrame(()=>{let e=`.group-column[data-bucket-kind="${CSS.escape(o)}"][data-bucket-id="${CSS.escape(a)}"]`,n=document.querySelector(`${e} .report-card[data-report-id="${CSS.escape(t)}"]`)||document.querySelector(`.search-results-cards .report-card[data-report-id="${CSS.escape(t)}"]`);un(n),n?.classList.add(`is-drop-landed`),window.setTimeout(()=>n?.classList.remove(`is-drop-landed`),900)}),X(o===`featured`?`已加入精选成果`:o===`tag`?`已添加目标标签`:o===`type`?`工作类型已更新`:i?`报告顺序已更新`:`已移入新主题`))}),e.addEventListener(`pointercancel`,f)}),document.querySelectorAll(`.group-drag-handle`).forEach(e=>{let t=()=>{F=``,e.closest(`.group-column`)?.classList.remove(`is-group-dragging`),document.querySelectorAll(`.group-column`).forEach(e=>{e.classList.remove(`is-group-drop-target`,`is-drop-ready`)})};e.addEventListener(`pointerdown`,t=>{t.preventDefault(),F=e.dataset.groupDragId,P=``,e.setPointerCapture?.(t.pointerId),e.closest(`.group-column`)?.classList.add(`is-group-dragging`)}),e.addEventListener(`pointermove`,e=>{F&&document.querySelectorAll(`.group-column`).forEach(t=>{t.classList.toggle(`is-group-drop-target`,t===document.elementFromPoint(e.clientX,e.clientY)?.closest(`.group-column`))})}),e.addEventListener(`pointerup`,e=>{if(!F)return;let n=F,r=document.elementFromPoint(e.clientX,e.clientY)?.closest(`.group-column`);if(r&&Wt(n,r.dataset.bucketId,r.dataset.bucketKind)){F=``,$(),X(`分组顺序已更新`);return}t()}),e.addEventListener(`pointercancel`,t),e.addEventListener(`keydown`,t=>{if(![`ArrowLeft`,`ArrowRight`].includes(t.key))return;t.preventDefault();let n=[...document.querySelectorAll(`.group-column`)],r=n.findIndex(t=>t.dataset.bucketId===e.dataset.groupDragId),i=n[t.key===`ArrowLeft`?r-1:r+1];!i||!Wt(e.dataset.groupDragId,i.dataset.bucketId,e.dataset.groupDragKind)||($(),X(`分组顺序已更新`),document.querySelector(`[data-group-drag-id="${CSS.escape(e.dataset.groupDragId)}"]`)?.focus())})}),document.querySelectorAll(`.group-column`).forEach(e=>{e.addEventListener(`dragover`,t=>{t.preventDefault(),e.classList.add(F?`is-group-drop-target`:`is-drop-ready`)}),e.addEventListener(`dragleave`,()=>{e.classList.remove(`is-drop-ready`,`is-group-drop-target`)}),e.addEventListener(`drop`,t=>{if(t.preventDefault(),F){if(Wt(F,e.dataset.bucketId,e.dataset.bucketKind)){F=``,$(),X(`分组顺序已更新`);return}F=``,e.classList.remove(`is-group-drop-target`);return}let n=D.reports.find(e=>e.id===P),r=e.dataset.bucketKind||M;n&&$t(P,r,e.dataset.bucketId)&&(P=``,$(),X(r===`tag`?`已添加目标标签`:r===`type`?`工作类型已更新`:`已移入新主题`)),P=``})});let t=document.getElementById(`group-form`);t?.addEventListener(`submit`,e=>{e.preventDefault();let n=new FormData(t).get(`name`)?.trim(),r=new FormData(t).get(`description`)?.trim();if(!n)return;if(z.mode===`edit`){let e=D.groups.find(e=>e.id===z.groupId);if(!e)return;e.name=n.slice(0,60),e.description=r?.slice(0,80)||`自定义工作主题`}else D.groups.push({id:rn(`group`),name:n.slice(0,60),description:r?.slice(0,80)||`自定义工作主题`,accent:[`blue`,`violet`,`amber`,`green`][D.groups.length%4],position:D.groups.length}),M=`topic`,localStorage.setItem(S,M);U();let i=z.mode===`edit`?`工作主题已更新`:`工作主题已创建，可直接拖入报告`;z=null,$(),X(i)});let n=document.getElementById(`report-form`),r=n?.elements.tags,i=e=>{let t=Y(r?.value).includes(e.dataset.reportTag);e.classList.toggle(`selected`,t),e.setAttribute(`aria-pressed`,String(t))},a=e=>{e.addEventListener(`click`,()=>{let t=Y(r.value),n=e.dataset.reportTag;r.value=t.includes(n)?t.filter(e=>e!==n).join(`、`):[...t,n].slice(0,8).join(`、`),i(e)})};n?.querySelectorAll(`[data-report-tag]`).forEach(a);let o=n?.querySelector(`[data-add-report-tag]`),s=n?.querySelector(`.new-report-tag-row`),c=n?.querySelector(`[data-new-report-tag]`),l=()=>{s.hidden=!1,c.focus()},u=()=>{let[e]=Y(c.value);if(!e)return;let t=Y(r.value);if(!t.includes(e)&&t.length>=8){X(`最多选择 8 个标签`);return}r.value=[...new Set([...t,e])].slice(0,8).join(`、`);let l=[...n.querySelectorAll(`[data-report-tag]`)].find(t=>t.dataset.reportTag===e);l||(l=document.createElement(`button`),l.type=`button`,l.dataset.reportTag=e,l.textContent=e,o.before(l),a(l)),i(l),c.value=``,s.hidden=!0,o.focus()};o?.addEventListener(`click`,l),n?.querySelector(`[data-confirm-report-tag]`)?.addEventListener(`click`,u),c?.addEventListener(`keydown`,e=>{e.key===`Enter`?(e.preventDefault(),u()):e.key===`Escape`&&(e.preventDefault(),c.value=``,s.hidden=!0,o.focus())}),n?.addEventListener(`submit`,async e=>{e.preventDefault();let t=n.elements.url.value.trim();if(!on(t))return;let r=n.querySelector(`button[type="submit"]`),i=n.querySelector(`.field-hint`);r.disabled=!0,r.innerHTML=`<span class="mini-spinner"></span>`;let a=Pt({material:t,files:[],url:t,excludeId:z.mode===`edit`?z.reportId:``});if(a){r.disabled=!1,r.textContent=`Save`,i.textContent=`成果库已有“${a.title}”，未重复保存`,X(`成果库已有“${a.title}”，未重复保存`);return}let o=await Bt({material:t,files:[],url:t},e=>{i.textContent=e});if(!o.allowed){r.disabled=!1,r.textContent=`Save`,i.textContent=o.reason,X(o.reason);return}let s=n.elements.title.value.trim()||o.metadata.title,c=n.elements.groupId.value,l=n.elements.workType.value,u=Y(n.elements.tags.value),d={title:s||J(t),url:t,groupId:c,workType:l,source:`手动添加`,access:o.access,detectedDescription:o.metadata.description,manualSaved:!0,isProduction:o.access===`production`,isPersonal:Ft(t),isHtml:o.isHtml,loginProvider:o.loginProvider},f=z.mode===`edit`?u:[...new Set([...E(d,l),...u])].slice(0,8);if(z.mode===`edit`){let e=D.reports.find(e=>e.id===z.reportId);Object.assign(e,d,{tags:f}),W(e)}else{let e=new Date().toISOString(),t={id:rn(`report`),groupId:c,...d,pinned:!1,position:D.reports.filter(e=>e.groupId===c).length,createdAt:e,modifiedAt:e,archived:!1,archivedAt:``,tags:f};D.reports.push(t)}U(),z=null,$(),X(`报告已保存`)});let d=k&&D.reports.find(e=>e.id===k);d&&Ze(d)}function An(){wt(),$()}An(document.getElementById(`app`));