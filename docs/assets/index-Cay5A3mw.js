...
`}function De(e,t){let n=new DOMParser().parseFromString(e,`text/html`);n.querySelectorAll(`meta[http-equiv="Content-Security-Policy" i]`).forEach(e=>{e.dataset.clairEditorHttpEquiv=e.getAttribute(`http-equiv`)||`Content-Security-Policy`,e.setAttribute(`http-equiv`,`x-clair-csp-disabled`)}),Te(n);let r=n.createElement(`base`);r.href=t,r.dataset.clairEditorBase=`true`,n.head.prepend(r);let i=n.createElement(`style`);i.id=`clair-editor-style`,i.textContent=`
  ...
  `,n.head.append(i);let a=n.createElement(`script`);return a.id=`clair-editor-bridge`,a.textContent=Ee(),n.body.append(a),`<!DOCTYPE html>\n${n.documentElement.outerHTML}`}function Oe(e){if(e.url)return``;if(e.savedHtml)return e.savedHtml;let t=(e.savedFiles||[]).find(e=>/\.html?$/i.test(e.name||``));return t?.content||t?.excerpt?t.content||t.excerpt:/<!doctype\s+html|<html[\s>]/i.test(e.savedContent||``)?e.savedContent.trim():``}async function ke(e){try{let t=Oe(e),n=t?null:Se(e.url),r=null;if(t)r={html:t,target:null};else if(n)try{r=await we(n)}catch{}if(!r&&e.url){let t=await fetch(e.url,{cache:`no-store`});if(!t.ok)throw Error(`报告读取失败（HTTP ${t.status}）`);r={html:await t.text(),target:n}}let i=await be(r.html);_.protection=i.protection,_.target=r.target||n;let a=i.html,o=de(e.id);if(o?.html)try{let e=await be(o.html);a=e.html,_.hasDraft=!0,_.draftHtml=e.html,_.draftAt=o.savedAt||``,o.baseFiles&&_.target&&(_.target.baseFiles=o.baseFiles)}catch{fe(e.id)}_.html=a,_.editorDocument=De(a,e.url||window.location.href),_.status=`ready`,_.currentPage=0,_.pageCount=1,_.error=``}catch(e){_.status=`error`,_.error=e?.message||`无法读取这份 HTML`}finally{_.loadPromise=null,_.render?.()}}function Ae(){let e=_.render,t=_.showToast;Object.assign(_,{reportId:``,reportTitle:``,reportUrl:``,status:`idle`,error:``,html:``,editorDocument:``,dirty:!1,hasDraft:!1,draftHtml:``,draftAt:``,target:null,settingsOpen:!1,publishConfirmOpen:!1,pendingSave:!1,saving:!1,lastCommit:``,isLocal:!1,saveLocal:null,protection:null,currentPage:0,pageCount:1,loadPromise:null,render:e,showToast:t})}function y(){return document.querySelector(`.report-editor-frame`)}function je(e,t=null){y()?.contentWindow?.postMessage({channel:g,type:`command`,command:e,value:t},`*`)}function Me(e){let t=y();if(!t?.contentWindow)return;let n=Math.max(1,Number(_.pageCount)||1),r=Number.isFinite(Number(e))?Number(e):_.currentPage,i=Math.max(0,Math.min(n-1,r));t.contentWindow.postMessage({channel:g,type:`set-page`,page:i},`*`),_.currentPage=i,v()}function b(){let e=y();if(!e?.contentWindow)return Promise.reject(Error(`编辑画布尚未就绪`));let t=crypto.randomUUID?.()||`${Date.now()}-${Math.random()}`;return new Promise((n,r)=>{let i=window.setTimeout(()=>{oe.delete(t),r(Error(`读取编辑内容超时`))},1e4);oe.set(t,{resolve:e=>{clearTimeout(i),n(e)}}),e.contentWindow.postMessage({channel:g,type:`serialize`,requestId:t},`*`)})}function Ne(e){return`${String(e||`report`).replace(/[\\/:*?"<>|]+/g,`-`).replace(/\s+/g,`-`).replace(/-+/g,`-`).replace(/^-|-$/g,``).slice(0,80)||`report`}.html`}function Pe(e,t){let n=new Blob([e],{type:`text/html;charset=utf-8`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=Ne(t),document.body.append(i),i.click(),i.remove(),window.setTimeout(()=>URL.revokeObjectURL(r),1e3)}async function Fe(e){await navigator.clipboard.writeText(e)}function Ie(e,t){let n=new DOMParser().parseFromString(e,`text/html`);n.querySelector(`base[data-clair-preview-base]`)?.remove();let r=n.createElement(`base`);return r.href=t,r.dataset.clairPreviewBase=`true`,n.head.prepend(r),`<!DOCTYPE html>\n${n.documentElement.outerHTML}`}function Le(e){if(!_.hasDraft||!_.draftHtml)throw Error(`请先暂存当前修订，再另开预览`);let t=new Blob([Ie(_.draftHtml,e.url||window.location.href)],{type:`text/html;charset=utf-8`}),n=URL.createObjectURL(t),r=window.open(n,`_blank`);if(!r)throw URL.revokeObjectURL(n),Error(`浏览器拦截了新窗口，请允许弹窗后重试`);r.opener=null,window.setTimeout(()=>URL.revokeObjectURL(n),6e4)}async function x(e,{silent:t=!1}={}){let n=await b(),r=await xe(n),i=new Date().toISOString();try{sessionStorage.setItem(ue(e.id),JSON.stringify({reportId:e.id,reportUrl:e.url,savedAt:i,baseFiles:le(),html:r}))}catch{throw Error(`浏览器暂存空间不足，请先下载 HTML 备份`)}return _.html=n,_.draftHtml=n,_.draftAt=i,_.hasDraft=!0,_.dirty=!1,_.lastCommit=``,v(),t||_.showToast?.(_.isLocal?`已暂存在当前浏览器会话，尚未写回成果库`:`已暂存在当前浏览器会话，尚未更新 GitHub`),n}async function Re(e){if(!(_.saving||!_.saveLocal)){_.saving=!0,v();try{let t=_.dirty?await x(e,{silent:!0}):_.draftHtml||await b();await _.saveLocal(t),_.html=t,_.dirty=!1,_.hasDraft=!1,_.draftHtml=``,_.draftAt=``,_.lastCommit=`local`,fe(e.id),_.showToast?.(`已更新成果库中的 HTML`)}catch(e){_.showToast?.(e?.message||`保存失败，请下载 HTML 备份`)}finally{_.saving=!1,v()}}}async function ze(e){let t=_.target;if(!t?.owner||!t.repository||!t.path||!t.branch)throw Error(`请先填写 GitHub 仓库、分支和 HTML 路径`);if(!_.token)throw Error(`请先提供 GitHub Fine-grained Token`);let n=await xe(e),r=(t.mirrors||[]).map(e=>e.path),i=ce([...r.filter(e=>e.startsWith(`public/`)),...r.filter(e=>!e.startsWith(`public/`)&&e!==t.path),t.path]),a=``,o=[];for(let e of i)try{let r=e.split(`/`).map(encodeURIComponent).join(`/`),i=`/repos/${encodeURIComponent(t.owner)}/${encodeURIComponent(t.repository)}/contents/${r}`,s=await Ce(`${i}?ref=${encodeURIComponent(t.branch)}`,{token:_.token}),c=le(t)[e];if(c&&s.sha!==c)throw Error(`生产文件 ${e} 已在本次编辑后更新，请重新打开报告合并修改`);let l=await Ce(i,{token:_.token,method:`PUT`,body:{message:`Update ${_.reportTitle} from Clair's Studio`,content:ge(n),sha:s.sha,branch:t.branch}});a=l?.commit?.sha||a,t.baseFiles={...le(t),[e]:l?.content?.sha||s.sha},o.push(e)}catch(t){throw o.length?Error(`已更新 ${o.join(`、`)}，但 ${e} 同步失败：${t.message}`):t}return{commit:a,files:o.length}}async function Be(e){if(!_.saving){_.saving=!0,v();try{let t=_.dirty?await x(e,{silent:!0}):_.draftHtml||await b(),n=await ze(t);_.html=t,_.dirty=!1,_.hasDraft=!1,_.draftHtml=``,_.draftAt=``,_.lastCommit=n.commit,fe(e.id),_.showToast?.(n.files>1?`已同步 ${n.files} 个 GitHub 文件，Pages 正在更新`:`已提交 GitHub，Pages 正在更新`)}catch(e){_.showToast?.(e?.message||`保存失败，请下载 HTML 备份`)}finally{_.saving=!1,v()}}}function Ve(e){let t=_.target||{owner:`ClairKu`,repository:``,branch:`main`,path:``};return`
    ...
    </div>`}function He(e){let t=_.target?`${_.target.owner}/${_.target.repository} · ${_.target.path}`:`尚未识别 GitHub 文件路径`;return`
    ...
    <section class="generated-results">
      <div class="section-heading">
        ...
        <span>${e.length} 份命中</span>
      </div>
      <div class="search-result-grid search-results-grid">
        ${e.length?e.map(e=>Ht(e)).join(``):`
          <p class="generated-empty">未检索到匹配内容，换一个关键词试试。</p>
        `}
      </div>
    </section>`}function Ht(e){let t=w(t),n=[e.source?J(e.source):`手动添加`,(e.tags||[]).length?`标签：${J((e.tags||[]).slice(0,4).join(` / `))}`:``,Ut(e,t)].filter(Boolean);return`
    <article class="report-card search-result-card" data-report-id="${J(e.id)}">
      <a class="card-main card-link" href="#"
        data-action="open" data-id="${J(e.id)}" aria-label="打开${J(e.title)}">
        <span class="report-preview">
          ${e.url?`<img src="./previews/${J(e.id)}.png" alt="" loading="lazy" decoding="async" onerror="this.remove()" />`:`<span class="search-result-placeholder">R</span>`}
        </span>
        <span class="report-copy">
          <span class="report-source">${n[0]||`搜索结果`}</span>
          <strong>${J(e.title)}</strong>
          ${n.slice(1).map(e=>`<span class=\"search-result-meta\">${e}</span>`).join(``)}
        </span>
      </a>
      ...
      <div class="card-actions">
        <button type="button" data-action="toggle-pin" data-id="${J(e.id)}" title="${e.pinned?`取消精选`:`设置精选`}">
          ${e.pinned?`★`:`☆`}
        </button>
      </div>
      ${e.pinned?`<span class="report-pin-note">精选</span>`:``}
    </article>`}function Ut(e,t=``){if(!t)return``;let n=[e.source,e.title,e.url,(e.tags||[]).join(` `)].filter(Boolean).join(` | `),r=String(n).replace(/<[^>]+>/g,` `).replace(/\\s+/g,` `).trim(),i=r.toLocaleLowerCase(`zh-CN`),a=searchTokens(t)[0];if(!a||i.length<a.length)return``;let o=i.indexOf(a);if(o===-1)return``;let s=Math.max(0,o-30),c=Math.min(r.length,o+a.length+36),l=r.slice(s,c).trim();return`${s>0?`…`:``}${J(l)}${c<r.length?`…`:``}`}function Wt(e){return`${e}-${crypto.randomUUID?.()||`${Date.now()}-${Math.random()}`}`}function J(e=``){return String(e).replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`).replaceAll(`'`,`&#39;`)}var Gt={back:`
    <svg viewBox="0 0 24 24" aria-hidden="true">
    ...
    </svg>`};function Y(e){return Gt[e]||``}function X(e){try{return new URL(e).hostname.replace(/^www\./,``)}catch{return e}}function Kt(e){try{return[`http:`,`https:`].includes(new URL(e).protocol)}catch{return!1}}function qt(e=``){return[...new Set(String(e).split(/[、,，;；\n]+/).map(e=>e.trim()).filter(Boolean).map(e=>e.slice(0,20)))].slice(0,8)}function Z(e){document.querySelector(`.toast`)?.remove();let t=document.createElement(`div`);t.className=`toast`,t.setAttribute(`role`,`status`),t.textContent=e,document.body.append(t),clearTimeout(dt),dt=window.setTimeout(()=>t.remove(),2600)}function Q(e=`auto`){requestAnimationFrame(()=>{window.scrollTo({top:0,left:0,behavior:e})})}function Jt(e){return e.savedHtml||wt(e.savedContent,e.savedFiles)}function Yt(e){return`${String(e.title||`report`).replace(/[\\/:*?"<>|]+/g,`-`).replace(/\s+/g,` `).trim().slice(0,80)||`report`}.html`}function Xt(e){let t=Jt(e);return t?URL.createObjectURL(new Blob([t],{type:`text/html;charset=utf-8`})):``}function Zt(e){let t=Xt(e);if(!t)return!1;let n=document.createElement(`a`);return n.href=t,n.download=Yt(e),document.body.append(n),n.click(),n.remove(),window.setTimeout(()=>URL.revokeObjectURL(t),1e3),!0}function Qt(e){let t=e.url||Xt(e);return t?(window.open(t,`_blank`,`noopener,noreferrer`),e.url||window.setTimeout(()=>URL.revokeObjectURL(t),6e4),!0):!1}function $t(e,t=!1){let n=!e.url&&(!!e.savedContent||!!(e.savedFiles||[]).length),r=[`org`,`account`].includes(e.access),i=e.access===`org`?`需组织登录`:e.access===`account`?`需账号登录`:`生产可访问`,a=Jt(e),o=F===`time`?Rt(e.createdAt):e.source||`手动添加`,s=!!e.pinned,c=!r&&O.reports.some(t=>t.id===e.id),l=e.preview||`${e.id}.png`,u=a&&e.isHtml?`<iframe class="local-html-preview-frame" title="${J(e.title)}视觉预览"
        srcdoc="${J(a)}" sandbox="allow-scripts" loading="lazy"
        tabindex="-1" aria-hidden="true"></iframe>`:c?`<img src="./previews/${J(l)}" alt="" loading="lazy" decoding="async" />`:`
      <div class="preview-placeholder ${r?`preview-restricted`:``}">
        <span>${r?`ACCESS`:J(e.title.slice(0,2))}</span>
        <strong>${r?i:n?`本地内容`:`预览待补充`}</strong>
      </div>`;return`
    <article class="report-card ${r?`restricted-card`:``} ${t?`archived-card`:``} ${s?`is-pinned`:``} ${z===e.id?`is-move-selected`:``}" data-report-id="${J(e.id)}">
      <button class="card-main" type="button" data-action="open" data-id="${J(e.id)}" aria-label="打开${J(e.title)}">
        ...
        <span class="report-copy">
          ...
          <strong>${J(e.title)}</strong>
          ${s?`<span class="report-pin-note">精选</span>`:``}
      ...
      <div class="card-actions">
        ${t?`
            <button type="button" data-action="restore" data-id="${J(e.id)}">Restore</button>
            <button type="button" data-action="delete" data-id="${J(e.id)}">Delete permanently</button>`:`
            <button type="button" class="tag-edit-action" data-action="edit-tags" data-id="${J(e.id)}" title="编辑标签" aria-label="编辑标签">#</button>
            ${e.url?`<button type="button" data-action="edit" data-id="${J(e.id)}">Edit</button>`:``}
            <button type="button" data-action="toggle-pin" data-id="${J(e.id)}" title="${s?`取消精选`:`设置精选`}" aria-label="${s?`取消精选`:`设置精选`}">${s?`★`:`☆`}</button>
            <button type="button" data-action="archive" data-id="${J(e.id)}">Archive</button>`}
      </div>
    </article>`}function en(){if(!H)return``;if(H.type===`tags`){let e=A.reports.find(e=>e.id===H.reportId);return e?`
      <div class="dialog-backdrop">
        <form class="dialog compact-dialog tag-dialog" id="tag-form">
          ...
          <div class="tag-suggestions">
            ${D.map(t=>`<button type="button" class="${(e.tags||[]).includes(t)?`selected`:``}" data-tag-suggestion="${J(t)}">${J(t)}</button>`).join(``)}
          </div>
          <div class="dialog-actions">
      ...
      </div>`:``}if(H.type===`group`){let e=H.mode===`edit`?A.groups.find(e=>e.id===H.groupId):null;return`
      <div class="dialog-backdrop">
      ...
      </div>`}let e=H.mode===`edit`?A.reports.find(e=>e.id===H.reportId):null,t=e?.groupId||H.groupId||A.groups[0]?.id||``;return`
    <div class="dialog-backdrop">
      <form class="dialog" id="report-form">
        ...
        <label>放入分组
          <select name="groupId">
            ${A.groups.map(e=>`<option value="${J(e.id)}" ${e.id===t?`selected`:``}>${J(e.name)}</option>`).join(``)}
          </select>
        </label>
        <label>工作类型
          <select name="workType">
            ${at.map(t=>`<option value="${J(t.id)}" ${t.id===(e?.workType||`product-planning`)?`selected`:``}>${J(t.name)}</option>`).join(``)}
          </select>
        </label>
        <label>关键标签
    ...
    </main>`}function nn(e){if(Ge(e.id))return qe(e,J);let t=!e.url&&(!!e.savedContent||!!(e.savedFiles||[]).length),n=[`org`,`account`].includes(e.access),r=e.loginProvider||Tt(e.url)?.provider||(e.access===`org`?`组织帐号`:`站点帐号`),i=e.savedHtml||wt(e.savedContent,e.savedFiles),a=i?`edit-local-document`:e.url?n?`edit`:`edit-document`:``,o=i?`
...
`,`<br />`)}</div>`:``}
    ...
    <header class="topbar">
      ...
      ${P?`<div class="top-actions"><button class="quiet-button" type="button" data-action="show-catalog">← Library</button></div>`:``}
    </header>`}function an(){let e=A.reports.filter(e=>e.archived).filter(e=>$e(e,M,{group:A.groups.find(t=>t.id===e.groupId),workTypeName:G(e.workType)})).sort((e,t)=>new Date(t.archivedAt||0)-new Date(e.archivedAt||0)),t=A.reports.filter(e=>e.archived).length;return`
    <main class="app-shell archive-shell">
      ...
      <section class="workspace archive-workspace">
        ...
        <label class="search archive-search">
          <span aria-hidden="true">⌕</span>
          <input id="search-input" value="${J(M)}"
            placeholder="搜索归档标题、来源或网址" aria-label="搜索归档" />
          ${M?`<button type="button" data-action="clear-search">Clear</button>`:``}
        </label>
        ${e.length?`
          <section class="archive-results">
            <div class="archive-heading">
              <div><h2>${M?`搜索结果`:`归档内容`}</h2><p>按最近归档时间排列</p></div>
              <span>${e.length} 份</span>
            </div>
            <div class="archive-grid">${e.map(e=>$t(e,!0)).join(``)}</div>
          </section>`:`
          <section class="archive-empty">
            <span>ARCHIVE</span>
            <h2>${M?`没有找到相关归档`:`归档区还是空的`}</h2>
            <p>${M?`换个关键词，或返回查看全部归档内容。`:`在主目录的报告卡片上选择“归档”，内容就会安全收纳在这里。`}</p>
            <button class="quiet-button" type="button" data-action="${M?`clear-search`:`show-catalog`}">${M?`Clear search`:`Back to library`}</button>
          </section>`}
      ...
      ${en()}
    </main>`}function on(){if(P)return an();let e=w(M),t=A.reports.filter(e=>!e.archived),n=e?Bt(t,e):t,r=t.filter(e=>e.pinned).sort((e,t)=>{let n=Ft(e)||Pt(e);return(Ft(t)||Pt(t))-n}),i=zt(t,``),a=zt(t,e).filter(t=>t.reports.length||z||F===`topic`&&!e),o=A.reports.filter(e=>e.archived).length,s=t.filter(e=>e.access===`production`).length,c=t.filter(e=>e.access!==`production`).length,l=F===`type`?`工作类型`:F===`tag`?`关键标签`:F===`time`?`新增时间`:`工作主题`;return`
    <main class="app-shell">
        ...
        <div class="results-toolbar unified-results-toolbar">
          ...
          <div class="results-toolbar-side">
            <label class="search results-search">
              <span aria-hidden="true">⌕</span>
              <input id="search-input" type="search" value="${J(M)}"
            ...
            <div class="studio-summary compact-summary" aria-label="成果统计">
              ...
              <i></i>
              <strong>${A.groups.length}</strong><span>主题</span>
              <i></i>
              <strong>${s}</strong><span>直达</span>
              <i></i>
              <strong>${I===`modified`?`修改`:`创建`}倒序</strong><span>时间</span>
            </div>
            <label class="time-sort-control">
              <span>时间排序</span>
              <select data-action="set-time-sort">
                <option value="created"${I===`created`?` selected`:``}>创建时间</option>
                <option value="modified"${I===`modified`?` selected`:``}>修改时间</option>
              </select>
            </label>
          </div>
        </div>
        <section class="groups-section">
          ${r.length?`
            <section class="pinned-section">
              <div class="section-heading">
                <div>
                  <span class="section-kicker">精选成就</span>
                  <h2>精选成果</h2>
                </div>
                <span>${r.length} 份</span>
              </div>
              <div class="pinned-grid">${r.map(e=>$t(e)).join(``)}</div>
            </section>`:``}
          ${z?`
          ...
          ${a.length||e?`
            <div class="library-layout">
              <nav class="topic-nav" aria-label="报告${l}">
                <div class="library-nav-controls">
                  <div class="library-view-switcher" role="tablist" aria-label="成果分类方式">
                    <button type="button" role="tab" aria-selected="${F===`topic`}" class="${F===`topic`?`active`:``}" data-action="set-view" data-id="topic">Topic</button>
                    <button type="button" role="tab" aria-selected="${F===`type`}" class="${F===`type`?`active`:``}" data-action="set-view" data-id="type">Type</button>
                    <button type="button" role="tab" aria-selected="${F===`tag`}" class="${F===`tag`?`active`:``}" data-action="set-view" data-id="tag">Tag</button>
                    <button type="button" role="tab" aria-selected="${F===`time`}" class="${F===`time`?`active`:``}" data-action="set-view" data-id="time">Time</button>
                  </div>
                  <button class="add-topic-icon" type="button" data-action="add-group"
                    aria-label="Add topic" title="Add topic">＋</button>
                </div>
                ${i.map((e,t)=>`
                  <a href="#bucket-${t}" class="topic-nav-item" data-nav-bucket-kind="${J(e.kind)}"
                    data-nav-bucket-id="${J(e.id)}" data-nav-index="${t}" title="拖到此处快速归类：${J(e.name)}">
                    ${J(e.name)}<span>${e.reports.length}</span>
                  </a>`).join(``)}
                <span class="library-nav-spacer" aria-hidden="true"></span>
                ${F===`tag`?`
                  <button class="library-nav-utility" type="button" data-create-tag="1" data-action="add-tag"
                    aria-label="新增标签" title="新增标签（拖拽后快速创建）">
                    <span aria-hidden="true">＋</span>
                    <strong>新增标签</strong>
                  </button>`:``}
              ...
              <div class="board catalog-view-${F}">
              ${e?`${n.length?Vt(n):`<div class="no-results">
                      <strong>没有找到“${J(M.trim())}”</strong>
                  ...
                  <section id="bucket-${t}" class="group-column topic-section bucket-${J(e.kind)} accent-${J(e.accent||`blue`)}"
                    ...
                    data-group-id="${J(e.id)}">
                    <header class="group-header">
                      <div class="group-heading-copy group-drag-handle" role="button" tabindex="0"
                        data-group-drag-id="${J(e.id)}"
                        data-group-drag-kind="${J(e.kind)}"
                        aria-label="Drag ${J(e.name)} to reorder"
                        title="Drag to reorder · use left or right arrow keys">
                        <div><h2>${J(e.name)}</h2></div>
                        <span class="count">${e.reports.length} 份</span>
                      </div>
                      <div class="group-menu">
                        ${z?`<button class="move-here-button" type="button" data-action="move-here" data-id="${J(e.id)}" data-bucket-kind="${J(e.kind)}">Move here</button>`:``}
                        ${e.kind===`topic`?`<button type="button" data-action="add-to-group" data-id="${J(e.id)}">Add report</button>
                             <button type="button" data-action="rename-group" data-id="${J(e.id)}">Rename</button>
                             ${e.id===`inbox`?``:`<button type="button" data-action="delete-group" data-id="${J(e.id)}">Delete</button>`}`:``}
                      </div>
                    </header>
                    <div class="group-cards">
                      ${e.reports.length?e.reports.map(e=>$t(e)).join(``):e.kind===`topic`?`<button class="empty-topic-drop" type="button" data-action="add-to-group" data-id="${J(e.id)}">
      ...
      ${en()}
    </main>`}function $(){let e=document.getElementById(`app`);if(sessionStorage.getItem(nt)!==`ok`){e.innerHTML=tn(),sn();return}let t=N&&A.reports.find(e=>e.id===N);e.innerHTML=t?nn(t):on(),ln(),p({render:$,showToast:Z,saveToLibrary:Ot})}function sn(){let e=document.getElementById(`login-form`);e?.addEventListener(`submit`,t=>{if(t.preventDefault(),new FormData(e).get(`password`)!==`2026`){let t=e.querySelector(`.form-error`);t.hidden=!1,t.textContent=`口令不正确，请再试一次`;return}sessionStorage.setItem(nt,`ok`),$()})}async function cn(e){let t=e.elements.url,n=e.elements.title,r=e.querySelector(`[data-action="detect-title"]`),i=e.querySelector(`.field-hint`),a=t.value.trim();if(!Kt(a))return i.textContent=`请输入完整的 http 或 https 网址`,``;r.disabled=!0,r.innerHTML=`<span class="mini-spinner"></span>`,i.textContent=`正在读取网页标题…`;try{let{title:e}=await Et(a);if(!e)throw Error(`read failed`);return n.value=e,i.textContent=`已识别网页标题`,n.value}catch{let e=X(a);return n.value||=e,i.textContent=`网页暂时无法读取，已用域名作为标题，你可以手动修改`,n.value}finally{r.disabled=!1,r.textContent=`Detect title`}}function ln(){let e=document.getElementById(`search-input`);e?.addEventListener(`input`,e=>{if(e.isComposing)return;M=e.target.value;let t=e.target.selectionStart,n=e.target.selectionEnd;$();let r=document.getElementById(`search-input`);r?.focus(),r?.setSelectionRange(t,n)}),e?.addEventListener(`keydown`,e=>{e.key!==`Escape`||!M||(e.preventDefault(),M=``,$(),document.getElementById(`search-input`)?.focus())}),document.querySelectorAll(`[data-action]`).forEach(e=>{e.addEventListener(`click`,async e=>{let t=e.currentTarget.dataset.action,n=e.currentTarget.dataset.id;if(t===`scroll-top`)Q(`smooth`);else if(t===`open`)N=n,$(),Q();else if(t===`edit-document`){let e=A.reports.find(e=>e.id===n);if(!e||e.access!==`production`)return;Ke(e,{render:$,showToast:Z})}else if(t===`edit-local-document`){let e=A.reports.find(e=>e.id===n);if(!e||!Jt(e))return;Ke(e,{render:$,showToast:Z,saveLocal:async t=>{let n=e.savedHtml;e.savedHtml=t,e.isHtml=!0,e.tags=k(e,e.workType);try{W()}catch{throw e.savedHtml=n,Error(`修改后的 HTML 超过当前浏览器可保存容量，请先下载备份`)}}})}else if(t===`download-report`){let e=A.reports.find(e=>e.id===n);if(!e)return;Jt(e)?Zt(e)&&Z(`HTML 已下载`):await Ye(e,Z)}else if(t===`share-report`||t===`copy-production-url`){let e=A.reports.find(e=>e.id===n);e?.url&&await Xe(e,e=>{Z(e===`报告链接已复制`?`生产 URL 已复制`:e)})}else if(t===`open-browser`){let e=A.reports.find(e=>e.id===n);if(!e)return;Qt(e)||Z(`浏览器未能打开该报告`)}else if(t===`back`)N=``,H=null,$(),Q();else if(t===`lock`)sessionStorage.removeItem(nt),$();else if(t===`clear-search`)M=``,$(),document.getElementById(`search-input`)?.focus();else if(t===`set-view`){if(![`topic`,`type`,`tag`,`time`].includes(n))return;F=n,z=``,localStorage.setItem(T,F),$(),Q()}else if(t===`set-time-sort`){let t=e.currentTarget.value||`created`;[`created`,`modified`].includes(t)&&(I=t,localStorage.setItem(rt,I),$())}else if(t===`toggle-pin`){let e=A.reports.find(e=>e.id===n);if(!e)return;e.pinned=!e.pinned,e.pinned&&Mt(e),W(),$(),Z(e.pinned?`已设为精选`:`已取消精选`)}else if(t===`cancel-move`)z=``,$();else if(t===`move-here`){let t=e.currentTarget.dataset.bucketKind||F;z&&K(z,t,n)&&(z=``,$(),Z(t===`tag`?`已添加目标标签`:`报告已移入目标${q()}`))}else if(t===`show-archive`)P=!0,M=``,N=``,$(),Q();else if(t===`show-catalog`)P=!1,M=``,N=``,$(),Q();else if(t===`add-report`)H={type:`report`,mode:`create`,groupId:A.groups[1]?.id||A.groups[0]?.id},$();else if(t===`add-to-group`)H={type:`report`,mode:`create`,groupId:n},$();else if(t===`add-tag`){let e=window.prompt(`输入新标签名称（例如：用户反馈）`);if(!e)return;let t=String(e).trim().slice(0,20);if(!t)return;if(D.includes(t)||j.has(t)){Z(`标签「${t}」已存在`);return}j.add(t),ft(),Z(`已新增标签：${t}`),$()}else if(t===`edit`)H={type:`report`,mode:`edit`,reportId:n},$();else if(t===`edit-tags`)H={type:`tags`,reportId:n},$();else if(t===`close-modal`)H=null,$();else if(t===`detect-title`)await cn(e.currentTarget.closest(`form`));else if(t===`archive`){let e=A.reports.find(e=>e.id===n);if(!e)return;e.archived=!0,e.archivedAt=new Date().toISOString(),W(),$(),Z(`已归档，可随时恢复`)}else if(t===`restore`){let e=A.reports.find(e=>e.id===n);if(!e)return;e.archived=!1,e.archivedAt=``,W(),$(),Z(`报告已恢复到原主题`)}else if(t===`delete`){let e=A.reports.find(e=>e.id===n);e?.archived&&confirm(`二次确认：永久删除“${e.title}”？\n\n删除后无法从归档区恢复。`)&&(A.reports=A.reports.filter(e=>e.id!==n),N===n&&(N=``),W(),$(),Z(`报告已永久删除`))}else if(t===`add-group`)H={type:`group`,mode:`create`},$();else if(t===`rename-group`)A.groups.find(e=>e.id===n)&&(H={type:`group`,mode:`edit`,groupId:n},$());else if(t===`delete-group`){let e=A.groups.find(e=>e.id===n);e&&confirm(`删除“${e.name}”？其中的报告会移到“待整理”。`)&&(A.reports.forEach(e=>{e.groupId===n&&(e.groupId=`inbox`)}),A.groups=A.groups.filter(e=>e.id!==n),W(),$(),Z(`分组已删除，报告已移到待整理`))}})}),document.querySelector(`.topbar`)?.addEventListener(`click`,e=>{e.target.closest(`button, a`)||Q(`smooth`)}),document.querySelectorAll(`.report-drag-handle`).forEach(e=>{let t=null,n=!1,r=0,i=()=>{B=null,V?.remove(),V=null,document.querySelectorAll(`.topic-nav-item, .group-column, .library-nav-utility, .report-card`).forEach(e=>{e.classList.remove(`is-nav-drop-target`,`is-drop-ready`,`is-card-drop-target`,`is-card-drop-before`,`is-card-drop-after`)})},a=()=>{r&&=(cancelAnimationFrame(r),0)},o=e=>{L&&(e<80?r||=requestAnimationFrame(()=>{window.scrollBy(0,-14),r=0,o(e)}):e>window.innerHeight-80?r||=requestAnimationFrame(()=>{window.scrollBy(0,14),r=0,o(e)}):a())},s=(e,t,n)=>{if(e)if(V||(V=document.createElement(`div`),V.className=`report-card report-card-placeholder`,V.innerHTML=`<span>放置位置</span>`),t&&t.parentElement===e){let r=n?t.nextSibling:t;e.insertBefore(V,r)}else e.appendChild(V)},c=t=>{let n=e.closest(`.report-card`),r=document.elementFromPoint(t.clientX,t.clientY),a=r?.closest(`.topic-nav-item`),o=r?.closest(`[data-create-tag='1']`);if(r?.closest(`.report-card-placeholder`))return;if(o){i(),o.classList.add(`is-nav-drop-target`),B={bucketKind:`__create-tag__`,bucketId:`__create-tag__`,reportId:``,placeAfter:!1,nav:!0};return}if(a){i(),a.classList.add(`is-nav-drop-target`),B={bucketKind:a.dataset.navBucketKind||F,bucketId:a.dataset.navBucketId||``,reportId:``,placeAfter:!1,nav:!0};return}let c=r?.closest(`.report-card`);if(c&&c.classList.contains(`report-card-placeholder`))return;let l=r?.closest(`.group-column`);if(i(),c){if(c===n)return;let e=c.closest(`.group-column`),r=c.getBoundingClientRect(),i=t.clientY>r.top+r.height/2;c.classList.add(`is-card-drop-target`,i?`is-card-drop-after`:`is-card-drop-before`),s(e.querySelector(`.group-cards`),c,i),B={bucketKind:e?.dataset.bucketKind||F,bucketId:e?.dataset.bucketId||``,reportId:c.dataset.reportId||``,placeAfter:i,nav:!1};return}if(l){l.classList.add(`is-drop-ready`);let e=l.querySelector(`.group-cards`);s(e,null,!1),B={bucketKind:l.dataset.bucketKind||F,bucketId:l.dataset.bucketId||``,reportId:``,placeAfter:!1,nav:!1}}},l=()=>{L=``,t=null,n=!1,a(),i(),e.closest(`.report-card`)?.classList.remove(`is-dragging`)};e.addEventListener(`pointerdown`,r=>{r.preventDefault(),L=e.dataset.reportDragId,R=``,t={x:r.clientX,y:r.clientY},n=!1,e.setPointerCapture?.(r.pointerId),e.closest(`.report-card`)?.classList.add(`is-dragging`)}),e.addEventListener(`pointermove`,e=>{L&&(t&&Math.hypot(e.clientX-t.x,e.clientY-t.y)<7||(n=!0,o(e.clientY),c(e)))}),e.addEventListener(`pointerup`,e=>{if(!L)return;let t=L;if(!n){z=t,l(),$(),Z(`请选择目标${q()}`);return}let r=B,i=document.elementFromPoint(e.clientX,e.clientY),a=r?.reportId||``,o=r?.bucketId||i?.closest(`.group-column`)?.dataset.bucketId||``,s=r?.bucketKind||i?.closest(`.group-column`)?.dataset.bucketKind||F,c=r?.placeAfter||!1,u=A.reports.find(e=>e.id===t),d=!1;if(r?.nav)if(s===`__create-tag__`&&u){let e=window.prompt(`请输入新标签（将“${u.title}”加入此标签）：`)?.trim();e&&(!D.includes(e)&&!j.has(e)&&(j.add(e),ft()),d=K(t,`tag`,e))}else d=K(t,s,o);else a&&a!==t?d=K(t,s,o,a,c):o&&(d=K(t,s,o));l(),d&&($(),Z(s===`tag`?`已添加目标标签`:s===`type`?`工作类型已更新`:a?`报告顺序已更新`:`已移入新主题`))}),e.addEventListener(`pointercancel`,l)}),document.querySelectorAll(`.group-drag-handle`).forEach(e=>{let t=()=>{R=``,e.closest(`.group-column`)?.classList.remove(`is-group-dragging`),document.querySelectorAll(`.group-column`).forEach(e=>{e.classList.remove(`is-group-drop-target`,`is-drop-ready`)})};e.addEventListener(`pointerdown`,t=>{t.preventDefault(),R=e.dataset.groupDragId,L=``,e.setPointerCapture?.(t.pointerId),e.closest(`.group-column`)?.classList.add(`is-group-dragging`)}),e.addEventListener(`pointermove`,e=>{R&&document.querySelectorAll(`.group-column`).forEach(t=>{t.classList.toggle(`is-group-drop-target`,t===document.elementFromPoint(e.clientX,e.clientY)?.closest(`.group-column`))})}),e.addEventListener(`pointerup`,e=>{if(!R)return;let n=R,r=document.elementFromPoint(e.clientX,e.clientY)?.closest(`.group-column`);if(r&&jt(n,r.dataset.bucketId,r.dataset.bucketKind)){R=``,$(),Z(`分组顺序已更新`);return}t()}),e.addEventListener(`pointercancel`,t),e.addEventListener(`keydown`,t=>{if(![`ArrowLeft`,`ArrowRight`].includes(t.key))return;t.preventDefault();let n=[...document.querySelectorAll(`.group-column`)],r=n.findIndex(t=>t.dataset.bucketId===e.dataset.groupDragId),i=n[t.key===`ArrowLeft`?r-1:r+1];!i||!jt(e.dataset.groupDragId,i.dataset.bucketId,e.dataset.groupDragKind)||($(),Z(`分组顺序已更新`),document.querySelector(`[data-group-drag-id="${CSS.escape(e.dataset.groupDragId)}"]`)?.focus())})}),document.querySelectorAll(`.group-column`).forEach(e=>{e.addEventListener(`dragover`,t=>{t.preventDefault(),e.classList.add(R?`is-group-drop-target`:`is-drop-ready`)}),e.addEventListener(`dragleave`,()=>{e.classList.remove(`is-drop-ready`,`is-group-drop-target`)}),e.addEventListener(`drop`,t=>{if(t.preventDefault(),R){if(jt(R,e.dataset.bucketId,e.dataset.bucketKind)){R=``,$(),Z(`分组顺序已更新`);return}R=``,e.classList.remove(`is-group-drop-target`);return}let n=A.reports.find(e=>e.id===L),r=e.dataset.bucketKind||F;n&&K(L,r,e.dataset.bucketId)&&(L=``,$(),Z(r===`tag`?`已添加目标标签`:r===`type`?`工作类型已更新`:`已移入新主题`)),L=``})}),document.querySelectorAll(`[data-tag-suggestion]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=document.querySelector(`#tag-form input[name="tags"]`);if(!t)return;let n=qt(t.value),r=e.dataset.tagSuggestion;t.value=n.includes(r)?n.filter(e=>e!==r).join(`、`):[...n,r].slice(0,8).join(`、`),e.classList.toggle(`selected`,!n.includes(r)),t.focus()})});let t=document.getElementById(`tag-form`);t?.addEventListener(`submit`,e=>{e.preventDefault();let n=A.reports.find(e=>e.id===H.reportId);n&&(n.tags=qt(new FormData(t).get(`tags`)),W(),H=null,$(),Z(`标签已更新`))});let n=document.getElementById(`group-form`);n?.addEventListener(`submit`,e=>{e.preventDefault();let t=new FormData(n).get(`name`)?.trim(),r=new FormData(n).get(`description`)?.trim();if(!t)return;if(H.mode===`edit`){let e=A.groups.find(e=>e.id===H.groupId);if(!e)return;e.name=t.slice(0,60),e.description=r?.slice(0,80)||`自定义工作主题`}else A.groups.push({id:Wt(`group`),name:t.slice(0,60),description:r?.slice(0,80)||`自定义工作主题`,accent:[`blue`,`violet`,`amber`,`green`][A.groups.length%4],position:A.groups.length}),F=`topic`,localStorage.setItem(T,F);W();let i=H.mode===`edit`?`工作主题已更新`:`工作主题已创建，可直接拖入报告`;H=null,$(),Z(i)});let r=document.getElementById(`report-form`);r?.addEventListener(`submit`,async e=>{e.preventDefault();let t=r.elements.url.value.trim();if(!Kt(t))return;let n=r.querySelector(`button[type="submit"]`),i=r.querySelector(`.field-hint`);n.disabled=!0,n.innerHTML=`<span class="mini-spinner"></span>`;let a=xt({material:t,files:[],url:t,excludeId:H.mode===`edit`?H.reportId:``});if(a){n.disabled=!1,n.textContent=`Save`,i.textContent=`成果库已有“${a.title}”，未重复保存`,Z(`成果库已有“${a.title}”，未重复保存`);return}let o=await Dt({material:t,files:[],url:t},e=>{i.textContent=e});if(!o.allowed){n.disabled=!1,n.textContent=`Save`,i.textContent=o.reason,Z(o.reason);return}let s=r.elements.title.value.trim()||o.metadata.title,c=r.elements.groupId.value,l=r.elements.workType.value,u=qt(r.elements.tags.value),d={title:s||X(t),url:t,groupId:c,workType:l,source:`手动添加`,access:o.access,detectedDescription:o.metadata.description,manualSaved:!0,isProduction:o.access===`production`,isPersonal:St(t),isHtml:o.isHtml,loginProvider:o.loginProvider},f=[...new Set([...k(d,l),...u])].slice(0,8);if(H.mode===`edit`){let e=A.reports.find(e=>e.id===H.reportId);Object.assign(e,d,{tags:f})}else{let e={id:Wt(`report`),groupId:c,...d,pinned:!1,position:A.reports.filter(e=>e.groupId===c).length,createdAt:new Date().toISOString(),archived:!1,archivedAt:``,tags:f};A.reports.push(e)}W(),H=null,$(),Z(`报告已保存`)});let i=N&&A.reports.find(e=>e.id===N);i&&Je(i)}function un(){$()}un(document.getElementById(`app`));