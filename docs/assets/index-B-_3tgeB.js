(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const o of n)if(o.type==="childList")for(const l of o.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&i(l)}).observe(document,{childList:!0,subtree:!0});function a(n){const o={};return n.integrity&&(o.integrity=n.integrity),n.referrerPolicy&&(o.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?o.credentials="include":n.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(n){if(n.ep)return;n.ep=!0;const o=a(n);fetch(n.href,o)}})();const Ct="clair-ai-studio-tasks-v1",ot=[{id:"save",name:"仅保存",icon:"◇",hint:"收进成果区，不执行分析"},{id:"review",name:"执行评审",icon:"审",hint:"智能判断最合适的评审路径"},{id:"skill",name:"触发 Skill",icon:"✦",hint:"明确指定一个工具执行"}],st=[{id:"auto",name:"智能识别",summon:"自动派单",icon:"✦",hint:"让 AI 判断最适合的任务"},{id:"requirement",name:"需求评审",summon:"需求专家",icon:"需",hint:"价值、范围、规则、验收"},{id:"solution",name:"方案评审",summon:"方案专家",icon:"案",hint:"体验、逻辑、可行性、风险"},{id:"decision",name:"决策推演",summon:"决策顾问",icon:"决",hint:"选项、证据、取舍、止损"},{id:"agreement",name:"协议审查",summon:"协议专家",icon:"协",hint:"权责、数据、责任、退出"},{id:"career",name:"履历评估",summon:"履历顾问",icon:"历",hint:"事实、能力、匹配、核验"}];let x=Yt(),g=Dt(),T="",D="compose";function Dt(){return{action:"review",skillId:"auto",goal:"",material:"",files:[],collabEnabled:!1,collabUrl:""}}function Yt(){try{const t=JSON.parse(localStorage.getItem(Ct));return Array.isArray(t)?t:[]}catch{return[]}}function _(){localStorage.setItem(Ct,JSON.stringify(x))}function Ot(){var t;return((t=crypto.randomUUID)==null?void 0:t.call(crypto))||`${Date.now()}-${Math.random()}`}function pt(t){return st.find(e=>e.id===t)||st[0]}function Jt(t){return ot.find(e=>e.id===t)||ot[1]}function Xt(t){try{return["http:","https:"].includes(new URL(t).protocol)}catch{return!1}}function Qt(t){return(t.match(/https?:\/\/[^\s<>"'）)]+/gi)||[]).find(a=>/feishu\.cn|docs\.qq\.com|notion\.(so|site)|docs\.google\.com|yuque\.com|shimo\.im|office\.com|sharepoint\.com/i.test(a))||""}function te(t){var i;const e=t.toLowerCase();return((i=[["agreement",["协议","合同","条款","保密","签署"]],["career",["简历","履历","候选人","晋升","岗位","面试"]],["decision",["决策","选型","取舍","是否推进","选择"]],["requirement",["需求","prd","用户故事","验收","原型"]],["solution",["方案","流程","架构","设计","上线"]]].find(([,n])=>n.some(o=>e.includes(o))))==null?void 0:i[0])||"solution"}function Pt(t){return new Intl.DateTimeFormat("zh-CN",{month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"}).format(new Date(t))}function ee(t,e){const a=t.files.length?t.files.map(o=>`${o.name}（${o.sizeLabel}）`).join("、"):"无附件",i=t.material.trim().length,n=t.collabUrl?`<li>协作文档：<a href="${e(t.collabUrl)}" target="_blank" rel="noreferrer">打开协作文档 ↗</a></li>`:"";return`
    <h2>材料已收齐</h2>
...
`,"<br />")}</p>
    ...
    </button>`).join("")}function oe(t){return ot.map(e=>`
    ...
    <section class="inline-task-launcher prompt-launcher" aria-label="发起任务">
      <form class="prompt-composer" id="task-composer">
        ...
        <div class="intake-route-row">
          <div class="intake-actions" aria-label="处理方式">${oe(t)}</div>
        ...
        <div class="prompt-footer">
          ...
          ${g.action==="save"?'<div class="save-mode-note"><span>◇</span><small>只保存记录，不执行分析</small></div>':`<div class="expert-summoner" aria-label="召唤专家">
                ...
                <div class="expert-strip">${re(t)}</div>
              </div>`}
...
`}function Se(t,e){const i=new DOMParser().parseFromString(t,"text/html");i.querySelectorAll('meta[http-equiv="Content-Security-Policy" i]').forEach(s=>{s.dataset.clairEditorHttpEquiv=s.getAttribute("http-equiv")||"Content-Security-Policy",s.setAttribute("http-equiv","x-clair-csp-disabled")}),Ie(i);const n=i.createElement("base");n.href=e,n.dataset.clairEditorBase="true",i.head.prepend(n);const o=i.createElement("style");o.id="clair-editor-style",o.textContent=`
  ...
  `,i.head.append(o);const l=i.createElement("script");return l.id="clair-editor-bridge",l.textContent=Ae(),i.body.append(l),`<!DOCTYPE html>
${i.documentElement.outerHTML}`}async function Nt(t){var e;try{const a=ke(t.url);let i=null;if(a)try{i=await $e(a)}catch{}if(!i){const s=await fetch(t.url,{cache:"no-store"});if(!s.ok)throw new Error(`报告读取失败（HTTP ${s.status}）`);i={html:await s.text(),target:a}}const n=await St(i.html);r.protection=n.protection,r.target=i.target||a;let o=n.html;const l=he(t.id);if(l!=null&&l.html)try{const s=await St(l.html);o=s.html,r.hasDraft=!0,r.draftHtml=s.html,r.draftAt=l.savedAt||"",l.baseFiles&&r.target&&(r.target.baseFiles=l.baseFiles)}catch{Ut(t.id)}r.html=o,r.editorDocument=Se(o,t.url),r.status="ready",r.error=""}catch(a){r.status="error",r.error=(a==null?void 0:a.message)||"无法读取这份 HTML"}finally{r.loadPromise=null,(e=r.render)==null||e.call(r)}}function jt(){const t=r.render,e=r.showToast;Object.assign(r,{reportId:"",reportTitle:"",reportUrl:"",status:"idle",error:"",html:"",editorDocument:"",dirty:!1,hasDraft:!1,draftHtml:"",draftAt:"",target:null,settingsOpen:!1,publishConfirmOpen:!1,pendingSave:!1,saving:!1,lastCommit:"",protection:null,loadPromise:null,render:t,showToast:e})}function bt(){return document.querySelector(".report-editor-frame")}function at(t,e=null){var i;const a=bt();(i=a==null?void 0:a.contentWindow)==null||i.postMessage({channel:X,type:"command",command:t,value:e},"*")}function ht(){var a;const t=bt();if(!(t!=null&&t.contentWindow))return Promise.reject(new Error("编辑画布尚未就绪"));const e=((a=crypto.randomUUID)==null?void 0:a.call(crypto))||`${Date.now()}-${Math.random()}`;return new Promise((i,n)=>{const o=window.setTimeout(()=>{V.delete(e),n(new Error("读取编辑内容超时"))},1e4);V.set(e,{resolve:l=>{clearTimeout(o),i(l)}}),t.contentWindow.postMessage({channel:X,type:"serialize",requestId:e},"*")})}function Ee(t){return`${String(t||"report").replace(/[\\/:*?"<>|]+/g,"-").replace(/\s+/g,"-").replace(/-+/g,"-").replace(/^-|-$/g,"").slice(0,80)||"report"}.html`}function Ft(t,e){const a=new Blob([t],{type:"text/html;charset=utf-8"}),i=URL.createObjectURL(a),n=document.createElement("a");n.href=i,n.download=Ee(e),document.body.append(n),n.click(),n.remove(),window.setTimeout(()=>URL.revokeObjectURL(i),1e3)}async function Gt(t){await navigator.clipboard.writeText(t)}function qe(t,e){var n;const a=new DOMParser().parseFromString(t,"text/html");(n=a.querySelector("base[data-clair-preview-base]"))==null||n.remove();const i=a.createElement("base");return i.href=e,i.dataset.clairPreviewBase="true",a.head.prepend(i),`<!DOCTYPE html>
${a.documentElement.outerHTML}`}function Te(t){if(!r.hasDraft||!r.draftHtml)throw new Error("请先暂存当前修订，再另开预览");const e=new Blob([qe(r.draftHtml,t.url)],{type:"text/html;charset=utf-8"}),a=URL.createObjectURL(e),i=window.open(a,"_blank");if(!i)throw URL.revokeObjectURL(a),new Error("浏览器拦截了新窗口，请允许弹窗后重试");i.opener=null,window.setTimeout(()=>URL.revokeObjectURL(a),6e4)}async function ct(t,{silent:e=!1}={}){var o;const a=await ht(),i=await ft(a),n=new Date().toISOString();try{sessionStorage.setItem(gt(t.id),JSON.stringify({reportId:t.id,reportUrl:t.url,savedAt:n,baseFiles:lt(),html:i}))}catch{throw new Error("浏览器暂存空间不足，请先下载 HTML 备份")}return r.html=a,r.draftHtml=a,r.draftAt=n,r.hasDraft=!0,r.dirty=!1,r.lastCommit="",Y(),e||(o=r.showToast)==null||o.call(r,"已暂存在当前浏览器会话，尚未更新 GitHub"),a}async function xe(t){var s,d;const e=r.target;if(!(e!=null&&e.owner)||!e.repository||!e.path||!e.branch)throw new Error("请先填写 GitHub 仓库、分支和 HTML 路径");if(!r.token)throw new Error("请先提供 GitHub Fine-grained Token");const a=await ft(t),i=(e.mirrors||[]).map(c=>c.path),n=mt([...i.filter(c=>c.startsWith("public/")),...i.filter(c=>!c.startsWith("public/")&&c!==e.path),e.path]);let o="";const l=[];for(const c of n)try{const p=c.split("/").map(encodeURIComponent).join("/"),u=`/repos/${encodeURIComponent(e.owner)}/${encodeURIComponent(e.repository)}/contents/${p}`,b=await J(`${u}?ref=${encodeURIComponent(e.branch)}`,{token:r.token}),f=lt(e)[c];if(f&&b.sha!==f)throw new Error(`生产文件 ${c} 已在本次编辑后更新，请重新打开报告合并修改`);const w=await J(u,{token:r.token,method:"PUT",body:{message:`Update ${r.reportTitle} from Clair's Studio`,content:we(a),sha:b.sha,branch:e.branch}});o=((s=w==null?void 0:w.commit)==null?void 0:s.sha)||o,e.baseFiles={...lt(e),[c]:((d=w==null?void 0:w.content)==null?void 0:d.sha)||b.sha},l.push(c)}catch(p){throw l.length?new Error(`已更新 ${l.join("、")}，但 ${c} 同步失败：${p.message}`):p}return{commit:o,files:l.length}}async function Et(t){var e,a;if(!r.saving){r.saving=!0,Y();try{const i=r.dirty?await ct(t,{silent:!0}):r.draftHtml||await ht(),n=await xe(i);r.html=i,r.dirty=!1,r.hasDraft=!1,r.draftHtml="",r.draftAt="",r.lastCommit=n.commit,Ut(t.id),(e=r.showToast)==null||e.call(r,n.files>1?`已同步 ${n.files} 个 GitHub 文件，Pages 正在更新`:"已提交 GitHub，Pages 正在更新")}catch(i){(a=r.showToast)==null||a.call(r,(i==null?void 0:i.message)||"保存失败，请下载 HTML 备份")}finally{r.saving=!1,Y()}}}function Le(t){const e=r.target||{owner:"ClairKu",repository:"",branch:"main",path:""};return`
    <div class="dialog-backdrop editor-settings-backdrop" ${r.settingsOpen?"":"hidden"}>
      <form class="dialog editor-settings-dialog" id="editor-settings-form">
        ...
        <label>Fine-grained personal access token
          <input class="github-token-input" name="github-token-not-password" type="text" value=""
            ...
            placeholder="${r.token?"已连接；留空可继续使用当前 Token":"github_pat_…"}" ${r.token?"":"required"} />
        </label>
        ...
        <div class="dialog-actions">
          ...
          <button type="submit" class="primary-button">${r.pendingSave?"连接并保存":"保存设置"}</button>
        </div>
      </form>
    </div>`}function Ce(t){const e=r.target?`${r.target.owner}/${r.target.repository} · ${r.target.path}`:"尚未识别 GitHub 文件路径";return`
    <div class="dialog-backdrop editor-publish-backdrop" ${r.publishConfirmOpen?"":"hidden"}>
    ...
    </div>`}function qt({pendingSave:t=!1}={}){r.settingsOpen=!0,r.pendingSave=t;const e=document.querySelector(".editor-settings-backdrop");if(!e)return;e.hidden=!1;const a=e.querySelector("#editor-settings-form"),i=r.target||{};if(a){a.elements.owner.value=i.owner||"ClairKu",a.elements.repository.value=i.repository||"",a.elements.branch.value=i.branch||"main",a.elements.path.value=i.path||"";const n=a.querySelector('button[type="submit"]');n&&(n.textContent=t?"连接并保存":"保存设置")}}function z(){r.settingsOpen=!1,r.pendingSave=!1;const t=document.querySelector(".editor-settings-backdrop");t&&(t.hidden=!0)}function De(){r.publishConfirmOpen=!0;const t=document.querySelector(".editor-publish-backdrop");t&&(t.hidden=!1)}function H(){r.publishConfirmOpen=!1;const t=document.querySelector(".editor-publish-backdrop");t&&(t.hidden=!0)}function Zt(t=""){return!!(r.reportId&&(!t||r.reportId===t))}function Oe(t,{render:e,showToast:a}){jt(),Object.assign(r,{reportId:t.id,reportTitle:t.title,reportUrl:t.url,status:"loading",render:e,showToast:a}),e(),r.loadPromise=Nt(t)}function Pe(t,e){var l;const a=r.target?`${r.target.owner}/${r.target.repository} · ${r.target.path}${(l=r.target.mirrors)!=null&&l.length?` · 同步 ${r.target.mirrors.length+1} 处`:""}`:"尚未识别 GitHub 源文件",i=Bt(),n=r.status==="ready"?`
      ...
      </div>`:"",o=r.status==="loading"?'<div class="editor-state"><span class="editor-loader"></span><strong>正在载入可编辑 HTML…</strong><p>会自动识别对应 GitHub 仓库与源文件。</p></div>':r.status==="error"?`<div class="editor-state editor-error"><strong>这份报告暂时无法进入编辑模式</strong><p>${e(r.error)}</p><div><button class="quiet-button" type="button" data-editor-action="retry">重试</button><button class="primary-button" type="button" data-editor-action="download-published">下载原 HTML</button></div></div>`:`<div class="report-editor-frame-wrap"><iframe class="report-editor-frame" title="${e(t.title)}编辑画布"
          sandbox="allow-scripts allow-modals" srcdoc="${ve(r.editorDocument)}"></iframe></div>`;return`
      ...
      <header class="reader-header editor-header">
        ...
        <div class="reader-actions editor-actions">
          ...
          <button class="quiet-button" type="button" data-editor-action="stash"
            ${r.status!=="ready"||r.saving||!r.dirty?"disabled":""}>${!r.dirty&&r.hasDraft?"已暂存":"暂存"}</button>
          <button class="quiet-button" type="button" data-editor-action="preview"
            title="在新窗口打开已暂存修订" ${r.status!=="ready"||!r.hasDraft?"disabled":""}>另开启</button>
          ...
          <button class="primary-button" type="button" data-editor-action="publish"
            ${r.status!=="ready"||r.saving||!r.dirty&&!r.hasDraft?"disabled":""}>${r.saving?"推送中…":"推送生产"}</button>
        </div>
      </header>
      ${n}
      ${o}
    ...
    <main class="app-shell archive-shell">
      ...
      ${$t()}
    </main>`}function ze(){if(F)return _e();const t=E.trim().toLowerCase(),e=t.split(/\s+/).filter(Boolean),a=v.reports.filter(c=>!c.archived),i=e.length?a.filter(c=>{const p=`${c.title} ${c.source||""} ${c.access||""} ${je(c.workType)} ${(c.tags||[]).join(" ")}`.toLowerCase();return e.every(u=>p.includes(u))}):a,n=v.reports.filter(c=>c.archived).length,o=a.filter(c=>c.access==="production").length,l=a.filter(c=>c.access!=="production").length,s=Fe(i,t).filter(c=>c.reports.length||P),d=S==="type"?"工作类型":S==="tag"?"关键标签":"工作主题";return`
    <main class="app-shell">
      ...
      ${$t()}
    </main>`}function k(){const t=document.getElementById("app");if(sessionStorage.getItem(yt)!=="ok"){t.innerHTML=Ge(),He();return}const e=O&&v.reports.find(a=>a.id===O);t.innerHTML=e?Ze(e):ze(),Ke(),pe({render:k,escapeHtml:h,showToast:A,showResults:()=>{F=!1}})}function He(){const t=document.getElementById("login-form");t==null||t.addEventListener("submit",e=>{if(e.preventDefault(),new FormData(t).get("password")!=="2026"){const i=t.querySelector(".form-error");i.hidden=!1,i.textContent="口令不正确，请再试一次";return}sessionStorage.setItem(yt,"ok"),k()})}async function Lt(t){var l,s;const e=t.elements.url,a=t.elements.title,i=t.querySelector('[data-action="detect-title"]'),n=t.querySelector(".field-hint"),o=e.value.trim();if(!Kt(o))return n.textContent="请输入完整的 http 或 https 网址","";i.disabled=!0,i.innerHTML='<span class="mini-spinner"></span>',n.textContent="正在读取网页标题…";try{const d=`https://api.microlink.io/?url=${encodeURIComponent(o)}`,c=await fetch(d,{headers:{Accept:"application/json"},signal:AbortSignal.timeout(1e4)});if(!c.ok)throw new Error("read failed");const p=await c.json(),u=((s=(l=p==null?void 0:p.data)==null?void 0:l.title)==null?void 0:s.trim())||Z(o);return a.value=u.slice(0,180),n.textContent="已识别网页标题",a.value}catch{const d=Z(o);return a.value||(a.value=d),n.textContent="网页暂时无法读取，已用域名作为标题，你可以手动修改",a.value}finally{i.disabled=!1,i.textContent="识别标题"}}function Ke(){var n;(n=document.getElementById("search-input"))==null||n.addEventListener("input",o=>{E=o.target.value,k();const l=document.getElementById("search-input");l==null||l.focus(),l==null||l.setSelectionRange(E.length,E.length)}),document.querySelectorAll("[data-action]").forEach(o=>{o.addEventListener("click",async l=>{var c,p;const s=l.currentTarget.dataset.action,d=l.currentTarget.dataset.id;if(s==="open")O=d,k();else if(s==="edit-document"){const u=v.reports.find(b=>b.id===d);if(!u||u.access!=="production")return;Oe(u,{render:k,showToast:A})}else if(s==="download-report"){const u=v.reports.find(b=>b.id===d);u&&await _t(u,A)}else if(s==="share-report"){const u=v.reports.find(b=>b.id===d);u&&await Ue(u,A)}else if(s==="back")O="",I=null,k();else if(s==="lock")sessionStorage.removeItem(yt),k();else if(s==="clear-search")E="",k();else if(s==="set-view"){if(!["topic","type","tag"].includes(d))return;S=d,P="",localStorage.setItem(dt,S),k()}else if(s==="cancel-move")P="",k();else if(s==="move-here"){const u=l.currentTarget.dataset.bucketKind||S;P&&K(P,u,d)&&(P="",k(),A(u==="tag"?"已添加目标标签":`报告已移入目标${G()}`))}else if(s==="show-archive")F=!0,E="",O="",k();else if(s==="show-catalog")F=!1,E="",O="",k();else if(s==="add-report")I={type:"report",mode:"create",groupId:((c=v.groups[1])==null?void 0:c.id)||((p=v.groups[0])==null?void 0:p.id)},k();else if(s==="add-to-group")I={type:"report",mode:"create",groupId:d},k();else if(s==="edit")I={type:"report",mode:"edit",reportId:d},k();else if(s==="edit-tags")I={type:"tags",reportId:d},k();else if(s==="close-modal")I=null,k();else if(s==="detect-title")await Lt(l.currentTarget.closest("form"));else if(s==="archive"){const u=v.reports.find(b=>b.id===d);if(!u)return;u.archived=!0,u.archivedAt=new Date().toISOString(),L(),k(),A("已归档，可随时恢复")}else if(s==="restore"){const u=v.reports.find(b=>b.id===d);if(!u)return;u.archived=!1,u.archivedAt="",L(),k(),A("报告已恢复到原主题")}else if(s==="delete"){const u=v.reports.find(b=>b.id===d);u!=null&&u.archived&&confirm(`二次确认：永久删除“${u.title}”？

删除后无法从归档区恢复。`)&&(v.reports=v.reports.filter(b=>b.id!==d),O===d&&(O=""),L(),k(),A("报告已永久删除"))}else if(s==="add-group")I={type:"group",mode:"create"},k();else if(s==="rename-group")v.groups.find(b=>b.id===d)&&(I={type:"group",mode:"edit",groupId:d},k());else if(s==="delete-group"){const u=v.groups.find(b=>b.id===d);u&&confirm(`删除“${u.name}”？其中的报告会移到“待整理”。`)&&(v.reports.forEach(b=>{b.groupId===d&&(b.groupId="inbox")}),v.groups=v.groups.filter(b=>b.id!==d),L(),k(),A("分组已删除，报告已移到待整理"))}})}),document.querySelectorAll(".report-drag-handle").forEach(o=>{let l=null,s=!1;const d=()=>{var c;C="",l=null,s=!1,(c=o.closest(".report-card"))==null||c.classList.remove("is-dragging"),document.querySelectorAll(".report-card, .group-column").forEach(p=>{p.classList.remove("is-card-drop-target","is-drop-ready")})};o.addEventListener("pointerdown",c=>{var p,u;c.preventDefault(),C=o.dataset.reportDragId,q="",l={x:c.clientX,y:c.clientY},s=!1,(p=o.setPointerCapture)==null||p.call(o,c.pointerId),(u=o.closest(".report-card"))==null||u.classList.add("is-dragging")}),o.addEventListener("pointermove",c=>{if(!C||l&&Math.hypot(c.clientX-l.x,c.clientY-l.y)<7)return;s=!0;const p=document.elementFromPoint(c.clientX,c.clientY),u=p==null?void 0:p.closest(".report-card"),b=p==null?void 0:p.closest(".group-column");document.querySelectorAll(".report-card").forEach(f=>{f.classList.toggle("is-card-drop-target",!!(u&&u!==o.closest(".report-card")&&f===u))}),document.querySelectorAll(".group-column").forEach(f=>{f.classList.toggle("is-drop-ready",!!(b&&f===b))})}),o.addEventListener("pointerup",c=>{if(!C)return;const p=C;if(!s){P=p,d(),k(),A(`请选择目标${G()}`);return}const u=document.elementFromPoint(c.clientX,c.clientY),b=u==null?void 0:u.closest(".report-card"),f=u==null?void 0:u.closest(".group-column"),w=(b==null?void 0:b.dataset.reportId)||"",m=(f==null?void 0:f.dataset.bucketId)||"",$=(f==null?void 0:f.dataset.bucketKind)||S,y=w&&w!==p?K(p,$,m,w):m?K(p,$,m):!1;d(),y&&(k(),A($==="tag"?"已添加目标标签":$==="type"?"工作类型已更新":w?"报告顺序已更新":"已移入新主题"))}),o.addEventListener("pointercancel",d)}),document.querySelectorAll(".group-drag-handle").forEach(o=>{const l=()=>{var s;q="",(s=o.closest(".group-column"))==null||s.classList.remove("is-group-dragging"),document.querySelectorAll(".group-column").forEach(d=>{d.classList.remove("is-group-drop-target","is-drop-ready")})};o.addEventListener("pointerdown",s=>{var d,c;s.preventDefault(),q=o.dataset.groupDragId,C="",(d=o.setPointerCapture)==null||d.call(o,s.pointerId),(c=o.closest(".group-column"))==null||c.classList.add("is-group-dragging")}),o.addEventListener("pointermove",s=>{q&&document.querySelectorAll(".group-column").forEach(d=>{var c;d.classList.toggle("is-group-drop-target",d===((c=document.elementFromPoint(s.clientX,s.clientY))==null?void 0:c.closest(".group-column")))})}),o.addEventListener("pointerup",s=>{var p;if(!q)return;const d=q,c=(p=document.elementFromPoint(s.clientX,s.clientY))==null?void 0:p.closest(".group-column");if(c&&nt(d,c.dataset.groupId)){q="",k(),A("分组顺序已更新");return}l()}),o.addEventListener("pointercancel",l),o.addEventListener("keydown",s=>{var u;if(!["ArrowLeft","ArrowRight"].includes(s.key))return;s.preventDefault();const d=v.groups.findIndex(b=>b.id===o.dataset.groupDragId),c=s.key==="ArrowLeft"?d-1:d+1,p=v.groups[c];!p||!nt(o.dataset.groupDragId,p.id)||(k(),A("分组顺序已更新"),(u=document.querySelector(`[data-group-drag-id="${CSS.escape(o.dataset.groupDragId)}"]`))==null||u.focus())})}),document.querySelectorAll(".group-column").forEach(o=>{o.addEventListener("dragover",l=>{l.preventDefault(),o.classList.add(q?"is-group-drop-target":"is-drop-ready")}),o.addEventListener("dragleave",()=>{o.classList.remove("is-drop-ready","is-group-drop-target")}),o.addEventListener("drop",l=>{if(l.preventDefault(),q){if(o.dataset.bucketKind==="topic"&&nt(q,o.dataset.groupId)){q="",k(),A("分组顺序已更新");return}q="",o.classList.remove("is-group-drop-target");return}const s=v.reports.find(c=>c.id===C),d=o.dataset.bucketKind||S;s&&K(C,d,o.dataset.bucketId)&&(C="",k(),A(d==="tag"?"已添加目标标签":d==="type"?"工作类型已更新":"已移入新主题")),C=""})}),document.querySelectorAll("[data-tag-suggestion]").forEach(o=>{o.addEventListener("click",()=>{const l=document.querySelector('#tag-form input[name="tags"]');if(!l)return;const s=rt(l.value),d=o.dataset.tagSuggestion;l.value=s.includes(d)?s.filter(c=>c!==d).join("、"):[...s,d].slice(0,8).join("、"),o.classList.toggle("selected",!s.includes(d)),l.focus()})});const t=document.getElementById("tag-form");t==null||t.addEventListener("submit",o=>{o.preventDefault();const l=v.reports.find(s=>s.id===I.reportId);l&&(l.tags=rt(new FormData(t).get("tags")),L(),I=null,k(),A("标签已更新"))});const e=document.getElementById("group-form");e==null||e.addEventListener("submit",o=>{var c,p;o.preventDefault();const l=(c=new FormData(e).get("name"))==null?void 0:c.trim(),s=(p=new FormData(e).get("description"))==null?void 0:p.trim();if(!l)return;if(I.mode==="edit"){const u=v.groups.find(b=>b.id===I.groupId);if(!u)return;u.name=l.slice(0,60),u.description=(s==null?void 0:s.slice(0,80))||"自定义工作主题"}else v.groups.push({id:xt("group"),name:l.slice(0,60),description:(s==null?void 0:s.slice(0,80))||"自定义工作主题",accent:["blue","violet","amber","green"][v.groups.length%4],position:v.groups.length});L();const d=I.mode==="edit"?"工作主题已更新":"工作主题已创建，可直接拖入报告";I=null,k(),A(d)});const a=document.getElementById("report-form");a==null||a.addEventListener("submit",async o=>{o.preventDefault();const l=a.elements.url.value.trim();if(!Kt(l))return;const s=a.querySelector('button[type="submit"]');s.disabled=!0,s.innerHTML='<span class="mini-spinner"></span>';let d=a.elements.title.value.trim();d||(d=await Lt(a));const c=a.elements.groupId.value,p=a.elements.workType.value,u=rt(a.elements.tags.value);if(I.mode==="edit"){const b=v.reports.find(f=>f.id===I.reportId);Object.assign(b,{title:d,url:l,groupId:c,workType:p,tags:u})}else{const b={id:xt("report"),groupId:c,title:d||Z(l),url:l,pinned:!1,position:v.reports.filter(f=>f.groupId===c).length,createdAt:new Date().toISOString(),source:"手动添加",access:"production",archived:!1,archivedAt:"",workType:p,tags:u};b.tags.length||(b.tags=kt(b,b.workType)),v.reports.push(b)}L(),I=null,k(),A("报告已保存")});const i=O&&v.reports.find(o=>o.id===O);i&&Re(i)}function We(){k()}We(document.getElementById("app"));
