const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./lib-B6JveqHd.js","./jszip.min-Cw_ndgJv.js","./pptx-preview.es-CylJg-AV.js"])))=>i.map(i=>d[i]);
...
`)}function kt(e=``){let t=document.createElement(`template`);return t.innerHTML=String(e),t.content.querySelectorAll(`script, style, link, meta, base, iframe, frame, object, embed, form, input, button, textarea, select`).forEach(e=>e.remove()),t.content.querySelectorAll(`*`).forEach(e=>{[...e.attributes].forEach(t=>{let n=t.name.toLowerCase(),r=t.value.trim().toLowerCase();(n.startsWith(`on`)||n===`srcdoc`||(n===`href`||n===`src`||n===`xlink:href`)&&/^(javascript|vbscript):/.test(r))&&e.removeAttribute(t.name)})}),t.innerHTML}function At(e,t,n=`document`){return`<!doctype html>
    ...
    </svg>`};function er(e){return $n[e]||``}function tr(e){try{return new URL(e).hostname.replace(/^www\./,``)}catch{return e}}function nr(e){try{return[`http:`,`https:`].includes(new URL(e).protocol)}catch{return!1}}function G(e=``){return[...new Set(String(e).split(/[、,，;；\n]+/).map(e=>e.trim()).filter(Boolean).map(e=>e.slice(0,20)))].slice(0,8)}function rr(){let e=new Set(Zt);return D.reports.forEach(t=>{(t.tags||[]).forEach(t=>e.add(t))}),[...e].filter(e=>![`HTML`,`手动保存`,`生产`].includes(e))}function K(e,{duration:t=2600,actionLabel:n=``,onAction:r=null}={}){document.querySelector(`.toast`)?.remove();let i=document.createElement(`div`);i.className=`toast`,i.setAttribute(`role`,`status`);let a=document.createElement(`span`);if(a.textContent=e,i.append(a),n&&typeof r==`function`){let e=document.createElement(`button`);e.type=`button`,e.className=`toast-action`,e.textContent=n,e.addEventListener(`click`,()=>{clearTimeout(on),i.remove(),r()}),i.append(e)}document.body.append(i),clearTimeout(on),on=window.setTimeout(()=>i.remove(),t)}function ir(e,t){K(e,{duration:8e3,actionLabel:`撤销`,onAction:()=>{t(),K(`已撤销刚才的操作`)}})}function ar(e){let t=e?.closest?.(`[data-action]`);return t?{action:t.dataset.action||``,id:t.dataset.id||``,bucketKind:t.dataset.bucketKind||``,direction:t.dataset.direction||``}:null}function or(e){if(!e?.action)return null;let t=[`[data-action="${CSS.escape(e.action)}"]`,e.id?`[data-id="${CSS.escape(e.id)}"]`:``,e.bucketKind?`[data-bucket-kind="${CSS.escape(e.bucketKind)}"]`:``,e.direction?`[data-direction="${CSS.escape(e.direction)}"]`:``].join(``);return document.querySelector(t)}function sr(e,t,n){pn=t||X(),sn=ar(n||document.activeElement),cn=window.scrollY,L=e,Z(pn)}function q({fallbackSelector:e=`.results-toolbar, .archive-search, .reader-header`}={}){if(!L)return;let t=pn||{scrollY:cn},n=sn;L=null,Z(t),pn=null,sn=null,requestAnimationFrame(()=>{let t=document.querySelector(e);(or(n)||t?.querySelector(`button, input, [tabindex]:not([tabindex='-1'])`)||t)?.focus?.({preventScroll:!0})})}function cr(){let e=document.querySelector(`.app-shell > .dialog-backdrop`);if(e?document.body.style.setProperty(`--studio-modal-scroll-top`,`${-cn}px`):document.body.style.removeProperty(`--studio-modal-scroll-top`),document.body.classList.toggle(`studio-modal-open`,!!e),!e)return;[...e.parentElement.children].forEach(t=>{t!==e&&(t.inert=!0,t.setAttribute(`aria-hidden`,`true`))});let t=e.querySelector(`[role="dialog"]`);if(!t)return;let n=()=>[...t.querySelectorAll([`a[href]`,`button:not([disabled])`,`input:not([disabled]):not([type='hidden'])`,`select:not([disabled])`,`textarea:not([disabled])`,`[tabindex]:not([tabindex='-1'])`].join(`,`))].filter(e=>!e.hidden&&e.getClientRects().length);e.addEventListener(`keydown`,e=>{if(e.key===`Escape`){e.preventDefault(),q();return}if(e.key!==`Tab`)return;let r=n();if(!r.length){e.preventDefault(),t.focus({preventScroll:!0});return}let i=r[0],a=r.at(-1);e.shiftKey&&document.activeElement===i?(e.preventDefault(),a.focus({preventScroll:!0})):!e.shiftKey&&document.activeElement===a&&(e.preventDefault(),i.focus({preventScroll:!0}))}),e.addEventListener(`click`,t=>{t.target===e&&q()}),requestAnimationFrame(()=>{(t.querySelector(`[autofocus]`)||n()[0]||t)?.focus?.({preventScroll:!0})})}function lr(e=`auto`){ur(),z=requestAnimationFrame(()=>{z=0,window.scrollTo({top:0,left:0,behavior:e})})}function ur(){R&&cancelAnimationFrame(R),z&&cancelAnimationFrame(z),ln&&cancelAnimationFrame(ln),R=0,z=0,ln=0}function dr(){let e=document.querySelector(`.topbar`)?.getBoundingClientRect().bottom||0,t=document.querySelector(`.topic-nav`),n=t?getComputedStyle(t):null,r=t?.getBoundingClientRect();if(!window.matchMedia(`(max-width: 840px)`).matches){let t=n?.position===`sticky`&&Number.parseFloat(n.top)||0;return Math.max(e+22,t)}let i=n?.position===`sticky`&&r?.bottom>0?r.bottom:0;return Math.max(e,i)+10}function fr(e,t=`smooth`){if(!e)return;ur();let n=Math.max(0,document.documentElement.scrollHeight-window.innerHeight),r=Math.max(0,Math.min(n,window.scrollY+e.getBoundingClientRect().top-dr())),i=window.scrollY,a=r-i;if(Math.abs(a)<2)return;if(t!==`smooth`||matchMedia(`(prefers-reduced-motion: reduce)`).matches){window.scrollTo({top:r,left:0,behavior:`auto`});return}let o=Math.min(360,Math.max(180,Math.abs(a)*.22)),s=performance.now(),c=t=>{if(!e.isConnected){R=0;return}let n=Math.min(1,(t-s)/o),r=1-(1-n)**3,l=Math.max(0,Math.min(Math.max(0,document.documentElement.scrollHeight-window.innerHeight),window.scrollY+e.getBoundingClientRect().top-dr()));a=l-i,window.scrollTo(0,i+a*r),n<1?R=requestAnimationFrame(c):(R=0,window.scrollTo(0,l))};R=requestAnimationFrame(c)}function J(e,t){return document.querySelector(`.group-column[data-bucket-kind="${CSS.escape(e)}"][data-bucket-id="${CSS.escape(t)}"]`)}function Y(e){return document.querySelector(`.board .report-card[data-report-id="${CSS.escape(e)}"]`)}function pr(e){if(!e)return null;let t=e.closest?.(`.report-card[data-report-id]`);if(t){let e=t.closest(`.group-column[data-bucket-kind][data-bucket-id]`);return{type:`report`,id:t.dataset.reportId,bucketKind:e?.dataset.bucketKind||``,bucketId:e?.dataset.bucketId||``}}let n=e.closest?.(`.group-column[data-bucket-kind][data-bucket-id]`);if(n)return{type:`bucket`,kind:n.dataset.bucketKind,id:n.dataset.bucketId};let r=e.closest?.(`.results-toolbar, .archive-search, .prompt-composer, .groups-section, .library-layout`);return r?{type:`selector`,selector:r.classList.contains(`results-toolbar`)?`.results-toolbar`:r.classList.contains(`archive-search`)?`.archive-search`:r.classList.contains(`prompt-composer`)?`.prompt-composer`:r.classList.contains(`groups-section`)?`.groups-section`:`.library-layout`}:null}function mr(e){return e?e.type===`report`?(e.bucketKind&&e.bucketId?J(e.bucketKind,e.bucketId):null)?.querySelector(`.report-card[data-report-id="${CSS.escape(e.id)}"]`)||Y(e.id):e.type===`bucket`?J(e.kind,e.id):e.type===`selector`?document.querySelector(e.selector):null:null}function hr(){let e=dr(),t=t=>[...document.querySelectorAll(t)].filter(t=>{let n=t.getBoundingClientRect();return n.bottom>e&&n.top<window.innerHeight}).sort((t,n)=>Math.abs(t.getBoundingClientRect().top-e)-Math.abs(n.getBoundingClientRect().top-e))[0];return t(`.board .report-card[data-report-id]`)||t(`.group-column[data-bucket-id]`)||t(`.results-toolbar, .archive-search, .prompt-composer`)||document.querySelector(`.results-toolbar, .archive-search, .groups-section, .library-layout`)}function X(e=null){ur();let t=e||hr();return{scrollY:window.scrollY,identity:pr(t),viewportTop:t?.getBoundingClientRect().top??null}}function gr(e){if(!e)return;ur();let t=()=>{let t=mr(e.identity),n=Math.max(0,document.documentElement.scrollHeight-window.innerHeight),r=t&&Number.isFinite(e.viewportTop)?window.scrollY+t.getBoundingClientRect().top-e.viewportTop:e.scrollY;window.scrollTo({top:Math.max(0,Math.min(n,r)),left:0,behavior:`auto`})};t(),ln=requestAnimationFrame(()=>{ln=requestAnimationFrame(()=>{ln=0,t()})})}function Z(e){$(),gr(e)}function _r(e,t=null){let n=t||Y(e),r=n?.nextElementSibling?.matches?.(`.report-card[data-report-id]`)?n.nextElementSibling:n?.previousElementSibling?.matches?.(`.report-card[data-report-id]`)?n.previousElementSibling:null;if(!r){let e=n?.closest(`.group-column[data-bucket-id]`),t=e?.nextElementSibling?.matches?.(`.group-column[data-bucket-id]`)?e.nextElementSibling:e?.previousElementSibling?.matches?.(`.group-column[data-bucket-id]`)?e.previousElementSibling:null;r=t?.querySelector(`.report-card[data-report-id]`)||t||n}return X(r)}function vr(e,t){let n=J(e,t);return X(n?.nextElementSibling?.matches?.(`.group-column[data-bucket-id]`)?n.nextElementSibling:n?.previousElementSibling?.matches?.(`.group-column[data-bucket-id]`)?n.previousElementSibling:n)}function yr(e,t=`smooth`){ur(),z=requestAnimationFrame(()=>{z=requestAnimationFrame(()=>{z=0,fr(e(),t)})})}function Q(e=null){Z(X(typeof e==`function`?e():e))}[`wheel`,`touchstart`,`pointerdown`].forEach(e=>{window.addEventListener(e,ur,{passive:!0})});var br=null,xr=new Set;function Sr(){return br||(br=new Promise((e,t)=>{if(!window.indexedDB){t(Error(`IndexedDB unavailable`));return}let n=indexedDB.open(Yt,1);n.onupgradeneeded=()=>{let e=n.result;e.objectStoreNames.contains(C)||e.createObjectStore(C,{keyPath:`id`}).createIndex(`reportId`,`reportId`,{unique:!1})},n.onsuccess=()=>e(n.result),n.onerror=()=>t(n.error||Error(`File database failed`))}),br)}async function Cr(e,t=[]){let n=t.map(t=>{let{blob:n,...r}=t;return{...r,storageId:n instanceof Blob?`${e}:${t.id}`:t.storageId||``,blob:n}}),r=n.filter(e=>e.blob instanceof Blob&&e.storageId);if(r.length){let t=await Sr();await new Promise((n,i)=>{let a=t.transaction(C,`readwrite`),o=a.objectStore(C);r.forEach(t=>{o.put({id:t.storageId,reportId:e,name:t.name,type:t.type,size:t.size,blob:t.blob,updatedAt:new Date().toISOString()})}),a.oncomplete=n,a.onerror=()=>i(a.error||Error(`File save failed`)),a.onabort=()=>i(a.error||Error(`File save aborted`))})}return n.map(({blob:e,...t})=>t)}async function wr(e){if(e?.storageId)try{let t=await Sr(),n=await new Promise((n,r)=>{let i=t.transaction(C,`readonly`).objectStore(C).get(e.storageId);i.onsuccess=()=>n(i.result),i.onerror=()=>r(i.error||Error(`File read failed`))});if(n?.blob instanceof Blob)return n.blob}catch{return null}let t=e?.content||e?.excerpt;return t?new Blob([t],{type:e.type||`text/plain;charset=utf-8`}):null}async function Tr(e){try{let t=await Sr();await new Promise((n,r)=>{let i=t.transaction(C,`readwrite`),a=i.objectStore(C),o=a.index(`reportId`).openKeyCursor(IDBKeyRange.only(e));o.onsuccess=()=>{let e=o.result;e&&(a.delete(e.primaryKey),e.continue())},o.onerror=()=>r(o.error||Error(`File cleanup failed`)),i.oncomplete=n,i.onerror=()=>r(i.error||Error(`File cleanup failed`))})}catch{}}function Er(){xr.forEach(e=>URL.revokeObjectURL(e)),xr.clear()}function Dr(e){let t=URL.createObjectURL(e);return xr.add(t),t}async function Or(){let e=[...document.querySelectorAll(`[data-saved-file-preview]`)];await Promise.all(e.map(async e=>{let t=D.reports.find(t=>t.id===e.dataset.reportId)?.savedFiles?.find(t=>t.id===e.dataset.fileId);if(!t)return;e.classList.add(`is-loading`),e.setAttribute(`aria-busy`,`true`);let n=await wr(t);if(!n||!e.isConnected){e.classList.remove(`is-loading`),e.setAttribute(`aria-busy`,`false`);return}let r=e.dataset.previewMode;try{if(r===`image`){let r=document.createElement(`img`);r.src=Dr(n),r.alt=t.name||`图片预览`,r.draggable=!1,e.replaceChildren(r)}else if(r===`html`){let r=document.createElement(`iframe`);r.src=Dr(n),r.title=`${t.name||`HTML`}内容`,r.setAttribute(`sandbox`,`allow-forms allow-modals allow-popups allow-scripts`),e.replaceChildren(r)}else await It(e,n,t,r);e.classList.add(`is-ready`)}catch(t){let n=document.createElement(`div`);n.className=`saved-file-render-error`;let r=document.createElement(`strong`);r.textContent=`暂时无法直接显示这个文件`;let i=document.createElement(`p`);i.textContent=t?.message||`请下载原文件后使用对应应用打开`,n.append(r,i),e.replaceChildren(n),e.classList.add(`has-error`)}finally{e.classList.remove(`is-loading`),e.setAttribute(`aria-busy`,`false`)}}))}async function kr(e){let t=await wr(e);if(!t)return!1;let n=Dr(t),r=document.createElement(`a`);return r.href=n,r.download=e.name||`download`,document.body.append(r),r.click(),r.remove(),window.setTimeout(()=>{URL.revokeObjectURL(n),xr.delete(n)},1e3),!0}async function Ar(e){let t=window.open(``,`_blank`),n=await wr(e);if(!n)return t?.close(),!1;let r=Dr(n);return t?t.location.href=r:window.open(r,`_blank`,`noopener,noreferrer`),window.setTimeout(()=>{URL.revokeObjectURL(r),xr.delete(r)},6e4),!0}function jr(e){return e.savedHtml||Nn(e.savedContent,e.savedFiles)}function Mr(e){return`${String(e.title||`report`).replace(/[\\/:*?"<>|]+/g,`-`).replace(/\s+/g,` `).trim().slice(0,80)||`report`}.html`}function Nr(e){let t=jr(e);return t?URL.createObjectURL(new Blob([t],{type:`text/html;charset=utf-8`})):``}function Pr(e){let t=Nr(e);if(!t)return!1;let n=document.createElement(`a`);return n.href=t,n.download=Mr(e),document.body.append(n),n.click(),n.remove(),window.setTimeout(()=>URL.revokeObjectURL(t),1e3),!0}function Fr(e){let t=e.url||Nr(e);return t?(window.open(t,`_blank`,`noopener,noreferrer`),e.url||window.setTimeout(()=>URL.revokeObjectURL(t),6e4),!0):!1}function Ir(e,t,n=!1){let r=g(t),i=t.format||r.label,a=!!(t.storageId&&[`image`,`pdf`,`html`].includes(r.preview)),o=n&&r.preview===`pdf`?`pdf-thumb`:r.preview,s=a?`data-saved-file-preview data-report-id="${W(e.id)}" data-file-id="${W(t.id)}" data-preview-mode="${o}"`:``,c=r.preview===`text`&&(t.excerpt||t.content)?`<pre>${W((t.excerpt||t.content).slice(0,n?280:8e3))}</pre>`:``;return`
    ...
    </div>`}function Lr(e,t,n,r){let i=g(t),a=t.format||i.label,o=t.storageId||t.content||t.excerpt?`data-saved-file-preview data-report-id="${W(e.id)}" data-file-id="${W(t.id)}" data-preview-mode="${W(i.preview)}"`:``;return`<article class="saved-file-embed file-kind-${i.kind}">
    <header class="saved-file-embed-header">
      <div class="saved-file-identity">
        <span class="saved-file-format">${W(a)}</span>
        <div><b>${W(t.name||`未命名文件`)}</b><small>${W(t.sizeLabel||``)} · ${n+1}/${r}</small></div>
      </div>
      <button type="button" class="saved-file-download" data-action="download-saved-file" data-id="${W(e.id)}" data-file-id="${W(t.id)}">下载原文件</button>
    </header>
    <div class="saved-file-embedded-content" ${o}>
      <div class="saved-file-loading" aria-hidden="true"><span></span><strong>正在展开 ${W(a)} 内容</strong></div>
    </div>
  </article>`}function Rr(e){let t=e.savedFiles||[];return t.length?`<section class="saved-file-list embedded-file-list" aria-label="档案正文">
    <div class="embedded-file-list-heading"><strong>档案正文</strong><span>${t.length} 个文件 · 已直接展开</span></div>
    ${t.map((n,r)=>Lr(e,n,r,t.length)).join(``)}
  </section>`:``}function zr(e,t=!1){let n=!e.url&&(!!e.savedContent||!!(e.savedFiles||[]).length),r=[`org`,`account`].includes(e.access),i=e.access===`org`?`需组织登录`:e.access===`account`?`需账号登录`:`生产可访问`,a=jr(e),o=!!e.pinned,s=D.groups.find(t=>t.id===e.groupId)?.name||`未归类`,c=new Set([`HTML`,`手动保存`,`生产`]),l=[...new Set([s,Wn(e.workType),...e.tags||[]])].filter(e=>!c.has(e)),u=!r&&E.reports.some(t=>t.id===e.id),d=e.preview||`${e.id}.png`,f=(e.savedFiles||[])[0],p=a&&e.isHtml?`<iframe class="local-html-preview-frame" title="${W(e.title)}视觉预览"
        srcdoc="${W(a)}" sandbox="allow-scripts" loading="lazy"
        tabindex="-1" aria-hidden="true"></iframe>`:u?`<img src="./previews/${W(d)}" alt="" loading="lazy" decoding="async" draggable="false" />`:n&&f?Ir(e,f,!0):`
      <div class="preview-placeholder ${r?`preview-restricted`:``}">
        <span>${r?`ACCESS`:W(e.title.slice(0,2))}</span>
        <strong>${r?i:n?`本地内容`:`预览待补充`}</strong>
      </div>`;return`
    <article class="report-card ${r?`restricted-card`:``} ${t?`archived-card`:``} ${o?`is-featured`:``} ${P===e.id?`is-move-selected`:``}"
      data-report-id="${W(e.id)}" ${t||j===`time`?``:`data-report-draggable="true"`}>
      <button class="card-main" type="button" data-action="open" data-id="${W(e.id)}" aria-label="打开${W(e.title)}">
        ...
        <span class="report-copy">
          ...
          ${r?`<span class="report-access-note">${W(i)}</span>`:``}
        </span>
      </button>
      <div class="card-actions">
        ${t?`
            ...
            <button type="button" class="studio-icon-button feature-action" data-action="toggle-pin" data-id="${W(e.id)}"
              title="${o?`取消精选`:`设为精选`}" aria-label="${o?`取消精选`:`设为精选`}">${T.star}</button>`}
      </div>
    </article>`}function Br(){if(!L)return``;if(L.type===`delete-report`){let e=D.reports.find(e=>e.id===L.reportId);return e?`
      <div class="dialog-backdrop">
        <section class="dialog compact-dialog destructive-dialog" role="dialog" aria-modal="true"
          ...
          <div class="dialog-title-row">
            ...
            <button type="button" class="studio-icon-button dialog-close-button" data-action="close-modal" title="关闭" aria-label="关闭">${T.close}</button>
          </div>
          <p id="delete-report-dialog-description" class="destructive-dialog-copy">
            将永久删除“<strong>${W(e.title)}</strong>”。此操作完成后无法从归档区恢复。
          </p>
          <div class="dialog-actions">
            <button type="button" class="quiet-button" data-action="close-modal" autofocus>Cancel</button>
            <button type="button" class="danger-button" data-action="confirm-delete" data-id="${W(e.id)}">Delete permanently</button>
          </div>
        </section>
      </div>`:``}if(L.type===`group`){let e=L.mode===`edit`?D.groups.find(e=>e.id===L.groupId):null;return`
      <div class="dialog-backdrop">
      ...
      </div>`}let e=L.mode===`edit`?D.reports.find(e=>e.id===L.reportId):null,t=e?.groupId||L.groupId||D.groups[0]?.id||``,n=G((e?.tags||[]).join(`、`)).filter(e=>![`HTML`,`手动保存`,`生产`].includes(e));return`
    <div class="dialog-backdrop">
      <form class="dialog" id="report-form" role="dialog" aria-modal="true"
        ...
        <label>放入分组
          <select name="groupId">
            ${D.groups.map(e=>`<option value="${W(e.id)}" ${e.id===t?`selected`:``}>${W(e.name)}</option>`).join(``)}
          </select>
        </label>
        <label>工作类型
          <select name="workType">
            ${Xt.map(t=>`<option value="${W(t.id)}" ${t.id===(e?.workType||`product-planning`)?`selected`:``}>${W(t.name)}</option>`).join(``)}
          </select>
        </label>
        <fieldset class="report-tag-field">
          ...
          <div class="report-tag-picker" aria-label="选择关键标签">
            ${rr().map(e=>`<button type="button" class="${n.includes(e)?`selected`:``}"
    ...
    </div>`}function Vr(e){if(_t(e.id))return yt(e,W);let t=!e.url&&(!!e.savedContent||!!(e.savedFiles||[]).length),n=[`org`,`account`].includes(e.access),r=e.loginProvider||Pn(e.url)?.provider||(e.access===`org`?`组织帐号`:`站点帐号`),i=e.savedHtml||Nn(e.savedContent,e.savedFiles),a=i?`edit-local-document`:e.url?n?`edit`:`edit-document`:``,o=i?`
...
`,`<br />`)}</div>`:``}
      ...
      <div class="login-handoff-wrap">
        <section class="login-handoff-card" aria-labelledby="login-handoff-title">
          ...
          <p class="login-handoff-domain">${W(tr(e.url))}</p>
        </section>
      </div>`:`
      <div class="reader-frame-wrap">
    ...
    <main class="reader-shell compact-reader-shell">
      <header class="reader-header compact-reader-header">
        <button class="reader-icon-button back-button" type="button" data-action="back"
          aria-label="返回成果库" title="返回成果库">${er(`back`)}</button>
        <div class="reader-title">
          <strong>${W(e.title)}</strong>
          <span>${t?`本地保存`:W(tr(e.url))}</span>
        </div>
    ...
    <header class="topbar">
      ...
      ${an?`<div class="top-actions"><button class="quiet-button" type="button" data-action="show-catalog">← Library</button></div>`:``}
    </header>`}function Ur(){let e=D.reports.filter(e=>e.archived).filter(e=>Ut(e,k,{group:D.groups.find(t=>t.id===e.groupId),workTypeName:Wn(e.workType)})).sort((e,t)=>new Date(t.archivedAt||0)-new Date(e.archivedAt||0)),t=D.reports.filter(e=>e.archived).length;return`
    <main class="app-shell archive-shell">
      ...
      <section class="workspace archive-workspace">
        ...
        <label class="search archive-search">
          ...
          <input id="search-input" value="${W(k)}"
            placeholder="搜索归档标题、来源或网址" aria-label="搜索归档" />
          ${k?`<button type="button" class="studio-icon-button search-clear-button" data-action="clear-search" title="清除搜索" aria-label="清除搜索">${T.close}</button>`:``}
        </label>
        ${e.length?`
          <section class="archive-results">
            ...
            <div class="archive-grid">${e.map(e=>zr(e,!0)).join(``)}</div>
          </section>`:`
      ...
      ${Br()}
    </main>`}function Wr(e,t){if(![`topic`,`type`,`tag`].includes(e.kind))return``;let n=t.filter(t=>t.kind===e.kind),r=n.findIndex(t=>t.id===e.id);if(r<0)return``;let i=r===0,a=r===n.length-1,o=(t,n,r,i)=>`
    <button type="button" class="studio-icon-button group-order-button" data-action="move-group"
      ...
      title="${n}" aria-label="${n}" ${i?`disabled`:``}>${r}</button>`;return[o(`top`,`置顶`,T.top,i),o(`up`,`上移`,T.up,i),o(`down`,`下移`,T.down,a),o(`bottom`,`置底`,T.bottom,a)].join(``)}function Gr(){if(an)return Ur();let e=S(k),t=D.reports.filter(e=>!e.archived),n=e?Xn(t,e):[],r=B.size?n.filter(e=>e.dimensions.some(e=>B.has(e))):n,i=e?r.map(e=>e.report):t,a=Vn(t.filter(e=>e.pinned),`featured`,`featured`,(e,t)=>Kn(t)-Kn(e)),o={id:`featured`,name:`精选成果`,kind:`featured`,accent:`violet`,reports:a},s=D.reports.filter(e=>e.archived).length,c=t.filter(e=>e.access===`production`).length,l=t.filter(e=>e.access!==`production`).length,u=qn(t,``),d=j===`topic`&&a.length?[o,...u]:u,f=j===`time`&&u[0]?.reports||[],p=e?[]:(j===`topic`&&a.length?[o,...u]:u).filter(e=>e.reports.length||P||j===`topic`),m=j===`type`?`工作类型`:j===`tag`?`关键标签`:j===`time`?`新增时间`:`工作主题`;return`
    <main class="app-shell">
        ...
        <div class="results-toolbar unified-results-toolbar">
          ...
          <div class="results-toolbar-side">
            <label class="search results-search">
              <span aria-hidden="true">⌕</span>
              <input id="search-input" type="search" value="${W(k)}"
                placeholder="Rediscover your work" aria-label="找到一个成果"
                autocomplete="off" spellcheck="false" enterkeyhint="search" />
              ${k?`<button type="button" class="studio-icon-button search-clear-button" data-action="clear-search" title="清除搜索" aria-label="清除搜索">${T.close}</button>`:``}
            </label>
            <div class="studio-summary compact-summary" aria-label="成果统计">
              <strong>${e?i.length:t.length}</strong><span>${e?`匹配`:`成果`}</span>
              <i></i>
              <strong>${D.groups.length}</strong><span>主题</span>
              <i></i>
              <strong>${c}</strong><span>直达</span>
            </div>
          </div>
        </div>
        <section class="groups-section">
          ${P?`
            <div class="move-mode-banner" role="status">
              <div><strong>正在整理报告</strong><span>选择目标${Yn()}的“移到这里”，或直接拖动卡片。</span></div>
              <button type="button" data-action="cancel-move">Cancel</button>
            </div>`:``}
          ${e||p.length?`
            <div class="library-layout">
              <nav class="topic-nav" aria-label="报告${m}">
                <div class="library-nav-controls">
                  ...
                  <button class="studio-icon-button add-topic-icon" type="button" data-action="add-group"
                    aria-label="Add topic" title="Add topic">${T.plus}</button>
                </div>
                ${j===`time`?`
              ...
              ${e?`
                <section class="search-results-panel">
                  <header class="search-results-header">
                    <div><span>SEARCH RESULTS</span><h2>“${W(k.trim())}”</h2></div>
                    <strong class="search-results-announcement" role="status" aria-live="polite">匹配到了 ${i.length} 份</strong>
                  </header>
                  ${Zn(n)}
                  ${i.length?`<div class="group-cards search-results-cards">${i.map(e=>zr(e)).join(``)}</div>`:n.length?`<div class="no-results search-no-results">
                  ...
                  <header class="group-header">
                    <div class="group-heading-area">
                      ...
                      ${e.kind===`topic`?`<div class="group-primary-actions" aria-label="分组快捷操作">
                        <button type="button" class="studio-icon-button" data-action="add-to-group" data-id="${W(e.id)}" title="新增成果" aria-label="新增成果">${T.plus}</button>
                        <button type="button" class="studio-icon-button" data-action="rename-group" data-id="${W(e.id)}" title="编辑分组" aria-label="编辑分组">${T.edit}</button>
                      </div>`:``}
                    </div>
                    <div class="group-menu" aria-label="分组排序操作">
                      ${P&&e.kind!==`time`?`<button class="move-here-button" type="button" data-action="move-here" data-id="${W(e.id)}" data-bucket-kind="${W(e.kind)}">Move here</button>`:``}
                      ${Wr(e,u)}
                      ${e.kind===`topic`?`<button type="button" class="studio-icon-button group-delete-button" data-action="delete-group" data-id="${W(e.id)}" title="删除分组" aria-label="删除分组">${T.minus}</button>`:``}
                    </div>
                  </header>
                  <div class="group-cards">
                    ${e.reports.length?e.reports.map(e=>zr(e)).join(``):e.kind===`topic`?`<button class="empty-topic-drop" type="button" data-action="add-to-group" data-id="${W(e.id)}">
          ...
          <div class="catalog-note">
            <span>${l} 份报告需要组织或账号登录${s?` · ${s} 份已安全归档`:``}</span>
      ...
      ${Br()}
    </main>`}function $(){let e=document.getElementById(`app`),t=A&&D.reports.find(e=>e.id===A);Er(),e.innerHTML=t?Vr(t):Gr(),cr(),Jr(),he({render:()=>Q(()=>document.querySelector(`.prompt-composer`)),showToast:K,saveToLibrary:Ln}),Or()}async function Kr(e){let t=e.elements.url,n=e.elements.title,r=e.querySelector(`[data-action="detect-title"]`),i=e.querySelector(`.field-hint`),a=t.value.trim();if(!nr(a))return i.textContent=`请输入完整的 http 或 https 网址`,``;r.disabled=!0,r.innerHTML=`<span class="mini-spinner"></span>`,i.textContent=`正在读取网页标题…`;try{let{title:e}=await Fn(a);if(!e)throw Error(`read failed`);return n.value=e,i.textContent=`已识别网页标题`,n.value}catch{let e=tr(a);return n.value||=e,i.textContent=`网页暂时无法读取，已用域名作为标题，你可以手动修改`,n.value}finally{r.disabled=!1,r.textContent=`Detect title`}}function qr(){let e=document.querySelector(`.board`);if(!e)return;let t=null,n=()=>{document.querySelectorAll(`.report-card, .group-column, .topic-nav a`).forEach(e=>{e.classList.remove(`is-nav-drop-target`)})},r=e=>e?[e.bucketKind,e.bucketId,e.targetReportId,Number(e.placeAfter),Number(e.nav)].join(`|`):``,i=()=>{let e=t.sourceCard.getBoundingClientRect(),n=t.sourceCard.cloneNode(!0);return n.removeAttribute(`id`),n.className=`report-card report-drag-preview`,n.style.width=`${e.width}px`,n.style.height=`${e.height}px`,n.querySelectorAll(`button, [role='button'], iframe`).forEach(e=>{e.removeAttribute(`data-action`),e.setAttribute(`tabindex`,`-1`)}),document.body.append(n),t.previewWidth=e.width,t.previewHeight=e.height,t.previewOffsetX=Math.max(18,Math.min(e.width-18,t.startX-e.left)),t.previewOffsetY=Math.max(18,Math.min(e.height-18,t.startY-e.top)),n},a=()=>{if(!t?.preview)return;let e=Math.max(8,window.innerWidth-t.previewWidth-8),n=Math.max(8,window.innerHeight-t.previewHeight-8),r=Math.max(8,Math.min(e,t.x-t.previewOffsetX)),i=Math.max(8,Math.min(n,t.y-t.previewOffsetY));t.preview.style.transform=`translate3d(${r}px, ${i}px, 0)`},o=()=>{!t?.active||t.updateFrame||(t.updateFrame=requestAnimationFrame(()=>{t?.active&&(t.updateFrame=0,a(),u())}))},s=(e,n=null,r=!1)=>{if(!(!e||!t?.placeholder)){if(!n||n.parentElement!==e){e.append(t.placeholder);return}e.insertBefore(t.placeholder,r?n.nextSibling:n)}},c=()=>{let e=document.elementFromPoint(t.x,t.y);if(e?.closest(`.report-card-placeholder`))return t.target;let n=e?.closest(`.topic-nav a[data-nav-bucket-id]`);if(n)return{bucketKind:n.dataset.navBucketKind,bucketId:n.dataset.navBucketId,targetReportId:``,placeAfter:!1,nav:!0,element:n};let r=e?.closest(`.report-card:not(.report-card-placeholder):not(.report-drag-preview)`),i=e?.closest(`.group-column`);if(r&&r!==t.sourceCard){let e=r.closest(`.group-column`);if(!e||e.dataset.bucketKind===`time`)return null;let n=r.getBoundingClientRect(),i=t.y>n.bottom-n.height*.22||t.y>=n.top+n.height*.22&&t.y<=n.bottom-n.height*.22&&t.x>n.left+n.width/2;return{bucketKind:e.dataset.bucketKind||j,bucketId:e.dataset.bucketId||``,targetReportId:r.dataset.reportId||``,placeAfter:i,nav:!1,element:r,container:e.querySelector(`.group-cards`)}}return i&&i.dataset.bucketKind!==`time`?{bucketKind:i.dataset.bucketKind||j,bucketId:i.dataset.bucketId||``,targetReportId:``,placeAfter:!1,nav:!1,element:i,container:i.querySelector(`.group-cards`)}:null},l=e=>{if(!(!t||r(e)===r(t.target))&&(n(),t.target=e,F=e,e)){if(e.nav){e.element.classList.add(`is-nav-drop-target`);return}if(e.targetReportId){s(e.container,e.element,e.placeAfter);return}s(e.container)}},u=()=>{t?.active&&l(c())},d=()=>{let e=Math.min(window.innerHeight*.34,dr()+72),n=window.innerHeight-72;if(t.y<e){let n=Math.min(1,(e-t.y)/84);return-Math.max(1,Math.round(12*n*n))}if(t.y>n){let e=Math.min(1,(t.y-n)/84);return Math.max(1,Math.round(12*e*e))}return 0},f=()=>{if(!t?.active)return;let e=d();if(e){let t=window.scrollY;window.scrollBy(0,e),window.scrollY!==t&&u()}t.autoScrollFrame=requestAnimationFrame(f)},p=()=>{!t||t.active||(clearTimeout(t.holdTimer),t.sourceCard.setPointerCapture?.(t.pointerId),t.active=!0,N=t.reportId,t.preview=i(),t.placeholder=document.createElement(`div`),t.placeholder.className=`report-card report-card-placeholder`,t.placeholder.innerHTML=`<span>放在这里</span>`,t.placeholder.style.minHeight=`${t.sourceCard.getBoundingClientRect().height}px`,t.sourceCard.before(t.placeholder),t.sourceCard.classList.add(`is-dragging`),document.body.classList.add(`report-drag-session`),a(),u(),t.autoScrollFrame=requestAnimationFrame(f))},m=()=>{t&&(clearTimeout(t.holdTimer),t.autoScrollFrame&&cancelAnimationFrame(t.autoScrollFrame),t.updateFrame&&cancelAnimationFrame(t.updateFrame),t.preview?.remove(),t.placeholder?.remove(),t.sourceCard.classList.remove(`is-dragging`),document.body.classList.remove(`report-drag-session`),n(),N=``,F=null)},h=()=>{if(!t)return;let e=t,n=e.active?e.target:null,r=e.reportId;if(e.active&&(un=r,dn=Date.now()+500),m(),t=null,!n?.bucketId||n.bucketKind===`time`)return;let i=V(D),a=V(O);Jn(r,n.bucketKind,n.bucketId,n.targetReportId||``,!!n.placeAfter)&&(Q(()=>Y(r)),n.nav&&yr(()=>J(n.bucketKind,n.bucketId)),ir(n.bucketKind===`featured`?`已加入精选成果`:n.bucketKind===`tag`?`已添加目标标签`:n.bucketKind===`type`?`工作类型已更新`:n.targetReportId?`报告顺序已更新`:`已移入新主题`,()=>{D=i,O=a,H(),Sn(),Q(()=>Y(r))}))};e.addEventListener(`pointerdown`,e=>{if(e.button!==0||e.target.closest(`.card-actions`))return;let n=e.target.closest(`.report-card[data-report-draggable="true"]`);n?.closest(`.group-column`)&&(e.pointerType===`mouse`&&e.preventDefault(),t={pointerId:e.pointerId,reportId:n.dataset.reportId,sourceCard:n,startX:e.clientX,startY:e.clientY,x:e.clientX,y:e.clientY,active:!1,target:null,preview:null,placeholder:null,autoScrollFrame:0,updateFrame:0,holdTimer:0},t.holdTimer=window.setTimeout(()=>p(),240))}),e.addEventListener(`pointermove`,e=>{!t||e.pointerId!==t.pointerId||(t.x=e.clientX,t.y=e.clientY,!t.active&&Math.hypot(t.x-t.startX,t.y-t.startY)>=8&&p(),t.active&&(e.preventDefault(),o()))}),e.addEventListener(`pointerup`,e=>{!t||e.pointerId!==t.pointerId||h()}),e.addEventListener(`pointercancel`,()=>{m(),t=null})}function Jr(){let e=document.getElementById(`search-input`),t=!1,n=e=>{let t=e?.value||``;if(t===k)return;let n=e.selectionStart,r=e.selectionEnd;(!S(k)||!S(t))&&B.clear(),k=t,Q(()=>document.querySelector(`.results-toolbar, .archive-search`));let i=document.getElementById(`search-input`);i?.focus({preventScroll:!0}),i?.setSelectionRange(n,r)};e?.addEventListener(`compositionstart`,()=>{t=!0}),e?.addEventListener(`compositionend`,e=>{t=!1,n(e.currentTarget)}),e?.addEventListener(`input`,e=>{e.isComposing||t||n(e.currentTarget)}),e?.addEventListener(`search`,e=>n(e.currentTarget)),e?.addEventListener(`keydown`,e=>{e.key!==`Escape`||!k||(e.preventDefault(),k=``,B.clear(),Q(()=>document.querySelector(`.results-toolbar, .archive-search`)),document.getElementById(`search-input`)?.focus({preventScroll:!0}))}),document.querySelectorAll(`[data-action]`).forEach(e=>{e.addEventListener(`click`,async e=>{let t=e.currentTarget.dataset.action,n=e.currentTarget.dataset.id,r=e.currentTarget.closest(`.report-card`);if(t===`scroll-top`)lr(`smooth`);else if(t===`open`){if(n===un&&Date.now()<dn)return;fn=X(r||Y(n)),A=n,$(),lr()}else if(t===`preview-saved-file`||t===`download-saved-file`){let r=D.reports.find(e=>e.id===n)?.savedFiles?.find(t=>t.id===e.currentTarget.dataset.fileId);if(!r)return;(t===`preview-saved-file`?await Ar(r):await kr(r))||K(`原文件未找到，请重新上传后保存`)}else if(t===`edit-document`){let e=D.reports.find(e=>e.id===n);if(!e||e.access!==`production`)return;vt(e,{render:$,showToast:K})}else if(t===`edit-local-document`){let e=D.reports.find(e=>e.id===n);if(!e||!jr(e))return;vt(e,{render:$,showToast:K,saveLocal:async t=>{let n=e.savedHtml;e.savedHtml=t,e.isHtml=!0,e.tags=tn(e,e.workType),U(e);try{H()}catch{throw e.savedHtml=n,Error(`修改后的 HTML 超过当前浏览器可保存容量，请先下载备份`)}}})}else if(t===`download-report`){let e=D.reports.find(e=>e.id===n);if(!e)return;jr(e)?Pr(e)&&K(`HTML 已下载`):await xt(e,K)}else if(t===`share-report`||t===`copy-production-url`){let e=D.reports.find(e=>e.id===n);e?.url&&await St(e,e=>{K(e===`报告链接已复制`?`生产 URL 已复制`:e)})}else if(t===`open-browser`){let e=D.reports.find(e=>e.id===n);if(!e)return;Fr(e)||K(`浏览器未能打开该报告`)}else if(t===`back`)A=``,L=null,Z(fn||{scrollY:0}),fn=null;else if(t===`clear-search`)k=``,B.clear(),Q(()=>document.querySelector(`.results-toolbar, .archive-search`)),document.getElementById(`search-input`)?.focus({preventScroll:!0});else if(t===`toggle-search-dimension`)n===`all`?B.clear():vn.some(e=>e.id===n)&&(B.has(n)?B.delete(n):B.add(n)),Q(()=>document.querySelector(`.search-dimension-controls`)),requestAnimationFrame(()=>{document.querySelector(`[data-action="toggle-search-dimension"][data-id="${CSS.escape(n)}"]`)?.focus({preventScroll:!0})});else if(t===`reset-search-dimensions`)B.clear(),Q(()=>document.querySelector(`.search-dimension-controls`)),requestAnimationFrame(()=>{document.querySelector(`[data-action="toggle-search-dimension"][data-id="all"]`)?.focus({preventScroll:!0})});else if(t===`set-view`){if(![`topic`,`type`,`tag`,`time`].includes(n))return;let e={scrollY:window.scrollY,identity:null,viewportTop:null};j=n,P=``,localStorage.setItem(Gt,j),Z(e),requestAnimationFrame(()=>{document.querySelector(`[data-action="set-view"][data-id="${CSS.escape(n)}"]`)?.focus({preventScroll:!0})})}else if(t===`toggle-time-sort`)M=M===`created`?`modified`:`created`,localStorage.setItem(Kt,M),Q();else if(t===`cancel-move`)P=``,Q();else if(t===`move-here`){let t=e.currentTarget.dataset.bucketKind||j,r=V(D),i=V(O);if(P&&Jn(P,t,n)){let e=P;P=``,Q(()=>Y(e)),ir(t===`tag`?`已添加目标标签`:`报告已移入目标${Yn()}`,()=>{D=r,O=i,H(),Sn(),Q(()=>Y(e))})}}else if(t===`show-archive`)an=!0,k=``,B.clear(),A=``,$(),lr();else if(t===`show-catalog`)an=!1,k=``,B.clear(),A=``,$(),lr();else if(t===`add-report`)sr({type:`report`,mode:`create`,groupId:D.groups[0]?.id},X(document.querySelector(`.results-toolbar`)),e.currentTarget);else if(t===`add-to-group`)sr({type:`report`,mode:`create`,groupId:n},X(J(`topic`,n)),e.currentTarget);else if(t===`edit`)sr({type:`report`,mode:`edit`,reportId:n},X(r||Y(n)),e.currentTarget);else if(t===`toggle-pin`){let e=D.reports.find(e=>e.id===n);if(!e)return;let t=V(e),i=e.pinned&&r?.closest(`[data-bucket-kind="featured"]`)?_r(n,r):X(r||Y(n));e.pinned=!e.pinned,U(e),H(),Z(i),ir(e.pinned?`已加入精选成果`:`已移出精选成果`,()=>{let e=D.reports.find(e=>e.id===n);e&&(Object.assign(e,t),e.archived=!!t.archived,e.archivedAt=t.archivedAt||``,H(),Q(()=>Y(n)))})}else if(t===`close-modal`)q();else if(t===`detect-title`)await Kr(e.currentTarget.closest(`form`));else if(t===`archive`){let e=D.reports.find(e=>e.id===n);if(!e)return;let t=V(e),i=_r(n,r);e.archived=!0,e.archivedAt=new Date().toISOString(),H(),Z(i),ir(`已归档，可随时恢复`,()=>{let e=D.reports.find(e=>e.id===n);e&&(Object.assign(e,t),e.archived=!!t.archived,e.archivedAt=t.archivedAt||``,H(),Q(()=>Y(n)))})}else if(t===`restore`){let e=D.reports.find(e=>e.id===n);if(!e)return;let t=V(e),r=_r(n);e.archived=!1,e.archivedAt=``,H(),Z(r),ir(`报告已恢复到原主题`,()=>{let e=D.reports.find(e=>e.id===n);e&&(Object.assign(e,t),H(),Q(()=>Y(n)))})}else if(t===`delete`){if(!D.reports.find(e=>e.id===n)?.archived)return;sr({type:`delete-report`,reportId:n},_r(n),e.currentTarget)}else if(t===`confirm-delete`){let e=D.reports.find(e=>e.id===n);if(!e?.archived||L?.type!==`delete-report`)return;D.reports=D.reports.filter(e=>e.id!==n),A===n&&(A=``),H(),await Tr(n),q({fallbackSelector:`.archive-grid, .archive-search`}),K(`已永久删除“${e.title}”`)}else if(t===`add-group`)sr({type:`group`,mode:`create`},X(document.querySelector(`.results-toolbar`)),e.currentTarget);else if(t===`rename-group`)D.groups.find(e=>e.id===n)&&sr({type:`group`,mode:`edit`,groupId:n},X(J(`topic`,n)),e.currentTarget);else if(t===`move-group`){let t=e.currentTarget.dataset.bucketKind,r=e.currentTarget.dataset.direction,i=X(J(t,n));zn(n,r,t)&&(Z(i),requestAnimationFrame(()=>{document.querySelector(`[data-action="move-group"][data-id="${CSS.escape(n)}"][data-direction="${CSS.escape(r)}"]`)?.focus({preventScroll:!0})}),K(`分组顺序已更新`))}else if(t===`delete-group`){let e=D.groups.find(e=>e.id===n),t=D.groups.find(e=>e.id!==n);if(e&&!t)K(`请先新增另一个分组，再删除当前分组`);else if(e&&confirm(`删除“${e.name}”？其中的报告会移到“${t.name}”。`)){let e=vr(`topic`,n);D.reports.forEach(e=>{e.groupId===n&&(e.groupId=t.id)}),D.groups=D.groups.filter(e=>e.id!==n),H(),Z(e),K(`分组已删除，报告已移到“${t.name}”`)}}})}),document.querySelector(`.topbar`)?.addEventListener(`click`,e=>{e.target.closest(`button, a`)||lr(`smooth`)}),document.querySelectorAll(`.topic-nav a[data-nav-bucket-id]`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault();let n=e.dataset.navBucketKind,r=e.dataset.navBucketId;if(k){k=``,Q(),yr(()=>J(n,r));return}fr(J(n,r))})}),document.querySelectorAll(`.topic-nav a[data-nav-report-id]`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault(),fr(Y(e.dataset.navReportId))})}),qr(),document.querySelectorAll(`.legacy-report-drag-handle`).forEach(e=>{let t=null,n=!1,r=null,i=0,a=0,o=()=>{i&&=(cancelAnimationFrame(i),0)},s=()=>{if(!N)return o();let e=Math.min(110,window.innerHeight*.18),t=a<e?a-e:a>window.innerHeight-e?a-(window.innerHeight-e):0;if(!t)return o();let n=Math.sign(t)*Math.min(24,5+Math.abs(t)*.2);window.scrollBy(0,n),i=requestAnimationFrame(s)},c=t=>{let n=e.closest(`.report-card`);if(n){if(!r){let e=n.getBoundingClientRect();r=n.cloneNode(!0),r.className=`report-card report-drag-preview`,r.style.width=`${e.width}px`,r.style.height=`${e.height}px`,r.querySelectorAll(`button, [role='button']`).forEach(e=>{e.removeAttribute(`data-action`),e.setAttribute(`tabindex`,`-1`)}),document.body.append(r)}r.style.transform=`translate3d(${t.clientX+16}px, ${t.clientY+16}px, 0)`}},l=()=>{F=null,I?.remove(),I=null,document.querySelectorAll(`.report-card, .group-column, .topic-nav a`).forEach(e=>e.classList.remove(`is-card-drop-target`,`is-card-drop-before`,`is-card-drop-after`,`is-drop-ready`,`is-nav-drop-target`))},u=(t,n,r)=>{if(t){if(!I){I=document.createElement(`div`),I.className=`report-card report-card-placeholder`,I.innerHTML=`<span>放在这里</span>`;let t=e.closest(`.report-card`)?.getBoundingClientRect().height;t&&(I.style.minHeight=`${t}px`)}if(!n||n.parentElement!==t){t.append(I);return}t.insertBefore(I,r?n.nextSibling:n)}},d=t=>{let n=document.elementFromPoint(t.clientX,t.clientY),r=n?.closest(`.topic-nav a[data-nav-bucket-id]`);if(l(),r){r.classList.add(`is-nav-drop-target`),F={bucketKind:r.dataset.navBucketKind,bucketId:r.dataset.navBucketId,targetReportId:``,placeAfter:!1,nav:!0};return}let i=e.closest(`.report-card`),a=n?.closest(`.report-card:not(.report-card-placeholder)`),o=n?.closest(`.group-column`);if(a&&a!==i){let e=a.closest(`.group-column`),n=a.getBoundingClientRect(),r=t.clientY>n.top+n.height/2;a.classList.add(`is-card-drop-target`,r?`is-card-drop-after`:`is-card-drop-before`),u(e?.querySelector(`.group-cards`),a,r),F={bucketKind:e?.dataset.bucketKind||j,bucketId:e?.dataset.bucketId||``,targetReportId:a.dataset.reportId||``,placeAfter:r,nav:!1};return}o&&o.dataset.bucketKind!==`time`&&(o.classList.add(`is-drop-ready`),u(o.querySelector(`.group-cards`),null,!1),F={bucketKind:o.dataset.bucketKind||j,bucketId:o.dataset.bucketId||``,targetReportId:``,placeAfter:!1,nav:!1})},f=()=>{N=``,t=null,n=!1,o(),r?.remove(),r=null,l(),e.closest(`.report-card`)?.classList.remove(`is-dragging`)};e.addEventListener(`pointerdown`,r=>{r.preventDefault(),N=e.dataset.reportDragId,t={x:r.clientX,y:r.clientY},n=!1,e.setPointerCapture?.(r.pointerId),e.closest(`.report-card`)?.classList.add(`is-dragging`)}),e.addEventListener(`pointermove`,e=>{N&&(t&&Math.hypot(e.clientX-t.x,e.clientY-t.y)<7||(n=!0,a=e.clientY,c(e),d(e),i||=requestAnimationFrame(s)))}),e.addEventListener(`pointerup`,e=>{if(!N)return;let t=N;if(!n){P=t,f(),Q(()=>Y(t)),K(`请选择目标${Yn()}`);return}let r=F,i=r?.targetReportId||``,a=r?.bucketId||``,o=r?.bucketKind||j,s=a&&o!==`time`?Jn(t,o,a,i,!!r?.placeAfter):!1;f(),s&&(Q(()=>Y(t)),requestAnimationFrame(()=>{let e=`.group-column[data-bucket-kind="${CSS.escape(o)}"][data-bucket-id="${CSS.escape(a)}"]`,n=document.querySelector(`${e} .report-card[data-report-id="${CSS.escape(t)}"]`)||document.querySelector(`.search-results-cards .report-card[data-report-id="${CSS.escape(t)}"]`);fr(n),n?.classList.add(`is-drop-landed`),window.setTimeout(()=>n?.classList.remove(`is-drop-landed`),900)}),K(o===`featured`?`已加入精选成果`:o===`tag`?`已添加目标标签`:o===`type`?`工作类型已更新`:i?`报告顺序已更新`:`已移入新主题`))}),e.addEventListener(`pointercancel`,f)});let r=document.getElementById(`group-form`);r?.addEventListener(`submit`,e=>{e.preventDefault();let t=new FormData(r).get(`name`)?.trim(),n=new FormData(r).get(`description`)?.trim();if(!t)return;if(L.mode===`edit`){let e=D.groups.find(e=>e.id===L.groupId);if(!e)return;e.name=t.slice(0,60),e.description=n?.slice(0,80)||`自定义工作主题`}else D.groups.push({id:Qn(`group`),name:t.slice(0,60),description:n?.slice(0,80)||`自定义工作主题`,accent:[`blue`,`violet`,`amber`,`green`][D.groups.length%4],position:D.groups.length}),j=`topic`,localStorage.setItem(Gt,j);H();let i=L.mode===`edit`?`工作主题已更新`:`工作主题已创建，可直接拖入报告`;q(),K(i)});let i=document.getElementById(`report-form`),a=i?.elements.tags,o=e=>{let t=G(a?.value).includes(e.dataset.reportTag);e.classList.toggle(`selected`,t),e.setAttribute(`aria-pressed`,String(t))},s=e=>{e.addEventListener(`click`,()=>{let t=G(a.value),n=e.dataset.reportTag;a.value=t.includes(n)?t.filter(e=>e!==n).join(`、`):[...t,n].slice(0,8).join(`、`),o(e)})};i?.querySelectorAll(`[data-report-tag]`).forEach(s);let c=i?.querySelector(`[data-add-report-tag]`),l=i?.querySelector(`.new-report-tag-row`),u=i?.querySelector(`[data-new-report-tag]`),d=()=>{l.hidden=!1,u.focus()},f=()=>{let[e]=G(u.value);if(!e)return;let t=G(a.value);if(!t.includes(e)&&t.length>=8){K(`最多选择 8 个标签`);return}a.value=[...new Set([...t,e])].slice(0,8).join(`、`);let n=[...i.querySelectorAll(`[data-report-tag]`)].find(t=>t.dataset.reportTag===e);n||(n=document.createElement(`button`),n.type=`button`,n.dataset.reportTag=e,n.textContent=e,c.before(n),s(n)),o(n),u.value=``,l.hidden=!0,c.focus()};c?.addEventListener(`click`,d),i?.querySelector(`[data-confirm-report-tag]`)?.addEventListener(`click`,f),u?.addEventListener(`keydown`,e=>{e.key===`Enter`?(e.preventDefault(),f()):e.key===`Escape`&&(e.preventDefault(),u.value=``,l.hidden=!0,c.focus())}),i?.addEventListener(`submit`,async e=>{e.preventDefault();let t=i.elements.url.value.trim(),n=L.mode===`edit`?L.reportId:``,r=n?D.reports.find(e=>e.id===n):null,a=!!(r&&!r.url&&!t);if(!nr(t)&&!a)return;let o=i.querySelector(`button[type="submit"]`),s=i.querySelector(`.field-hint`);if(o.disabled=!0,o.innerHTML=`<span class="mini-spinner"></span>`,a){let e=i.elements.title.value.trim()||r.title,t=i.elements.groupId.value,n=i.elements.workType.value,a=G(i.elements.tags.value).filter(e=>![`HTML`,`手动保存`,`生产`].includes(e));Object.assign(r,{title:e,groupId:t,workType:n,tags:a}),U(r),H(),q(),K(`报告已保存`);return}let c=An({material:t,files:[],url:t,excludeId:n});if(c){o.disabled=!1,o.textContent=`Save`,s.textContent=`成果库已有“${c.title}”，未重复保存`,K(`成果库已有“${c.title}”，未重复保存`);return}let l=await In({material:t,files:[],url:t},e=>{s.textContent=e});if(!l.allowed){o.disabled=!1,o.textContent=`Save`,s.textContent=l.reason,K(l.reason);return}let u=i.elements.title.value.trim()||l.metadata.title,d=i.elements.groupId.value,f=i.elements.workType.value,p=G(i.elements.tags.value),m={title:u||tr(t),url:t,groupId:d,workType:f,source:`手动添加`,access:l.access,detectedDescription:l.metadata.description,manualSaved:!0,isProduction:l.access===`production`,isPersonal:jn(t),isHtml:l.isHtml,loginProvider:l.loginProvider},h=L.mode===`edit`?p:[...new Set([...tn(m,f),...p])].slice(0,8);if(L.mode===`edit`){let e=D.reports.find(e=>e.id===L.reportId);Object.assign(e,m,{tags:h}),U(e)}else{let e=new Date().toISOString(),t={id:Qn(`report`),groupId:d,...m,pinned:!1,position:D.reports.filter(e=>e.groupId===d).length,createdAt:e,modifiedAt:e,archived:!1,archivedAt:``,tags:h};D.reports.push(t)}H(),q(),K(`报告已保存`)});let p=A&&D.reports.find(e=>e.id===A);p&&bt(p)}function Yr(){yn(),$()}Yr(document.getElementById(`app`));export{f as a,c as i,s as n,d as o,o as r,u as s,x as t};