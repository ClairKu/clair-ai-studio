(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))s(i);new MutationObserver(i=>{for(const r of i)if(r.type==="childList")for(const n of r.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&s(n)}).observe(document,{childList:!0,subtree:!0});function e(i){const r={};return i.integrity&&(r.integrity=i.integrity),i.referrerPolicy&&(r.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?r.credentials="include":i.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function s(i){if(i.ep)return;i.ep=!0;const r=e(i);fetch(i.href,r)}})();const ot="clair-ai-studio-tasks-v1",W=[{id:"auto",name:"智能识别",summon:"自动派单",icon:"✦",hint:"让 AI 判断最适合的任务"},{id:"requirement",name:"需求评审",summon:"需求专家",icon:"需",hint:"价值、范围、规则、验收"},{id:"solution",name:"方案评审",summon:"方案专家",icon:"案",hint:"体验、逻辑、可行性、风险"},{id:"decision",name:"决策推演",summon:"决策顾问",icon:"决",hint:"选项、证据、取舍、止损"},{id:"agreement",name:"协议审查",summon:"协议专家",icon:"协",hint:"权责、数据、责任、退出"},{id:"career",name:"履历评估",summon:"履历顾问",icon:"历",hint:"事实、能力、匹配、核验"}];let T=ft(),v={skillId:"auto",goal:"",material:"",files:[]},L="",P="compose";function ft(){try{const t=JSON.parse(localStorage.getItem(ot));return Array.isArray(t)?t:[]}catch{return[]}}function j(){localStorage.setItem(ot,JSON.stringify(T))}function ct(){var t;return((t=crypto.randomUUID)==null?void 0:t.call(crypto))||`${Date.now()}-${Math.random()}`}function Q(t){return W.find(a=>a.id===t)||W[0]}function ht(t){var s;const a=t.toLowerCase();return((s=[["agreement",["协议","合同","条款","保密","签署"]],["career",["简历","履历","候选人","晋升","岗位","面试"]],["decision",["决策","选型","取舍","是否推进","选择"]],["requirement",["需求","prd","用户故事","验收","原型"]],["solution",["方案","流程","架构","设计","上线"]]].find(([,i])=>i.some(r=>a.includes(r))))==null?void 0:s[0])||"solution"}function lt(t){return new Intl.DateTimeFormat("zh-CN",{month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"}).format(new Date(t))}function vt(t,a){const e=t.files.length?t.files.map(i=>`${i.name}（${i.sizeLabel}）`).join("、"):"无附件",s=t.material.trim().length;return`
    <h2>材料已收齐</h2>
    ...
    <h3>下一步</h3>
    <p>任务已保存。安全 AI 服务接通后会在这里生成完整初稿；在此之前可继续补充材料，或直接粘贴已完成的分析结果。</p>`}function bt(t,a){return`${t.trim().split(/\n/)[0].replace(/[。；;！!？?]+$/,"").slice(0,42)||"未命名任务"}｜${a}`}function yt(t){return t<1024?`${t} B`:t<1024*1024?`${Math.ceil(t/1024)} KB`:`${(t/1024/1024).toFixed(1)} MB`}async function K(t){const a=[...t].slice(0,20);return Promise.all(a.map(async e=>{const s=e.type.startsWith("text/")||/\.(md|txt|csv|json|html|xml)$/i.test(e.name);let i="";if(s&&e.size<=1024*1024)try{i=(await e.text()).slice(0,12e3)}catch{i=""}return{id:ct(),name:e.name,type:e.type||"文件",size:e.size,sizeLabel:yt(e.size),excerpt:i}}))}function kt(t){return W.map(a=>`
    <button class="expert-choice ${v.skillId===a.id?"selected":""}" type="button"
      data-task-action="choose-skill" data-skill-id="${a.id}"
      title="${t(a.hint)}" aria-pressed="${v.skillId===a.id}">
      <span>${t(a.icon)}</span>
      <strong>@${t(a.summon)}</strong>
    </button>`).join("")}function $t(t){return v.files.length?`<div class="attachment-list">${v.files.map(a=>`
    ...
    </span>`).join("")}</div>`:""}function It(t){const a=T.filter(e=>e.status!=="confirmed").sort((e,s)=>new Date(s.updatedAt)-new Date(e.updatedAt));return a.length?`
      ...
      <div class="progress-task-list">
        ${a.slice(0,3).map(e=>`
          <button type="button" data-task-action="open-task" data-task-id="${e.id}">
            ...
            <div><strong>${t(e.title)}</strong><small>${e.status==="review"?"待确认":"处理中"} · ${lt(e.updatedAt)}</small></div>
            <i>→</i>
          </button>`).join("")}
      </div>
    </div>`:""}function wt(t){if(L){const a=T.find(e=>e.id===L);if(a)return At(a,t);L=""}return`
    <section class="inline-task-launcher prompt-launcher" aria-label="发起任务">
      <form class="prompt-composer" id="task-composer">
        <div class="prompt-main">
          ...
          <textarea id="task-goal" rows="3" placeholder="描述你想完成的事，或把文档、图片直接拖进来……" aria-label="任务描述">${t(v.goal)}</textarea>
        </div>
        ${$t(t)}
        <div class="prompt-footer">
          ...
          <div class="expert-summoner" aria-label="召唤专家">
            ...
            <div class="expert-strip">${kt(t)}</div>
          </div>
      ...
      ${It(t)}
    </section>`}function At(t,a){var s;const e=t.status==="confirmed";return`
    <section class="task-center task-detail inline-task-detail">
      ...
      <div class="task-review-layout">
        ...
        <main class="task-result-editor">
          <div class="result-editor-heading"><div><span class="section-kicker">WORKING RESULT</span><h2>${e?"最终成果":"工作草稿"}</h2></div><small>最后更新 ${lt(t.updatedAt)}</small></div>
    ...
    </section>`}function St(t){const a=T.filter(e=>e.status==="confirmed").sort((e,s)=>new Date(s.confirmedAt)-new Date(e.confirmedAt));return a.length?`
...
当前安全 AI 服务尚未接通，任务已保存，可继续补充或粘贴分析结果。`,T.push(p),j(),L=p.id,v={skillId:"auto",goal:"",material:"",files:[]},t(),e(`已创建任务，并匹配“${l.name}”`)});const r=document.getElementById("task-files");r==null||r.addEventListener("change",async d=>{C(),v.files.push(...await K(d.target.files)),t(),e(`已加入 ${d.target.files.length} 个文件`)});const n=document.getElementById("material-drop")||document.querySelector(".prompt-composer");n==null||n.addEventListener("dragover",d=>{d.preventDefault(),n.classList.add("drag-over")}),n==null||n.addEventListener("dragleave",()=>n.classList.remove("drag-over")),n==null||n.addEventListener("drop",async d=>{d.preventDefault(),n.classList.remove("drag-over"),C();const o=d.dataTransfer.files;v.files.push(...await K(o)),t(),e(`已加入 ${o.length} 个文件`)});const c=document.getElementById("task-goal");c==null||c.addEventListener("input",()=>{v.goal=c.value}),c==null||c.addEventListener("paste",async d=>{var b;const o=[...((b=d.clipboardData)==null?void 0:b.items)||[]].filter(A=>A.kind==="file"&&A.type.startsWith("image/")).map(A=>A.getAsFile()).filter(Boolean);if(!o.length)return;d.preventDefault();const l=d.clipboardData.getData("text/plain"),m=c.selectionStart??c.value.length,p=c.selectionEnd??m;v.goal=`${c.value.slice(0,m)}${l}${c.value.slice(p)}`,v.files.push(...await K(o)),t(),e(`已从剪贴板加入 ${o.length} 张图片`)})}function C(){const t=document.getElementById("task-material"),a=document.getElementById("task-goal"),e=document.getElementById("task-quick-goal");t&&(v.material=t.value),a&&(v.goal=a.value),e&&(v.goal=e.value)}const H="clair-service-report-workbench-v1",tt="clair-service-report-workbench-access",X="clair-service-report-workbench-view",B=6,G=[{id:"requirement-review",name:"需求评审"},{id:"reporting",name:"汇报材料"},{id:"competitive-research",name:"竞品调研"},{id:"product-planning",name:"产品规划"},{id:"data-analysis",name:"数据分析"},{id:"investment-research",name:"投研分析"},{id:"governance-review",name:"治理审查"},{id:"product-demo",name:"原型 Demo"}],U=["本体","飞书","调研","AI 小顾","AI 工作台","AI 开放平台","且慢","OAP","MCP","Skills","投顾服务","投研","数据分析","需求评审","经营汇报","知识治理"],Z={version:B,groups:[{id:"inbox",name:"待整理",description:"临时入口，等待归档",accent:"slate",position:0},{id:"xiaogu",name:"AI 小顾与投顾服务",description:"AI 小顾、顾问服务与客户体验",accent:"green",position:1},{id:"ai-workbench",name:"AI 工作台与生产力",description:"个人工作台、评审工具与 AI 生产力",accent:"blue",position:2},{id:"ai-platform",name:"AI 开放平台",description:"OAP、MCP、Skills、Agents 与治理",accent:"violet",position:3},{id:"product-planning",name:"且慢产品与体验",description:"产品规划、体验分析与交互方案",accent:"blue",position:4},{id:"research",name:"投研与策略研究",description:"基金、策略与资产配置研究",accent:"amber",position:5},{id:"reporting",name:"经营分析与汇报",description:"业务分析、周报与管理汇报",accent:"blue",position:6},{id:"knowledge",name:"知识治理与组织协同",description:"本体、飞书、SOUL 与知识资产",accent:"slate",position:7}],reports:[{id:"seed-mcp-benchmark",groupId:"ai-platform",title:"三家金融 MCP / Skills 服务最完整对比｜010350 同题实测",url:"https://clairku.github.io/skills/reports/eastmoney-mcp-skills-benchmark-2026-07-28/",pinned:!0,position:0,createdAt:"2026-07-28T10:00:00.000Z",source:"近月新增",access:"production"},{id:"seed-fund-report",groupId:"research",title:"东方财富妙想版｜010350 基金深度诊断",url:"https://clairku.github.io/skills/reports/eastmoney-fund-report-010350-2026-07-28/",pinned:!1,position:1,createdAt:"2026-07-28T09:30:00.000Z",source:"近月新增",access:"production"},{id:"storage-big-three-fund-screening",groupId:"research",title:"存储三巨头基金筛选｜境内 QDII 与港股通",url:"https://clairku.github.io/skills/reports/storage-big-three-fund-screening-2026-07-29/",pinned:!0,position:0,createdAt:"2026-07-29T04:49:24.000Z",source:"盈米 Skills / MCP",access:"production"},{id:"seed-agreement",groupId:"ai-platform",title:"盈米 MCP 协议审查台",url:"https://clairku.github.io/yingmi-mcp-agreement-review/",pinned:!0,position:0,createdAt:"2026-07-28T08:50:00.000Z",source:"近月新增",access:"production"},{id:"seed-xiaogu",groupId:"xiaogu",title:"且慢小顾介绍｜AI 投资助手",url:"https://clairku.github.io/qieman-mcp-subscription-prototype/xiaogu-intro/",pinned:!1,position:1,createdAt:"2026-07-27T07:40:00.000Z",source:"近月新增",access:"production"},{id:"seed-strategy",groupId:"research",title:"公募策略多指标双轴探索器｜四笔钱",url:"https://clairku.github.io/qieman-product-research-library/pages/product-research/four-money-metric-axis-explorer-2026-07-27.html",pinned:!1,position:0,createdAt:"2026-07-27T07:20:00.000Z",source:"近月新增",access:"production"},{id:"seed-ecosystem",groupId:"ai-platform",title:"盈米 AI 实验室｜服务组件编排 Demo",url:"https://clairku.github.io/qieman-product-research-library/pages/ai-lab-demo/",pinned:!1,position:2,createdAt:"2026-07-26T14:40:00.000Z",source:"近月新增",access:"production"},{id:"qieman-library-index",groupId:"knowledge",title:"且慢产品研究页面库｜原始总入口",url:"https://clairku.github.io/qieman-product-research-library/",pinned:!0,position:0,createdAt:"2026-07-26T09:23:00.000Z",source:"研究库",access:"production"},{id:"qieman-advisor-inventory",groupId:"product-planning",title:"且慢投顾模块现况盘点报告",url:"https://clairku.github.io/qieman-product-research-library/pages/advisor-revamp/01-投顾模块现况盘点报告.html",pinned:!1,position:0,createdAt:"2026-07-24T09:00:00.000Z",source:"研究库",access:"production"},{id:"qieman-advisor-direction-research",groupId:"product-planning",title:"且慢 APP 投顾模块｜现况盘点与改版方向",url:"https://clairku.github.io/qieman-product-research-library/pages/product-research/且慢APP投顾模块现况与改版方向-2026-07-23.html",pinned:!1,position:1,createdAt:"2026-07-23T09:00:00.000Z",source:"研究库",access:"production"},{id:"qieman-advisor-v09",groupId:"product-planning",title:"且慢投顾页改版｜方向与方案设计 V0.9",url:"https://clairku.github.io/qieman-product-research-library/pages/advisor-revamp/02-改版方向与方案设计.html",pinned:!0,position:2,createdAt:"2026-07-24T09:10:00.000Z",source:"研究库",access:"production"},{id:"qieman-network-research",groupId:"product-planning",title:"且慢产品现况网络调研报告",url:"https://clairku.github.io/qieman-product-research-library/pages/advisor-revamp/00-且慢产品现况网络调研报告.html",pinned:!1,position:3,createdAt:"2026-07-24T09:20:00.000Z",source:"研究库",access:"production"},{id:"qieman-advisor-final",groupId:"product-planning",title:"且慢投顾页改版｜推荐方案定稿与备选",url:"https://clairku.github.io/qieman-product-research-library/pages/advisor-revamp/03-推荐方案定稿与备选.html",pinned:!1,position:4,createdAt:"2026-07-24T09:30:00.000Z",source:"研究库",access:"production"},{id:"qieman-advisor-demo",groupId:"product-planning",title:"且慢投顾页改版交互 Demo｜方案 B",url:"https://clairku.github.io/qieman-product-research-library/pages/advisor-revamp/03-投顾页交互DEMO.html",pinned:!1,position:5,createdAt:"2026-07-24T09:40:00.000Z",source:"研究库",access:"production"},{id:"qieman-advisor-plan",groupId:"product-planning",title:"且慢投顾页改版｜产品规划与计划书",url:"https://clairku.github.io/qieman-product-research-library/pages/advisor-revamp/04-产品规划与计划书.html",pinned:!1,position:6,createdAt:"2026-07-24T09:50:00.000Z",source:"研究库",access:"production"},{id:"qieman-home-entry-analysis",groupId:"xiaogu",title:"且慢 App 首页金刚位分析报告｜修正版",url:"https://ontology.yingmi-inc.com/a/e5qhqu6j5p?k=1FyXYxD8FOybCMdrKX71uT8J5KtbJ0s8",pinned:!1,position:2,createdAt:"2026-07-23T10:00:00.000Z",source:"研究库",access:"org"},{id:"qieman-advisor-click-analysis",groupId:"product-planning",title:"且慢投顾页点击与转化分析",url:"https://clairku.github.io/qieman-product-research-library/pages/product-research/且慢投顾页点击与转化分析-2026-07-24.html",pinned:!1,position:7,createdAt:"2026-07-24T10:00:00.000Z",source:"研究库",access:"production"},{id:"qieman-app-map",groupId:"xiaogu",title:"且慢 APP 完整功能全景",url:"https://clairku.github.io/qieman-product-research-library/pages/qieman-app-panorama/map.html",pinned:!1,position:3,createdAt:"2026-07-24T10:10:00.000Z",source:"研究库",access:"production"},{id:"qieman-app-deep-analysis",groupId:"xiaogu",title:"且慢 App 深度产品分析报告",url:"https://ontology.yingmi-inc.com/a/634fdzap43?k=Uism1cxnNVIql-cZAw0OwQvbcWib_ghN",pinned:!1,position:4,createdAt:"2026-07-24T10:20:00.000Z",source:"研究库",access:"org"},{id:"qieman-app-usage",groupId:"xiaogu",title:"且慢 APP 使用情况与证据",url:"https://clairku.github.io/qieman-product-research-library/pages/qieman-app-panorama/usage.html",pinned:!1,position:5,createdAt:"2026-07-24T10:30:00.000Z",source:"研究库",access:"production"},{id:"qieman-app-roadmap",groupId:"xiaogu",title:"且慢 APP 深度产品判断与路线图",url:"https://clairku.github.io/qieman-product-research-library/pages/qieman-app-panorama/roadmap.html",pinned:!1,position:6,createdAt:"2026-07-24T10:40:00.000Z",source:"研究库",access:"production"},{id:"qieman-ai-native",groupId:"xiaogu",title:"且慢 APP AI 原生转型三案",url:"https://clairku.github.io/qieman-product-research-library/pages/product-research/且慢APP-AI原生转型三案-2026-07-24.html",pinned:!0,position:7,createdAt:"2026-07-24T10:50:00.000Z",source:"研究库",access:"production"},{id:"oap-progress-roadmap",groupId:"ai-platform",title:"OAP 进展与规划汇报",url:"https://clairku.github.io/qieman-product-research-library/pages/oap/oap-progress-and-roadmap-2026-07-24.html",pinned:!1,position:3,createdAt:"2026-07-24T11:00:00.000Z",source:"研究库",access:"production"},{id:"oap-metrics-trend",groupId:"ai-platform",title:"盈米 AI 开放平台｜上线以来运营趋势",url:"https://clairku.github.io/qieman-product-research-library/pages/oap/oap-metrics-trend-2026-07-28.html",pinned:!0,position:4,createdAt:"2026-07-28T10:11:00.000Z",source:"近月新增",access:"production"},{id:"oap-reporting-framework",groupId:"ai-platform",title:"OAP 汇报框架｜动因、成果、复盘与规划",url:"https://clairku.github.io/qieman-product-research-library/pages/oap/oap-reporting-framework-2026-07-28.html",pinned:!1,position:5,createdAt:"2026-07-28T08:30:00.000Z",source:"近月新增",access:"production"},{id:"oap-traffic-analysis",groupId:"ai-platform",title:"盈米 AI 开放平台｜全站访问与点击分析",url:"https://clairku.github.io/qieman-product-research-library/pages/oap/oap-service-site-traffic-analysis-2026-07-28.html",pinned:!0,position:6,createdAt:"2026-07-28T12:10:00.000Z",source:"近月新增",access:"production"},{id:"eastmoney-platform",groupId:"ai-platform",title:"东方财富 AI Skills 平台深度竞品分析",url:"https://clairku.github.io/skills/reports/eastmoney-ai-skills-platform-analysis-2026-07-28/",pinned:!1,position:7,createdAt:"2026-07-28T08:57:00.000Z",source:"近月新增",access:"production"},{id:"qieman-strategy-explorer",groupId:"research",title:"四笔钱策略检视台｜筛选、对比与全指标分析",url:"https://clairku.github.io/qieman-strategy-explorer/",pinned:!1,position:2,createdAt:"2026-07-27T16:43:00.000Z",source:"近月新增",access:"production"},{id:"financial-planning-review",groupId:"research",title:"财务规划报告｜现金流与目标可达性改稿建议",url:"https://clairku.github.io/qieman-product-research-library/pages/financial-planning-report-review/",pinned:!1,position:3,createdAt:"2026-07-27T11:27:00.000Z",source:"近月新增",access:"production"},{id:"investment-behavior-report",groupId:"research",title:"投资行为画像｜行为金融洞察报告（脱敏版）",url:"https://clairku.github.io/my-investment-behavior-report/",pinned:!1,position:4,createdAt:"2026-07-16T14:56:00.000Z",source:"近月新增",access:"production"},{id:"product-review-workbench",groupId:"product-planning",title:"产品需求评审工作台",url:"https://clairku.github.io/product-review-workbench/",pinned:!0,position:8,createdAt:"2026-07-08T06:43:00.000Z",source:"近月新增",access:"production"},{id:"community-ai-review",groupId:"product-planning",title:"社区 AI 运营方案｜需求评审报告",url:"https://clairku.github.io/product-review-workbench/reviews/community-ai-operations-2026-07-28/",pinned:!1,position:9,createdAt:"2026-07-28T08:20:00.000Z",source:"近月新增",access:"production"},{id:"jinzhenzi-review",groupId:"reporting",title:"金榛子奖申报材料审查报告",url:"https://clairku.github.io/jinzhenzi-submission-review/",pinned:!1,position:0,createdAt:"2026-07-28T11:01:00.000Z",source:"近月新增",access:"production"},{id:"jinzhenzi-history",groupId:"reporting",title:"金榛子奖历届获奖项目档案",url:"https://clairku.github.io/jinzhenzi-submission-review/history.html",pinned:!1,position:1,createdAt:"2026-07-28T11:20:00.000Z",source:"近月新增",access:"production"},{id:"xiaogu-user-needs",groupId:"xiaogu",title:"小顾用户需求分析与关键钩子工具方案",url:"https://clairku.github.io/xiaogu-user-needs-report/",pinned:!1,position:8,createdAt:"2026-07-16T09:58:00.000Z",source:"近月新增",access:"production"},{id:"qieman-ai-advisor-ecosystem",groupId:"xiaogu",title:"且慢小顾 × AI 实验室 × 开放平台｜生态闭环 Demo",url:"https://qieman-ai-advisor-ecosystem.clairku.chatgpt.site",pinned:!0,position:9,createdAt:"2026-07-26T15:05:00.000Z",source:"近月新增",access:"account"},{id:"oap-h2-plan",groupId:"reporting",title:"2026 下半年 AI 开放平台目标计划与里程碑",url:"https://yingmi.feishu.cn/docx/QCTAd6QDTo6iKVxdA3LceZ4rnrf",pinned:!1,position:2,createdAt:"2026-07-26T09:00:00.000Z",source:"研究库",access:"org"},{id:"ai-weekly-2026-07-13",groupId:"reporting",title:"AI 项目周报｜2026-07-13",url:"https://clairku.github.io/clair-ai-studio/reports/ai-weekly-2026-07-13/",pinned:!1,position:3,createdAt:"2026-07-13T02:20:23.000Z",source:"近月补录",access:"production"},{id:"pension-business-analysis",groupId:"reporting",title:"盈米及且慢养老金业务分析",url:"https://clairku.github.io/clair-ai-studio/reports/pension-business-analysis-2026-07/",pinned:!1,position:4,createdAt:"2026-07-13T08:47:33.000Z",source:"近月补录",access:"production"},{id:"advisor-2-business-onboarding",groupId:"reporting",title:"盈米投顾 2.0｜新负责人业务入职报告",url:"https://clairku.github.io/clair-ai-studio/reports/advisor-2-business-onboarding-2026-07/",pinned:!1,position:5,createdAt:"2026-07-13T09:12:10.000Z",source:"近月补录",access:"production"},{id:"schwab-ria-benchmark",groupId:"reporting",title:"嘉信 2026 RIA 基准调研｜对盈米与且慢的启示",url:"https://clairku.github.io/clair-ai-studio/reports/schwab-ria-benchmark-2026/",pinned:!1,position:6,createdAt:"2026-07-22T02:40:53.000Z",source:"近月补录",access:"production"},{id:"skill-audit-2026-07-16",groupId:"ai-workbench",title:"25 项 Skills 可用性与一致性审查",url:"https://clairku.github.io/clair-ai-studio/reports/skill-audit-2026-07-16/",pinned:!1,position:0,createdAt:"2026-07-16T03:30:04.000Z",source:"近月补录",access:"production"},{id:"yingmi-ai-capability-system",groupId:"ai-platform",title:"盈米 AI 能力体系专业报告｜2026.07",url:"https://clairku.github.io/clair-ai-studio/reports/yingmi-ai-capability-system-2026-07/",pinned:!1,position:8,createdAt:"2026-07-13T09:43:42.000Z",source:"近月补录",access:"production"}]},J={"seed-mcp-benchmark":"competitive-research","seed-fund-report":"investment-research","storage-big-three-fund-screening":"investment-research","seed-agreement":"governance-review","seed-xiaogu":"product-planning","seed-strategy":"investment-research","seed-ecosystem":"product-demo","qieman-library-index":"governance-review","qieman-advisor-inventory":"product-planning","qieman-advisor-direction-research":"product-planning","qieman-advisor-v09":"product-planning","qieman-network-research":"competitive-research","qieman-advisor-final":"product-planning","qieman-advisor-demo":"product-demo","qieman-advisor-plan":"product-planning","qieman-home-entry-analysis":"data-analysis","qieman-advisor-click-analysis":"data-analysis","qieman-app-map":"product-planning","qieman-app-deep-analysis":"data-analysis","qieman-app-usage":"data-analysis","qieman-app-roadmap":"product-planning","qieman-ai-native":"product-planning","oap-progress-roadmap":"reporting","oap-metrics-trend":"data-analysis","oap-reporting-framework":"reporting","oap-traffic-analysis":"data-analysis","eastmoney-platform":"competitive-research","qieman-strategy-explorer":"investment-research","financial-planning-review":"requirement-review","investment-behavior-report":"data-analysis","product-review-workbench":"product-demo","community-ai-review":"requirement-review","jinzhenzi-review":"governance-review","jinzhenzi-history":"competitive-research","xiaogu-user-needs":"product-planning","qieman-ai-advisor-ecosystem":"product-demo","oap-h2-plan":"reporting","ai-weekly-2026-07-13":"reporting","pension-business-analysis":"reporting","advisor-2-business-onboarding":"reporting","schwab-ria-benchmark":"competitive-research","skill-audit-2026-07-16":"governance-review","yingmi-ai-capability-system":"reporting"},dt={"qieman-home-entry-analysis":"product-planning","qieman-app-map":"product-planning","qieman-app-deep-analysis":"product-planning","qieman-app-usage":"product-planning","qieman-app-roadmap":"product-planning","financial-planning-review":"xiaogu","investment-behavior-report":"xiaogu","product-review-workbench":"ai-workbench","community-ai-review":"ai-workbench","qieman-ai-advisor-ecosystem":"ai-platform","oap-h2-plan":"ai-platform"};function et(t){const a=`${t.title||""} ${t.source||""}`;return/需求评审|评审工作台/.test(a)?"requirement-review":/竞品|对比|调研|研究/.test(a)?"competitive-research":/周报|汇报|进展|规划|里程碑|业务分析/.test(a)?"reporting":/数据|趋势|点击|转化|画像|使用/.test(a)?"data-analysis":/基金|策略|投研|资产配置/.test(a)?"investment-research":/审查|治理|知识/.test(a)?"governance-review":/Demo|工作台|原型/.test(a)?"product-demo":"product-planning"}function it(t,a=et(t)){const e=`${t.id||""} ${t.groupId||""} ${t.title||""} ${t.url||""}`,s=[],i=r=>{s.includes(r)||s.push(r)};return/ontology\.yingmi-inc\.com|本体/.test(e)&&i("本体"),/feishu\.cn|飞书|community-ai-review|oap-h2-plan/.test(e)&&i("飞书"),(a==="competitive-research"||/调研|研究|盘点/.test(e))&&i("调研"),(/xiaogu|小顾|财务规划|投资行为/.test(e)||t.groupId==="xiaogu")&&i("AI 小顾"),(/workbench|工作台|skill-audit/.test(e)||t.groupId==="ai-workbench")&&i("AI 工作台"),(/ai-platform|开放平台|OAP|MCP|Skills|能力体系/.test(e)||t.groupId==="ai-platform")&&i("AI 开放平台"),/且慢|qieman/.test(e)&&i("且慢"),/投顾|advisor|财务规划/.test(e)&&i("投顾服务"),/OAP|oap-/.test(e)&&i("OAP"),/MCP|mcp-/.test(e)&&i("MCP"),/Skills|skill-/.test(e)&&i("Skills"),(a==="investment-research"||t.groupId==="research")&&i("投研"),a==="data-analysis"&&i("数据分析"),a==="requirement-review"&&i("需求评审"),a==="reporting"&&i("经营汇报"),(a==="governance-review"||t.groupId==="knowledge")&&i("知识治理"),s.slice(0,5)}Z.reports=Z.reports.map(t=>{const a=dt[t.id]||t.groupId,e=J[t.id]||et(t),s={...t,groupId:a,workType:e};return{...s,tags:it(s,e)}});let f=Tt(),w="",O="",N=!1,$=["topic","type","tag"].includes(localStorage.getItem(X))?localStorage.getItem(X):"topic",x="",q="",D="",y=null,st=0;function pt(t){return JSON.parse(JSON.stringify(t))}function _(t=""){try{const a=new URL(t);a.hash="",a.search="";const e=decodeURI(a.pathname).replace(/\/index\.html$/,"/").replace(/\/+$/,"/");return`${a.origin}${e}`}catch{return String(t).trim().replace(/\/+$/,"/")}}function Tt(){try{const t=JSON.parse(localStorage.getItem(H));if(Array.isArray(t==null?void 0:t.groups)&&Array.isArray(t==null?void 0:t.reports))return Et(t)}catch{}return pt(Z)}function Et(t){const a=pt(Z),e=new Set(a.groups.map(u=>u.id)),s=new Set(["inbox","today","product","research"]),i=new Map(t.groups.map(u=>[u.id,u])),r=a.groups.map(u=>{const k=i.get(u.id);return!k||t.version<B?u:{...u,name:k.name||u.name,description:k.description||u.description,position:Number.isFinite(k.position)?k.position:u.position}});t.groups.filter(u=>!e.has(u.id)&&!s.has(u.id)).forEach((u,k)=>{r.push({...u,description:u.description||"自定义工作分组",position:Number.isFinite(u.position)?u.position:Z.groups.length+k})});const n=r.filter((u,k,I)=>I.findIndex(F=>F.id===u.id)===k);n.sort((u,k)=>(u.position||0)-(k.position||0));const c={"seed-mcp-benchmark":"ai-platform","seed-fund-report":"research","seed-agreement":"ai-platform","seed-xiaogu":"xiaogu","seed-strategy":"research","seed-ecosystem":"ai-platform","storage-big-three-fund-screening":"research"},d={inbox:"inbox",today:"product-planning",product:"xiaogu",research:"research"},o=t.reports.map(u=>({...u,groupId:dt[u.id]||c[u.id]||d[u.groupId]||u.groupId||"inbox",workType:u.workType||J[u.id]||et(u),tags:Array.isArray(u.tags)&&u.tags.length?u.tags:it(u,u.workType||J[u.id])})),l=new Map(o.map(u=>[u.id,u])),m=new Map(o.map(u=>[_(u.url),u])),p=new Set,b=a.reports.map(u=>{const k=_(u.url);p.add(k);const I=l.get(u.id)||m.get(k);return I?{...u,title:I.title||u.title,groupId:t.version>=B&&n.some(F=>F.id===I.groupId)?I.groupId:u.groupId,workType:t.version>=B&&I.workType?I.workType:u.workType,tags:t.version>=B&&Array.isArray(I.tags)&&I.tags.length?I.tags:u.tags,pinned:!!I.pinned,position:Number.isFinite(I.position)?I.position:u.position,archived:!!I.archived,archivedAt:I.archivedAt||""}:u});o.forEach(u=>{const k=_(u.url);p.has(k)||(p.add(k),b.push(u))});const A={version:B,groups:n,reports:b};return localStorage.setItem(H,JSON.stringify(A)),A}function E(){f.version=B,f.groups.forEach((t,a)=>{t.position=a}),localStorage.setItem(H,JSON.stringify(f))}function Y(t,a){const e=f.groups.findIndex(r=>r.id===t),s=f.groups.findIndex(r=>r.id===a);if(e<0||s<0||e===s)return!1;const[i]=f.groups.splice(e,1);return f.groups.splice(s,0,i),E(),!0}function xt(t,a,e=""){const s=f.reports.find(c=>c.id===t);if(!s||s.archived||!f.groups.find(c=>c.id===a))return!1;const r=f.reports.filter(c=>!c.archived&&c.groupId===a&&c.id!==t).sort((c,d)=>(c.position||0)-(d.position||0)),n=e?r.findIndex(c=>c.id===e):r.length;return s.groupId=a,r.splice(n<0?r.length:n,0,s),r.forEach((c,d)=>{c.position=d}),E(),!0}function Lt(t){var a;return((a=G.find(e=>e.id===t))==null?void 0:a.name)||"产品规划"}function Dt(t,a=""){const e=s=>!a||s.toLowerCase().includes(a);if($==="type")return G.map(s=>({id:s.id,name:s.name,kind:"type",accent:"blue",reports:t.filter(i=>i.workType===s.id).sort((i,r)=>+!!r.pinned-+!!i.pinned||new Date(r.createdAt)-new Date(i.createdAt))})).filter(s=>!a||s.reports.length||e(s.name));if($==="tag"){const s=new Set(U);return f.reports.forEach(r=>{(r.tags||[]).forEach(n=>s.add(n))}),[...s].sort((r,n)=>{const c=U.indexOf(r),d=U.indexOf(n);return c>=0||d>=0?(c<0?Number.MAX_SAFE_INTEGER:c)-(d<0?Number.MAX_SAFE_INTEGER:d):r.localeCompare(n,"zh-CN")}).map(r=>({id:r,name:r,kind:"tag",accent:"violet",reports:t.filter(n=>(n.tags||[]).includes(r)).sort((n,c)=>+!!c.pinned-+!!n.pinned||new Date(c.createdAt)-new Date(n.createdAt))})).filter(r=>r.reports.length&&(!a||e(r.name)||r.reports.length))}return f.groups.map(s=>({...s,kind:"topic",reports:t.filter(i=>i.groupId===s.id).sort((i,r)=>(i.position||0)-(r.position||0))})).filter(s=>!a||s.reports.length||e(`${s.name} ${s.description||""}`))}function z(t,a,e,s=""){const i=f.reports.find(r=>r.id===t);return!i||i.archived?!1:a==="topic"?xt(t,e,s):a==="type"?G.some(r=>r.id===e)?(i.workType=e,E(),!0):!1:a==="tag"?(i.tags=Array.isArray(i.tags)?i.tags:[],i.tags.includes(e)||i.tags.push(e),E(),!0):!1}function R(){return $==="type"?"工作类型":$==="tag"?"标签":"主题"}function rt(t){var a;return`${t}-${((a=crypto.randomUUID)==null?void 0:a.call(crypto))||`${Date.now()}-${Math.random()}`}`}function g(t=""){return String(t).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function M(t){try{return new URL(t).hostname.replace(/^www\./,"")}catch{return t}}function ut(t){try{return["http:","https:"].includes(new URL(t).protocol)}catch{return!1}}function V(t=""){return[...new Set(String(t).split(/[、,，;；\n]+/).map(a=>a.trim()).filter(Boolean).map(a=>a.slice(0,20)))].slice(0,8)}function S(t){var e;(e=document.querySelector(".toast"))==null||e.remove();const a=document.createElement("div");a.className="toast",a.setAttribute("role","status"),a.textContent=t,document.body.append(a),clearTimeout(st),st=window.setTimeout(()=>a.remove(),2600)}function gt(t,a=!1){const e=t.access!=="production",s=t.access==="org"?"需组织登录":t.access==="account"?"需账号登录":"生产可访问",r=!e&&Z.reports.some(n=>n.id===t.id)?`<img src="./previews/${g(t.id)}.png" alt="" loading="lazy" decoding="async" />`:`
    ...
    <article class="report-card ${e?"restricted-card":""} ${a?"archived-card":""} ${D===t.id?"is-move-selected":""}" data-report-id="${g(t.id)}">
      <button class="card-main" type="button" data-action="open" data-id="${g(t.id)}" aria-label="打开${g(t.title)}">
        ...
        <span class="report-copy">
          ...
          ${(t.tags||[]).length?`<span class="report-tags">${t.tags.slice(0,3).map(n=>`<span>${g(n)}</span>`).join("")}</span>`:""}
          ${e?`<span class="report-access-note">${g(s)}</span>`:""}
        </span>
      </button>
      ${a?"":`
        <span class="report-drag-handle" role="button" tabindex="0" data-report-drag-id="${g(t.id)}"
          aria-label="拖动《${g(t.title)}》到其他${R()}" title="拖动到其他${R()}">
          <span aria-hidden="true">⠿</span>
        </span>`}
    ...
    </article>`}function at(){var e;if(!y)return"";if(y.type==="tags"){const s=f.reports.find(i=>i.id===y.reportId);return s?`
      <div class="dialog-backdrop">
        <form class="dialog compact-dialog tag-dialog" id="tag-form">
          ...
          <div class="tag-suggestions">
            ${U.map(i=>`<button type="button" class="${(s.tags||[]).includes(i)?"selected":""}" data-tag-suggestion="${g(i)}">${g(i)}</button>`).join("")}
          </div>
      ...
      </div>`:""}if(y.type==="group"){const s=y.mode==="edit"?f.groups.find(i=>i.id===y.groupId):null;return`
      <div class="dialog-backdrop">
      ...
      </div>`}const t=y.mode==="edit"?f.reports.find(s=>s.id===y.reportId):null,a=(t==null?void 0:t.groupId)||y.groupId||((e=f.groups[0])==null?void 0:e.id)||"";return`
    <div class="dialog-backdrop">
      <form class="dialog" id="report-form">
        ...
        <label>工作类型
          <select name="workType">
            ${G.map(s=>`<option value="${g(s.id)}" ${s.id===((t==null?void 0:t.workType)||"product-planning")?"selected":""}>${g(s.name)}</option>`).join("")}
          </select>
        </label>
        <label>关键标签
    ...
    <main class="gate-shell">
    ...
    </main>`}function Ot(t){const a=t.access!=="production",e=t.access==="org"?"组织账号":"站点账号",s=a?`
      <div class="login-handoff-wrap">
        <section class="login-handoff-card" aria-labelledby="login-handoff-title">
          ...
          <p class="login-handoff-domain">${g(M(t.url))}</p>
        </section>
      </div>`:`
    ...
    </header>`}function Ct(){const t=f.reports.filter(e=>e.archived).filter(e=>{if(!w.trim())return!0;const s=w.trim().toLowerCase();return`${e.title} ${e.url} ${e.source||""}`.toLowerCase().includes(s)}).sort((e,s)=>new Date(s.archivedAt||0)-new Date(e.archivedAt||0)),a=f.reports.filter(e=>e.archived).length;return`
    <main class="app-shell archive-shell">
      ...
      <section class="workspace archive-workspace">
        ...
        ${t.length?`
          <section class="archive-results">
            ...
            <div class="archive-grid">${t.map(e=>gt(e,!0)).join("")}</div>
          </section>`:`
      ...
      ${at()}
    </main>`}function Bt(){if(N)return Ct();const t=w.trim().toLowerCase(),a=t.split(/\s+/).filter(Boolean),e=f.reports.filter(o=>!o.archived),s=a.length?e.filter(o=>{const l=`${o.title} ${o.source||""} ${o.access||""} ${Lt(o.workType)} ${(o.tags||[]).join(" ")}`.toLowerCase();return a.every(m=>l.includes(m))}):e,i=f.reports.filter(o=>o.archived).length,r=e.filter(o=>o.access==="production").length,n=e.filter(o=>o.access!=="production").length,c=Dt(s,t).filter(o=>o.reports.length||D),d=$==="type"?"工作类型":$==="tag"?"关键标签":"工作主题";return`
    <main class="app-shell">
        ...
        <div class="results-toolbar unified-results-toolbar">
          ...
          <div class="results-toolbar-side">
            <div class="studio-summary compact-summary" aria-label="成果统计">
              <strong>${e.length}</strong><span>成果</span>
              <i></i>
        ...
        <section class="groups-section">
          ${D?`
            <div class="move-mode-banner" role="status">
              <div><strong>正在整理报告</strong><span>选择目标${R()}的“移到这里”，或直接拖动卡片。</span></div>
          ...
          ${c.length?`
            <div class="library-layout">
              <nav class="topic-nav" aria-label="报告${d}">
                ${c.map((o,l)=>`<a href="#bucket-${l}"><span class="nav-index">${String(l+1).padStart(2,"0")}</span>${g(o.name)}<span>${o.reports.length}</span></a>`).join("")}
              ...
              ${c.map((o,l)=>`
                <section id="bucket-${l}" class="group-column topic-section bucket-${g(o.kind)} accent-${g(o.accent||"blue")}"
                  ...
                  <div class="group-cards">
                    ${o.reports.length?o.reports.map(m=>gt(m)).join(""):o.kind==="topic"?`<button class="empty-topic-drop" type="button" data-action="add-to-group" data-id="${g(o.id)}">
          ...
          <div class="catalog-note">
            <span>${n} 份报告需要组织或账号登录${i?` · ${i} 份已安全归档`:""}</span>
      ...
      ${at()}
    </main>`}function h(){const t=document.getElementById("app");if(sessionStorage.getItem(tt)!=="ok"){t.innerHTML=Pt(),Zt();return}const a=O&&f.reports.find(e=>e.id===O);t.innerHTML=a?Ot(a):Bt(),Nt(),qt({render:h,escapeHtml:g,showToast:S,showResults:()=>{N=!1}})}function Zt(){const t=document.getElementById("login-form");t==null||t.addEventListener("submit",a=>{if(a.preventDefault(),new FormData(t).get("password")!=="2026"){const s=t.querySelector(".form-error");s.hidden=!1,s.textContent="口令不正确，请再试一次";return}sessionStorage.setItem(tt,"ok"),h()})}async function nt(t){var n,c;const a=t.elements.url,e=t.elements.title,s=t.querySelector('[data-action="detect-title"]'),i=t.querySelector(".field-hint"),r=a.value.trim();if(!ut(r))return i.textContent="请输入完整的 http 或 https 网址","";s.disabled=!0,s.innerHTML='<span class="mini-spinner"></span>',i.textContent="正在读取网页标题…";try{const d=`https://api.microlink.io/?url=${encodeURIComponent(r)}`,o=await fetch(d,{headers:{Accept:"application/json"},signal:AbortSignal.timeout(1e4)});if(!o.ok)throw new Error("read failed");const l=await o.json(),m=((c=(n=l==null?void 0:l.data)==null?void 0:n.title)==null?void 0:c.trim())||M(r);return e.value=m.slice(0,180),i.textContent="已识别网页标题",e.value}catch{const d=M(r);return e.value||(e.value=d),i.textContent="网页暂时无法读取，已用域名作为标题，你可以手动修改",e.value}finally{s.disabled=!1,s.textContent="识别标题"}}function Nt(){var s;(s=document.getElementById("search-input"))==null||s.addEventListener("input",i=>{w=i.target.value,h();const r=document.getElementById("search-input");r==null||r.focus(),r==null||r.setSelectionRange(w.length,w.length)}),document.querySelectorAll("[data-action]").forEach(i=>{i.addEventListener("click",async r=>{var d,o;const n=r.currentTarget.dataset.action,c=r.currentTarget.dataset.id;if(n==="open")O=c,h();else if(n==="back")O="",y=null,h();else if(n==="lock")sessionStorage.removeItem(tt),h();else if(n==="clear-search")w="",h();else if(n==="set-view"){if(!["topic","type","tag"].includes(c))return;$=c,D="",localStorage.setItem(X,$),h()}else if(n==="cancel-move")D="",h();else if(n==="move-here"){const l=r.currentTarget.dataset.bucketKind||$;D&&z(D,l,c)&&(D="",h(),S(l==="tag"?"已添加目标标签":`报告已移入目标${R()}`))}else if(n==="show-archive")N=!0,w="",O="",h();else if(n==="show-catalog")N=!1,w="",O="",h();else if(n==="add-report")y={type:"report",mode:"create",groupId:((d=f.groups[1])==null?void 0:d.id)||((o=f.groups[0])==null?void 0:o.id)},h();else if(n==="add-to-group")y={type:"report",mode:"create",groupId:c},h();else if(n==="edit")y={type:"report",mode:"edit",reportId:c},h();else if(n==="edit-tags")y={type:"tags",reportId:c},h();else if(n==="close-modal")y=null,h();else if(n==="detect-title")await nt(r.currentTarget.closest("form"));else if(n==="archive"){const l=f.reports.find(m=>m.id===c);if(!l)return;l.archived=!0,l.archivedAt=new Date().toISOString(),E(),h(),S("已归档，可随时恢复")}else if(n==="restore"){const l=f.reports.find(m=>m.id===c);if(!l)return;l.archived=!1,l.archivedAt="",E(),h(),S("报告已恢复到原主题")}else if(n==="delete"){const l=f.reports.find(m=>m.id===c);l!=null&&l.archived&&confirm(`二次确认：永久删除“${l.title}”？

删除后无法从归档区恢复。`)&&(f.reports=f.reports.filter(m=>m.id!==c),O===c&&(O=""),E(),h(),S("报告已永久删除"))}else if(n==="add-group")y={type:"group",mode:"create"},h();else if(n==="rename-group")f.groups.find(m=>m.id===c)&&(y={type:"group",mode:"edit",groupId:c},h());else if(n==="delete-group"){const l=f.groups.find(m=>m.id===c);l&&confirm(`删除“${l.name}”？其中的报告会移到“待整理”。`)&&(f.reports.forEach(m=>{m.groupId===c&&(m.groupId="inbox")}),f.groups=f.groups.filter(m=>m.id!==c),E(),h(),S("分组已删除，报告已移到待整理"))}})}),document.querySelectorAll(".report-drag-handle").forEach(i=>{let r=null,n=!1;const c=()=>{var d;x="",r=null,n=!1,(d=i.closest(".report-card"))==null||d.classList.remove("is-dragging"),document.querySelectorAll(".report-card, .group-column").forEach(o=>{o.classList.remove("is-card-drop-target","is-drop-ready")})};i.addEventListener("pointerdown",d=>{var o,l;d.preventDefault(),x=i.dataset.reportDragId,q="",r={x:d.clientX,y:d.clientY},n=!1,(o=i.setPointerCapture)==null||o.call(i,d.pointerId),(l=i.closest(".report-card"))==null||l.classList.add("is-dragging")}),i.addEventListener("pointermove",d=>{if(!x||r&&Math.hypot(d.clientX-r.x,d.clientY-r.y)<7)return;n=!0;const o=document.elementFromPoint(d.clientX,d.clientY),l=o==null?void 0:o.closest(".report-card"),m=o==null?void 0:o.closest(".group-column");document.querySelectorAll(".report-card").forEach(p=>{p.classList.toggle("is-card-drop-target",!!(l&&l!==i.closest(".report-card")&&p===l))}),document.querySelectorAll(".group-column").forEach(p=>{p.classList.toggle("is-drop-ready",!!(m&&p===m))})}),i.addEventListener("pointerup",d=>{if(!x)return;const o=x;if(!n){D=o,c(),h(),S(`请选择目标${R()}`);return}const l=document.elementFromPoint(d.clientX,d.clientY),m=l==null?void 0:l.closest(".report-card"),p=l==null?void 0:l.closest(".group-column"),b=(m==null?void 0:m.dataset.reportId)||"",A=(p==null?void 0:p.dataset.bucketId)||"",u=(p==null?void 0:p.dataset.bucketKind)||$,k=b&&b!==o?z(o,u,A,b):A?z(o,u,A):!1;c(),k&&(h(),S(u==="tag"?"已添加目标标签":u==="type"?"工作类型已更新":b?"报告顺序已更新":"已移入新主题"))}),i.addEventListener("pointercancel",c)}),document.querySelectorAll(".group-drag-handle").forEach(i=>{const r=()=>{var n;q="",(n=i.closest(".group-column"))==null||n.classList.remove("is-group-dragging"),document.querySelectorAll(".group-column").forEach(c=>{c.classList.remove("is-group-drop-target","is-drop-ready")})};i.addEventListener("pointerdown",n=>{var c,d;n.preventDefault(),q=i.dataset.groupDragId,x="",(c=i.setPointerCapture)==null||c.call(i,n.pointerId),(d=i.closest(".group-column"))==null||d.classList.add("is-group-dragging")}),i.addEventListener("pointermove",n=>{q&&document.querySelectorAll(".group-column").forEach(c=>{var d;c.classList.toggle("is-group-drop-target",c===((d=document.elementFromPoint(n.clientX,n.clientY))==null?void 0:d.closest(".group-column")))})}),i.addEventListener("pointerup",n=>{var o;if(!q)return;const c=q,d=(o=document.elementFromPoint(n.clientX,n.clientY))==null?void 0:o.closest(".group-column");if(d&&Y(c,d.dataset.groupId)){q="",h(),S("分组顺序已更新");return}r()}),i.addEventListener("pointercancel",r),i.addEventListener("keydown",n=>{var l;if(!["ArrowLeft","ArrowRight"].includes(n.key))return;n.preventDefault();const c=f.groups.findIndex(m=>m.id===i.dataset.groupDragId),d=n.key==="ArrowLeft"?c-1:c+1,o=f.groups[d];!o||!Y(i.dataset.groupDragId,o.id)||(h(),S("分组顺序已更新"),(l=document.querySelector(`[data-group-drag-id="${CSS.escape(i.dataset.groupDragId)}"]`))==null||l.focus())})}),document.querySelectorAll(".group-column").forEach(i=>{i.addEventListener("dragover",r=>{r.preventDefault(),i.classList.add(q?"is-group-drop-target":"is-drop-ready")}),i.addEventListener("dragleave",()=>{i.classList.remove("is-drop-ready","is-group-drop-target")}),i.addEventListener("drop",r=>{if(r.preventDefault(),q){if(i.dataset.bucketKind==="topic"&&Y(q,i.dataset.groupId)){q="",h(),S("分组顺序已更新");return}q="",i.classList.remove("is-group-drop-target");return}const n=f.reports.find(d=>d.id===x),c=i.dataset.bucketKind||$;n&&z(x,c,i.dataset.bucketId)&&(x="",h(),S(c==="tag"?"已添加目标标签":c==="type"?"工作类型已更新":"已移入新主题")),x=""})}),document.querySelectorAll("[data-tag-suggestion]").forEach(i=>{i.addEventListener("click",()=>{const r=document.querySelector('#tag-form input[name="tags"]');if(!r)return;const n=V(r.value),c=i.dataset.tagSuggestion;r.value=n.includes(c)?n.filter(d=>d!==c).join("、"):[...n,c].slice(0,8).join("、"),i.classList.toggle("selected",!n.includes(c)),r.focus()})});const t=document.getElementById("tag-form");t==null||t.addEventListener("submit",i=>{i.preventDefault();const r=f.reports.find(n=>n.id===y.reportId);r&&(r.tags=V(new FormData(t).get("tags")),E(),y=null,h(),S("标签已更新"))});const a=document.getElementById("group-form");a==null||a.addEventListener("submit",i=>{var d,o;i.preventDefault();const r=(d=new FormData(a).get("name"))==null?void 0:d.trim(),n=(o=new FormData(a).get("description"))==null?void 0:o.trim();if(!r)return;if(y.mode==="edit"){const l=f.groups.find(m=>m.id===y.groupId);if(!l)return;l.name=r.slice(0,60),l.description=(n==null?void 0:n.slice(0,80))||"自定义工作主题"}else f.groups.push({id:rt("group"),name:r.slice(0,60),description:(n==null?void 0:n.slice(0,80))||"自定义工作主题",accent:["blue","violet","amber","green"][f.groups.length%4],position:f.groups.length});E();const c=y.mode==="edit"?"工作主题已更新":"工作主题已创建，可直接拖入报告";y=null,h(),S(c)});const e=document.getElementById("report-form");e==null||e.addEventListener("submit",async i=>{i.preventDefault();const r=e.elements.url.value.trim();if(!ut(r))return;const n=e.querySelector('button[type="submit"]');n.disabled=!0,n.innerHTML='<span class="mini-spinner"></span>';let c=e.elements.title.value.trim();c||(c=await nt(e));const d=e.elements.groupId.value,o=e.elements.workType.value,l=V(e.elements.tags.value);if(y.mode==="edit"){const m=f.reports.find(p=>p.id===y.reportId);Object.assign(m,{title:c,url:r,groupId:d,workType:o,tags:l})}else{const m={id:rt("report"),groupId:d,title:c||M(r),url:r,pinned:!1,position:f.reports.filter(p=>p.groupId===d).length,createdAt:new Date().toISOString(),source:"手动添加",access:"production",archived:!1,archivedAt:"",workType:o,tags:l};m.tags.length||(m.tags=it(m,m.workType)),f.reports.push(m)}E(),y=null,h(),S("报告已保存")})}function Rt(){h()}Rt(document.getElementById("app"));
