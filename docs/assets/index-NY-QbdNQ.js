const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./lib-BGD60R8z.js","./jszip.min-BbgLlslo.js","./pptx-preview.es-Cov77fBy.js"])))=>i.map(i=>d[i]);
...
`);let a=r.length?await ue(r):null;e(),requestAnimationFrame(ve),t(a?`已从剪贴板${de(a)}`:`已把粘贴内容放入输入框`)},document.ondragover=e=>{[...e.dataTransfer?.types||[]].includes(`Files`)&&e.preventDefault()},document.ondrop=async n=>{if(n.target?.closest?.(`.prompt-composer`))return;let r=n.dataTransfer?.files||[];if(!r.length)return;n.preventDefault();let i=await ue(r);e(),requestAnimationFrame(ve),t(de(i))}}var xe=`clair-report-editor-v1`,Se=`https://api.github.com`,Ce=`clair-report-editor-draft-v1:`,v={reportId:``,reportTitle:``,reportUrl:``,status:`idle`,error:``,html:``,editorDocument:``,dirty:!1,hasDraft:!1,draftHtml:``,draftAt:``,target:null,token:``,settingsOpen:!1,publishConfirmOpen:!1,pendingSave:!1,saving:!1,lastCommit:``,isLocal:!1,saveLocal:null,protection:null,loadPromise:null,render:null,showToast:null,currentPage:0,pageCount:1},we=new Map,Te=!1;function Ee(e){return[...new Set(e.filter(Boolean))]}function De(e=v.target){return e?{...e.path&&e.sha?{[e.path]:e.sha}:{},...Object.fromEntries((e.mirrors||[]).map(e=>[e.path,e.sha])),...e.baseFiles||{}}:{}}function Oe(e){return`${Ce}${e}`}function ke(e){try{let t=sessionStorage.getItem(Oe(e));if(!t)return null;let n=JSON.parse(t);return!n?.html||typeof n.html!=`string`?null:n}catch{return null}}function Ae(e=v.reportId){try{sessionStorage.removeItem(Oe(e))}catch{}}function je(){return v.dirty&&v.hasDraft?{tone:`changed`,label:v.isLocal?`有新修订 · 上次暂存待保存`:`有新修订 · 上次暂存待推送`}:v.dirty?{tone:`changed`,label:`已修订 · 未暂存`}:v.hasDraft?{tone:`staged`,label:v.isLocal?`已暂存 · 待保存成果库`:`已暂存 · 待推送生产`}:v.lastCommit?{tone:`published`,label:v.isLocal?`成果库 HTML 已更新`:`生产档案已更新`}:{tone:`clean`,label:`未修改`}}function y(){let e=je(),t=document.querySelector(`.editor-revision-status`);t&&(t.className=`editor-revision-status is-${e.tone}`,t.textContent=e.label);let n=document.querySelector(`[data-editor-action="stash"]`);if(n){n.disabled=v.status!==`ready`||v.saving||!v.dirty;let e=!v.dirty&&v.hasDraft?`已暂存`:`暂存修改`;n.setAttribute(`aria-label`,e),n.title=e}let r=document.querySelector(`[data-editor-action="publish"]`);if(r){r.disabled=v.status!==`ready`||v.saving||!v.dirty&&!v.hasDraft;let e=v.saving?v.isLocal?`正在保存到成果库`:`正在推送生产`:v.isLocal?`保存到成果库`:`推送生产`;r.setAttribute(`aria-label`,e),r.title=e,r.classList.toggle(`is-saving`,v.saving)}let i=document.querySelector(`[data-editor-action="preview"]`);i&&(i.disabled=v.status!==`ready`||v.saving||!v.hasDraft);let a=document.querySelector(`[data-editor-page-counter]`),o=document.querySelector(`[data-editor-page-controls]`);a&&(a.textContent=`${v.currentPage+1} / ${Math.max(1,v.pageCount)}`),o&&(o.hidden=v.pageCount<=1);let s=document.querySelector(`[data-editor-action="prev-page"]`),c=document.querySelector(`[data-editor-action="next-page"]`);s&&(s.disabled=v.currentPage<=0),c&&(c.disabled=v.currentPage>=v.pageCount-1)}function Me(e=``){return String(e).replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`).replaceAll(`'`,`&#39;`)}function Ne(e){let t=atob(String(e||``).replace(/\s/g,``)),n=Uint8Array.from(t,e=>e.charCodeAt(0));return new TextDecoder().decode(n)}function Pe(e){let t=new TextEncoder().encode(e),n=``,r=32768;for(let e=0;e<t.length;e+=r)n+=String.fromCharCode(...t.subarray(e,e+r));return btoa(n)}async function Fe(e){return{html:e,protection:null}}async function Ie(e){return e}function Le(e){try{let t=new URL(e);if(t.hostname.toLowerCase()!==`clairku.github.io`)return null;let n=t.pathname.split(`/`).filter(Boolean).map(decodeURIComponent),r=n.shift()||`ClairKu.github.io`,i=n.join(`/`);(!i||t.pathname.endsWith(`/`))&&(i=`${i?`${i}/`:``}index.html`);let a=Ee([`docs/${i}`,i,`public/${i}`]);return{owner:`ClairKu`,repository:r,branch:`main`,path:a[0],candidates:a,source:`auto`}}catch{return null}}async function Re(e,{token:t=``,method:n=`GET`,body:r}={}){let i={Accept:`application/vnd.github+json`,"X-GitHub-Api-Version":`2022-11-28`};t&&(i.Authorization=`Bearer ${t}`),r!==void 0&&(i[`Content-Type`]=`application/json`);let a=await fetch(`${Se}${e}`,{method:n,headers:i,body:r===void 0?void 0:JSON.stringify(r)});if(!a.ok){let e=``;try{e=(await a.json())?.message||``}catch{e=await a.text()}let t=Error(e||`GitHub API ${a.status}`);throw t.status=a.status,t}return a.status===204?null:a.json()}async function ze(e){e.branch=(await Re(`/repos/${encodeURIComponent(e.owner)}/${encodeURIComponent(e.repository)}`)).default_branch||e.branch||`main`;let t=Ee(e.candidates?.length?e.candidates:[e.path]),n=null,r=null,i=[];for(let a of t)try{let n=a.split(`/`).map(encodeURIComponent).join(`/`),o=await Re(`/repos/${encodeURIComponent(e.owner)}/${encodeURIComponent(e.repository)}/contents/${n}?ref=${encodeURIComponent(e.branch)}`),s=``;if(o.encoding===`base64`&&o.content)s=Ne(o.content);else if(o.download_url){let e=await fetch(o.download_url,{cache:`no-store`});if(!e.ok)throw Error(`无法读取 GitHub 原始文件`);s=await e.text()}if(!s)throw Error(`GitHub 文件内容为空`);r?s===r.html&&i.push({path:a,sha:o.sha}):r={html:s,target:{...e,path:a,sha:o.sha,candidates:t}}}catch(e){if(n=e,e.status&&![403,404].includes(e.status))break}if(r)return r.target.mirrors=i,r;throw n||Error(`没有找到对应的 GitHub HTML 文件`)}function Be(e){e.querySelectorAll(`script`).forEach(e=>{e.dataset.clairOriginalType=e.getAttribute(`type`)??`__empty__`,e.setAttribute(`type`,`application/x-clair-disabled`)}),e.querySelectorAll(`*`).forEach(e=>{[...e.attributes].forEach(t=>{/^on/i.test(t.name)&&(e.setAttribute(`data-clair-event-${t.name.toLowerCase()}`,t.value),e.removeAttribute(t.name))});let t=e.getAttribute(`href`);t&&/^\s*javascript:/i.test(t)&&(e.dataset.clairJavascriptHref=t,e.removeAttribute(`href`))})}function Ve(){return`
(() => {
...
`}function He(e,t){let n=new DOMParser().parseFromString(e,`text/html`);n.querySelectorAll(`meta[http-equiv="Content-Security-Policy" i]`).forEach(e=>{e.dataset.clairEditorHttpEquiv=e.getAttribute(`http-equiv`)||`Content-Security-Policy`,e.setAttribute(`http-equiv`,`x-clair-csp-disabled`)}),Be(n);let r=n.createElement(`base`);r.href=t,r.dataset.clairEditorBase=`true`,n.head.prepend(r);let i=n.createElement(`style`);i.id=`clair-editor-style`,i.textContent=`
  ...
  `,n.head.append(i);let a=n.createElement(`script`);return a.id=`clair-editor-bridge`,a.textContent=Ve(),n.body.append(a),`<!DOCTYPE html>\n${n.documentElement.outerHTML}`}function Ue(e){if(e.url)return``;if(e.savedHtml)return e.savedHtml;let t=(e.savedFiles||[]).find(e=>/\.html?$/i.test(e.name||``));return t?.content||t?.excerpt?t.content||t.excerpt:/<!doctype\s+html|<html[\s>]/i.test(e.savedContent||``)?e.savedContent.trim():``}async function We(e){try{let t=Ue(e),n=t?null:Le(e.url),r=null;if(t)r={html:t,target:null};else if(n)try{r=await ze(n)}catch{}if(!r&&e.url){let t=await fetch(e.url,{cache:`no-store`});if(!t.ok)throw Error(`报告读取失败（HTTP ${t.status}）`);r={html:await t.text(),target:n}}let i=await Fe(r.html);v.protection=i.protection,v.target=r.target||n;let a=i.html,o=ke(e.id);if(o?.html)try{let e=await Fe(o.html);a=e.html,v.hasDraft=!0,v.draftHtml=e.html,v.draftAt=o.savedAt||``,o.baseFiles&&v.target&&(v.target.baseFiles=o.baseFiles)}catch{Ae(e.id)}v.html=a,v.editorDocument=He(a,e.url||window.location.href),v.status=`ready`,v.error=``}catch(e){v.status=`error`,v.error=e?.message||`无法读取这份 HTML`}finally{v.loadPromise=null,v.render?.()}}function Ge(){let e=v.render,t=v.showToast;Object.assign(v,{reportId:``,reportTitle:``,reportUrl:``,status:`idle`,error:``,html:``,editorDocument:``,dirty:!1,hasDraft:!1,draftHtml:``,draftAt:``,target:null,settingsOpen:!1,publishConfirmOpen:!1,pendingSave:!1,saving:!1,lastCommit:``,isLocal:!1,saveLocal:null,protection:null,loadPromise:null,currentPage:0,pageCount:1,render:e,showToast:t})}function Ke(){return document.querySelector(`.report-editor-frame`)}function qe(e,t=null){Ke()?.contentWindow?.postMessage({channel:xe,type:`command`,command:e,value:t},`*`)}function b(e,t={}){Ke()?.contentWindow?.postMessage({channel:xe,type:e,...t},`*`)}function Je(e=``){return String(e).replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`)}function Ye(e){let t=Ke();if(!t?.contentWindow)return;let n=Math.max(0,Math.min(v.pageCount-1,Number(e)||0));v.currentPage=n,t.contentWindow.postMessage({channel:xe,type:`set-page`,page:n},`*`),y()}function Xe(){let e=Ke();if(!e?.contentWindow)return Promise.reject(Error(`编辑画布尚未就绪`));let t=crypto.randomUUID?.()||`${Date.now()}-${Math.random()}`;return new Promise((n,r)=>{let i=window.setTimeout(()=>{we.delete(t),r(Error(`读取编辑内容超时`))},1e4);we.set(t,{resolve:e=>{clearTimeout(i),n(e)}}),e.contentWindow.postMessage({channel:xe,type:`serialize`,requestId:t},`*`)})}function Ze(e){return`${String(e||`report`).replace(/[\\/:*?"<>|]+/g,`-`).replace(/\s+/g,`-`).replace(/-+/g,`-`).replace(/^-|-$/g,``).slice(0,80)||`report`}.html`}function Qe(e,t){let n=new Blob([e],{type:`text/html;charset=utf-8`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=Ze(t),document.body.append(i),i.click(),i.remove(),window.setTimeout(()=>URL.revokeObjectURL(r),1e3)}async function $e(e){await navigator.clipboard.writeText(e)}function et(e,t){let n=new DOMParser().parseFromString(e,`text/html`);n.querySelector(`base[data-clair-preview-base]`)?.remove();let r=n.createElement(`base`);return r.href=t,r.dataset.clairPreviewBase=`true`,n.head.prepend(r),`<!DOCTYPE html>\n${n.documentElement.outerHTML}`}function tt(e){if(!v.hasDraft||!v.draftHtml)throw Error(`请先暂存当前修订，再另开预览`);let t=new Blob([et(v.draftHtml,e.url||window.location.href)],{type:`text/html;charset=utf-8`}),n=URL.createObjectURL(t),r=window.open(n,`_blank`);if(!r)throw URL.revokeObjectURL(n),Error(`浏览器拦截了新窗口，请允许弹窗后重试`);r.opener=null,window.setTimeout(()=>URL.revokeObjectURL(n),6e4)}async function nt(e,{silent:t=!1}={}){let n=await Xe(),r=await Ie(n),i=new Date().toISOString();try{sessionStorage.setItem(Oe(e.id),JSON.stringify({reportId:e.id,reportUrl:e.url,savedAt:i,baseFiles:De(),html:r}))}catch{throw Error(`浏览器暂存空间不足，请先下载 HTML 备份`)}return v.html=n,v.draftHtml=n,v.draftAt=i,v.hasDraft=!0,v.dirty=!1,v.lastCommit=``,y(),t||v.showToast?.(v.isLocal?`已暂存在当前浏览器会话，尚未写回成果库`:`已暂存在当前浏览器会话，尚未更新 GitHub`),n}async function rt(e){if(!(v.saving||!v.saveLocal)){v.saving=!0,y();try{let t=v.dirty?await nt(e,{silent:!0}):v.draftHtml||await Xe();await v.saveLocal(t),v.html=t,v.dirty=!1,v.hasDraft=!1,v.draftHtml=``,v.draftAt=``,v.lastCommit=`local`,Ae(e.id),v.showToast?.(`已更新成果库中的 HTML`)}catch(e){v.showToast?.(e?.message||`保存失败，请下载 HTML 备份`)}finally{v.saving=!1,y()}}}async function it(e){let t=v.target;if(!t?.owner||!t.repository||!t.path||!t.branch)throw Error(`请先填写 GitHub 仓库、分支和 HTML 路径`);if(!v.token)throw Error(`请先提供 GitHub Fine-grained Token`);let n=await Ie(e),r=(t.mirrors||[]).map(e=>e.path),i=Ee([...r.filter(e=>e.startsWith(`public/`)),...r.filter(e=>!e.startsWith(`public/`)&&e!==t.path),t.path]),a=``,o=[];for(let e of i)try{let r=e.split(`/`).map(encodeURIComponent).join(`/`),i=`/repos/${encodeURIComponent(t.owner)}/${encodeURIComponent(t.repository)}/contents/${r}`,s=await Re(`${i}?ref=${encodeURIComponent(t.branch)}`,{token:v.token}),c=De(t)[e];if(c&&s.sha!==c)throw Error(`生产文件 ${e} 已在本次编辑后更新，请重新打开报告合并修改`);let l=await Re(i,{token:v.token,method:`PUT`,body:{message:`Update ${v.reportTitle} from Clair's Studio`,content:Pe(n),sha:s.sha,branch:t.branch}});a=l?.commit?.sha||a,t.baseFiles={...De(t),[e]:l?.content?.sha||s.sha},o.push(e)}catch(t){throw o.length?Error(`已更新 ${o.join(`、`)}，但 ${e} 同步失败：${t.message}`):t}return{commit:a,files:o.length}}async function at(e){if(!v.saving){v.saving=!0,y();try{let t=v.dirty?await nt(e,{silent:!0}):v.draftHtml||await Xe(),n=await it(t);v.html=t,v.dirty=!1,v.hasDraft=!1,v.draftHtml=``,v.draftAt=``,v.lastCommit=n.commit,Ae(e.id),v.showToast?.(n.files>1?`已同步 ${n.files} 个 GitHub 文件，Pages 正在更新`:`已提交 GitHub，Pages 正在更新`)}catch(e){v.showToast?.(e?.message||`保存失败，请下载 HTML 备份`)}finally{v.saving=!1,y()}}}function ot(e){let t=v.target||{owner:`ClairKu`,repository:``,branch:`main`,path:``};return`
    ...
    </div>`}function st(e){let t=v.target?`${v.target.owner}/${v.target.repository} · ${v.target.path}`:`尚未识别 GitHub 文件路径`;return`
    ...
    </div>`}function ct({pendingSave:e=!1}={}){v.settingsOpen=!0,v.pendingSave=e;let t=document.querySelector(`.editor-settings-backdrop`);if(!t)return;t.hidden=!1;let n=t.querySelector(`#editor-settings-form`),r=v.target||{};if(n){n.elements.owner.value=r.owner||`ClairKu`,n.elements.repository.value=r.repository||``,n.elements.branch.value=r.branch||`main`,n.elements.path.value=r.path||``;let t=n.querySelector(`button[type="submit"]`);t&&(t.textContent=e?`Connect & save`:`Save settings`)}}function lt(){v.settingsOpen=!1,v.pendingSave=!1;let e=document.querySelector(`.editor-settings-backdrop`);e&&(e.hidden=!0)}function ut(){v.publishConfirmOpen=!0;let e=document.querySelector(`.editor-publish-backdrop`);e&&(e.hidden=!1)}function dt(){v.publishConfirmOpen=!1;let e=document.querySelector(`.editor-publish-backdrop`);e&&(e.hidden=!0)}function ft(e=``){return!!(v.reportId&&(!e||v.reportId===e))}function pt(e,{render:t,showToast:n,saveLocal:r=null}){Ge(),Object.assign(v,{reportId:e.id,reportTitle:e.title,reportUrl:e.url,status:`loading`,render:t,showToast:n,isLocal:!!(Ue(e)&&r),saveLocal:r,currentPage:0,pageCount:1}),t(),v.loadPromise=We(e)}function mt(e,t){let n=v.isLocal?`本地成果 · 保存在当前浏览器`:v.target?`${v.target.owner}/${v.target.repository} · ${v.target.path}${v.target.mirrors?.length?` · 同步 ${v.target.mirrors.length+1} 处`:``}`:`尚未识别 GitHub 源文件`,r=je(),i=v.status===`ready`?`
    ...
    <main class="reader-shell report-editor-shell compact-editor-shell">
      ...
      ${ot(t)}
      ${st(t)}
    </main>`}function ht(e){if(!ft(e.id))return;Te||(Te=!0,window.addEventListener(`message`,e=>{let t=Ke();if(!(!t?.contentWindow||e.source!==t.contentWindow)&&e.data?.channel===xe){if(e.data.type===`dirty`&&(v.dirty=!0,v.lastCommit=``,y()),e.data.type===`page-info`&&(v.pageCount=Math.max(1,Number(e.data.pageCount)||1),v.currentPage=Math.max(0,Math.min(v.pageCount-1,Number(e.data.page)||0)),y()),e.data.type===`serialized`){let t=we.get(e.data.requestId);if(!t)return;we.delete(e.data.requestId),t.resolve(e.data.html)}e.data.type===`selection`&&document.querySelectorAll(`[data-editor-command]`).forEach(t=>{let n=t.dataset.editorCommand;[`bold`,`italic`,`underline`].includes(n)&&t.classList.toggle(`active`,!!e.data[n])}),e.data.type===`block-selection`&&document.querySelectorAll(`[data-editor-block]`).forEach(t=>{t.disabled=!e.data.selected&&t.dataset.editorBlock!==`paste`}),e.data.type===`block-feedback`&&v.showToast?.(e.data.message||`区块操作已完成`)}}),window.addEventListener(`beforeunload`,e=>{!v.reportId||!v.dirty||(e.preventDefault(),e.returnValue=``)}),window.addEventListener(`keydown`,e=>{e.key!==`Escape`||!v.reportId||(v.publishConfirmOpen?dt():v.settingsOpen&&lt())})),document.querySelectorAll(`[data-editor-command]`).forEach(e=>{e.addEventListener(`mousedown`,e=>e.preventDefault()),e.addEventListener(`click`,()=>qe(e.dataset.editorCommand))});let t=document.querySelector(`[data-editor-format]`);t?.addEventListener(`change`,()=>{qe(`formatBlock`,t.value),t.value=`p`}),document.querySelectorAll(`[data-editor-color]`).forEach(e=>{e.addEventListener(`input`,()=>qe(e.dataset.editorColor,e.value))}),document.querySelectorAll(`[data-editor-block]`).forEach(e=>{e.addEventListener(`mousedown`,e=>e.preventDefault()),e.addEventListener(`click`,()=>{b(`block-command`,{command:e.dataset.editorBlock})})});let n=document.querySelector(`[data-editor-image-input]`),r=document.querySelector(`[data-editor-file-input]`),i=e=>new Promise((t,n)=>{let r=new FileReader;r.onload=()=>t(r.result),r.onerror=()=>n(r.error||Error(`文件读取失败`)),r.readAsDataURL(e)});n?.addEventListener(`change`,async()=>{let e=n.files?.[0];if(e)try{b(`insert-block`,{html:`<figure data-block-kind="image"><img src="${await i(e)}" alt="${Je(e.name)}" style="max-width:100%;height:auto" /><figcaption>${Je(e.name)}</figcaption></figure>`})}catch{v.showToast?.(`图片读取失败`)}finally{n.value=``}}),r?.addEventListener(`change`,async()=>{let e=r.files?.[0];if(e)try{b(`insert-block`,{html:`<div data-block-kind="file"><a href="${await i(e)}" download="${Je(e.name)}">📎 ${Je(e.name)}</a></div>`})}catch{v.showToast?.(`文件读取失败`)}finally{r.value=``}});let a=document.querySelector(`[data-editor-insert]`);a?.addEventListener(`change`,()=>{let e=a.value;if(a.value=``,e){if(e===`text`)b(`insert-block`,{html:`<p data-block-kind="text">新文本</p>`});else if(e===`rich`)b(`insert-block`,{html:`<section data-block-kind="rich"><h2>新区块</h2><p>在这里编辑富文本内容。</p></section>`});else if(e===`image`)n?.click();else if(e===`file`)r?.click();else if(e===`table`){let e=(prompt(`表格大小，例如 3x4`,`3x3`)||`3x3`).toLowerCase().split(`x`).map(e=>Math.max(1,Math.min(12,Number(e)||3))),t=e[0]||3,n=e[1]||e[0]||3,r=e=>Array.from({length:n},(t,n)=>`<${e}>${e===`th`?`标题 ${n+1}`:`内容`}</${e}>`).join(``);b(`insert-block`,{html:`<table data-block-kind="table"><thead><tr>${r(`th`)}</tr></thead><tbody>${Array.from({length:t},()=>`<tr>${r(`td`)}</tr>`).join(``)}</tbody></table>`})}else if(e===`markdown`){let e=prompt(`输入 Markdown 内容`);e!==null&&b(`insert-block`,{html:`<pre data-block-kind="markdown" data-format="markdown">${Je(e)}</pre>`})}else if(e===`html`){let e=prompt(`输入 HTML 区块（脚本与危险属性会自动移除）`);e&&b(`insert-block`,{html:`<div data-block-kind="html">${e}</div>`})}}}),document.querySelectorAll(`[data-editor-action]`).forEach(t=>{t.addEventListener(`click`,async()=>{let n=t.dataset.editorAction;if(n===`exit`){if(v.dirty&&!confirm(`还有未暂存的修改。确定退出编辑模式吗？`))return;let e=v.render;Ge(),e?.()}else if(n===`settings`)ct();else if(n===`close-settings`)lt();else if(n===`stash`)try{await nt(e)}catch(e){v.showToast?.(e?.message||`暂存失败，请下载 HTML 备份`)}else if(n===`preview`)try{tt(e),v.showToast?.(`已在新窗口打开暂存修订`)}catch(e){v.showToast?.(e?.message||`无法打开预览`)}else if(n===`publish`)try{if(v.isLocal){await rt(e);return}if(v.dirty&&await nt(e,{silent:!0}),!v.hasDraft){v.showToast?.(`当前没有待推送的修订`);return}ut()}catch(e){v.showToast?.(e?.message||`暂存失败，请下载 HTML 备份`)}else if(n===`close-publish`)dt();else if(n===`confirm-publish`)dt(),!v.token||!v.target?.path?ct({pendingSave:!0}):await at(e);else if(n===`download`)try{Qe(await Ie(await Xe()),e.title),v.showToast?.(`HTML 已下载`)}catch(e){v.showToast?.(e?.message||`下载失败`)}else if(n===`download-published`)await gt(e,v.showToast);else if(n===`share`)try{await $e(e.url),v.showToast?.(`报告链接已复制`)}catch{v.showToast?.(`复制失败，请从地址栏复制`)}else if(n===`link`){let e=prompt(`输入链接地址（https://…）`);if(!e)return;try{let t=new URL(e);if(![`http:`,`https:`,`mailto:`].includes(t.protocol))throw Error();qe(`createLink`,t.href)}catch{v.showToast?.(`请输入有效的 http、https 或 mailto 链接`)}}else if(n===`paste`)try{let e=await navigator.clipboard.readText();if(!e)return;qe(`insertText`,e)}catch{v.showToast?.(`请在编辑区域使用 ⌘V 粘贴`)}else n===`prev-page`?Ye(v.currentPage-1):n===`next-page`?Ye(v.currentPage+1):n===`retry`&&(v.status=`loading`,v.error=``,v.render?.(),v.loadPromise||=We(e))})}),document.querySelectorAll(`.editor-settings-backdrop, .editor-publish-backdrop`).forEach(e=>{e.addEventListener(`click`,t=>{t.target===e&&(e.classList.contains(`editor-settings-backdrop`)?lt():dt())})});let o=document.getElementById(`editor-settings-form`);o?.addEventListener(`submit`,async t=>{t.preventDefault();let n=new FormData(o),r=String(n.get(`github-token-not-password`)||``).trim();r&&(v.token=r);let i=String(n.get(`path`)||``).trim().replace(/^\/+/,``);v.target={...v.target||{},owner:String(n.get(`owner`)||``).trim(),repository:String(n.get(`repository`)||``).trim(),branch:String(n.get(`branch`)||`main`).trim(),path:i,mirrors:i===v.target?.path&&v.target?.mirrors||[],source:`manual`};let a=v.pendingSave;lt();let s=document.querySelector(`.editor-target-label`);if(s){let e=`${v.target.owner}/${v.target.repository} · ${v.target.path}`;s.textContent=e,s.title=e}v.showToast?.(`保存权限已连接`),a&&await at(e)})}async function gt(e,t){try{let n=await fetch(e.url,{cache:`no-store`});if(!n.ok)throw Error();Qe(await n.text(),e.title),t?.(`HTML 已下载`)}catch{window.open(e.url,`_blank`,`noopener,noreferrer`),t?.(`浏览器限制了直接下载，已打开原页面`)}}async function _t(e,t){try{await $e(e.url),t?.(`报告链接已复制`)}catch{t?.(`复制失败，请从地址栏复制`)}}var vt=`modulepreload`,yt=function(e,t){return new URL(e,t).href},bt={},x=function(e,t,n){let r=Promise.resolve();if(t&&t.length>0){let e=document.getElementsByTagName(`link`),i=document.querySelector(`meta[property=csp-nonce]`),a=i?.nonce||i?.getAttribute(`nonce`);function o(e){return Promise.all(e.map(e=>Promise.resolve(e).then(e=>({status:`fulfilled`,value:e}),e=>({status:`rejected`,reason:e}))))}function s(e){return import.meta.resolve?import.meta.resolve(e):new URL(e,import.meta.url).href}r=o(t.map(t=>{if(t=yt(t,n),t=s(t),t in bt)return;bt[t]=!0;let r=t.endsWith(`.css`);for(let n=e.length-1;n>=0;n--){let i=e[n];if(i.href===t&&(!r||i.rel===`stylesheet`))return}let i=document.createElement(`link`);if(i.rel=r?`stylesheet`:vt,r||(i.as=`script`),i.crossOrigin=``,i.href=t,a&&i.setAttribute(`nonce`,a),document.head.appendChild(i),r)return new Promise((e,n)=>{i.addEventListener(`load`,e),i.addEventListener(`error`,()=>n(Error(`Unable to preload CSS for ${t}`)))})}))}function i(e){let t=new Event(`vite:preloadError`,{cancelable:!0});if(t.payload=e,window.dispatchEvent(t),!t.defaultPrevented)throw e}return r.then(t=>{for(let e of t||[])e.status===`rejected`&&i(e.reason);return e().catch(i)})};function S(e=``){return String(e).replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`).replaceAll(`'`,`&#039;`)}function xt(e=``){return S(e).replace(/`([^`]+)`/g,`<code>$1</code>`).replace(/\*\*([^*]+)\*\*/g,`<strong>$1</strong>`).replace(/__([^_]+)__/g,`<strong>$1</strong>`).replace(/(^|[^*])\*([^*]+)\*/g,`$1<em>$2</em>`).replace(/\[([^\]]+)\]\((https?:\/\/[^\s)]+|mailto:[^\s)]+)\)/g,`<a href="$2" target="_blank" rel="noreferrer">$1</a>`)}function St(e=``){let t=String(e).replaceAll(`\r
`,`
`).split(`
`),n=[],r=[],i=``,a=[],o=[],s=``,c=()=>{r.length&&(n.push(`<p>${r.map(xt).join(`<br>`)}</p>`),r=[])},l=()=>{i&&=(n.push(`</${i}>`),``)},u=()=>{a.length&&(n.push(`<blockquote>${a.map(xt).join(`<br>`)}</blockquote>`),a=[])},d=()=>{!o.length&&!s||(n.push(`<pre><code${s?` data-language="${S(s)}"`:``}>${S(o.join(`
`))}</code></pre>`),o=[],s=``)},f=()=>{c(),l(),u()},p=!1;for(let e of t){let t=e.match(/^```\s*([\w.+-]*)\s*$/);if(t){p?d():(f(),s=t[1]||``),p=!p;continue}if(p){o.push(e);continue}if(!e.trim()){f();continue}let m=e.match(/^(#{1,6})\s+(.+)$/);if(m){f();let e=m[1].length;n.push(`<h${e}>${xt(m[2])}</h${e}>`);continue}let h=e.match(/^\s*[-*+]\s+(.+)$/),g=e.match(/^\s*\d+[.)]\s+(.+)$/);if(h||g){c(),u();let e=g?`ol`:`ul`;i!==e&&(l(),i=e,n.push(`<${i}>`)),n.push(`<li>${xt((h||g)[1])}</li>`);continue}let ee=e.match(/^>\s?(.*)$/);if(ee){c(),l(),a.push(ee[1]);continue}l(),u(),r.push(e)}return p&&d(),f(),n.join(`
`)}function Ct(e=``){let t=document.createElement(`template`);return t.innerHTML=String(e),t.content.querySelectorAll(`script, style, link, meta, base, iframe, frame, object, embed, form, input, button, textarea, select`).forEach(e=>e.remove()),t.content.querySelectorAll(`*`).forEach(e=>{[...e.attributes].forEach(t=>{let n=t.name.toLowerCase(),r=t.value.trim().toLowerCase();(n.startsWith(`on`)||n===`srcdoc`||(n===`href`||n===`src`||n===`xlink:href`)&&/^(javascript|vbscript):/.test(r))&&e.removeAttribute(t.name)})}),t.innerHTML}function wt(e,t,n=`document`){return`<!doctype html>
    <html lang="zh-CN">
      <head>
        <meta charset="utf-8">
    ...
    </svg>`};function Hn(e){return Vn[e]||``}function Un(e){try{return new URL(e).hostname.replace(/^www\./,``)}catch{return e}}function Wn(e){try{return[`http:`,`https:`].includes(new URL(e).protocol)}catch{return!1}}function K(e=``){return[...new Set(String(e).split(/[、,，;；\n]+/).map(e=>e.trim()).filter(Boolean).map(e=>e.slice(0,20)))].slice(0,8)}function Gn(){let e=new Set(Ut);return O.reports.forEach(t=>{(t.tags||[]).forEach(t=>e.add(t))}),[...e].filter(e=>![`HTML`,`手动保存`,`生产`].includes(e))}function q(e){document.querySelector(`.toast`)?.remove();let t=document.createElement(`div`);t.className=`toast`,t.setAttribute(`role`,`status`),t.textContent=e,document.body.append(t),clearTimeout(Zt),Zt=window.setTimeout(()=>t.remove(),2600)}function Kn(e=`auto`){qn(),B=requestAnimationFrame(()=>{B=0,window.scrollTo({top:0,left:0,behavior:e})})}function qn(){z&&cancelAnimationFrame(z),B&&cancelAnimationFrame(B),Qt&&cancelAnimationFrame(Qt),z=0,B=0,Qt=0}function Jn(){let e=document.querySelector(`.topbar`)?.getBoundingClientRect().bottom||0,t=document.querySelector(`.topic-nav`),n=t?getComputedStyle(t):null,r=t?.getBoundingClientRect();if(!window.matchMedia(`(max-width: 840px)`).matches){let t=n?.position===`sticky`&&Number.parseFloat(n.top)||0;return Math.max(e+22,t)}let i=n?.position===`sticky`&&r?.bottom>0?r.bottom:0;return Math.max(e,i)+10}function Yn(e,t=`smooth`){if(!e)return;qn();let n=Math.max(0,document.documentElement.scrollHeight-window.innerHeight),r=Math.max(0,Math.min(n,window.scrollY+e.getBoundingClientRect().top-Jn())),i=window.scrollY,a=r-i;if(Math.abs(a)<2)return;if(t!==`smooth`||matchMedia(`(prefers-reduced-motion: reduce)`).matches){window.scrollTo({top:r,left:0,behavior:`auto`});return}let o=Math.min(360,Math.max(180,Math.abs(a)*.22)),s=performance.now(),c=t=>{if(!e.isConnected){z=0;return}let n=Math.min(1,(t-s)/o),r=1-(1-n)**3,l=Math.max(0,Math.min(Math.max(0,document.documentElement.scrollHeight-window.innerHeight),window.scrollY+e.getBoundingClientRect().top-Jn()));a=l-i,window.scrollTo(0,i+a*r),n<1?z=requestAnimationFrame(c):(z=0,window.scrollTo(0,l))};z=requestAnimationFrame(c)}function J(e,t){return document.querySelector(`.group-column[data-bucket-kind="${CSS.escape(e)}"][data-bucket-id="${CSS.escape(t)}"]`)}function Y(e){return document.querySelector(`.board .report-card[data-report-id="${CSS.escape(e)}"]`)}function Xn(e){if(!e)return null;let t=e.closest?.(`.report-card[data-report-id]`);if(t){let e=t.closest(`.group-column[data-bucket-kind][data-bucket-id]`);return{type:`report`,id:t.dataset.reportId,bucketKind:e?.dataset.bucketKind||``,bucketId:e?.dataset.bucketId||``}}let n=e.closest?.(`.group-column[data-bucket-kind][data-bucket-id]`);if(n)return{type:`bucket`,kind:n.dataset.bucketKind,id:n.dataset.bucketId};let r=e.closest?.(`.results-toolbar, .archive-search, .prompt-composer, .groups-section, .library-layout`);return r?{type:`selector`,selector:r.classList.contains(`results-toolbar`)?`.results-toolbar`:r.classList.contains(`archive-search`)?`.archive-search`:r.classList.contains(`prompt-composer`)?`.prompt-composer`:r.classList.contains(`groups-section`)?`.groups-section`:`.library-layout`}:null}function Zn(e){return e?e.type===`report`?(e.bucketKind&&e.bucketId?J(e.bucketKind,e.bucketId):null)?.querySelector(`.report-card[data-report-id="${CSS.escape(e.id)}"]`)||Y(e.id):e.type===`bucket`?J(e.kind,e.id):e.type===`selector`?document.querySelector(e.selector):null:null}function Qn(){let e=Jn(),t=t=>[...document.querySelectorAll(t)].filter(t=>{let n=t.getBoundingClientRect();return n.bottom>e&&n.top<window.innerHeight}).sort((t,n)=>Math.abs(t.getBoundingClientRect().top-e)-Math.abs(n.getBoundingClientRect().top-e))[0];return t(`.board .report-card[data-report-id]`)||t(`.group-column[data-bucket-id]`)||t(`.results-toolbar, .archive-search, .prompt-composer`)||document.querySelector(`.results-toolbar, .archive-search, .groups-section, .library-layout`)}function X(e=null){qn();let t=e||Qn();return{scrollY:window.scrollY,identity:Xn(t),viewportTop:t?.getBoundingClientRect().top??null}}function $n(e){if(!e)return;qn();let t=()=>{let t=Zn(e.identity),n=Math.max(0,document.documentElement.scrollHeight-window.innerHeight),r=t&&Number.isFinite(e.viewportTop)?window.scrollY+t.getBoundingClientRect().top-e.viewportTop:e.scrollY;window.scrollTo({top:Math.max(0,Math.min(n,r)),left:0,behavior:`auto`})};t(),Qt=requestAnimationFrame(()=>{Qt=requestAnimationFrame(()=>{Qt=0,t()})})}function Z(e){$(),$n(e)}function er(e,t=null){let n=t||Y(e),r=n?.nextElementSibling?.matches?.(`.report-card[data-report-id]`)?n.nextElementSibling:n?.previousElementSibling?.matches?.(`.report-card[data-report-id]`)?n.previousElementSibling:null;if(!r){let e=n?.closest(`.group-column[data-bucket-id]`),t=e?.nextElementSibling?.matches?.(`.group-column[data-bucket-id]`)?e.nextElementSibling:e?.previousElementSibling?.matches?.(`.group-column[data-bucket-id]`)?e.previousElementSibling:null;r=t?.querySelector(`.report-card[data-report-id]`)||t||n}return X(r)}function tr(e,t){let n=J(e,t);return X(n?.nextElementSibling?.matches?.(`.group-column[data-bucket-id]`)?n.nextElementSibling:n?.previousElementSibling?.matches?.(`.group-column[data-bucket-id]`)?n.previousElementSibling:n)}function nr(e,t=`smooth`){qn(),B=requestAnimationFrame(()=>{B=requestAnimationFrame(()=>{B=0,Yn(e(),t)})})}function Q(e=null){Z(X(typeof e==`function`?e():e))}[`wheel`,`touchstart`,`pointerdown`].forEach(e=>{window.addEventListener(e,qn,{passive:!0})});var rr=null,ir=new Set;function ar(){return rr||(rr=new Promise((e,t)=>{if(!window.indexedDB){t(Error(`IndexedDB unavailable`));return}let n=indexedDB.open(Vt,1);n.onupgradeneeded=()=>{let e=n.result;e.objectStoreNames.contains(w)||e.createObjectStore(w,{keyPath:`id`}).createIndex(`reportId`,`reportId`,{unique:!1})},n.onsuccess=()=>e(n.result),n.onerror=()=>t(n.error||Error(`File database failed`))}),rr)}async function or(e,t=[]){let n=t.map(t=>{let{blob:n,...r}=t;return{...r,storageId:n instanceof Blob?`${e}:${t.id}`:t.storageId||``,blob:n}}),r=n.filter(e=>e.blob instanceof Blob&&e.storageId);if(r.length){let t=await ar();await new Promise((n,i)=>{let a=t.transaction(w,`readwrite`),o=a.objectStore(w);r.forEach(t=>{o.put({id:t.storageId,reportId:e,name:t.name,type:t.type,size:t.size,blob:t.blob,updatedAt:new Date().toISOString()})}),a.oncomplete=n,a.onerror=()=>i(a.error||Error(`File save failed`)),a.onabort=()=>i(a.error||Error(`File save aborted`))})}return n.map(({blob:e,...t})=>t)}async function sr(e){if(e?.storageId)try{let t=await ar(),n=await new Promise((n,r)=>{let i=t.transaction(w,`readonly`).objectStore(w).get(e.storageId);i.onsuccess=()=>n(i.result),i.onerror=()=>r(i.error||Error(`File read failed`))});if(n?.blob instanceof Blob)return n.blob}catch{return null}let t=e?.content||e?.excerpt;return t?new Blob([t],{type:e.type||`text/plain;charset=utf-8`}):null}async function cr(e){try{let t=await ar();await new Promise((n,r)=>{let i=t.transaction(w,`readwrite`),a=i.objectStore(w),o=a.index(`reportId`).openKeyCursor(IDBKeyRange.only(e));o.onsuccess=()=>{let e=o.result;e&&(a.delete(e.primaryKey),e.continue())},o.onerror=()=>r(o.error||Error(`File cleanup failed`)),i.oncomplete=n,i.onerror=()=>r(i.error||Error(`File cleanup failed`))})}catch{}}function lr(){ir.forEach(e=>URL.revokeObjectURL(e)),ir.clear()}function ur(e){let t=URL.createObjectURL(e);return ir.add(t),t}async function dr(){let e=[...document.querySelectorAll(`[data-saved-file-preview]`)];await Promise.all(e.map(async e=>{let t=O.reports.find(t=>t.id===e.dataset.reportId)?.savedFiles?.find(t=>t.id===e.dataset.fileId);if(!t)return;e.classList.add(`is-loading`),e.setAttribute(`aria-busy`,`true`);let n=await sr(t);if(!n||!e.isConnected){e.classList.remove(`is-loading`),e.setAttribute(`aria-busy`,`false`);return}let r=e.dataset.previewMode;try{if(r===`image`){let r=document.createElement(`img`);r.src=ur(n),r.alt=t.name||`图片预览`,r.draggable=!1,e.replaceChildren(r)}else if(r===`html`){let r=document.createElement(`iframe`);r.src=ur(n),r.title=`${t.name||`HTML`}内容`,r.setAttribute(`sandbox`,`allow-forms allow-modals allow-popups allow-scripts`),e.replaceChildren(r)}else await At(e,n,t,r);e.classList.add(`is-ready`)}catch(t){let n=document.createElement(`div`);n.className=`saved-file-render-error`;let r=document.createElement(`strong`);r.textContent=`暂时无法直接显示这个文件`;let i=document.createElement(`p`);i.textContent=t?.message||`请下载原文件后使用对应应用打开`,n.append(r,i),e.replaceChildren(n),e.classList.add(`has-error`)}finally{e.classList.remove(`is-loading`),e.setAttribute(`aria-busy`,`false`)}}))}async function fr(e){let t=await sr(e);if(!t)return!1;let n=ur(t),r=document.createElement(`a`);return r.href=n,r.download=e.name||`download`,document.body.append(r),r.click(),r.remove(),window.setTimeout(()=>{URL.revokeObjectURL(n),ir.delete(n)},1e3),!0}async function pr(e){let t=window.open(``,`_blank`),n=await sr(e);if(!n)return t?.close(),!1;let r=ur(n);return t?t.location.href=r:window.open(r,`_blank`,`noopener,noreferrer`),window.setTimeout(()=>{URL.revokeObjectURL(r),ir.delete(r)},6e4),!0}function mr(e){return e.savedHtml||xn(e.savedContent,e.savedFiles)}function hr(e){return`${String(e.title||`report`).replace(/[\\/:*?"<>|]+/g,`-`).replace(/\s+/g,` `).trim().slice(0,80)||`report`}.html`}function gr(e){let t=mr(e);return t?URL.createObjectURL(new Blob([t],{type:`text/html;charset=utf-8`})):``}function _r(e){let t=gr(e);if(!t)return!1;let n=document.createElement(`a`);return n.href=t,n.download=hr(e),document.body.append(n),n.click(),n.remove(),window.setTimeout(()=>URL.revokeObjectURL(t),1e3),!0}function vr(e){let t=e.url||gr(e);return t?(window.open(t,`_blank`,`noopener,noreferrer`),e.url||window.setTimeout(()=>URL.revokeObjectURL(t),6e4),!0):!1}function yr(e,t,n=!1){let r=g(t),i=t.format||r.label,a=t.storageId&&[`image`,`pdf`,`html`].includes(r.preview)?`data-saved-file-preview data-report-id="${G(e.id)}" data-file-id="${G(t.id)}" data-preview-mode="${r.preview}"`:``,o=r.preview===`text`&&(t.excerpt||t.content)?`<pre>${G((t.excerpt||t.content).slice(0,n?280:8e3))}</pre>`:``;return`
    <div class="saved-file-visual file-kind-${r.kind} ${n?`compact`:``}" ${a}>
      <span class="saved-file-format">${G(i)}</span>
      ${o||`<div class="saved-file-fallback">
        <strong>${G(i)}</strong>
        <small>${G(t.name||`未命名文件`)}</small>
      </div>`}
    </div>`}function br(e,t,n,r){let i=g(t),a=t.format||i.label,o=t.storageId||t.content||t.excerpt?`data-saved-file-preview data-report-id="${G(e.id)}" data-file-id="${G(t.id)}" data-preview-mode="${G(i.preview)}"`:``;return`<article class="saved-file-embed file-kind-${i.kind}">
  ...
  </article>`}function xr(e){let t=e.savedFiles||[];return t.length?`<section class="saved-file-list embedded-file-list" aria-label="档案正文">
    <div class="embedded-file-list-heading"><strong>档案正文</strong><span>${t.length} 个文件 · 已直接展开</span></div>
    ${t.map((n,r)=>br(e,n,r,t.length)).join(``)}
  </section>`:``}function Sr(e,t=!1,n={}){let r=!e.url&&(!!e.savedContent||!!(e.savedFiles||[]).length),i=[`org`,`account`].includes(e.access),a=e.access===`org`?`需组织登录`:e.access===`account`?`需账号登录`:`生产可访问`,o=mr(e),s=!!e.pinned,c=O.groups.find(t=>t.id===e.groupId)?.name||`未归类`,l=new Set([`HTML`,`手动保存`,`生产`]),u=[...new Set([c,Mn(e.workType),...e.tags||[]])].filter(e=>!l.has(e)),d=!i&&D.reports.some(t=>t.id===e.id),f=e.preview||`${e.id}.png`,p=(e.savedFiles||[])[0],m=o&&e.isHtml?`<iframe class="local-html-preview-frame" title="${G(e.title)}视觉预览"
        srcdoc="${G(o)}" sandbox="allow-scripts" loading="lazy"
        tabindex="-1" aria-hidden="true"></iframe>`:d?`<img src="./previews/${G(f)}" alt="" loading="lazy" decoding="async" draggable="false" />`:r&&p?yr(e,p,!0):`
      <div class="preview-placeholder ${i?`preview-restricted`:``}">
        <span>${i?`ACCESS`:G(e.title.slice(0,2))}</span>
    ...
    <article class="report-card ${i?`restricted-card`:``} ${t?`archived-card`:``} ${s?`is-featured`:``} ${F===e.id?`is-move-selected`:``}"
      data-report-id="${G(e.id)}" ${t||M===`time`?``:`data-report-draggable="true"`}>
      <button class="card-main" type="button" data-action="open" data-id="${G(e.id)}" aria-label="打开${G(e.title)}">
        ...
        <span class="report-copy">
          ...
          <span class="report-tags">${u.map((e,t)=>`<span class="${t<2?`report-context-tag`:``}">${G(e)}</span>`).join(``)}</span>
          ${n.searchExcerpt?`<span class="report-search-excerpt">${G(n.searchExcerpt)}</span>`:``}
          ${i?`<span class="report-access-note">${G(a)}</span>`:``}
        </span>
      </button>
      <div class="card-actions">
        ${t?`
            ...
            <button type="button" class="studio-icon-button feature-action" data-action="toggle-pin" data-id="${G(e.id)}"
              title="${s?`取消精选`:`设为精选`}" aria-label="${s?`取消精选`:`设为精选`}">${E.star}</button>`}
      </div>
    </article>`}function Cr(){if(!R)return``;if(R.type===`group`){let e=R.mode===`edit`?O.groups.find(e=>e.id===R.groupId):null;return`
      <div class="dialog-backdrop">
        <form class="dialog compact-dialog" id="group-form">
          <div class="dialog-title-row">
            ...
            <button type="button" class="studio-icon-button dialog-close-button" data-action="close-modal" title="关闭" aria-label="关闭">${E.close}</button>
          </div>
          <label>主题 / 分组名称
      ...
      </div>`}let e=R.mode===`edit`?O.reports.find(e=>e.id===R.reportId):null,t=e?.groupId||R.groupId||O.groups[0]?.id||``,n=K((e?.tags||[]).join(`、`)).filter(e=>![`HTML`,`手动保存`,`生产`].includes(e));return`
    <div class="dialog-backdrop">
      <form class="dialog" id="report-form">
        ...
        <label>放入分组
          <select name="groupId">
            ${O.groups.map(e=>`<option value="${G(e.id)}" ${e.id===t?`selected`:``}>${G(e.name)}</option>`).join(``)}
          </select>
        </label>
        <label>工作类型
          <select name="workType">
            ${Ht.map(t=>`<option value="${G(t.id)}" ${t.id===(e?.workType||`product-planning`)?`selected`:``}>${G(t.name)}</option>`).join(``)}
          </select>
        </label>
        <fieldset class="report-tag-field">
          ...
          <div class="report-tag-picker" aria-label="选择关键标签">
            ${Gn().map(e=>`<button type="button" class="${n.includes(e)?`selected`:``}"
    ...
    </div>`}function wr(e){if(ft(e.id))return mt(e,G);let t=!e.url&&(!!e.savedContent||!!(e.savedFiles||[]).length),n=[`org`,`account`].includes(e.access),r=e.loginProvider||Sn(e.url)?.provider||(e.access===`org`?`组织帐号`:`站点帐号`),i=e.savedHtml||xn(e.savedContent,e.savedFiles),a=i?`edit-local-document`:e.url?n?`edit`:`edit-document`:``,o=i?`
...
`,`<br />`)}</div>`:``}
      ...
      <div class="login-handoff-wrap">
        <section class="login-handoff-card" aria-labelledby="login-handoff-title">
          ...
          <p class="login-handoff-domain">${G(Un(e.url))}</p>
        </section>
      </div>`:`
      <div class="reader-frame-wrap">
    ...
    <header class="topbar">
      ...
      ${j?`<div class="top-actions"><button class="quiet-button" type="button" data-action="show-catalog">← Library</button></div>`:``}
    </header>`}function Er(){let e=O.reports.filter(e=>e.archived).filter(e=>Nt(e,k,{group:O.groups.find(t=>t.id===e.groupId),workTypeName:Mn(e.workType)})).sort((e,t)=>new Date(t.archivedAt||0)-new Date(e.archivedAt||0)),t=O.reports.filter(e=>e.archived).length;return`
    <main class="app-shell archive-shell">
      ...
      <section class="workspace archive-workspace">
        ...
        <label class="search archive-search">
          ...
          <input id="search-input" value="${G(k)}"
            placeholder="搜索归档标题、来源或网址" aria-label="搜索归档" />
          ${k?`<button type="button" class="studio-icon-button search-clear-button" data-action="clear-search" title="清除搜索" aria-label="清除搜索">${E.close}</button>`:``}
        </label>
        ${e.length?`
          <section class="archive-results">
            <div class="archive-heading">
              <div><h2>${k?`搜索结果`:`归档内容`}</h2><p>按最近归档时间排列</p></div>
              <span>${e.length} 份</span>
            </div>
            <div class="archive-grid">${e.map(e=>Sr(e,!0)).join(``)}</div>
          </section>`:`
          <section class="archive-empty">
            <span>ARCHIVE</span>
            <h2>${k?`没有找到相关归档`:`归档区还是空的`}</h2>
            <p>${k?`换个关键词，或返回查看全部归档内容。`:`在主目录的报告卡片上选择“归档”，内容就会安全收纳在这里。`}</p>
            <button class="quiet-button" type="button" data-action="${k?`clear-search`:`show-catalog`}">${k?`Clear search`:`Back to library`}</button>
          </section>`}
      ...
      ${Cr()}
    </main>`}function Dr(e,t){if(![`topic`,`type`,`tag`].includes(e.kind))return``;let n=t.filter(t=>t.kind===e.kind),r=n.findIndex(t=>t.id===e.id);if(r<0)return``;let i=r===0,a=r===n.length-1,o=(t,n,r,i)=>`
    <button type="button" class="studio-icon-button group-order-button" data-action="move-group"
      ...
      title="${n}" aria-label="${n}" ${i?`disabled`:``}>${r}</button>`;return[o(`top`,`置顶`,E.top,i),o(`up`,`上移`,E.up,i),o(`down`,`下移`,E.down,a),o(`bottom`,`置底`,E.bottom,a)].join(``)}function Or(){if(j)return Er();let e=C(k),t=O.reports.filter(e=>!e.archived),n=e?Rn(t,e):t,r=kn(t.filter(e=>e.pinned),`featured`,`featured`,(e,t)=>Pn(t)-Pn(e)),i={id:`featured`,name:`精选成果`,kind:`featured`,accent:`violet`,reports:r},a=O.reports.filter(e=>e.archived).length,o=t.filter(e=>e.access===`production`).length,s=t.filter(e=>e.access!==`production`).length,c=Fn(t,``),l=M===`topic`&&r.length?[i,...c]:c,u=M===`time`&&c[0]?.reports||[],d=e?[]:(M===`topic`&&r.length?[i,...c]:c).filter(e=>e.reports.length||F||M===`topic`),f=M===`type`?`工作类型`:M===`tag`?`关键标签`:M===`time`?`新增时间`:`工作主题`;return`
    <main class="app-shell">
        ...
        <div class="results-toolbar unified-results-toolbar">
          ...
          <div class="results-toolbar-side">
            <label class="search results-search">
              <span aria-hidden="true">⌕</span>
              <input id="search-input" type="search" value="${G(k)}"
            ...
            <div class="studio-summary compact-summary" aria-label="成果统计">
              ...
              <i></i>
              <strong>${O.groups.length}</strong><span>主题</span>
              <i></i>
        ...
        <section class="groups-section">
          ${F?`
            <div class="move-mode-banner" role="status">
              <div><strong>正在整理报告</strong><span>选择目标${Ln()}的“移到这里”，或直接拖动卡片。</span></div>
          ...
          ${e||d.length?`
            <div class="library-layout">
              <nav class="topic-nav" aria-label="报告${f}">
                <div class="library-nav-controls">
                  <div class="library-view-switcher" role="tablist" aria-label="成果分类方式">
                    <button type="button" role="tab" aria-selected="${M===`topic`}" class="${M===`topic`?`active`:``}" data-action="set-view" data-id="topic">Topic</button>
                    <button type="button" role="tab" aria-selected="${M===`type`}" class="${M===`type`?`active`:``}" data-action="set-view" data-id="type">Type</button>
                    <button type="button" role="tab" aria-selected="${M===`tag`}" class="${M===`tag`?`active`:``}" data-action="set-view" data-id="tag">Tag</button>
                    <button type="button" role="tab" aria-selected="${M===`time`}" class="${M===`time`?`active`:``}" data-action="set-view" data-id="time">Time</button>
                  </div>
                  <button class="studio-icon-button add-topic-icon" type="button" data-action="add-group"
                    aria-label="Add topic" title="Add topic">${E.plus}</button>
                </div>
                ${M===`time`?`
                  <div class="library-time-order" aria-label="时间排序">
                    <span>${N===`modified`?`Modified`:`Created`}</span>
                    <button type="button" data-action="toggle-time-sort"
                      title="切换为按${N===`modified`?`创建`:`修改`}时间排序"
                      aria-label="切换为按${N===`modified`?`创建`:`修改`}时间倒序">
                      <svg viewBox="0 0 16 16" aria-hidden="true"><path d="M4 3v9m0 0L2 10m2 2 2-2M12 13V4m0 0-2 2m2-2 2 2"></path></svg>
                    </button>
                  </div>
                  <div class="library-time-titles" aria-label="按${N===`modified`?`修改`:`创建`}时间排列的成果">
                    ${u.map(e=>`
              ...
              <div class="board catalog-view-${M}">
              ${e?`
                <section class="search-results-panel" aria-live="polite">
                  <header class="search-results-header">
                    <div><span>SEARCH RESULTS</span><h2>“${G(k.trim())}”</h2></div>
                    <strong>${n.length} 份匹配</strong>
                  </header>
                  ${n.length?`<div class="group-cards search-results-cards">${n.map(t=>Sr(t,!1,{searchExcerpt:zn(t,e)})).join(``)}</div>`:`<div class="no-results search-no-results">
                        <strong>没有找到“${G(k.trim())}”</strong>
                  ...
                  <header class="group-header">
                    <div class="group-heading-area">
                      ...
                      ${e.kind===`topic`?`<div class="group-primary-actions" aria-label="分组快捷操作">
                        <button type="button" class="studio-icon-button" data-action="add-to-group" data-id="${G(e.id)}" title="新增成果" aria-label="新增成果">${E.plus}</button>
                        <button type="button" class="studio-icon-button" data-action="rename-group" data-id="${G(e.id)}" title="编辑分组" aria-label="编辑分组">${E.edit}</button>
                      </div>`:``}
                    </div>
                    <div class="group-menu" aria-label="分组排序操作">
                      ${F&&e.kind!==`time`?`<button class="move-here-button" type="button" data-action="move-here" data-id="${G(e.id)}" data-bucket-kind="${G(e.kind)}">Move here</button>`:``}
                      ${Dr(e,c)}
                      ${e.kind===`topic`?`<button type="button" class="studio-icon-button group-delete-button" data-action="delete-group" data-id="${G(e.id)}" title="删除分组" aria-label="删除分组">${E.minus}</button>`:``}
                    </div>
                  </header>
                  <div class="group-cards">
                    ${e.reports.length?e.reports.map(e=>Sr(e)).join(``):e.kind===`topic`?`<button class="empty-topic-drop" type="button" data-action="add-to-group" data-id="${G(e.id)}">
      ...
      ${Cr()}
    </main>`}function $(){let e=document.getElementById(`app`),t=A&&O.reports.find(e=>e.id===A);lr(),e.innerHTML=t?wr(t):Or(),jr(),he({render:()=>Q(()=>document.querySelector(`.prompt-composer`)),showToast:q,saveToLibrary:Tn}),dr()}async function kr(e){let t=e.elements.url,n=e.elements.title,r=e.querySelector(`[data-action="detect-title"]`),i=e.querySelector(`.field-hint`),a=t.value.trim();if(!Wn(a))return i.textContent=`请输入完整的 http 或 https 网址`,``;r.disabled=!0,r.innerHTML=`<span class="mini-spinner"></span>`,i.textContent=`正在读取网页标题…`;try{let{title:e}=await Cn(a);if(!e)throw Error(`read failed`);return n.value=e,i.textContent=`已识别网页标题`,n.value}catch{let e=Un(a);return n.value||=e,i.textContent=`网页暂时无法读取，已用域名作为标题，你可以手动修改`,n.value}finally{r.disabled=!1,r.textContent=`Detect title`}}function Ar(){let e=document.querySelector(`.board`);if(!e)return;let t=null,n=()=>{document.querySelectorAll(`.report-card, .group-column, .topic-nav a`).forEach(e=>{e.classList.remove(`is-nav-drop-target`)})},r=e=>e?[e.bucketKind,e.bucketId,e.targetReportId,Number(e.placeAfter),Number(e.nav)].join(`|`):``,i=()=>{let e=t.sourceCard.getBoundingClientRect(),n=t.sourceCard.cloneNode(!0);return n.removeAttribute(`id`),n.className=`report-card report-drag-preview`,n.style.width=`${e.width}px`,n.style.height=`${e.height}px`,n.querySelectorAll(`button, [role='button'], iframe`).forEach(e=>{e.removeAttribute(`data-action`),e.setAttribute(`tabindex`,`-1`)}),document.body.append(n),t.previewWidth=e.width,t.previewHeight=e.height,t.previewOffsetX=Math.max(18,Math.min(e.width-18,t.startX-e.left)),t.previewOffsetY=Math.max(18,Math.min(e.height-18,t.startY-e.top)),n},a=()=>{if(!t?.preview)return;let e=Math.max(8,window.innerWidth-t.previewWidth-8),n=Math.max(8,window.innerHeight-t.previewHeight-8),r=Math.max(8,Math.min(e,t.x-t.previewOffsetX)),i=Math.max(8,Math.min(n,t.y-t.previewOffsetY));t.preview.style.transform=`translate3d(${r}px, ${i}px, 0)`},o=()=>{!t?.active||t.updateFrame||(t.updateFrame=requestAnimationFrame(()=>{t?.active&&(t.updateFrame=0,a(),u())}))},s=(e,n=null,r=!1)=>{if(!(!e||!t?.placeholder)){if(!n||n.parentElement!==e){e.append(t.placeholder);return}e.insertBefore(t.placeholder,r?n.nextSibling:n)}},c=()=>{let e=document.elementFromPoint(t.x,t.y);if(e?.closest(`.report-card-placeholder`))return t.target;let n=e?.closest(`.topic-nav a[data-nav-bucket-id]`);if(n)return{bucketKind:n.dataset.navBucketKind,bucketId:n.dataset.navBucketId,targetReportId:``,placeAfter:!1,nav:!0,element:n};let r=e?.closest(`.report-card:not(.report-card-placeholder):not(.report-drag-preview)`),i=e?.closest(`.group-column`);if(r&&r!==t.sourceCard){let e=r.closest(`.group-column`);if(!e||e.dataset.bucketKind===`time`)return null;let n=r.getBoundingClientRect(),i=t.y>n.bottom-n.height*.22||t.y>=n.top+n.height*.22&&t.y<=n.bottom-n.height*.22&&t.x>n.left+n.width/2;return{bucketKind:e.dataset.bucketKind||M,bucketId:e.dataset.bucketId||``,targetReportId:r.dataset.reportId||``,placeAfter:i,nav:!1,element:r,container:e.querySelector(`.group-cards`)}}return i&&i.dataset.bucketKind!==`time`?{bucketKind:i.dataset.bucketKind||M,bucketId:i.dataset.bucketId||``,targetReportId:``,placeAfter:!1,nav:!1,element:i,container:i.querySelector(`.group-cards`)}:null},l=e=>{if(!(!t||r(e)===r(t.target))&&(n(),t.target=e,I=e,e)){if(e.nav){e.element.classList.add(`is-nav-drop-target`);return}if(e.targetReportId){s(e.container,e.element,e.placeAfter);return}s(e.container)}},u=()=>{t?.active&&l(c())},d=()=>{let e=Math.min(window.innerHeight*.34,Jn()+72),n=window.innerHeight-72;if(t.y<e){let n=Math.min(1,(e-t.y)/84);return-Math.max(1,Math.round(12*n*n))}if(t.y>n){let e=Math.min(1,(t.y-n)/84);return Math.max(1,Math.round(12*e*e))}return 0},f=()=>{if(!t?.active)return;let e=d();if(e){let t=window.scrollY;window.scrollBy(0,e),window.scrollY!==t&&u()}t.autoScrollFrame=requestAnimationFrame(f)},p=()=>{!t||t.active||(clearTimeout(t.holdTimer),t.sourceCard.setPointerCapture?.(t.pointerId),t.active=!0,P=t.reportId,t.preview=i(),t.placeholder=document.createElement(`div`),t.placeholder.className=`report-card report-card-placeholder`,t.placeholder.innerHTML=`<span>放在这里</span>`,t.placeholder.style.minHeight=`${t.sourceCard.getBoundingClientRect().height}px`,t.sourceCard.before(t.placeholder),t.sourceCard.classList.add(`is-dragging`),document.body.classList.add(`report-drag-session`),a(),u(),t.autoScrollFrame=requestAnimationFrame(f))},m=()=>{t&&(clearTimeout(t.holdTimer),t.autoScrollFrame&&cancelAnimationFrame(t.autoScrollFrame),t.updateFrame&&cancelAnimationFrame(t.updateFrame),t.preview?.remove(),t.placeholder?.remove(),t.sourceCard.classList.remove(`is-dragging`),document.body.classList.remove(`report-drag-session`),n(),P=``,I=null)},h=()=>{if(!t)return;let e=t,n=e.active?e.target:null,r=e.reportId;e.active&&($t=r,en=Date.now()+500),m(),t=null,!(!n?.bucketId||n.bucketKind===`time`)&&In(r,n.bucketKind,n.bucketId,n.targetReportId||``,!!n.placeAfter)&&(Q(()=>Y(r)),n.nav&&nr(()=>J(n.bucketKind,n.bucketId)),q(n.bucketKind===`featured`?`已加入精选成果`:n.bucketKind===`tag`?`已添加目标标签`:n.bucketKind===`type`?`工作类型已更新`:n.targetReportId?`报告顺序已更新`:`已移入新主题`))};e.addEventListener(`pointerdown`,e=>{if(e.button!==0||e.target.closest(`.card-actions`))return;let n=e.target.closest(`.report-card[data-report-draggable="true"]`);n?.closest(`.group-column`)&&(e.pointerType===`mouse`&&e.preventDefault(),t={pointerId:e.pointerId,reportId:n.dataset.reportId,sourceCard:n,startX:e.clientX,startY:e.clientY,x:e.clientX,y:e.clientY,active:!1,target:null,preview:null,placeholder:null,autoScrollFrame:0,updateFrame:0,holdTimer:0},t.holdTimer=window.setTimeout(()=>p(),240))}),e.addEventListener(`pointermove`,e=>{!t||e.pointerId!==t.pointerId||(t.x=e.clientX,t.y=e.clientY,!t.active&&Math.hypot(t.x-t.startX,t.y-t.startY)>=8&&p(),t.active&&(e.preventDefault(),o()))}),e.addEventListener(`pointerup`,e=>{!t||e.pointerId!==t.pointerId||h()}),e.addEventListener(`pointercancel`,()=>{m(),t=null})}function jr(){let e=document.getElementById(`search-input`),t=!1,n=e=>{let t=e?.value||``;if(t===k)return;let n=e.selectionStart,r=e.selectionEnd;k=t,Q(()=>document.querySelector(`.results-toolbar, .archive-search`));let i=document.getElementById(`search-input`);i?.focus({preventScroll:!0}),i?.setSelectionRange(n,r)};e?.addEventListener(`compositionstart`,()=>{t=!0}),e?.addEventListener(`compositionend`,e=>{t=!1,n(e.currentTarget)}),e?.addEventListener(`input`,e=>{e.isComposing||t||n(e.currentTarget)}),e?.addEventListener(`search`,e=>n(e.currentTarget)),e?.addEventListener(`keydown`,e=>{e.key!==`Escape`||!k||(e.preventDefault(),k=``,Q(()=>document.querySelector(`.results-toolbar, .archive-search`)),document.getElementById(`search-input`)?.focus({preventScroll:!0}))}),document.querySelectorAll(`[data-action]`).forEach(e=>{e.addEventListener(`click`,async e=>{let t=e.currentTarget.dataset.action,n=e.currentTarget.dataset.id,r=e.currentTarget.closest(`.report-card`);if(t===`scroll-top`)Kn(`smooth`);else if(t===`open`){if(n===$t&&Date.now()<en)return;tn=X(r||Y(n)),A=n,$(),Kn()}else if(t===`preview-saved-file`||t===`download-saved-file`){let r=O.reports.find(e=>e.id===n)?.savedFiles?.find(t=>t.id===e.currentTarget.dataset.fileId);if(!r)return;(t===`preview-saved-file`?await pr(r):await fr(r))||q(`原文件未找到，请重新上传后保存`)}else if(t===`edit-document`){let e=O.reports.find(e=>e.id===n);if(!e||e.access!==`production`)return;pt(e,{render:$,showToast:q})}else if(t===`edit-local-document`){let e=O.reports.find(e=>e.id===n);if(!e||!mr(e))return;pt(e,{render:$,showToast:q,saveLocal:async t=>{let n=e.savedHtml;e.savedHtml=t,e.isHtml=!0,e.tags=qt(e,e.workType),W(e);try{U()}catch{throw e.savedHtml=n,Error(`修改后的 HTML 超过当前浏览器可保存容量，请先下载备份`)}}})}else if(t===`download-report`){let e=O.reports.find(e=>e.id===n);if(!e)return;mr(e)?_r(e)&&q(`HTML 已下载`):await gt(e,q)}else if(t===`share-report`||t===`copy-production-url`){let e=O.reports.find(e=>e.id===n);e?.url&&await _t(e,e=>{q(e===`报告链接已复制`?`生产 URL 已复制`:e)})}else if(t===`open-browser`){let e=O.reports.find(e=>e.id===n);if(!e)return;vr(e)||q(`浏览器未能打开该报告`)}else if(t===`back`)A=``,R=null,Z(tn||{scrollY:0}),tn=null;else if(t===`clear-search`)k=``,Q(()=>document.querySelector(`.results-toolbar, .archive-search`)),document.getElementById(`search-input`)?.focus({preventScroll:!0});else if(t===`set-view`){if(![`topic`,`type`,`tag`,`time`].includes(n))return;M=n,F=``,localStorage.setItem(Lt,M),Q()}else if(t===`toggle-time-sort`)N=N===`created`?`modified`:`created`,localStorage.setItem(Rt,N),Q();else if(t===`cancel-move`)F=``,Q();else if(t===`move-here`){let t=e.currentTarget.dataset.bucketKind||M;if(F&&In(F,t,n)){let e=F;F=``,Q(()=>Y(e)),q(t===`tag`?`已添加目标标签`:`报告已移入目标${Ln()}`)}}else if(t===`show-archive`)j=!0,k=``,A=``,$(),Kn();else if(t===`show-catalog`)j=!1,k=``,A=``,$(),Kn();else if(t===`add-report`)V=X(document.querySelector(`.results-toolbar`)),R={type:`report`,mode:`create`,groupId:O.groups[0]?.id},Z(V);else if(t===`add-to-group`)V=X(J(`topic`,n)),R={type:`report`,mode:`create`,groupId:n},Z(V);else if(t===`edit`)V=X(r||Y(n)),R={type:`report`,mode:`edit`,reportId:n},Z(V);else if(t===`toggle-pin`){let e=O.reports.find(e=>e.id===n);if(!e)return;let t=e.pinned&&r?.closest(`[data-bucket-kind="featured"]`)?er(n,r):X(r||Y(n));e.pinned=!e.pinned,W(e),U(),Z(t),q(e.pinned?`已加入精选成果`:`已移出精选成果`)}else if(t===`close-modal`)R=null,Z(V||X()),V=null;else if(t===`detect-title`)await kr(e.currentTarget.closest(`form`));else if(t===`archive`){let e=O.reports.find(e=>e.id===n);if(!e)return;let t=er(n,r);e.archived=!0,e.archivedAt=new Date().toISOString(),U(),Z(t),q(`已归档，可随时恢复`)}else if(t===`restore`){let e=O.reports.find(e=>e.id===n);if(!e)return;let t=er(n);e.archived=!1,e.archivedAt=``,U(),Z(t),q(`报告已恢复到原主题`)}else if(t===`delete`){let e=O.reports.find(e=>e.id===n);if(e?.archived&&confirm(`二次确认：永久删除“${e.title}”？\n\n删除后无法从归档区恢复。`)){let e=er(n);O.reports=O.reports.filter(e=>e.id!==n),A===n&&(A=``),U(),await cr(n),Z(e),q(`报告已永久删除`)}}else if(t===`add-group`)V=X(document.querySelector(`.results-toolbar`)),R={type:`group`,mode:`create`},Z(V);else if(t===`rename-group`)O.groups.find(e=>e.id===n)&&(V=X(J(`topic`,n)),R={type:`group`,mode:`edit`,groupId:n},Z(V));else if(t===`move-group`){let t=e.currentTarget.dataset.bucketKind,r=e.currentTarget.dataset.direction,i=X(J(t,n));Dn(n,r,t)&&(Z(i),requestAnimationFrame(()=>{document.querySelector(`[data-action="move-group"][data-id="${CSS.escape(n)}"][data-direction="${CSS.escape(r)}"]`)?.focus({preventScroll:!0})}),q(`分组顺序已更新`))}else if(t===`delete-group`){let e=O.groups.find(e=>e.id===n),t=O.groups.find(e=>e.id!==n);if(e&&!t)q(`请先新增另一个分组，再删除当前分组`);else if(e&&confirm(`删除“${e.name}”？其中的报告会移到“${t.name}”。`)){let e=tr(`topic`,n);O.reports.forEach(e=>{e.groupId===n&&(e.groupId=t.id)}),O.groups=O.groups.filter(e=>e.id!==n),U(),Z(e),q(`分组已删除，报告已移到“${t.name}”`)}}})}),document.querySelector(`.topbar`)?.addEventListener(`click`,e=>{e.target.closest(`button, a`)||Kn(`smooth`)}),document.querySelectorAll(`.topic-nav a[data-nav-bucket-id]`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault();let n=e.dataset.navBucketKind,r=e.dataset.navBucketId;if(k){k=``,Q(),nr(()=>J(n,r));return}Yn(J(n,r))})}),document.querySelectorAll(`.topic-nav a[data-nav-report-id]`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault(),Yn(Y(e.dataset.navReportId))})}),Ar(),document.querySelectorAll(`.legacy-report-drag-handle`).forEach(e=>{let t=null,n=!1,r=null,i=0,a=0,o=()=>{i&&=(cancelAnimationFrame(i),0)},s=()=>{if(!P)return o();let e=Math.min(110,window.innerHeight*.18),t=a<e?a-e:a>window.innerHeight-e?a-(window.innerHeight-e):0;if(!t)return o();let n=Math.sign(t)*Math.min(24,5+Math.abs(t)*.2);window.scrollBy(0,n),i=requestAnimationFrame(s)},c=t=>{let n=e.closest(`.report-card`);if(n){if(!r){let e=n.getBoundingClientRect();r=n.cloneNode(!0),r.className=`report-card report-drag-preview`,r.style.width=`${e.width}px`,r.style.height=`${e.height}px`,r.querySelectorAll(`button, [role='button']`).forEach(e=>{e.removeAttribute(`data-action`),e.setAttribute(`tabindex`,`-1`)}),document.body.append(r)}r.style.transform=`translate3d(${t.clientX+16}px, ${t.clientY+16}px, 0)`}},l=()=>{I=null,L?.remove(),L=null,document.querySelectorAll(`.report-card, .group-column, .topic-nav a`).forEach(e=>e.classList.remove(`is-card-drop-target`,`is-card-drop-before`,`is-card-drop-after`,`is-drop-ready`,`is-nav-drop-target`))},u=(t,n,r)=>{if(t){if(!L){L=document.createElement(`div`),L.className=`report-card report-card-placeholder`,L.innerHTML=`<span>放在这里</span>`;let t=e.closest(`.report-card`)?.getBoundingClientRect().height;t&&(L.style.minHeight=`${t}px`)}if(!n||n.parentElement!==t){t.append(L);return}t.insertBefore(L,r?n.nextSibling:n)}},d=t=>{let n=document.elementFromPoint(t.clientX,t.clientY),r=n?.closest(`.topic-nav a[data-nav-bucket-id]`);if(l(),r){r.classList.add(`is-nav-drop-target`),I={bucketKind:r.dataset.navBucketKind,bucketId:r.dataset.navBucketId,targetReportId:``,placeAfter:!1,nav:!0};return}let i=e.closest(`.report-card`),a=n?.closest(`.report-card:not(.report-card-placeholder)`),o=n?.closest(`.group-column`);if(a&&a!==i){let e=a.closest(`.group-column`),n=a.getBoundingClientRect(),r=t.clientY>n.top+n.height/2;a.classList.add(`is-card-drop-target`,r?`is-card-drop-after`:`is-card-drop-before`),u(e?.querySelector(`.group-cards`),a,r),I={bucketKind:e?.dataset.bucketKind||M,bucketId:e?.dataset.bucketId||``,targetReportId:a.dataset.reportId||``,placeAfter:r,nav:!1};return}o&&o.dataset.bucketKind!==`time`&&(o.classList.add(`is-drop-ready`),u(o.querySelector(`.group-cards`),null,!1),I={bucketKind:o.dataset.bucketKind||M,bucketId:o.dataset.bucketId||``,targetReportId:``,placeAfter:!1,nav:!1})},f=()=>{P=``,t=null,n=!1,o(),r?.remove(),r=null,l(),e.closest(`.report-card`)?.classList.remove(`is-dragging`)};e.addEventListener(`pointerdown`,r=>{r.preventDefault(),P=e.dataset.reportDragId,t={x:r.clientX,y:r.clientY},n=!1,e.setPointerCapture?.(r.pointerId),e.closest(`.report-card`)?.classList.add(`is-dragging`)}),e.addEventListener(`pointermove`,e=>{P&&(t&&Math.hypot(e.clientX-t.x,e.clientY-t.y)<7||(n=!0,a=e.clientY,c(e),d(e),i||=requestAnimationFrame(s)))}),e.addEventListener(`pointerup`,e=>{if(!P)return;let t=P;if(!n){F=t,f(),Q(()=>Y(t)),q(`请选择目标${Ln()}`);return}let r=I,i=r?.targetReportId||``,a=r?.bucketId||``,o=r?.bucketKind||M,s=a&&o!==`time`?In(t,o,a,i,!!r?.placeAfter):!1;f(),s&&(Q(()=>Y(t)),requestAnimationFrame(()=>{let e=`.group-column[data-bucket-kind="${CSS.escape(o)}"][data-bucket-id="${CSS.escape(a)}"]`,n=document.querySelector(`${e} .report-card[data-report-id="${CSS.escape(t)}"]`)||document.querySelector(`.search-results-cards .report-card[data-report-id="${CSS.escape(t)}"]`);Yn(n),n?.classList.add(`is-drop-landed`),window.setTimeout(()=>n?.classList.remove(`is-drop-landed`),900)}),q(o===`featured`?`已加入精选成果`:o===`tag`?`已添加目标标签`:o===`type`?`工作类型已更新`:i?`报告顺序已更新`:`已移入新主题`))}),e.addEventListener(`pointercancel`,f)});let r=document.getElementById(`group-form`);r?.addEventListener(`submit`,e=>{e.preventDefault();let t=new FormData(r).get(`name`)?.trim(),n=new FormData(r).get(`description`)?.trim();if(!t)return;if(R.mode===`edit`){let e=O.groups.find(e=>e.id===R.groupId);if(!e)return;e.name=t.slice(0,60),e.description=n?.slice(0,80)||`自定义工作主题`}else O.groups.push({id:Bn(`group`),name:t.slice(0,60),description:n?.slice(0,80)||`自定义工作主题`,accent:[`blue`,`violet`,`amber`,`green`][O.groups.length%4],position:O.groups.length}),M=`topic`,localStorage.setItem(Lt,M);U();let i=R.mode===`edit`?`工作主题已更新`:`工作主题已创建，可直接拖入报告`;R=null,Z(V||X()),V=null,q(i)});let i=document.getElementById(`report-form`),a=i?.elements.tags,o=e=>{let t=K(a?.value).includes(e.dataset.reportTag);e.classList.toggle(`selected`,t),e.setAttribute(`aria-pressed`,String(t))},s=e=>{e.addEventListener(`click`,()=>{let t=K(a.value),n=e.dataset.reportTag;a.value=t.includes(n)?t.filter(e=>e!==n).join(`、`):[...t,n].slice(0,8).join(`、`),o(e)})};i?.querySelectorAll(`[data-report-tag]`).forEach(s);let c=i?.querySelector(`[data-add-report-tag]`),l=i?.querySelector(`.new-report-tag-row`),u=i?.querySelector(`[data-new-report-tag]`),d=()=>{l.hidden=!1,u.focus()},f=()=>{let[e]=K(u.value);if(!e)return;let t=K(a.value);if(!t.includes(e)&&t.length>=8){q(`最多选择 8 个标签`);return}a.value=[...new Set([...t,e])].slice(0,8).join(`、`);let n=[...i.querySelectorAll(`[data-report-tag]`)].find(t=>t.dataset.reportTag===e);n||(n=document.createElement(`button`),n.type=`button`,n.dataset.reportTag=e,n.textContent=e,c.before(n),s(n)),o(n),u.value=``,l.hidden=!0,c.focus()};c?.addEventListener(`click`,d),i?.querySelector(`[data-confirm-report-tag]`)?.addEventListener(`click`,f),u?.addEventListener(`keydown`,e=>{e.key===`Enter`?(e.preventDefault(),f()):e.key===`Escape`&&(e.preventDefault(),u.value=``,l.hidden=!0,c.focus())}),i?.addEventListener(`submit`,async e=>{e.preventDefault();let t=i.elements.url.value.trim(),n=R.mode===`edit`?R.reportId:``,r=n?O.reports.find(e=>e.id===n):null,a=!!(r&&!r.url&&!t);if(!Wn(t)&&!a)return;let o=i.querySelector(`button[type="submit"]`),s=i.querySelector(`.field-hint`);if(o.disabled=!0,o.innerHTML=`<span class="mini-spinner"></span>`,a){let e=i.elements.title.value.trim()||r.title,t=i.elements.groupId.value,n=i.elements.workType.value,a=K(i.elements.tags.value).filter(e=>![`HTML`,`手动保存`,`生产`].includes(e));Object.assign(r,{title:e,groupId:t,workType:n,tags:a}),W(r),U(),R=null,Z(V||X()),V=null,q(`报告已保存`);return}let c=vn({material:t,files:[],url:t,excludeId:n});if(c){o.disabled=!1,o.textContent=`Save`,s.textContent=`成果库已有“${c.title}”，未重复保存`,q(`成果库已有“${c.title}”，未重复保存`);return}let l=await wn({material:t,files:[],url:t},e=>{s.textContent=e});if(!l.allowed){o.disabled=!1,o.textContent=`Save`,s.textContent=l.reason,q(l.reason);return}let u=i.elements.title.value.trim()||l.metadata.title,d=i.elements.groupId.value,f=i.elements.workType.value,p=K(i.elements.tags.value),m={title:u||Un(t),url:t,groupId:d,workType:f,source:`手动添加`,access:l.access,detectedDescription:l.metadata.description,manualSaved:!0,isProduction:l.access===`production`,isPersonal:yn(t),isHtml:l.isHtml,loginProvider:l.loginProvider},h=R.mode===`edit`?p:[...new Set([...qt(m,f),...p])].slice(0,8);if(R.mode===`edit`){let e=O.reports.find(e=>e.id===R.reportId);Object.assign(e,m,{tags:h}),W(e)}else{let e=new Date().toISOString(),t={id:Bn(`report`),groupId:d,...m,pinned:!1,position:O.reports.filter(e=>e.groupId===d).length,createdAt:e,modifiedAt:e,archived:!1,archivedAt:``,tags:h};O.reports.push(t)}U(),R=null,Z(V||X()),V=null,q(`报告已保存`)});let p=A&&O.reports.find(e=>e.id===A);p&&ht(p)}function Mr(){sn(),$()}Mr(document.getElementById(`app`));export{f as a,c as i,s as n,d as o,o as r,u as s,x as t};