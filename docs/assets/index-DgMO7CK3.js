const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./lib-u__c6mEd.js","./jszip.min-4nB2MEZN.js","./pptx-preview.es-iqT4ONkg.js"])))=>i.map(i=>d[i]);
...
`)}function At(e=``){let t=document.createElement(`template`);return t.innerHTML=String(e),t.content.querySelectorAll(`script, style, link, meta, base, iframe, frame, object, embed, form, input, button, textarea, select`).forEach(e=>e.remove()),t.content.querySelectorAll(`*`).forEach(e=>{[...e.attributes].forEach(t=>{let n=t.name.toLowerCase(),r=t.value.trim().toLowerCase();(n.startsWith(`on`)||n===`srcdoc`||(n===`href`||n===`src`||n===`xlink:href`)&&/^(javascript|vbscript):/.test(r))&&e.removeAttribute(t.name)})}),t.innerHTML}function jt(e,t,n=`document`){return`<!doctype html>
    ...
    <div class="search-dimension-options">
      ${r(`all`,`全部`,e.length)}
      ${V.map(({id:e,label:t})=>r(e,t,n(e))).join(``)}
    </div>
    <span class="sr-only search-results-announcement" role="status" aria-live="polite">匹配到了 ${t} 份</span>
  </div>`}function $n(e){let t=z.size?new Set(z):new Set(V.map(e=>e.id)),n=new Map(V.map(({id:e})=>[e,[]]));return e.forEach(e=>{let r=V.find(({id:n})=>t.has(n)&&e.dimensions.includes(n));r&&n.get(r.id).push(e)}),`<div class="search-priority-groups" aria-label="按最高匹配优先级排列的搜索结果">
    ${V.map((e,t)=>({...e,priority:String(t+1).padStart(2,`0`),hits:n.get(e.id).sort(Zn)})).filter(({id:e})=>t.has(e)).filter(e=>e.hits.length).map(e=>`<section class="search-priority-group" data-search-dimension="${e.id}">
      ...
      <div class="group-cards search-results-cards">
        ${e.hits.map(({report:e})=>Vr(e)).join(``)}
      </div>
    </section>`).join(``)}
  </div>`}function er(e){return`${e}-${crypto.randomUUID?.()||`${Date.now()}-${Math.random()}`}`}function W(e=``){return String(e).replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`).replaceAll(`'`,`&#39;`)}var tr={back:`
    <svg viewBox="0 0 24 24" aria-hidden="true">
    ...
    </svg>`};function nr(e){return tr[e]||``}function rr(e){try{return new URL(e).hostname.replace(/^www\./,``)}catch{return e}}function ir(e){try{return[`http:`,`https:`].includes(new URL(e).protocol)}catch{return!1}}function G(e=``){return[...new Set(String(e).split(/[、,，;；\n]+/).map(e=>e.trim()).filter(Boolean).map(e=>e.slice(0,20)))].slice(0,8)}function ar(){let e=new Set(Qt);return E.reports.forEach(t=>{(t.tags||[]).forEach(t=>e.add(t))}),[...e].filter(e=>![`HTML`,`手动保存`,`生产`].includes(e))}function K(e,{duration:t=2600,actionLabel:n=``,onAction:r=null}={}){document.querySelector(`.toast`)?.remove();let i=document.createElement(`div`);i.className=`toast`,i.setAttribute(`role`,`status`);let a=document.createElement(`span`);if(a.textContent=e,i.append(a),n&&typeof r==`function`){let e=document.createElement(`button`);e.type=`button`,e.className=`toast-action`,e.textContent=n,e.addEventListener(`click`,()=>{clearTimeout(sn),i.remove(),r()}),i.append(e)}document.body.append(i),clearTimeout(sn),sn=window.setTimeout(()=>i.remove(),t)}function or(e,t){K(e,{duration:8e3,actionLabel:`撤销`,onAction:()=>{t(),K(`已撤销刚才的操作`)}})}function sr(e){let t=e?.closest?.(`[data-action]`);return t?{action:t.dataset.action||``,id:t.dataset.id||``,bucketKind:t.dataset.bucketKind||``,direction:t.dataset.direction||``}:null}function cr(e){if(!e?.action)return null;let t=[`[data-action="${CSS.escape(e.action)}"]`,e.id?`[data-id="${CSS.escape(e.id)}"]`:``,e.bucketKind?`[data-bucket-kind="${CSS.escape(e.bucketKind)}"]`:``,e.direction?`[data-direction="${CSS.escape(e.direction)}"]`:``].join(``);return document.querySelector(t)}function lr(e,t,n){mn=t||X(),cn=sr(n||document.activeElement),ln=window.scrollY,I=e,Z(mn)}function q({fallbackSelector:e=`.results-toolbar, .archive-search, .reader-header`}={}){if(!I)return;let t=mn||{scrollY:ln},n=cn;I=null,Z(t),mn=null,cn=null,requestAnimationFrame(()=>{let t=document.querySelector(e);(cr(n)||t?.querySelector(`button, input, [tabindex]:not([tabindex='-1'])`)||t)?.focus?.({preventScroll:!0})})}function ur(){let e=document.querySelector(`.app-shell > .dialog-backdrop`);if(e?document.body.style.setProperty(`--studio-modal-scroll-top`,`${-ln}px`):document.body.style.removeProperty(`--studio-modal-scroll-top`),document.body.classList.toggle(`studio-modal-open`,!!e),!e)return;[...e.parentElement.children].forEach(t=>{t!==e&&(t.inert=!0,t.setAttribute(`aria-hidden`,`true`))});let t=e.querySelector(`[role="dialog"]`);if(!t)return;let n=()=>[...t.querySelectorAll([`a[href]`,`button:not([disabled])`,`input:not([disabled]):not([type='hidden'])`,`select:not([disabled])`,`textarea:not([disabled])`,`[tabindex]:not([tabindex='-1'])`].join(`,`))].filter(e=>!e.hidden&&e.getClientRects().length);e.addEventListener(`keydown`,e=>{if(e.key===`Escape`){e.preventDefault(),q();return}if(e.key!==`Tab`)return;let r=n();if(!r.length){e.preventDefault(),t.focus({preventScroll:!0});return}let i=r[0],a=r.at(-1);e.shiftKey&&document.activeElement===i?(e.preventDefault(),a.focus({preventScroll:!0})):!e.shiftKey&&document.activeElement===a&&(e.preventDefault(),i.focus({preventScroll:!0}))}),e.addEventListener(`click`,t=>{t.target===e&&q()}),requestAnimationFrame(()=>{(t.querySelector(`[autofocus]`)||n()[0]||t)?.focus?.({preventScroll:!0})})}function dr(e=`auto`){fr(),R=requestAnimationFrame(()=>{R=0,window.scrollTo({top:0,left:0,behavior:e})})}function fr(){L&&cancelAnimationFrame(L),R&&cancelAnimationFrame(R),un&&cancelAnimationFrame(un),L=0,R=0,un=0}function pr(){let e=document.querySelector(`.topbar`)?.getBoundingClientRect().bottom||0,t=document.querySelector(`.topic-nav`),n=t?getComputedStyle(t):null,r=t?.getBoundingClientRect();if(!window.matchMedia(`(max-width: 840px)`).matches){let t=n?.position===`sticky`&&Number.parseFloat(n.top)||0;return Math.max(e+22,t)}let i=n?.position===`sticky`&&r?.bottom>0?r.bottom:0;return Math.max(e,i)+10}function mr(e,t=`smooth`){if(!e)return;fr();let n=Math.max(0,document.documentElement.scrollHeight-window.innerHeight),r=Math.max(0,Math.min(n,window.scrollY+e.getBoundingClientRect().top-pr())),i=window.scrollY,a=r-i;if(Math.abs(a)<2)return;if(t!==`smooth`||matchMedia(`(prefers-reduced-motion: reduce)`).matches){window.scrollTo({top:r,left:0,behavior:`auto`});return}let o=Math.min(360,Math.max(180,Math.abs(a)*.22)),s=performance.now(),c=t=>{if(!e.isConnected){L=0;return}let n=Math.min(1,(t-s)/o),r=1-(1-n)**3,l=Math.max(0,Math.min(Math.max(0,document.documentElement.scrollHeight-window.innerHeight),window.scrollY+e.getBoundingClientRect().top-pr()));a=l-i,window.scrollTo(0,i+a*r),n<1?L=requestAnimationFrame(c):(L=0,window.scrollTo(0,l))};L=requestAnimationFrame(c)}function J(e,t){return document.querySelector(`.group-column[data-bucket-kind="${CSS.escape(e)}"][data-bucket-id="${CSS.escape(t)}"]`)}function Y(e){return document.querySelector(`.board .report-card[data-report-id="${CSS.escape(e)}"]`)}function hr(e){if(!e)return null;let t=e.closest?.(`.report-card[data-report-id]`);if(t){let e=t.closest(`.group-column[data-bucket-kind][data-bucket-id]`);return{type:`report`,id:t.dataset.reportId,bucketKind:e?.dataset.bucketKind||``,bucketId:e?.dataset.bucketId||``}}let n=e.closest?.(`.group-column[data-bucket-kind][data-bucket-id]`);if(n)return{type:`bucket`,kind:n.dataset.bucketKind,id:n.dataset.bucketId};let r=e.closest?.(`.results-toolbar, .archive-search, .prompt-composer, .groups-section, .library-layout`);return r?{type:`selector`,selector:r.classList.contains(`results-toolbar`)?`.results-toolbar`:r.classList.contains(`archive-search`)?`.archive-search`:r.classList.contains(`prompt-composer`)?`.prompt-composer`:r.classList.contains(`groups-section`)?`.groups-section`:`.library-layout`}:null}function gr(e){return e?e.type===`report`?(e.bucketKind&&e.bucketId?J(e.bucketKind,e.bucketId):null)?.querySelector(`.report-card[data-report-id="${CSS.escape(e.id)}"]`)||Y(e.id):e.type===`bucket`?J(e.kind,e.id):e.type===`selector`?document.querySelector(e.selector):null:null}function _r(){let e=pr(),t=t=>[...document.querySelectorAll(t)].filter(t=>{let n=t.getBoundingClientRect();return n.bottom>e&&n.top<window.innerHeight}).sort((t,n)=>Math.abs(t.getBoundingClientRect().top-e)-Math.abs(n.getBoundingClientRect().top-e))[0];return t(`.board .report-card[data-report-id]`)||t(`.group-column[data-bucket-id]`)||t(`.results-toolbar, .archive-search, .prompt-composer`)||document.querySelector(`.results-toolbar, .archive-search, .groups-section, .library-layout`)}function X(e=null){fr();let t=e||_r();return{scrollY:window.scrollY,identity:hr(t),viewportTop:t?.getBoundingClientRect().top??null}}function vr(e){if(!e)return;fr();let t=()=>{let t=gr(e.identity),n=Math.max(0,document.documentElement.scrollHeight-window.innerHeight),r=t&&Number.isFinite(e.viewportTop)?window.scrollY+t.getBoundingClientRect().top-e.viewportTop:e.scrollY;window.scrollTo({top:Math.max(0,Math.min(n,r)),left:0,behavior:`auto`})};t(),un=requestAnimationFrame(()=>{un=requestAnimationFrame(()=>{un=0,t()})})}function Z(e){$(),vr(e)}function yr(e,t=null){let n=t||Y(e),r=n?.nextElementSibling?.matches?.(`.report-card[data-report-id]`)?n.nextElementSibling:n?.previousElementSibling?.matches?.(`.report-card[data-report-id]`)?n.previousElementSibling:null;if(!r){let e=n?.closest(`.group-column[data-bucket-id]`),t=e?.nextElementSibling?.matches?.(`.group-column[data-bucket-id]`)?e.nextElementSibling:e?.previousElementSibling?.matches?.(`.group-column[data-bucket-id]`)?e.previousElementSibling:null;r=t?.querySelector(`.report-card[data-report-id]`)||t||n}return X(r)}function br(e,t){let n=J(e,t);return X(n?.nextElementSibling?.matches?.(`.group-column[data-bucket-id]`)?n.nextElementSibling:n?.previousElementSibling?.matches?.(`.group-column[data-bucket-id]`)?n.previousElementSibling:n)}function xr(e,t=`smooth`){fr(),R=requestAnimationFrame(()=>{R=requestAnimationFrame(()=>{R=0,mr(e(),t)})})}function Q(e=null){Z(X(typeof e==`function`?e():e))}[`wheel`,`touchstart`,`pointerdown`].forEach(e=>{window.addEventListener(e,fr,{passive:!0})});var Sr=null,Cr=new Set;function wr(){return Sr||(Sr=new Promise((e,t)=>{if(!window.indexedDB){t(Error(`IndexedDB unavailable`));return}let n=indexedDB.open(Xt,1);n.onupgradeneeded=()=>{let e=n.result;e.objectStoreNames.contains(S)||e.createObjectStore(S,{keyPath:`id`}).createIndex(`reportId`,`reportId`,{unique:!1})},n.onsuccess=()=>e(n.result),n.onerror=()=>t(n.error||Error(`File database failed`))}),Sr)}async function Tr(e,t=[]){let n=t.map(t=>{let{blob:n,...r}=t;return{...r,storageId:n instanceof Blob?`${e}:${t.id}`:t.storageId||``,blob:n}}),r=n.filter(e=>e.blob instanceof Blob&&e.storageId);if(r.length){let t=await wr();await new Promise((n,i)=>{let a=t.transaction(S,`readwrite`),o=a.objectStore(S);r.forEach(t=>{o.put({id:t.storageId,reportId:e,name:t.name,type:t.type,size:t.size,blob:t.blob,updatedAt:new Date().toISOString()})}),a.oncomplete=n,a.onerror=()=>i(a.error||Error(`File save failed`)),a.onabort=()=>i(a.error||Error(`File save aborted`))})}return n.map(({blob:e,...t})=>t)}async function Er(e){if(e?.storageId)try{let t=await wr(),n=await new Promise((n,r)=>{let i=t.transaction(S,`readonly`).objectStore(S).get(e.storageId);i.onsuccess=()=>n(i.result),i.onerror=()=>r(i.error||Error(`File read failed`))});if(n?.blob instanceof Blob)return n.blob}catch{return null}let t=e?.content||e?.excerpt;return t?new Blob([t],{type:e.type||`text/plain;charset=utf-8`}):null}async function Dr(e){try{let t=await wr();await new Promise((n,r)=>{let i=t.transaction(S,`readwrite`),a=i.objectStore(S),o=a.index(`reportId`).openKeyCursor(IDBKeyRange.only(e));o.onsuccess=()=>{let e=o.result;e&&(a.delete(e.primaryKey),e.continue())},o.onerror=()=>r(o.error||Error(`File cleanup failed`)),i.oncomplete=n,i.onerror=()=>r(i.error||Error(`File cleanup failed`))})}catch{}}function Or(){Cr.forEach(e=>URL.revokeObjectURL(e)),Cr.clear()}function kr(e){let t=URL.createObjectURL(e);return Cr.add(t),t}async function Ar(){let e=[...document.querySelectorAll(`[data-saved-file-preview]`)];await Promise.all(e.map(async e=>{let t=E.reports.find(t=>t.id===e.dataset.reportId)?.savedFiles?.find(t=>t.id===e.dataset.fileId);if(!t)return;e.classList.add(`is-loading`),e.setAttribute(`aria-busy`,`true`);let n=await Er(t);if(!n||!e.isConnected){e.classList.remove(`is-loading`),e.setAttribute(`aria-busy`,`false`);return}let r=e.dataset.previewMode;try{if(r===`image`){let r=document.createElement(`img`);r.src=kr(n),r.alt=t.name||`图片预览`,r.draggable=!1,e.replaceChildren(r)}else if(r===`html`){let r=document.createElement(`iframe`);r.src=kr(n),r.title=`${t.name||`HTML`}内容`,r.setAttribute(`sandbox`,`allow-forms allow-modals allow-popups allow-scripts`),e.replaceChildren(r)}else await Lt(e,n,t,r);e.classList.add(`is-ready`)}catch(t){let n=document.createElement(`div`);n.className=`saved-file-render-error`;let r=document.createElement(`strong`);r.textContent=`暂时无法直接显示这个文件`;let i=document.createElement(`p`);i.textContent=t?.message||`请下载原文件后使用对应应用打开`,n.append(r,i),e.replaceChildren(n),e.classList.add(`has-error`)}finally{e.classList.remove(`is-loading`),e.setAttribute(`aria-busy`,`false`)}}))}async function jr(e){let t=await Er(e);if(!t)return!1;let n=kr(t),r=document.createElement(`a`);return r.href=n,r.download=e.name||`download`,document.body.append(r),r.click(),r.remove(),window.setTimeout(()=>{URL.revokeObjectURL(n),Cr.delete(n)},1e3),!0}async function Mr(e){let t=window.open(``,`_blank`),n=await Er(e);if(!n)return t?.close(),!1;let r=kr(n);return t?t.location.href=r:window.open(r,`_blank`,`noopener,noreferrer`),window.setTimeout(()=>{URL.revokeObjectURL(r),Cr.delete(r)},6e4),!0}function Nr(e){return e.savedHtml||Nn(e.savedContent,e.savedFiles)}function Pr(e){return`${String(e.title||`report`).replace(/[\\/:*?"<>|]+/g,`-`).replace(/\s+/g,` `).trim().slice(0,80)||`report`}.html`}function Fr(e){let t=Nr(e);return t?URL.createObjectURL(new Blob([t],{type:`text/html;charset=utf-8`})):``}function Ir(e){let t=Fr(e);if(!t)return!1;let n=document.createElement(`a`);return n.href=t,n.download=Pr(e),document.body.append(n),n.click(),n.remove(),window.setTimeout(()=>URL.revokeObjectURL(t),1e3),!0}function Lr(e){let t=e.url||Fr(e);return t?(window.open(t,`_blank`,`noopener,noreferrer`),e.url||window.setTimeout(()=>URL.revokeObjectURL(t),6e4),!0):!1}function Rr(e,t,n=!1){let r=g(t),i=t.format||r.label,a=!!(t.storageId&&[`image`,`pdf`,`html`].includes(r.preview)),o=n&&r.preview===`pdf`?`pdf-thumb`:r.preview,s=a?`data-saved-file-preview data-report-id="${W(e.id)}" data-file-id="${W(t.id)}" data-preview-mode="${o}"`:``,c=r.preview===`text`&&(t.excerpt||t.content)?`<pre>${W((t.excerpt||t.content).slice(0,n?280:8e3))}</pre>`:``;return`
    ...
    </div>`}function zr(e,t,n,r){let i=g(t),a=t.format||i.label,o=t.storageId||t.content||t.excerpt?`data-saved-file-preview data-report-id="${W(e.id)}" data-file-id="${W(t.id)}" data-preview-mode="${W(i.preview)}"`:``;return`<article class="saved-file-embed file-kind-${i.kind}">
  ...
  </article>`}function Br(e){let t=e.savedFiles||[];return t.length?`<section class="saved-file-list embedded-file-list" aria-label="档案正文">
    <div class="embedded-file-list-heading"><strong>档案正文</strong><span>${t.length} 个文件 · 已直接展开</span></div>
    ${t.map((n,r)=>zr(e,n,r,t.length)).join(``)}
  </section>`:``}function Vr(e,t=!1){let n=!e.url&&(!!e.savedContent||!!(e.savedFiles||[]).length),r=[`org`,`account`].includes(e.access),i=e.access===`org`?`需组织登录`:e.access===`account`?`需账号登录`:`生产可访问`,a=Nr(e),o=!!e.pinned,s=E.groups.find(t=>t.id===e.groupId)?.name||`未归类`,c=new Set([`HTML`,`手动保存`,`生产`]),l=[...new Set([s,Wn(e.workType),...e.tags||[]])].filter(e=>!c.has(e)),u=!r&&T.reports.some(t=>t.id===e.id),d=e.preview||`${e.id}.png`,f=(e.savedFiles||[])[0],p=a&&e.isHtml?`<iframe class="local-html-preview-frame" title="${W(e.title)}视觉预览"
        srcdoc="${W(a)}" sandbox="allow-scripts" loading="lazy"
        tabindex="-1" aria-hidden="true"></iframe>`:u?`<img src="./previews/${W(d)}" alt="" loading="lazy" decoding="async" draggable="false" />`:n&&f?Rr(e,f,!0):`
    ...
    <article class="report-card ${r?`restricted-card`:``} ${t?`archived-card`:``} ${o?`is-featured`:``} ${N===e.id?`is-move-selected`:``}"
      data-report-id="${W(e.id)}" ${t||A===`time`?``:`data-report-draggable="true"`}>
    ...
    </article>`}function Hr(){if(!I)return``;if(I.type===`delete-report`){let e=E.reports.find(e=>e.id===I.reportId);return e?`
      ...
      </div>`}let e=I.mode===`edit`?E.reports.find(e=>e.id===I.reportId):null,t=e?.groupId||I.groupId||E.groups[0]?.id||``,n=G((e?.tags||[]).join(`、`)).filter(e=>![`HTML`,`手动保存`,`生产`].includes(e));return`
    <div class="dialog-backdrop">
      <form class="dialog" id="report-form" role="dialog" aria-modal="true"
        ...
        <label>放入分组
          <select name="groupId">
            ${E.groups.map(e=>`<option value="${W(e.id)}" ${e.id===t?`selected`:``}>${W(e.name)}</option>`).join(``)}
          </select>
        </label>
        <label>工作类型
          <select name="workType">
            ${Zt.map(t=>`<option value="${W(t.id)}" ${t.id===(e?.workType||`product-planning`)?`selected`:``}>${W(t.name)}</option>`).join(``)}
          </select>
        </label>
        <fieldset class="report-tag-field">
          ...
          <div class="report-tag-picker" aria-label="选择关键标签">
            ${ar().map(e=>`<button type="button" class="${n.includes(e)?`selected`:``}"
    ...
    </div>`}function Ur(e){if(_t(e.id))return yt(e,W);let t=!e.url&&(!!e.savedContent||!!(e.savedFiles||[]).length),n=[`org`,`account`].includes(e.access),r=e.loginProvider||Pn(e.url)?.provider||(e.access===`org`?`组织帐号`:`站点帐号`),i=e.savedHtml||Nn(e.savedContent,e.savedFiles),a=i?`edit-local-document`:e.url?n?`edit`:`edit-document`:``,o=i?`
...
`,`<br />`)}</div>`:``}
      ...
      <div class="login-handoff-wrap">
        <section class="login-handoff-card" aria-labelledby="login-handoff-title">
          ...
          <p class="login-handoff-domain">${W(rr(e.url))}</p>
        </section>
      </div>`:`
      <div class="reader-frame-wrap">
    ...
    <main class="reader-shell compact-reader-shell">
      <header class="reader-header compact-reader-header">
        <button class="reader-icon-button back-button" type="button" data-action="back"
          aria-label="返回成果库" title="返回成果库">${nr(`back`)}</button>
        <div class="reader-title">
          <strong>${W(e.title)}</strong>
          <span>${t?`本地保存`:W(rr(e.url))}</span>
        </div>
    ...
    </header>`}function Gr(){let e=E.reports.filter(e=>e.archived).filter(e=>Wt(e,O,{group:E.groups.find(t=>t.id===e.groupId),workTypeName:Wn(e.workType)})).sort((e,t)=>new Date(t.archivedAt||0)-new Date(e.archivedAt||0)),t=E.reports.filter(e=>e.archived).length;return`
    <main class="app-shell archive-shell">
      ...
      <section class="workspace archive-workspace">
        ...
        <label class="search archive-search">
          ...
          <input id="search-input" value="${W(O)}"
        ...
        ${e.length?`
          <section class="archive-results">
            ...
            <div class="archive-grid">${e.map(e=>Vr(e,!0)).join(``)}</div>
          </section>`:`
      ...
      ${Hr()}
    </main>`}function Kr(e,t){if(![`topic`,`type`,`tag`].includes(e.kind))return``;let n=t.filter(t=>t.kind===e.kind),r=n.findIndex(t=>t.id===e.id);if(r<0)return``;let i=r===0,a=r===n.length-1,o=(t,n,r,i)=>`
    <button type="button" class="studio-icon-button group-order-button" data-action="move-group"
      ...
      title="${n}" aria-label="${n}" ${i?`disabled`:``}>${r}</button>`;return[o(`top`,`置顶`,w.top,i),o(`up`,`上移`,w.up,i),o(`down`,`下移`,w.down,a),o(`bottom`,`置底`,w.bottom,a)].join(``)}function qr(){if(on)return Gr();let e=x(O),t=E.reports.filter(e=>!e.archived),n=e?Xn(t,e):[],r=z.size?n.filter(e=>e.dimensions.some(e=>z.has(e))):n,i=Vn(t.filter(e=>e.pinned),`featured`,`featured`,(e,t)=>Kn(t)-Kn(e)),a={id:`featured`,name:`精选成果`,kind:`featured`,accent:`violet`,reports:i},o=E.reports.filter(e=>e.archived).length,s=t.filter(e=>e.access===`production`).length,c=t.filter(e=>e.access!==`production`).length,l=qn(t,``),u=A===`topic`&&i.length?[a,...l]:l,d=A===`time`&&l[0]?.reports||[],f=e?[]:(A===`topic`&&i.length?[a,...l]:l).filter(e=>e.reports.length||N||A===`topic`),p=A===`type`?`工作类型`:A===`tag`?`关键标签`:A===`time`?`新增时间`:`工作主题`;return`
        ...
        <div class="results-toolbar unified-results-toolbar">
          ...
          <div class="results-toolbar-side ${e?`has-search-results`:``}">
            <label class="search results-search">
              <span aria-hidden="true">⌕</span>
              <input id="search-input" type="search" value="${W(O)}"
                placeholder="Rediscover your work" aria-label="找到一个成果"
                autocomplete="off" spellcheck="false" enterkeyhint="search" />
              ${O?`<button type="button" class="studio-icon-button search-clear-button" data-action="clear-search" title="清除搜索" aria-label="清除搜索">${w.close}</button>`:``}
            </label>
            ${e?Qn(n,r.length):`<div class="studio-summary compact-summary" aria-label="成果统计">
              ...
              ${e?`
                <section class="search-results-panel">
                  <header class="search-results-header">
                    <div><span>SEARCH RESULTS</span><h2>“${W(O.trim())}”</h2></div>
                    ${r.length?`<p>按最高匹配优先级归类</p>`:``}
                  </header>
                  ${r.length?$n(r):n.length?`<div class="no-results search-no-results">
                  ...
                  <header class="group-header">
                    <div class="group-heading-area">
                      ...
                      ${e.kind===`topic`?`<div class="group-primary-actions" aria-label="分组快捷操作">
                        <button type="button" class="studio-icon-button" data-action="add-to-group" data-id="${W(e.id)}" title="新增成果" aria-label="新增成果">${w.plus}</button>
                        <button type="button" class="studio-icon-button" data-action="rename-group" data-id="${W(e.id)}" title="编辑分组" aria-label="编辑分组">${w.edit}</button>
                      </div>`:``}
                    </div>
                    <div class="group-menu" aria-label="分组排序操作">
                      ${N&&e.kind!==`time`?`<button class="move-here-button" type="button" data-action="move-here" data-id="${W(e.id)}" data-bucket-kind="${W(e.kind)}">Move here</button>`:``}
                      ${Kr(e,l)}
                      ${e.kind===`topic`?`<button type="button" class="studio-icon-button group-delete-button" data-action="delete-group" data-id="${W(e.id)}" title="删除分组" aria-label="删除分组">${w.minus}</button>`:``}
                    </div>
                  </header>
                  <div class="group-cards">
                    ${e.reports.length?e.reports.map(e=>Vr(e)).join(``):e.kind===`topic`?`<button class="empty-topic-drop" type="button" data-action="add-to-group" data-id="${W(e.id)}">
