const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./lib-Co3wvXuv.js","./jszip.min-BAXTQowQ.js","./pptx-preview.es-DFzdjs9Y.js"])))=>i.map(i=>d[i]);
...
`)}function At(e=``){let t=document.createElement(`template`);return t.innerHTML=String(e),t.content.querySelectorAll(`script, style, link, meta, base, iframe, frame, object, embed, form, input, button, textarea, select`).forEach(e=>e.remove()),t.content.querySelectorAll(`*`).forEach(e=>{[...e.attributes].forEach(t=>{let n=t.name.toLowerCase(),r=t.value.trim().toLowerCase();(n.startsWith(`on`)||n===`srcdoc`||(n===`href`||n===`src`||n===`xlink:href`)&&/^(javascript|vbscript):/.test(r))&&e.removeAttribute(t.name)})}),t.innerHTML}function jt(e,t,n=`document`){return`<!doctype html>
  ...
  </div>`}function xr(e){let t=z.size?new Set(z):new Set(H.map(e=>e.id)),n=new Map(H.map(({id:e})=>[e,[]]));return e.forEach(e=>{let r=H.find(({id:n})=>t.has(n)&&e.dimensions.includes(n));r&&n.get(r.id).push(e)}),`<div class="search-priority-groups" aria-label="按最高匹配优先级排列的搜索结果">
    ${H.map((e,t)=>({...e,priority:String(t+1).padStart(2,`0`),hits:n.get(e.id).sort(yr)})).filter(({id:e})=>t.has(e)).filter(e=>e.hits.length).map(e=>`<section class="search-priority-group" data-search-dimension="${e.id}">
      ...
      <div class="group-cards search-results-cards">
        ${e.hits.map(({report:e})=>vi(e)).join(``)}
      </div>
    </section>`).join(``)}
  </div>`}function Sr(e){return`${e}-${crypto.randomUUID?.()||`${Date.now()}-${Math.random()}`}`}function G(e=``){return String(e).replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`).replaceAll(`'`,`&#39;`)}var Cr={back:`
    <svg viewBox="0 0 24 24" aria-hidden="true">
    ...
    </svg>`};function wr(e){return Cr[e]||``}function Tr(e){try{return new URL(e).hostname.replace(/^www\./,``)}catch{return e}}function Er(e){try{return[`http:`,`https:`].includes(new URL(e).protocol)}catch{return!1}}function Dr(e=``){return[...new Set(String(e).split(/[、,，;；\n]+/).map(e=>e.trim()).filter(Boolean).map(e=>e.slice(0,20)))].slice(0,8)}function Or(){let e=new Set(_n);return T.reports.forEach(t=>{(t.tags||[]).forEach(t=>e.add(t))}),[...e].filter(e=>![`HTML`,`手动保存`,`生产`].includes(e))}function K(e,{duration:t=2600,actionLabel:n=``,onAction:r=null}={}){document.querySelector(`.toast`)?.remove();let i=document.createElement(`div`);i.className=`toast`,i.setAttribute(`role`,`status`);let a=document.createElement(`span`);if(a.textContent=e,i.append(a),n&&typeof r==`function`){let e=document.createElement(`button`);e.type=`button`,e.className=`toast-action`,e.textContent=n,e.addEventListener(`click`,()=>{clearTimeout(Tn),i.remove(),r()}),i.append(e)}document.body.append(i),clearTimeout(Tn),Tn=window.setTimeout(()=>i.remove(),t)}function kr(e,t){K(e,{duration:8e3,actionLabel:`撤销`,onAction:()=>{t(),K(`已撤销刚才的操作`)}})}function Ar(){let e=document.querySelector(`script[type="module"][src*="/assets/index-"]`);return e?.src?new URL(e.src,location.href):null}function jr(e){let t=new DOMParser().parseFromString(e,`text/html`).querySelector(`script[type="module"][src*="/assets/index-"]`)?.getAttribute(`src`)||``;return t?new URL(t,location.href):null}function Mr(){if(document.querySelector(`.app-update-notice`))return;let e=document.createElement(`div`);e.className=`app-update-notice`,e.setAttribute(`role`,`status`);let t=document.createElement(`span`);t.textContent=`Studio 已更新，刷新后使用最新修复`;let n=document.createElement(`button`);n.type=`button`,n.textContent=`立即刷新`,n.addEventListener(`click`,()=>{let e=new URL(location.href);e.searchParams.set(`v`,Date.now().toString(36)),location.replace(e)}),e.append(t,n),document.body.append(e)}function Nr({force:e=!1}={}){if(Rn)return Mr(),Promise.resolve(!0);let t=Ar();if(!t)return Promise.resolve(!1);let n=Date.now();if(!e&&n-Ln<hn)return In||Promise.resolve(!1);if(In)return In;Ln=n;let r=new URL(`./`,location.href);return r.searchParams.set(`studio-update-check`,n.toString(36)),In=fetch(r,{cache:`no-store`}).then(e=>e.ok?e.text():``).then(e=>{let n=jr(e);return Rn=!!(n&&n.pathname!==t.pathname),Rn&&Mr(),Rn}).catch(()=>!1).finally(()=>{In=null}),In}function Pr(){document.documentElement.dataset.studioUpdateBound!==`true`&&(document.documentElement.dataset.studioUpdateBound=`true`,window.addEventListener(`focus`,()=>Nr({force:!0})),window.addEventListener(`pageshow`,()=>Nr({force:!0})),window.addEventListener(`online`,()=>Nr({force:!0})),document.addEventListener(`visibilitychange`,()=>{document.visibilityState===`visible`&&Nr({force:!0})}),window.setTimeout(()=>Nr({force:!0}),1200),window.setInterval(()=>{document.visibilityState===`visible`&&Nr()},hn))}function Fr(e){let t=e?.closest?.(`[data-action]`);return t?{action:t.dataset.action||``,id:t.dataset.id||``,bucketKind:t.dataset.bucketKind||``,direction:t.dataset.direction||``}:null}function Ir(e){if(!e?.action)return null;let t=[`[data-action="${CSS.escape(e.action)}"]`,e.id?`[data-id="${CSS.escape(e.id)}"]`:``,e.bucketKind?`[data-bucket-kind="${CSS.escape(e.bucketKind)}"]`:``,e.direction?`[data-direction="${CSS.escape(e.direction)}"]`:``].join(``);return document.querySelector(t)}function Lr(e,t,n){Mn=t||Y(),En=Fr(n||document.activeElement),Dn=window.scrollY,I=e,X(Mn)}function Rr({fallbackSelector:e=`.results-toolbar, .archive-search, .reader-header`}={}){if(!I)return;let t=Mn||{scrollY:Dn},n=En;I=null,X(t),Mn=null,En=null,requestAnimationFrame(()=>{let t=document.querySelector(e);(Ir(n)||t?.querySelector(`button, input, [tabindex]:not([tabindex='-1'])`)||t)?.focus?.({preventScroll:!0})})}function zr(){let e=document.querySelector(`.app-shell > .dialog-backdrop`);if(e?document.body.style.setProperty(`--studio-modal-scroll-top`,`${-Dn}px`):document.body.style.removeProperty(`--studio-modal-scroll-top`),document.body.classList.toggle(`studio-modal-open`,!!e),!e)return;[...e.parentElement.children].forEach(t=>{t!==e&&(t.inert=!0,t.setAttribute(`aria-hidden`,`true`))});let t=e.querySelector(`[role="dialog"]`);if(!t)return;let n=()=>[...t.querySelectorAll([`a[href]`,`button:not([disabled])`,`input:not([disabled]):not([type='hidden'])`,`select:not([disabled])`,`textarea:not([disabled])`,`[tabindex]:not([tabindex='-1'])`].join(`,`))].filter(e=>!e.hidden&&e.getClientRects().length);e.addEventListener(`keydown`,e=>{if(e.key===`Escape`){e.preventDefault(),Rr();return}if(e.key!==`Tab`)return;let r=n();if(!r.length){e.preventDefault(),t.focus({preventScroll:!0});return}let i=r[0],a=r.at(-1);e.shiftKey&&document.activeElement===i?(e.preventDefault(),a.focus({preventScroll:!0})):!e.shiftKey&&document.activeElement===a&&(e.preventDefault(),i.focus({preventScroll:!0}))}),e.addEventListener(`click`,t=>{t.target===e&&Rr()}),requestAnimationFrame(()=>{(t.querySelector(`[autofocus]`)||n()[0]||t)?.focus?.({preventScroll:!0})})}function Br(e=`auto`){Vr(),R=requestAnimationFrame(()=>{R=0,window.scrollTo({top:0,left:0,behavior:e})})}function Vr(){L&&cancelAnimationFrame(L),R&&cancelAnimationFrame(R),On&&cancelAnimationFrame(On),L=0,R=0,On=0}function Hr(){let e=document.querySelector(`.topbar`)?.getBoundingClientRect().bottom||0,t=document.querySelector(`.topic-nav`),n=t?getComputedStyle(t):null,r=t?.getBoundingClientRect();if(!window.matchMedia(`(max-width: 840px)`).matches){let t=n?.position===`sticky`&&Number.parseFloat(n.top)||0;return Math.max(e+22,t)}let i=n?.position===`sticky`&&r?.bottom>0?r.bottom:0;return Math.max(e,i)+10}function Ur(e,t=`smooth`){if(!e)return;Vr();let n=Math.max(0,document.documentElement.scrollHeight-window.innerHeight),r=Math.max(0,Math.min(n,window.scrollY+e.getBoundingClientRect().top-Hr())),i=window.scrollY,a=r-i;if(Math.abs(a)<2)return;if(t!==`smooth`||matchMedia(`(prefers-reduced-motion: reduce)`).matches){window.scrollTo({top:r,left:0,behavior:`auto`});return}let o=Math.min(360,Math.max(180,Math.abs(a)*.22)),s=performance.now(),c=t=>{if(!e.isConnected){L=0;return}let n=Math.min(1,(t-s)/o),r=1-(1-n)**3,l=Math.max(0,Math.min(Math.max(0,document.documentElement.scrollHeight-window.innerHeight),window.scrollY+e.getBoundingClientRect().top-Hr()));a=l-i,window.scrollTo(0,i+a*r),n<1?L=requestAnimationFrame(c):(L=0,window.scrollTo(0,l))};L=requestAnimationFrame(c)}function q(e,t){return document.querySelector(`.group-column[data-bucket-kind="${CSS.escape(e)}"][data-bucket-id="${CSS.escape(t)}"]`)}function J(e){return document.querySelector(`.board .report-card[data-report-id="${CSS.escape(e)}"]`)}function Wr(e){if(!e)return null;let t=e.closest?.(`.report-card[data-report-id]`);if(t){let e=t.closest(`.group-column[data-bucket-kind][data-bucket-id]`);return{type:`report`,id:t.dataset.reportId,bucketKind:e?.dataset.bucketKind||``,bucketId:e?.dataset.bucketId||``}}let n=e.closest?.(`.group-column[data-bucket-kind][data-bucket-id]`);if(n)return{type:`bucket`,kind:n.dataset.bucketKind,id:n.dataset.bucketId};let r=e.closest?.(`.results-toolbar, .archive-search, .prompt-composer, .groups-section, .library-layout`);return r?{type:`selector`,selector:r.classList.contains(`results-toolbar`)?`.results-toolbar`:r.classList.contains(`archive-search`)?`.archive-search`:r.classList.contains(`prompt-composer`)?`.prompt-composer`:r.classList.contains(`groups-section`)?`.groups-section`:`.library-layout`}:null}function Gr(e){return e?e.type===`report`?(e.bucketKind&&e.bucketId?q(e.bucketKind,e.bucketId):null)?.querySelector(`.report-card[data-report-id="${CSS.escape(e.id)}"]`)||J(e.id):e.type===`bucket`?q(e.kind,e.id):e.type===`selector`?document.querySelector(e.selector):null:null}function Kr(){let e=Hr(),t=t=>[...document.querySelectorAll(t)].filter(t=>{let n=t.getBoundingClientRect();return n.bottom>e&&n.top<window.innerHeight}).sort((t,n)=>Math.abs(t.getBoundingClientRect().top-e)-Math.abs(n.getBoundingClientRect().top-e))[0];return t(`.board .report-card[data-report-id]`)||t(`.group-column[data-bucket-id]`)||t(`.results-toolbar, .archive-search, .prompt-composer`)||document.querySelector(`.results-toolbar, .archive-search, .groups-section, .library-layout`)}function Y(e=null){Vr();let t=e||Kr(),n=Wr(t),r=Gr(n)||t;return{scrollY:window.scrollY,identity:n,viewportTop:r?.getBoundingClientRect().top??null}}function qr(e){if(!e)return;Vr();let t=()=>{let t=Gr(e.identity),n=Math.max(0,document.documentElement.scrollHeight-window.innerHeight),r=t&&Number.isFinite(e.viewportTop)?window.scrollY+t.getBoundingClientRect().top-e.viewportTop:e.scrollY;window.scrollTo({top:Math.max(0,Math.min(n,r)),left:0,behavior:`auto`})};t(),On=requestAnimationFrame(()=>{On=requestAnimationFrame(()=>{On=0,t()})})}function X(e){$(),qr(e)}function Jr(e,t=null){let n=t||J(e),r=n?.nextElementSibling?.matches?.(`.report-card[data-report-id]`)?n.nextElementSibling:n?.previousElementSibling?.matches?.(`.report-card[data-report-id]`)?n.previousElementSibling:null;if(!r){let e=n?.closest(`.group-column[data-bucket-id]`),t=e?.nextElementSibling?.matches?.(`.group-column[data-bucket-id]`)?e.nextElementSibling:e?.previousElementSibling?.matches?.(`.group-column[data-bucket-id]`)?e.previousElementSibling:null;r=t?.querySelector(`.report-card[data-report-id]`)||t||n}return Y(r)}function Yr(e,t){let n=q(e,t);return Y(n?.nextElementSibling?.matches?.(`.group-column[data-bucket-id]`)?n.nextElementSibling:n?.previousElementSibling?.matches?.(`.group-column[data-bucket-id]`)?n.previousElementSibling:n)}function Xr(e,t=`smooth`){Vr(),R=requestAnimationFrame(()=>{R=requestAnimationFrame(()=>{R=0,Ur(e(),t)})})}function Z(e=null){X(Y(typeof e==`function`?e():e))}function Zr(){if(O||k||I)return!1;let e=document.querySelector(`.results-toolbar-side`),t=e?.querySelector(`.results-search`),n=document.querySelector(`.groups-section`);if(!e||!t||!n)return!1;let r=document.createElement(`template`);r.innerHTML=wi();let i=r.content.querySelector(`.results-toolbar-side`),a=i?.querySelector(`.results-search`),o=r.content.querySelector(`.groups-section`);if(!i||!a||!o)return!1;t.querySelector(`.search-clear-button`)?.remove();let s=a.querySelector(`.search-clear-button`);return s&&t.append(s),[...e.children].filter(e=>e!==t).forEach(e=>e.remove()),[...i.children].filter(e=>!e.classList.contains(`results-search`)).forEach(t=>e.append(t)),e.className=i.className,n.replaceWith(o),Di(),si(),!0}function Qr(e){Zr()||$(),qr(e)}function Q(e=null,t=null){let n=(typeof e==`function`?e():e)||document.querySelector(`.results-toolbar`);Qr(t||Y(n))}[`wheel`,`touchstart`,`pointerdown`].forEach(e=>{window.addEventListener(e,Vr,{passive:!0})});var $r=null,ei=new Set;function ti(){return $r||($r=new Promise((e,t)=>{if(!window.indexedDB){t(Error(`IndexedDB unavailable`));return}let n=indexedDB.open(pn,1);n.onupgradeneeded=()=>{let e=n.result;e.objectStoreNames.contains(S)||e.createObjectStore(S,{keyPath:`id`}).createIndex(`reportId`,`reportId`,{unique:!1})},n.onsuccess=()=>e(n.result),n.onerror=()=>t(n.error||Error(`File database failed`))}),$r)}async function ni(e,t=[]){let n=t.map(t=>{let{blob:n,...r}=t;return{...r,storageId:n instanceof Blob?`${e}:${t.id}`:t.storageId||``,blob:n}}),r=n.filter(e=>e.blob instanceof Blob&&e.storageId);if(r.length){let t=await ti();await new Promise((n,i)=>{let a=t.transaction(S,`readwrite`),o=a.objectStore(S);r.forEach(t=>{o.put({id:t.storageId,reportId:e,name:t.name,type:t.type,size:t.size,blob:t.blob,updatedAt:new Date().toISOString()})}),a.oncomplete=n,a.onerror=()=>i(a.error||Error(`File save failed`)),a.onabort=()=>i(a.error||Error(`File save aborted`))})}return n.map(({blob:e,...t})=>t)}async function ri(e){if(e?.storageId)try{let t=await ti(),n=await new Promise((n,r)=>{let i=t.transaction(S,`readonly`).objectStore(S).get(e.storageId);i.onsuccess=()=>n(i.result),i.onerror=()=>r(i.error||Error(`File read failed`))});if(n?.blob instanceof Blob)return n.blob}catch{return null}let t=e?.content||e?.excerpt;return t?new Blob([t],{type:e.type||`text/plain;charset=utf-8`}):null}async function ii(e){try{let t=await ti();await new Promise((n,r)=>{let i=t.transaction(S,`readwrite`),a=i.objectStore(S),o=a.index(`reportId`).openKeyCursor(IDBKeyRange.only(e));o.onsuccess=()=>{let e=o.result;e&&(a.delete(e.primaryKey),e.continue())},o.onerror=()=>r(o.error||Error(`File cleanup failed`)),i.oncomplete=n,i.onerror=()=>r(i.error||Error(`File cleanup failed`))})}catch{}}function ai(){ei.forEach(e=>URL.revokeObjectURL(e)),ei.clear()}function oi(e){let t=URL.createObjectURL(e);return ei.add(t),t}async function si(){let e=[...document.querySelectorAll(`[data-saved-file-preview]`)];await Promise.all(e.map(async e=>{let t=T.reports.find(t=>t.id===e.dataset.reportId)?.savedFiles?.find(t=>t.id===e.dataset.fileId);if(!t)return;e.classList.add(`is-loading`),e.setAttribute(`aria-busy`,`true`);let n=await ri(t);if(!n||!e.isConnected){e.classList.remove(`is-loading`),e.setAttribute(`aria-busy`,`false`);return}let r=e.dataset.previewMode;try{if(r===`image`){let r=document.createElement(`img`);r.src=oi(n),r.alt=t.name||`图片预览`,r.draggable=!1,e.replaceChildren(r)}else if(r===`html`){let r=document.createElement(`iframe`);r.src=oi(n),r.title=`${t.name||`HTML`}内容`,r.setAttribute(`sandbox`,`allow-forms allow-modals allow-popups allow-scripts`),e.replaceChildren(r)}else await Lt(e,n,t,r);e.classList.add(`is-ready`)}catch(t){let n=document.createElement(`div`);n.className=`saved-file-render-error`;let r=document.createElement(`strong`);r.textContent=`暂时无法直接显示这个文件`;let i=document.createElement(`p`);i.textContent=t?.message||`请下载原文件后使用对应应用打开`,n.append(r,i),e.replaceChildren(n),e.classList.add(`has-error`)}finally{e.classList.remove(`is-loading`),e.setAttribute(`aria-busy`,`false`)}}))}async function ci(e){let t=await ri(e);if(!t)return!1;let n=oi(t),r=document.createElement(`a`);return r.href=n,r.download=e.name||`download`,document.body.append(r),r.click(),r.remove(),window.setTimeout(()=>{URL.revokeObjectURL(n),ei.delete(n)},1e3),!0}async function li(e){let t=window.open(``,`_blank`),n=await ri(e);if(!n)return t?.close(),!1;let r=oi(n);return t?t.location.href=r:window.open(r,`_blank`,`noopener,noreferrer`),window.setTimeout(()=>{URL.revokeObjectURL(r),ei.delete(r)},6e4),!0}function ui(e){return e.savedHtml||tr(e.savedContent,e.savedFiles)}function di(e){return`${String(e.title||`report`).replace(/[\\/:*?"<>|]+/g,`-`).replace(/\s+/g,` `).trim().slice(0,80)||`report`}.html`}function fi(e){let t=ui(e);return t?URL.createObjectURL(new Blob([t],{type:`text/html;charset=utf-8`})):``}function pi(e){let t=fi(e);if(!t)return!1;let n=document.createElement(`a`);return n.href=t,n.download=di(e),document.body.append(n),n.click(),n.remove(),window.setTimeout(()=>URL.revokeObjectURL(t),1e3),!0}function mi(e){let t=e.url||fi(e);return t?(window.open(t,`_blank`,`noopener,noreferrer`),e.url||window.setTimeout(()=>URL.revokeObjectURL(t),6e4),!0):!1}function hi(e,t,n=!1){let r=g(t),i=t.format||r.label,a=!!(t.storageId&&[`image`,`pdf`,`html`].includes(r.preview)),o=n&&r.preview===`pdf`?`pdf-thumb`:r.preview,s=a?`data-saved-file-preview data-report-id="${G(e.id)}" data-file-id="${G(t.id)}" data-preview-mode="${o}"`:``,c=r.preview===`text`&&(t.excerpt||t.content)?`<pre>${G((t.excerpt||t.content).slice(0,n?280:8e3))}</pre>`:``;return`
    ...
    </div>`}function gi(e,t,n,r){let i=g(t),a=t.format||i.label,o=t.storageId||t.content||t.excerpt?`data-saved-file-preview data-report-id="${G(e.id)}" data-file-id="${G(t.id)}" data-preview-mode="${G(i.preview)}"`:``;return`<article class="saved-file-embed file-kind-${i.kind}">
  ...
  </article>`}function _i(e){let t=e.savedFiles||[];return t.length?`<section class="saved-file-list embedded-file-list" aria-label="档案正文">
    <div class="embedded-file-list-heading"><strong>档案正文</strong><span>${t.length} 个文件 · 已直接展开</span></div>
    ${t.map((n,r)=>gi(e,n,r,t.length)).join(``)}
  </section>`:``}function vi(e,t=!1){let n=!e.url&&(!!e.savedContent||!!(e.savedFiles||[]).length),r=[`org`,`account`].includes(e.access),i=e.access===`org`?`需组织登录`:e.access===`account`?`需账号登录`:`生产可访问`,a=ui(e),o=!!e.pinned,s=T.groups.find(t=>t.id===e.groupId)?.name||`未归类`,c=new Set([`HTML`,`手动保存`,`生产`]),l=[...new Set([s,fr(e.workType),...e.tags||[]])].filter(e=>!c.has(e)),u=!r&&vn.reports.some(t=>t.id===e.id),d=e.preview||`${e.id}.png`,f=(e.savedFiles||[])[0],p=a&&e.isHtml?`<iframe class="local-html-preview-frame" title="${G(e.title)}视觉预览"
        srcdoc="${G(a)}" sandbox="allow-scripts" loading="lazy"
        tabindex="-1" aria-hidden="true"></iframe>`:u?`<img src="./previews/${G(d)}" alt="" loading="lazy" decoding="async" draggable="false" />`:n&&f?hi(e,f,!0):`
      <div class="preview-placeholder ${r?`preview-restricted`:``}">
        <span>${r?`ACCESS`:G(e.title.slice(0,2))}</span>
        <strong>${r?i:n?`本地内容`:`预览待补充`}</strong>
      </div>`;return`
    <article class="report-card ${r?`restricted-card`:``} ${t?`archived-card`:``} ${o?`is-featured`:``} ${N===e.id?`is-move-selected`:``}"
      data-report-id="${G(e.id)}" ${t||A===`time`?``:`data-report-draggable="true"`}>
    ...
    </article>`}function yi(){if(!I)return``;if(I.type===`delete-report`){let e=T.reports.find(e=>e.id===I.reportId);return e?`
      ...
      </div>`:``}if(I.type===`group`){let e=I.mode===`edit`?T.groups.find(e=>e.id===I.groupId):null;return`
      <div class="dialog-backdrop">
      ...
      </div>`}let e=I.mode===`edit`?T.reports.find(e=>e.id===I.reportId):null,t=e?.groupId||I.groupId||T.groups[0]?.id||``,n=Dr((e?.tags||[]).join(`、`)).filter(e=>![`HTML`,`手动保存`,`生产`].includes(e));return`
    <div class="dialog-backdrop">
      <form class="dialog" id="report-form" role="dialog" aria-modal="true"
        ...
        <label>工作类型
          <select name="workType">
            ${gn.map(t=>`<option value="${G(t.id)}" ${t.id===(e?.workType||`product-planning`)?`selected`:``}>${G(t.name)}</option>`).join(``)}
          </select>
        </label>
        <fieldset class="report-tag-field">
          ...
          <div class="report-tag-picker" aria-label="选择关键标签">
            ${Or().map(e=>`<button type="button" class="${n.includes(e)?`selected`:``}"
    ...
    </div>`}function bi(e){if(_t(e.id))return yt(e,G);let t=!e.url&&(!!e.savedContent||!!(e.savedFiles||[]).length),n=[`org`,`account`].includes(e.access),r=e.loginProvider||nr(e.url)?.provider||(e.access===`org`?`组织帐号`:`站点帐号`),i=e.savedHtml||tr(e.savedContent,e.savedFiles),a=i?`edit-local-document`:e.url?n?`edit`:`edit-document`:``,o=i?`
...
`,`<br />`)}</div>`:``}
      ...
      <div class="login-handoff-wrap">
        <section class="login-handoff-card" aria-labelledby="login-handoff-title">
          ...
          <p class="login-handoff-domain">${G(Tr(e.url))}</p>
        </section>
      </div>`:`
      <div class="reader-frame-wrap">
    ...
    <main class="reader-shell compact-reader-shell">
      <header class="reader-header compact-reader-header">
        <button class="reader-icon-button back-button" type="button" data-action="back"
          aria-label="返回成果库" title="返回成果库">${wr(`back`)}</button>
        <div class="reader-title">
          <strong>${G(e.title)}</strong>
          <span>${t?`本地保存`:G(Tr(e.url))}</span>
        </div>
    ...
    </header>`}function Si(){let e=T.reports.filter(e=>e.archived).filter(e=>sn(e,D,{group:T.groups.find(t=>t.id===e.groupId),workTypeName:fr(e.workType)})).sort((e,t)=>new Date(t.archivedAt||0)-new Date(e.archivedAt||0)),t=T.reports.filter(e=>e.archived).length;return`
    <main class="app-shell archive-shell">
      ...
      <section class="workspace archive-workspace">
        ...
        <label class="search archive-search">
          ...
          <input id="search-input" value="${G(D)}"
            placeholder="搜索归档标题、来源或网址" aria-label="搜索归档" />
          ${D?`<button type="button" class="studio-icon-button search-clear-button" data-action="clear-search" title="清除搜索" aria-label="清除搜索">${w.close}</button>`:``}
        </label>
        ${e.length?`
          <section class="archive-results">
            ...
            <div class="archive-grid">${e.map(e=>vi(e,!0)).join(``)}</div>
          </section>`:`
      ...
      ${yi()}
    </main>`}function Ci(e,t){if(![`topic`,`type`,`tag`].includes(e.kind))return``;let n=t.filter(t=>t.kind===e.kind),r=n.findIndex(t=>t.id===e.id);if(r<0)return``;let i=r===0,a=r===n.length-1,o=(t,n,r,i)=>`
    <button type="button" class="studio-icon-button group-order-button" data-action="move-group"
      ...
      title="${n}" aria-label="${n}" ${i?`disabled`:``}>${r}</button>`;return[o(`top`,`置顶`,w.top,i),o(`up`,`上移`,w.up,i),o(`down`,`下移`,w.down,a),o(`bottom`,`置底`,w.bottom,a)].join(``)}function wi(){if(k)return Si();let e=x(D),t=T.reports.filter(e=>!e.archived),n=e?vr(t,e):[],r=z.size?n.filter(e=>e.dimensions.some(e=>z.has(e))):n,i=lr(t.filter(e=>e.pinned),`featured`,`featured`,(e,t)=>mr(t)-mr(e)),a={id:`featured`,name:`精选成果`,kind:`featured`,accent:`violet`,reports:i},o=T.reports.filter(e=>e.archived).length,s=t.filter(e=>e.access===`production`).length,c=t.filter(e=>e.access!==`production`).length,l=hr(t,``),u=A===`topic`&&i.length?[a,...l]:l,d=A===`time`&&l[0]?.reports||[],f=e?[]:(A===`topic`&&i.length?[a,...l]:l).filter(e=>e.reports.length||N||A===`topic`),p=A===`type`?`工作类型`:A===`tag`?`关键标签`:A===`time`?`新增时间`:`工作主题`;return`
    <main class="app-shell">
        ...
        <div class="results-toolbar unified-results-toolbar">
          ...
          <div class="results-toolbar-side ${e?`has-search-results`:``}">
            <label class="search results-search">
              <span aria-hidden="true">⌕</span>
              <input id="search-input" type="search" value="${G(D)}"
                placeholder="Rediscover your work" aria-label="找到一个成果"
                autocomplete="off" spellcheck="false" enterkeyhint="search" />
              ${D?`<button type="button" class="studio-icon-button search-clear-button" data-action="clear-search" title="清除搜索" aria-label="清除搜索">${w.close}</button>`:``}
            </label>
            ${e?br(n,r.length):`<div class="studio-summary compact-summary" aria-label="成果统计">
        ...
        <section class="groups-section">
          ${N?`
            <div class="move-mode-banner" role="status">
              <div><strong>正在整理报告</strong><span>选择目标${_r()}的“移到这里”，或直接拖动卡片。</span></div>
              ...
              ${e?`
                <section class="search-results-panel">
                  <header class="search-results-header">
                    <div><span>SEARCH RESULTS</span><h2>“${G(D.trim())}”</h2></div>
                    ${r.length?`<p>按最高匹配优先级归类</p>`:``}
                  </header>
                  ${r.length?xr(r):n.length?`<div class="no-results search-no-results">
                ...
                <section class="group-column topic-section bucket-${G(e.kind)} accent-${G(e.accent||`blue`)}"
                  ...
                  <header class="group-header">
                    ...
                    <div class="group-menu" aria-label="分组排序操作">
                      ${N&&e.kind!==`time`?`<button class="move-here-button" type="button" data-action="move-here" data-id="${G(e.id)}" data-bucket-kind="${G(e.kind)}">Move here</button>`:``}
                      ${Ci(e,l)}
                  ...
                  <div class="group-cards">
                    ${e.reports.length?e.reports.map(e=>vi(e)).join(``):e.kind===`topic`?`<button class="empty-topic-drop" type="button" data-action="add-to-group" data-id="${G(e.id)}">
      ...
      ${yi()}
    </main>`}function $(){let e=document.getElementById(`app`),t=O&&T.reports.find(e=>e.id===O);ai(),e.innerHTML=t?bi(t):wi(),zr(),Di(),he({render:()=>Z(()=>document.querySelector(`.prompt-composer`)),showToast:K,saveToLibrary:ar}),si()}async function Ti(e){let t=e.elements.url,n=e.elements.title,r=e.querySelector(`[data-action="detect-title"]`),i=e.querySelector(`.field-hint`),a=t.value.trim();if(!Er(a))return i.textContent=`请输入完整的 http 或 https 网址`,``;r.disabled=!0,r.innerHTML=`<span class="mini-spinner"></span>`,i.textContent=`正在读取网页标题…`;try{let{title:e}=await rr(a);if(!e)throw Error(`read failed`);return n.value=e,i.textContent=`已识别网页标题`,n.value}catch{let e=Tr(a);return n.value||=e,i.textContent=`网页暂时无法读取，已用域名作为标题，你可以手动修改`,n.value}finally{r.disabled=!1,r.textContent=`Detect title`}}function Ei(){let e=document.querySelector(`.board`);if(!e||e.dataset.appDraggingBound===`true`)return;e.dataset.appDraggingBound=`true`;let t=null,n=()=>{document.querySelectorAll(`.report-card, .group-column, .topic-nav a`).forEach(e=>{e.classList.remove(`is-nav-drop-target`)})},r=e=>e?[e.bucketKind,e.bucketId,e.targetReportId,Number(e.placeAfter),Number(e.nav)].join(`|`):``,i=()=>{let e=t.sourceCard.getBoundingClientRect(),n=t.sourceCard.cloneNode(!0);return n.removeAttribute(`id`),n.className=`report-card report-drag-preview`,n.style.width=`${e.width}px`,n.style.height=`${e.height}px`,n.querySelectorAll(`button, [role='button'], iframe`).forEach(e=>{e.removeAttribute(`data-action`),e.setAttribute(`tabindex`,`-1`)}),document.body.append(n),t.previewWidth=e.width,t.previewHeight=e.height,t.previewOffsetX=Math.max(18,Math.min(e.width-18,t.startX-e.left)),t.previewOffsetY=Math.max(18,Math.min(e.height-18,t.startY-e.top)),n},a=()=>{if(!t?.preview)return;let e=Math.max(8,window.innerWidth-t.previewWidth-8),n=Math.max(8,window.innerHeight-t.previewHeight-8),r=Math.max(8,Math.min(e,t.x-t.previewOffsetX)),i=Math.max(8,Math.min(n,t.y-t.previewOffsetY));t.preview.style.transform=`translate3d(${r}px, ${i}px, 0)`},o=()=>{!t?.active||t.updateFrame||(t.updateFrame=requestAnimationFrame(()=>{t?.active&&(t.updateFrame=0,a(),u())}))},s=(e,n=null,r=!1)=>{if(!(!e||!t?.placeholder)){if(!n||n.parentElement!==e){e.append(t.placeholder);return}e.insertBefore(t.placeholder,r?n.nextSibling:n)}},c=()=>{let e=document.elementFromPoint(t.x,t.y);if(e?.closest(`.report-card-placeholder`))return t.target;let n=e?.closest(`.topic-nav a[data-nav-bucket-id]`);if(n)return{bucketKind:n.dataset.navBucketKind,bucketId:n.dataset.navBucketId,targetReportId:``,placeAfter:!1,nav:!0,element:n};let r=e?.closest(`.report-card:not(.report-card-placeholder):not(.report-drag-preview)`),i=e?.closest(`.group-column`);if(r&&r!==t.sourceCard){let e=r.closest(`.group-column`);if(!e||e.dataset.bucketKind===`time`)return null;let n=r.getBoundingClientRect(),i=t.y>n.bottom-n.height*.22||t.y>=n.top+n.height*.22&&t.y<=n.bottom-n.height*.22&&t.x>n.left+n.width/2;return{bucketKind:e.dataset.bucketKind||A,bucketId:e.dataset.bucketId||``,targetReportId:r.dataset.reportId||``,placeAfter:i,nav:!1,element:r,container:e.querySelector(`.group-cards`)}}return i&&i.dataset.bucketKind!==`time`?{bucketKind:i.dataset.bucketKind||A,bucketId:i.dataset.bucketId||``,targetReportId:``,placeAfter:!1,nav:!1,element:i,container:i.querySelector(`.group-cards`)}:null},l=e=>{if(!(!t||r(e)===r(t.target))&&(n(),t.target=e,P=e,e)){if(e.nav){e.element.classList.add(`is-nav-drop-target`);return}if(e.targetReportId){s(e.container,e.element,e.placeAfter);return}s(e.container)}},u=()=>{t?.active&&l(c())},d=()=>{let e=Math.min(window.innerHeight*.34,Hr()+72),n=window.innerHeight-72;if(t.y<e){let n=Math.min(1,(e-t.y)/84);return-Math.max(1,Math.round(12*n*n))}if(t.y>n){let e=Math.min(1,(t.y-n)/84);return Math.max(1,Math.round(12*e*e))}return 0},f=()=>{if(!t?.active)return;let e=d();if(e){let t=window.scrollY;window.scrollBy(0,e),window.scrollY!==t&&u()}t.autoScrollFrame=requestAnimationFrame(f)},p=()=>{!t||t.active||(clearTimeout(t.holdTimer),t.sourceCard.setPointerCapture?.(t.pointerId),t.active=!0,M=t.reportId,t.preview=i(),t.placeholder=document.createElement(`div`),t.placeholder.className=`report-card report-card-placeholder`,t.placeholder.innerHTML=`<span>放在这里</span>`,t.placeholder.style.minHeight=`${t.sourceCard.getBoundingClientRect().height}px`,t.sourceCard.before(t.placeholder),t.sourceCard.classList.add(`is-dragging`),document.body.classList.add(`report-drag-session`),a(),u(),t.autoScrollFrame=requestAnimationFrame(f))},m=()=>{t&&(clearTimeout(t.holdTimer),t.autoScrollFrame&&cancelAnimationFrame(t.autoScrollFrame),t.updateFrame&&cancelAnimationFrame(t.updateFrame),t.preview?.remove(),t.placeholder?.remove(),t.sourceCard.classList.remove(`is-dragging`),document.body.classList.remove(`report-drag-session`),n(),M=``,P=null)},h=()=>{if(!t)return;let e=t,n=e.active?e.target:null,r=e.reportId;if(e.active&&(kn=r,An=Date.now()+500),m(),t=null,!n?.bucketId||n.bucketKind===`time`)return;let i=V(T),a=V(E);gr(r,n.bucketKind,n.bucketId,n.targetReportId||``,!!n.placeAfter)&&(Z(()=>J(r)),n.nav&&Xr(()=>q(n.bucketKind,n.bucketId)),kr(n.bucketKind===`featured`?`已加入精选成果`:n.bucketKind===`tag`?`已添加目标标签`:n.bucketKind===`type`?`工作类型已更新`:n.targetReportId?`报告顺序已更新`:`已移入新主题`,()=>{T=i,E=a,U(),Wn(),Z(()=>J(r))}))};e.addEventListener(`pointerdown`,e=>{if(e.button!==0||e.target.closest(`.card-actions`))return;let n=e.target.closest(`.report-card[data-report-draggable="true"]`);n?.closest(`.group-column`)&&(e.pointerType===`mouse`&&e.preventDefault(),t={pointerId:e.pointerId,reportId:n.dataset.reportId,sourceCard:n,startX:e.clientX,startY:e.clientY,x:e.clientX,y:e.clientY,active:!1,target:null,preview:null,placeholder:null,autoScrollFrame:0,updateFrame:0,holdTimer:0},t.holdTimer=window.setTimeout(()=>p(),240))}),e.addEventListener(`pointermove`,e=>{!t||e.pointerId!==t.pointerId||(t.x=e.clientX,t.y=e.clientY,!t.active&&Math.hypot(t.x-t.startX,t.y-t.startY)>=8&&p(),t.active&&(e.preventDefault(),o()))}),e.addEventListener(`pointerup`,e=>{!t||e.pointerId!==t.pointerId||h()}),e.addEventListener(`pointercancel`,()=>{m(),t=null})}function Di(){let e=document.getElementById(`search-input`);if(e&&e.dataset.appSearchBound!==`true`){e.dataset.appSearchBound=`true`;let t=!1,n=({value:e=``,selectionStart:t=0,selectionEnd:n=0,viewportSnapshot:r=null}={})=>{B&&window.clearTimeout(B),B=0,Fn=null;let i=e;if(i===D)return;(!x(D)||!x(i))&&z.clear(),D=i,Q(null,r);let a=document.getElementById(`search-input`);a?.focus({preventScroll:!0}),a?.setSelectionRange(t,n)},r=e=>({value:e?.value||``,selectionStart:e?.selectionStart??0,selectionEnd:e?.selectionEnd??e?.selectionStart??0,viewportSnapshot:Fn}),i=e=>{let t=r(e);Fn=null,B&&window.clearTimeout(B),B=window.setTimeout(()=>{B=0,n(t)},mn)};e.addEventListener(`compositionstart`,()=>{t=!0,Fn=Y(document.querySelector(`.results-toolbar`))}),e.addEventListener(`compositionend`,e=>{t=!1,n(r(e.currentTarget))}),e.addEventListener(`beforeinput`,()=>{Fn||=Y(document.querySelector(`.results-toolbar`))}),e.addEventListener(`input`,e=>{e.isComposing||t||i(e.currentTarget)}),e.addEventListener(`search`,e=>n(r(e.currentTarget))),e.addEventListener(`keydown`,e=>{if(e.key===`Enter`&&e.currentTarget.value!==D){e.preventDefault(),n(r(e.currentTarget));return}e.key!==`Escape`||!D&&!e.currentTarget.value||(e.preventDefault(),B&&window.clearTimeout(B),B=0,D=``,e.currentTarget.value=``,z.clear(),Q(),document.getElementById(`search-input`)?.focus({preventScroll:!0}))})}document.querySelectorAll(`[data-action]`).forEach(e=>{e.dataset.appActionBound!==`true`&&(e.dataset.appActionBound=`true`,e.addEventListener(`click`,async e=>{let t=e.currentTarget.dataset.action,n=e.currentTarget.dataset.id,r=e.currentTarget.closest(`.report-card`);if(B&&window.clearTimeout(B),B=0,t===`scroll-top`)Br(`smooth`);else if(t===`open`){if(n===kn&&Date.now()<An)return;jn=Y(r||J(n)),O=n,$(),Br()}else if(t===`preview-saved-file`||t===`download-saved-file`){let r=T.reports.find(e=>e.id===n)?.savedFiles?.find(t=>t.id===e.currentTarget.dataset.fileId);if(!r)return;(t===`preview-saved-file`?await li(r):await ci(r))||K(`原文件未找到，请重新上传后保存`)}else if(t===`edit-document`){let e=T.reports.find(e=>e.id===n);if(!e||e.access!==`production`)return;vt(e,{render:$,showToast:K})}else if(t===`edit-local-document`){let e=T.reports.find(e=>e.id===n);if(!e||!ui(e))return;vt(e,{render:$,showToast:K,saveLocal:async t=>{let n=e.savedHtml;e.savedHtml=t,e.isHtml=!0,e.tags=Sn(e,e.workType),W(e);try{U()}catch{throw e.savedHtml=n,Error(`修改后的 HTML 超过当前浏览器可保存容量，请先下载备份`)}}})}else if(t===`download-report`){let e=T.reports.find(e=>e.id===n);if(!e)return;ui(e)?pi(e)&&K(`HTML 已下载`):await xt(e,K)}else if(t===`share-report`||t===`copy-production-url`){let e=T.reports.find(e=>e.id===n);e?.url&&await St(e,e=>{K(e===`报告链接已复制`?`生产 URL 已复制`:e)})}else if(t===`open-browser`){let e=T.reports.find(e=>e.id===n);if(!e)return;mi(e)||K(`浏览器未能打开该报告`)}else if(t===`back`)O=``,I=null,X(jn||{scrollY:0}),jn=null;else if(t===`clear-search`){B&&window.clearTimeout(B),B=0,D=``;let e=document.getElementById(`search-input`);e&&(e.value=``),z.clear(),Q(),document.getElementById(`search-input`)?.focus({preventScroll:!0})}else if(t===`toggle-search-dimension`)n===`all`?z.clear():H.some(e=>e.id===n)&&(z.has(n)?z.delete(n):z.add(n)),Q(()=>document.querySelector(`.search-toolbar-dimensions`)),requestAnimationFrame(()=>{document.querySelector(`[data-action="toggle-search-dimension"][data-id="${CSS.escape(n)}"]`)?.focus({preventScroll:!0})});else if(t===`reset-search-dimensions`)z.clear(),Q(()=>document.querySelector(`.search-toolbar-dimensions`)),requestAnimationFrame(()=>{document.querySelector(`[data-action="toggle-search-dimension"][data-id="all"]`)?.focus({preventScroll:!0})});else if(t===`set-view`){if(![`topic`,`type`,`tag`,`time`].includes(n))return;let e={scrollY:window.scrollY,identity:null,viewportTop:null};A=n,N=``,localStorage.setItem(ln,A),x(D)?Q():X(e),requestAnimationFrame(()=>{document.querySelector(`[data-action="set-view"][data-id="${CSS.escape(n)}"]`)?.focus({preventScroll:!0})})}else if(t===`toggle-time-sort`)j=j===`created`?`modified`:`created`,localStorage.setItem(un,j),x(D)?Q():Z();else if(t===`cancel-move`)N=``,Z();else if(t===`move-here`){let t=e.currentTarget.dataset.bucketKind||A,r=V(T),i=V(E);if(N&&gr(N,t,n)){let e=N;N=``,Z(()=>J(e)),kr(t===`tag`?`已添加目标标签`:`报告已移入目标${_r()}`,()=>{T=r,E=i,U(),Wn(),Z(()=>J(e))})}}else if(t===`show-archive`)k=!0,D=``,z.clear(),O=``,$(),Br();else if(t===`show-catalog`)k=!1,D=``,z.clear(),O=``,$(),Br();else if(t===`add-report`)Lr({type:`report`,mode:`create`,groupId:T.groups[0]?.id},Y(document.querySelector(`.results-toolbar`)),e.currentTarget);else if(t===`add-to-group`)Lr({type:`report`,mode:`create`,groupId:n},Y(q(`topic`,n)),e.currentTarget);else if(t===`edit`)Lr({type:`report`,mode:`edit`,reportId:n},Y(r||J(n)),e.currentTarget);else if(t===`toggle-pin`){let e=T.reports.find(e=>e.id===n);if(!e)return;let t=V(e),i=e.pinned&&r?.closest(`[data-bucket-kind="featured"]`)?Jr(n,r):Y(r||J(n));e.pinned=!e.pinned,W(e),U(),x(D)?Qr(i):X(i),kr(e.pinned?`已加入精选成果`:`已移出精选成果`,()=>{let e=T.reports.find(e=>e.id===n);e&&(Object.assign(e,t),e.archived=!!t.archived,e.archivedAt=t.archivedAt||``,U(),x(D)?Q(()=>J(n)):Z(()=>J(n)))})}else if(t===`close-modal`)Rr();else if(t===`detect-title`)await Ti(e.currentTarget.closest(`form`));else if(t===`archive`){let e=T.reports.find(e=>e.id===n);if(!e)return;let t=V(e),i=Jr(n,r);e.archived=!0,e.archivedAt=new Date().toISOString(),U(),x(D)?Qr(i):X(i),kr(`已归档，可随时恢复`,()=>{let e=T.reports.find(e=>e.id===n);e&&(Object.assign(e,t),e.archived=!!t.archived,e.archivedAt=t.archivedAt||``,U(),x(D)?Q(()=>J(n)):Z(()=>J(n)))})}else if(t===`restore`){let e=T.reports.find(e=>e.id===n);if(!e)return;let t=V(e),r=Jr(n);e.archived=!1,e.archivedAt=``,U(),X(r),kr(`报告已恢复到原主题`,()=>{let e=T.reports.find(e=>e.id===n);e&&(Object.assign(e,t),U(),Z(()=>J(n)))})}else if(t===`delete`){if(!T.reports.find(e=>e.id===n)?.archived)return;Lr({type:`delete-report`,reportId:n},Jr(n),e.currentTarget)}else if(t===`confirm-delete`){let e=T.reports.find(e=>e.id===n);if(!e?.archived||I?.type!==`delete-report`)return;T.reports=T.reports.filter(e=>e.id!==n),O===n&&(O=``),U(),await ii(n),Rr({fallbackSelector:`.archive-grid, .archive-search`}),K(`已永久删除“${e.title}”`)}else if(t===`add-group`)Lr({type:`group`,mode:`create`},Y(document.querySelector(`.results-toolbar`)),e.currentTarget);else if(t===`rename-group`)T.groups.find(e=>e.id===n)&&Lr({type:`group`,mode:`edit`,groupId:n},Y(q(`topic`,n)),e.currentTarget);else if(t===`move-group`){let t=e.currentTarget.dataset.bucketKind,r=e.currentTarget.dataset.direction,i=Y(q(t,n));sr(n,r,t)&&(X(i),requestAnimationFrame(()=>{document.querySelector(`[data-action="move-group"][data-id="${CSS.escape(n)}"][data-direction="${CSS.escape(r)}"]`)?.focus({preventScroll:!0})}),K(`分组顺序已更新`))}else if(t===`delete-group`){let e=T.groups.find(e=>e.id===n),t=T.groups.find(e=>e.id!==n);if(e&&!t)K(`请先新增另一个分组，再删除当前分组`);else if(e&&confirm(`删除“${e.name}”？其中的报告会移到“${t.name}”。`)){let e=Yr(`topic`,n);T.reports.forEach(e=>{e.groupId===n&&(e.groupId=t.id)}),T.groups=T.groups.filter(e=>e.id!==n),U(),X(e),K(`分组已删除，报告已移到“${t.name}”`)}}}))});let t=document.querySelector(`.topbar`);t&&t.dataset.appTopbarBound!==`true`&&(t.dataset.appTopbarBound=`true`,t.addEventListener(`click`,e=>{e.target.closest(`button, a`)||Br(`smooth`)})),document.querySelectorAll(`.topic-nav a[data-nav-bucket-id]`).forEach(e=>{e.dataset.appNavBound!==`true`&&(e.dataset.appNavBound=`true`,e.addEventListener(`click`,t=>{t.preventDefault();let n=e.dataset.navBucketKind,r=e.dataset.navBucketId;if(D){D=``,Q(),Xr(()=>q(n,r));return}Ur(q(n,r))}))}),document.querySelectorAll(`.topic-nav a[data-nav-report-id]`).forEach(e=>{e.dataset.appNavBound!==`true`&&(e.dataset.appNavBound=`true`,e.addEventListener(`click`,t=>{t.preventDefault(),Ur(J(e.dataset.navReportId))}))}),Ei(),document.querySelectorAll(`.legacy-report-drag-handle`).forEach(e=>{let t=null,n=!1,r=null,i=0,a=0,o=()=>{i&&=(cancelAnimationFrame(i),0)},s=()=>{if(!M)return o();let e=Math.min(110,window.innerHeight*.18),t=a<e?a-e:a>window.innerHeight-e?a-(window.innerHeight-e):0;if(!t)return o();let n=Math.sign(t)*Math.min(24,5+Math.abs(t)*.2);window.scrollBy(0,n),i=requestAnimationFrame(s)},c=t=>{let n=e.closest(`.report-card`);if(n){if(!r){let e=n.getBoundingClientRect();r=n.cloneNode(!0),r.className=`report-card report-drag-preview`,r.style.width=`${e.width}px`,r.style.height=`${e.height}px`,r.querySelectorAll(`button, [role='button']`).forEach(e=>{e.removeAttribute(`data-action`),e.setAttribute(`tabindex`,`-1`)}),document.body.append(r)}r.style.transform=`translate3d(${t.clientX+16}px, ${t.clientY+16}px, 0)`}},l=()=>{P=null,F?.remove(),F=null,document.querySelectorAll(`.report-card, .group-column, .topic-nav a`).forEach(e=>e.classList.remove(`is-card-drop-target`,`is-card-drop-before`,`is-card-drop-after`,`is-drop-ready`,`is-nav-drop-target`))},u=(t,n,r)=>{if(t){if(!F){F=document.createElement(`div`),F.className=`report-card report-card-placeholder`,F.innerHTML=`<span>放在这里</span>`;let t=e.closest(`.report-card`)?.getBoundingClientRect().height;t&&(F.style.minHeight=`${t}px`)}if(!n||n.parentElement!==t){t.append(F);return}t.insertBefore(F,r?n.nextSibling:n)}},d=t=>{let n=document.elementFromPoint(t.clientX,t.clientY),r=n?.closest(`.topic-nav a[data-nav-bucket-id]`);if(l(),r){r.classList.add(`is-nav-drop-target`),P={bucketKind:r.dataset.navBucketKind,bucketId:r.dataset.navBucketId,targetReportId:``,placeAfter:!1,nav:!0};return}let i=e.closest(`.report-card`),a=n?.closest(`.report-card:not(.report-card-placeholder)`),o=n?.closest(`.group-column`);if(a&&a!==i){let e=a.closest(`.group-column`),n=a.getBoundingClientRect(),r=t.clientY>n.top+n.height/2;a.classList.add(`is-card-drop-target`,r?`is-card-drop-after`:`is-card-drop-before`),u(e?.querySelector(`.group-cards`),a,r),P={bucketKind:e?.dataset.bucketKind||A,bucketId:e?.dataset.bucketId||``,targetReportId:a.dataset.reportId||``,placeAfter:r,nav:!1};return}o&&o.dataset.bucketKind!==`time`&&(o.classList.add(`is-drop-ready`),u(o.querySelector(`.group-cards`),null,!1),P={bucketKind:o.dataset.bucketKind||A,bucketId:o.dataset.bucketId||``,targetReportId:``,placeAfter:!1,nav:!1})},f=()=>{M=``,t=null,n=!1,o(),r?.remove(),r=null,l(),e.closest(`.report-card`)?.classList.remove(`is-dragging`)};e.addEventListener(`pointerdown`,r=>{r.preventDefault(),M=e.dataset.reportDragId,t={x:r.clientX,y:r.clientY},n=!1,e.setPointerCapture?.(r.pointerId),e.closest(`.report-card`)?.classList.add(`is-dragging`)}),e.addEventListener(`pointermove`,e=>{M&&(t&&Math.hypot(e.clientX-t.x,e.clientY-t.y)<7||(n=!0,a=e.clientY,c(e),d(e),i||=requestAnimationFrame(s)))}),e.addEventListener(`pointerup`,e=>{if(!M)return;let t=M;if(!n){N=t,f(),Z(()=>J(t)),K(`请选择目标${_r()}`);return}let r=P,i=r?.targetReportId||``,a=r?.bucketId||``,o=r?.bucketKind||A,s=a&&o!==`time`?gr(t,o,a,i,!!r?.placeAfter):!1;f(),s&&(Z(()=>J(t)),requestAnimationFrame(()=>{let e=`.group-column[data-bucket-kind="${CSS.escape(o)}"][data-bucket-id="${CSS.escape(a)}"]`,n=document.querySelector(`${e} .report-card[data-report-id="${CSS.escape(t)}"]`)||document.querySelector(`.search-results-cards .report-card[data-report-id="${CSS.escape(t)}"]`);Ur(n),n?.classList.add(`is-drop-landed`),window.setTimeout(()=>n?.classList.remove(`is-drop-landed`),900)}),K(o===`featured`?`已加入精选成果`:o===`tag`?`已添加目标标签`:o===`type`?`工作类型已更新`:i?`报告顺序已更新`:`已移入新主题`))}),e.addEventListener(`pointercancel`,f)});let n=document.getElementById(`group-form`);n?.addEventListener(`submit`,e=>{e.preventDefault();let t=new FormData(n).get(`name`)?.trim(),r=new FormData(n).get(`description`)?.trim();if(!t)return;if(I.mode===`edit`){let e=T.groups.find(e=>e.id===I.groupId);if(!e)return;e.name=t.slice(0,60),e.description=r?.slice(0,80)||`自定义工作主题`}else T.groups.push({id:Sr(`group`),name:t.slice(0,60),description:r?.slice(0,80)||`自定义工作主题`,accent:[`blue`,`violet`,`amber`,`green`][T.groups.length%4],position:T.groups.length}),A=`topic`,localStorage.setItem(ln,A);U();let i=I.mode===`edit`?`工作主题已更新`:`工作主题已创建，可直接拖入报告`;Rr(),K(i)});let r=document.getElementById(`report-form`),i=r?.elements.tags,a=e=>{let t=Dr(i?.value).includes(e.dataset.reportTag);e.classList.toggle(`selected`,t),e.setAttribute(`aria-pressed`,String(t))},o=e=>{e.addEventListener(`click`,()=>{let t=Dr(i.value),n=e.dataset.reportTag;i.value=t.includes(n)?t.filter(e=>e!==n).join(`、`):[...t,n].slice(0,8).join(`、`),a(e)})};r?.querySelectorAll(`[data-report-tag]`).forEach(o);let s=r?.querySelector(`[data-add-report-tag]`),c=r?.querySelector(`.new-report-tag-row`),l=r?.querySelector(`[data-new-report-tag]`),u=()=>{c.hidden=!1,l.focus()},d=()=>{let[e]=Dr(l.value);if(!e)return;let t=Dr(i.value);if(!t.includes(e)&&t.length>=8){K(`最多选择 8 个标签`);return}i.value=[...new Set([...t,e])].slice(0,8).join(`、`);let n=[...r.querySelectorAll(`[data-report-tag]`)].find(t=>t.dataset.reportTag===e);n||(n=document.createElement(`button`),n.type=`button`,n.dataset.reportTag=e,n.textContent=e,s.before(n),o(n)),a(n),l.value=``,c.hidden=!0,s.focus()};s?.addEventListener(`click`,u),r?.querySelector(`[data-confirm-report-tag]`)?.addEventListener(`click`,d),l?.addEventListener(`keydown`,e=>{e.key===`Enter`?(e.preventDefault(),d()):e.key===`Escape`&&(e.preventDefault(),l.value=``,c.hidden=!0,s.focus())}),r?.addEventListener(`submit`,async e=>{e.preventDefault();let t=r.elements.url.value.trim(),n=I.mode===`edit`?I.reportId:``,i=n?T.reports.find(e=>e.id===n):null,a=!!(i&&!i.url&&!t);if(!Er(t)&&!a)return;let o=r.querySelector(`button[type="submit"]`),s=r.querySelector(`.field-hint`);if(o.disabled=!0,o.innerHTML=`<span class="mini-spinner"></span>`,a){let e=r.elements.title.value.trim()||i.title,t=r.elements.groupId.value,n=r.elements.workType.value,a=Dr(r.elements.tags.value).filter(e=>![`HTML`,`手动保存`,`生产`].includes(e));Object.assign(i,{title:e,groupId:t,workType:n,tags:a}),W(i),U(),Rr(),K(`报告已保存`);return}let c=Qn({material:t,files:[],url:t,excludeId:n});if(c){o.disabled=!1,o.textContent=`Save`,s.textContent=`成果库已有“${c.title}”，未重复保存`,K(`成果库已有“${c.title}”，未重复保存`);return}let l=await ir({material:t,files:[],url:t},e=>{s.textContent=e});if(!l.allowed){o.disabled=!1,o.textContent=`Save`,s.textContent=l.reason,K(l.reason);return}let u=r.elements.title.value.trim()||l.metadata.title,d=r.elements.groupId.value,f=r.elements.workType.value,p=Dr(r.elements.tags.value),m={title:u||Tr(t),url:t,groupId:d,workType:f,source:`手动添加`,access:l.access,detectedDescription:l.metadata.description,manualSaved:!0,isProduction:l.access===`production`,isPersonal:$n(t),isHtml:l.isHtml,loginProvider:l.loginProvider},h=I.mode===`edit`?p:[...new Set([...Sn(m,f),...p])].slice(0,8);if(I.mode===`edit`){let e=T.reports.find(e=>e.id===I.reportId);Object.assign(e,m,{tags:h}),W(e)}else{let e=new Date().toISOString(),t={id:Sr(`report`),groupId:d,...m,pinned:!1,position:T.reports.filter(e=>e.groupId===d).length,createdAt:e,modifiedAt:e,archived:!1,archivedAt:``,tags:h};T.reports.push(t)}U(),Rr(),K(`报告已保存`)});let f=O&&T.reports.find(e=>e.id===O);f&&bt(f)}function Oi(){Pr(),Vn(),$()}Oi(document.getElementById(`app`));export{f as a,c as i,s as n,d as o,o as r,u as s,Et as t};