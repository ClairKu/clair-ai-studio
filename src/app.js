...
const initialState = {
  ...
  reports: [
    ...
    },
    {
      ...
      source: "OAP OAuth 授权 × 且慢生产数仓只读聚合｜数据截至 2026-09-05 22:33（北京时间）｜341 人完成豆包授权，334 人产生至少一次 OAuth 会话令牌，共 2,330 次，使用代理覆盖率 97.9%；工具级实调用人数因缺少豆包 client_id × user_id 回连暂不可得｜以首次会话令牌为锚点：新增开户 0、绑卡 0、风测 0、首投 0；严格晚于首次会话日期的正向入账为 6 人 / 0.83 万元，另有 0.11 万元与首次会话同日、先后不可判定；自然日关联口径合计 7 人 / 0.94 万元｜input_amount 不等同独立现金充值；且慢券商 0008 存量保有 434.49 万元不归因于豆包｜仅公开聚合统计，不含用户身份、凭证、请求 ID 或原始对话",
      access: "production",
      workType: "data-analysis",
