...
const initialState = {
  ...
  reports: [
    ...
    },
    {
      ...
      source: "OAP OAuth 授权 × 且慢生产数仓 × OAP 调用 × SCRM 只读聚合｜数据截至 2026-09-05 22:00（北京时间）｜当前有效授权 341 人，其中授权时新建且慢身份 194 人、已有身份 147 人；194 名新身份的资金账户、绑卡、风测与非钱包买入均为 0｜且慢券商 0008 的完整资产快照显示 30 人持有 434.49 万元；11 人绑定后发生非钱包买入，均为绑定前已有投资者，不作因果归因｜35 名同主体用户在绑定后有 3,408 次 OAP 成功调用，但 API Key 缺少 OAuth 客户端/会话级血缘，不能解释为豆包直接使用｜身份键、券商范围和交易口径经双重审计；公开版仅保留聚合数据，不含用户身份、凭证或原始对话",
      access: "production",
      workType: "data-analysis",
