...
const initialState = {
  ...
  reports: [
    ...
    },
    {
      ...
      source: "盈米本体 × 生产数据库｜数据截至 2026-08-18 10:38（北京时间）｜新老口径按每个用户绑定当刻是否新注册判定：累计绑定用户 1,555，其中绑定时新注册 1,179、绑定时已有账户 376｜统计窗口往前延长一周至 8 月 3 日，含正式上线前灰度绑定 383（新 335 / 老 48）｜新增「在且慢的经营情况与用户画像」：保有规模 5,326 万元（171 人）、绑定后入金 144.8 万元（114 人）、绑定后买入 42.3 万元（74 人）、公众号绑定 224 人，并含年龄、性别、居住地分布｜走势图改为累计与每日新增上下分栏、默认显示各项最新累计值，划到某一天即读该日数值｜20 人以下分组隐藏，不含用户明细、单用户金额或原始对话",
      access: "production",
      workType: "data-analysis",
